...
public:
	ClientListEntry(int32 id, int32 iLSID, const char* iLoginName, const char* iLoginKey, sint16 iWorldAdmin = 0, int32 ip = 0, uint8 local=0, bool isLocalLogin=false);
	ClientListEntry(int32 id, int32 iAccID, const char* iAccName, MD5& iMD5Pass, sint16 iAdmin = 0, bool isLocalLogin=false);
	...
	void	SetPVPFlag(int32 flagID) { pvpFlag = flagID; }
	
	int32			GetPVPFlag()   { return pvpFlag; }
	...
	void	SetRulesFlag(int8 flag);
	inline uint8	GetRulesFlag() { return rulesAccepted; }
	
	void	SetSyncAccID(int32 accid);
	inline uint32	GetSyncAccID() { return syncAccID; }

	inline bool		UsingRemoteLoginServer() { return IsRemoteLoginServer; }

	void	SetAccountID(int32 newID) { paccountid = newID; }
	bool	IsSyncCharActive() { return IsSyncCharacterActive; }
	void	SetSyncCharActive(bool var ) { IsSyncCharacterActive = var; }
	void	SetStale(int8 var ) { stale = var; }
	sint8	GetOnlineStatus() { return pOnline; }
private:
	...
	bool	IsRemoteLoginServer;
	bool	IsSyncCharacterActive;
};
