	...
	eqs(ieqs)
{
	...
	SetOfficialClient(false);
}

Client::~Client() {
	if (cle && !didEnterWorld)
	{
		client_list.RemoveCLE(cle);
	}
	else if (RunLoops && cle && zoneID == 0)
...
void Client::SendLogServer()
{
	...
	// Kings & Bandits **NEED THIS ENABLED**
	switch(atoi(tmp))
	{
	case 1:
		{
	...
	case 3: // zek
		{
			// kab
		l->enable_pvp = RuleI(World,ServerType); // 4 = deity
		break;
		}
	default:
		{
...
void Client::SendEnterWorld(string name)
{
	...
	if (pZoning && database.GetLiveChar(GetAccountID(), char_name)) {
		int32 accountID = database.GetAccountIDByChar(char_name);
	if ( GetSyncAccountID() > 0 && GetSyncAccountID() == accountID )
	{
		if ( cle ) 
			cle->SetSyncCharActive(true);
	}
	else if(accountID != GetAccountID() ) {
			eqs->Close();
			return;
		} else {
...
void Client::SendCharInfo() {
	...
	database.GetCharSelectInfo(GetAccountID(),GetSyncAccountID(), cs);
...
bool Client::HandlePacket(const EQApplicationPacket *app) {
	...
	else if (GetAccountID() == 0 && opcode != OP_SendLoginInfo && opcode != OP_SendLoginInfoOld) {
	...
	case OP_SendLoginInfo:
		{
			...
			if ( opcode == OP_SendLoginInfo || opcode == OP_SendLoginInfoOld )
				newclient = true;
			...
			int nameLength = SanitizeCharArray(li->login_info, (char*)name, 18);
			int passLength = SanitizeCharArray((char*)&(li->login_info[nameLength+1]), (char*)password, 15);
		...
		//	strn0cpy(password, (char*)&(li->login_info[strlen(name)+1]), 15);
...
#endif
			{
				...
				// Kings & bandits - temporarily relieve their rules flag till we know they send the CRC
				tmpruleflag = cle->GetRulesFlag();
				cle->SetRulesFlag(0);
	...
	case OP_ApproveName: //Name approval
		{
			...
			int charNameLength = SanitizeCharArray((char*)app->pBuffer, (char*)char_name, 64);
			...
			//devn00b: removed this because it doesnt work. Leaving this fragment here just in case there really is an issue with my code...it is my code afterall.
			...
			//devn00b checked my code vs eqemum they did same thing I did but added this below. so I'll add it and assume its needed.
			if (!valid)
	...
	case OP_CharacterCreate: //Char create
		{
			...
			int32 char_acct_id = database.GetAccountIDByChar(char_name);
	...
	case OP_EnterWorld: // Enter world
		{
			if ( app->size != sizeof(EnterWorld_Struct) )
				break;
			...
			// Kings & Bandits - account ip check code
			char_name[0] = 0; // reset char_name so we don't compare against us if its already set - not sure why it would be.
			
			int nameLength = SanitizeCharArray(ew->name, (char*)char_name, 64);
			...
			int32 tmpaccid = 0;
			uint8 tmpLevel = 0;
			bool syncCharacter = false;

			// Kings & Bandits - world server pvpflag to track users
			charid = database.GetCharacterInfo(char_name, &tmpaccid, &zoneID, &instanceID, NULL, NULL, NULL, &pvpFlag, &tmpLevel);
			if ( charid > 0 && cle && cle->GetSyncAccID() > 0 && cle->GetSyncAccID() == tmpaccid )
			{
				syncCharacter = true;
				cle->SetSyncCharActive(true);
			}
			else if (charid == 0 || tmpaccid != GetAccountID()) {
				clog(WORLD__CLIENT_ERR,"Could not get CharInfo for '%s'",char_name);
				eqs->Close();
				break;
			}
			else if ( cle )
				cle->SetSyncCharActive(false);

			if ( cle )
				cle->SetPVPFlag(GetPVPFlag());
			
			if ( GetAdmin() < 80 && ( !cle || cle->GetRulesFlag() == 0 ) )
			{
				if ( !bypassLevelCRC || tmpLevel >= RuleI(World,MinLevelCheckBypass) )
				{
					clog(WORLD__CLIENT_ERR,"Incorrect client for '%s' (charid %i), user is level %i and must have the correct CRC.",char_name, charid, tmpLevel);
					ZoneUnavail(false);
					break;
				}
			}

			int maxClients = RuleI(World, MaxClientsPerIP);
			if (maxClients > -1 && GetAdmin() < 80) {
				// Kings & Bandits - designed to return ip connection count so we can handle it here
				int ipConnections = 0;
				if ( pZoning ) // if they are zoning lets kick down one ip connection - we aren't worried about someone logging back in
					ipConnections = client_list.GetCLEIP(this->GetIP(),charid,GetAccountID(),GetCLE());
				else // logging from world to a char in the same zone would be a concern
				{
					ipConnections = client_list.GetCLEIP(this->GetIP(),charid,GetAccountID(),GetCLE(), true);
					if ( ipConnections > 0 )
					{
						clog(WORLD__CLIENT_ERR,"That character is already logged in (charid %i)", charid);
						ZoneUnavail(false);
						break;
					}
					else // since we didn't have any connections on that character, we need to make sure they aren't logged in others
					{
						ipConnections = client_list.GetCLEIP(this->GetIP(),charid,GetAccountID(),NULL,false,true)-1;

						if ( ipConnections > 0 )
						{
							clog(WORLD__CLIENT_ERR,"That account is already logged in (charid %i), acctid %i (%i)", charid, GetAccountID(), ipConnections);
							ZoneUnavail(false);
							break;
						}
						else
				...
				if ( GetCLE() != NULL )
					exceptionMaxCount = this->GetCLE()->GetRulesFlag();

				if ( exceptionMaxCount > 1 && ipConnections > (exceptionMaxCount-2) )
				{
					clog(WORLD__CLIENT_ERR,"Hit max exception amount from rules flag for '%s' (charid %i) max connections allowed %i",char_name, charid, (exceptionMaxCount-1));
					ZoneUnavail(false);
					break;
				}
				else if ( exceptionMaxCount < 2 && ipConnections > maxClients )
				{
					clog(WORLD__CLIENT_ERR,"Hit max clients amount from rules flag for '%s' (charid %i) max connections allowed %i", char_name, charid, maxClients);
					ZoneUnavail(false);
					break;
				}
			}
			...
			if(!pZoning && ew->return_home)
			{
				...
				if ( syncCharacter )
					database.GetCharSelectInfo(GetSyncAccountID(), 0, cs);
				else
					database.GetCharSelectInfo(GetAccountID(), 0, cs);
			...
			if(instanceID > 0)
			{
				if ( instanceID < 10000 )
				{
					zoneID = database.GetZoneIDByDBID(instanceID);
				}
			...
			if (zoneID == 0 || !database.GetZoneName(zoneID,instanceID)) {
	...
	case OP_DeleteCharacter: {
		GuildsListEntry_Struct *gle = (GuildsListEntry_Struct*)app->pBuffer;

		char deleteName[64];

		int nameLength = SanitizeCharArray(gle->name, (char*)deleteName, 64);

		int32 char_acct_id = database.GetAccountIDByChar(deleteName);

		if(char_acct_id == GetAccountID() || ( GetSyncAccountID() > 0 && GetSyncAccountID() == char_acct_id ) )
		{
			clog(WORLD__CLIENT,"Delete character: %s",app->pBuffer);
			database.DeleteCharacter(deleteName);
			SendCharInfo();
		}
		break;
							 }
	...
	case OP_World_Client_CRC2: {
		...
		// Kings & Bandits - CRC check stop cheating mq2
		HideCorpse_Struct *cs = (HideCorpse_Struct*)app->pBuffer;
		// first 4 bytes: 7a a5  0xa57a
		// second 4 bytes: 9d d7 0xd79d
		// d79da57a 003cc000
		// current bad capture: 4A D3 D5 22 00 C0 3C 00 

		if ( cs->Action == RuleI(World,CRCCheck) ) // 0xd79da57a
		{
			if ( !newclient )
			{
				if ( RuleB(World,ForceCRCCheck) )
				{
					char query[256];
					snprintf(query,256,"UPDATE account SET rulesflag=0 where id=%i",GetAccountID());
					database.UpdateDatabaseQuery(query);
					this->GetCLE()->SetRulesFlag(0);
				}
				else // non kings and bandits - we dont worry about it and allow them past
				{
					if ( tmpruleflag < 1 )
						tmpruleflag = 1;

					this->GetCLE()->SetRulesFlag(tmpruleflag);
				}

				clog(WORLD__CLIENT,"%s Logged in suspiciously with this info set rule flag to 0! %08x %08x",GetAccountName(),cs->Action,cs->Unknown04);
			}
			else if ( !this->GetCLE()->GetRulesFlag() )
				{
					if ( tmpruleflag > 1 )
					{
						this->GetCLE()->SetRulesFlag(tmpruleflag);
					...
					{
						database.SetAgreementFlag(GetAccountID());
						this->GetCLE()->SetRulesFlag(1);
					}
			}
				this->SetOfficialClient(true);
				clog(WORLD__CLIENT,"Logged in. OK CRC.");
		}
		else
		{
			if ( RuleB(World,ForceCRCCheck) )
			...
			if ( cs->Action == RuleI(World,BaseTitaniumCRC) )
			{
				clog(WORLD__CLIENT,"%s Logged in with base Titanium CRC. %08x %08x",GetAccountName(),cs->Action,cs->Unknown04);
				bypassLevelCRC = true;
			}
			else
				clog(WORLD__CLIENT,"%s Logged in with bad CRC! %08x %08x",GetAccountName(),cs->Action,cs->Unknown04);
				//database.SetAccountStatus(this->GetAccountName(),-1);
				//clog(WORLD__CLIENT,"%s Account suspended.",GetAccountName());
				//eqs->Close();
				//break;
		}
...
bool Client::Process() {
	...
	if (autobootup_timeout.Check()) {		
		char msg[256];
		const char *zone_name=database.GetZoneName(zoneID, instanceID, true);
		_snprintf(msg,256,"Zone bootup timer expired for character %s (charid %i), bootup failed or too slow for zone %s (%d:%d)",GetCharName(),charid,zone_name,zoneID,instanceID);

		clog(WORLD__CLIENT_ERR, msg);
		database.logevents("ZoneBootupFailure",0,0,"ZoneBootupFailure","ZoneBoot","Zone Bootup Failure",msg,17);
		ZoneUnavail();
	}
	if(connect.Check()){
...
void Client::EnterWorld(bool TryBootup) {
	...
	ZoneServer* zs = NULL;
	// 10000 check hardcoded so zoneid's don't interfere, instances are 10000+t
	if(instanceID > 0)
	{
		if ( instanceID > 10000 )
		{
			if(database.VerifyInstanceAlive(instanceID, GetCharID()))
			{
				...
				else
				{
					...
					clog(WORLD__CLIENT_ERR,"Could not find instance %i for zoneid %i (charid %i), moving character to bind.",instanceID,zoneID,charid);
					ZoneUnavail();
					return;
				}
			}
	...
	if (zs) {
		...
		if (TryBootup && instanceID < 10000 ) {
			...
			if (pwaitingforbootup == 0) {
				clog(WORLD__CLIENT_ERR,"Could not find zone server to boot for instid %i zone %s zoneid %i (charid %i).",instanceID,zone_name,zoneID,charid);
				ZoneUnavail();
			}
			return;
		}
			...
			if ( instanceID > 9999 )
			{
				database.MoveCharacterToZone(GetCharID(), "nexus");
				clog(WORLD__CLIENT_ERR,"Could not find instance to boot for instid %i zone %s zoneid %i (charid %i), moving character to nexus.",instanceID,zone_name,zoneID,charid);
				ZoneUnavail();
				return;
			}
			else
			{
				clog(WORLD__CLIENT_ERR,"Zone %s is not running instid %i and zoneid %i (charid %i).",zone_name,instanceID,zoneID,charid);
				ZoneUnavail();
				return;
			}
		}
	...
	if (seencharsel) {
		if (GetAdmin() < 80 && zoneserver_list.IsZoneLocked(zoneID)) {
			clog(WORLD__CLIENT_ERR,"Zone locked %s instid %i zoneid %i (charid %i).",zone_name,instanceID,zoneID,charid);
			ZoneUnavail();
			return;
		}
...
void Client::Clearance(sint8 response)
{
	...
	const char* zonename = database.GetZoneName(zoneID,instanceID);
	if (zonename == 0) {
...
void Client::ZoneUnavail(bool populate) {
	...
	if ( populate )
	{
		const char* zonename = database.GetZoneName(zoneID, GetInstanceID());
		if (zonename)
			strcpy(ua->zonename, zonename);
	}
	else
	{
		snprintf(ua->zonename,16,"MaxClientsDeny");
	}
...
bool Client::OPCharCreate(char *name, CharCreate_Struct *cc)
{
	...
	// Kings & Bandits (Red69) - Force red69 to no class / race types
	if ( pvpFlag == 1 )
	{
	// Red Server fix for race & class
				int xpack = 0;
				char val[20] = {0};
				if (database.GetVariable("Expansions", val, 20)) {
					xpack = atoi(val);
				}
		if ( xpack < 1 ) // xpack is kunark just let them use it
		{
		switch(cc->race)
		{
		case GNOME:
	...
	else if ( pvpFlag > 1 ) // Zek+KAB
	{
				switch(cc->race)
		{
				case IKSAR:
					{
						if ( !RuleB(World,AllowIksars) )
						{
							clog(WORLD__CLIENT,"Character creation of Iksar Race by %s LS#%d (%s:%d) was disallowed.", GetCLE()->LSName(), GetCLE()->LSID(), inet_ntoa(in), GetPort());
							cc->race = HUMAN;
						}
						break;
					}
				case VAHSHIR:
					{
						if ( !RuleB(World,AllowVahShir) )
						{
							clog(WORLD__CLIENT,"Character creation of Vah Shir Race by %s LS#%d (%s:%d) was disallowed.", GetCLE()->LSName(), GetCLE()->LSID(), inet_ntoa(in), GetPort());
							cc->race = HUMAN;
						}
					break;
					}
				case FROGLOK:
					{
						if ( !RuleB(World,AllowFroglok) )
						{
							clog(WORLD__CLIENT,"Character creation of Froglok Race by %s LS#%d (%s:%d) was disallowed.", GetCLE()->LSName(), GetCLE()->LSID(), inet_ntoa(in), GetPort());
							cc->race = HUMAN;
						}
					break;
					}
	...
	switch(cc->class_)
	{
		case BEASTLORD:
		{
			if ( !RuleB(World,AllowBeastlord) )
			{
				clog(WORLD__CLIENT,"Character creation of Beastlord Class by %s LS#%d (%s:%d) was disallowed.", GetCLE()->LSName(), GetCLE()->LSID(), inet_ntoa(in), GetPort());
				cc->class_ = WARRIOR;
			}
			break;
		}
		case BERSERKER:
		{
			if ( !RuleB(World,AllowBerserker) )
			{
				clog(WORLD__CLIENT,"Character creation of Berserker Class by %s LS#%d (%s:%d) was disallowed.", GetCLE()->LSName(), GetCLE()->LSID(), inet_ntoa(in), GetPort());
				cc->class_ = WARRIOR;
			}
			break;
		}
	...
	// Kings & Bandits handles this in game, disabling world pvp flag by ServerType
	pp.pvp = 0;

	pp.PVPCareerPoints = RuleI(World,CharacterDefaultPVPPoints);
	pp.PVPCurrentPoints = RuleI(World,CharacterDefaultPVPPoints);
	...
	else
	{
		...
		if(database.GetVariable("startzone", startzone, 50))
		{
			...
			const ZoneShortName_Struct* zns = database.GetZoneInfo(startzone);

			if ( zns != NULL )
			{
			pp.zone_id = zns->zoneID;
			pp.zoneInstance = zns->dbID;
			zoneID = pp.zone_id;
			instanceID = pp.zoneInstance;
			clog(WORLD__CLIENT,"Character set to: %s  zoneid %i zoneinst %i zoneid %i instanceid %i",
				database.GetZoneName(pp.binds[0].zoneId,instanceID),pp.zone_id,pp.zoneInstance,zoneID,instanceID);
			}

			if(pp.zone_id)
				database.GetSafePoints(zns->zoneShortName, 0, &pp.x, &pp.y, &pp.z);
			else
		...
		else   // otherwise use normal starting zone logic
		{
			...
			if(pp.binds[0].x == 0 && pp.binds[0].y == 0 && pp.binds[0].z == 0)
				database.GetSafePoints(pp.binds[0].zoneId, 0, &pp.binds[0].x, &pp.binds[0].y, &pp.binds[0].z);
		}
	}
	...
	clog(WORLD__CLIENT,"Current location: %s  %0.2f, %0.2f, %0.2f",
		database.GetZoneName(zoneID,instanceID), pp.x, pp.y, pp.z);
	clog(WORLD__CLIENT,"Bind location: %s  %0.2f, %0.2f, %0.2f",
		database.GetZoneName(pp.binds[0].zoneId,instanceID), pp.binds[0].x, pp.binds[0].y, pp.binds[0].z);
	...
	else if (!database.StoreCharacter(GetAccountID(), &pp, &inv, &ext, &charID))
	{
	...
	{
		database.InsertCharacterStats(charID);
