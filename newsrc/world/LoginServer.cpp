...
LoginServer::LoginServer(const char* iAddress, int16 iPort, const char* Account, const char* Password, bool remoteServer)
: statusupdate_timer(LoginServer_StatusUpdateInterval)
{
	...
	LoginIsRemoteServer = false;
	if ( remoteServer )
		LoginIsRemoteServer = true;
...
bool LoginServer::Process() {
	...
	while((pack = tcpc->PopPacket()))
	{
		...
		case ServerOP_UsertoWorldReq: {
			...
			sint16 status = 0;
			int32 id = database.GetAccountIDFromLSID(LoginIsRemoteServer,utwr->lsaccountid, NULL, &status);

			_log(WORLD__LS, "Login to World Request: LSAccountID: %i, WorldAccountID: %i, Status: %i, LoginEQEmuServer?: %i",utwr->lsaccountid,id, status, LoginIsRemoteServer);
		...
		case ServerOP_LSClientAuth: {
			...
			client_list.CLEAdd(slsca->lsaccount_id, slsca->name, slsca->key, slsca->worldadmin, slsca->ip, slsca->local, !LoginIsRemoteServer);
			break;
		}
		...
		_log(WORLD__LS_ERR, "Login server responded with FatalError.");
			...
			break;
									...
									// removed because it lets eqemulator.net send broadcasts
		/*case ServerOP_SystemwideMessage: {
			if ( LoginIsRemoteServer )
				break;
...
void LoginServer::SendStatus() {
	...
	if (WorldConfig::get()->Locked)
		...
		lss->status = client_list.GetClientCount();
