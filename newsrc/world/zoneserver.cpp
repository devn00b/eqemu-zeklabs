...
bool ZoneServer::SetZone(int32 iZoneID, int32 iInstanceID, bool iStaticZone) {
	...
	const char* zn = MakeLowerString(database.GetZoneName(iZoneID,iInstanceID));
	char*	longname;
	...
	if (zn)
	{
		...
		// Kings & Bandits - might need to modify this Long Name to fix special zones
...
bool ZoneServer::Process() {
	...
	while((pack = tcpc->PopPacket())) {
			...
			if (scm->chan_num == 7 || scm->chan_num == 14) {
				...
				if (cle == 0 || cle->Online() < CLE_Status_Zoning || (cle->TellsOff() && ((cle->Anon() == 1 && scm->fromadmin < cle->Admin()) || scm->fromadmin < 80))) {
					if (!scm->noreply)
					{
						...
						if ( cle == 0 )
							charID = database.GetCharacterID(scm->to);

						if ( ( cle != 0 || charID > 0 ) && database.InsertMessageQueue(scm->to,scm->from,scm->chan_num,scm->language,0,(char*)scm->message) )
							zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0, "Your message was queued to %s, '%s'", scm->to, scm->message);
						else
							zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0, "You told %s, '%s is not online at this time'", scm->to, scm->to);
					}
				}
					...
					if (!scm->noreply) {
						if ( database.InsertMessageQueue(scm->to,scm->from,scm->chan_num,scm->language,0,(char*)scm->message) )
							zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0, "Your message was queued to %s, '%s'", scm->to, scm->message);
						else
							zoneserver_list.SendEmoteMessage(scm->from, 0, 0, 0, "You told %s, '%s is not online at this time'", scm->to, scm->to);
					}
		...
		case ServerOP_SetZone: {
			...
			if (szs->zoneid != 0) {
				if(database.GetZoneName(szs->zoneid,szs->instanceid))
			...
			zoneserver_list.SendServerStats();
		...
		case ServerOP_ZoneShutdown: {
			...
			if (s->ZoneServerID != 0)
				...
				zs = zoneserver_list.FindByName(database.GetZoneName(s->zoneid,(int32)s->instanceid));
			else
		...
		case ServerOP_ZoneBootup: {
			ServerZoneStateChange_struct* s = (ServerZoneStateChange_struct *) pack->pBuffer;
			zoneserver_list.SOPZoneBootup(s->adminname, s->ZoneServerID, database.GetZoneName(s->zoneid,(int32)s->instanceid), s->makestatic);
			break;
								  }
		...
		case ServerOP_ZoneToZoneRequest: {
			...
			if(GetZoneID() == ztz->current_zone_id && GetInstanceID() == ztz->current_instance_id)	// this is a request from the egress zone
			{
				...
				if(ztz->requested_instance_id > 10000)
				{
					ingress_server = zoneserver_list.FindByInstanceID(ztz->requested_instance_id);
				}
				else if ( ztz->requested_instance_id > 0)
				{
					ingress_server = zoneserver_list.FindByZoneID(ztz->requested_instance_id);
				}
				else
				{
					ztz->requested_instance_id = 0;
			...
			else	// this is response from the ingress server, route it back to the egress server
			{
				...
				if(ztz->current_instance_id > 10000)
				{
					egress_server = zoneserver_list.FindByInstanceID(ztz->current_instance_id);
				}
				else if ( ztz->requested_instance_id > 0)
				{
					egress_server = zoneserver_list.FindByZoneID(ztz->requested_instance_id);
				}
				else
				{
					ztz->requested_instance_id = 0;
					egress_server = zoneserver_list.FindByZoneID(ztz->requested_zone_id);
		...
		case ServerOP_Who: {
			...
			client_list.SendWhoAll(whoall->fromid,whoall->from, whoall->admin, whom, this, whoall->charid);
			delete whom;
			break;
						   }
...
void ZoneServer::IncommingClient(Client* client) {
	...
	s->officialClient = client->IsOfficialClient();
	SendPacket(pack);
	delete pack;
}
