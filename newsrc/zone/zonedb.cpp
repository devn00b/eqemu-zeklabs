...
bool ZoneDatabase::SaveZoneCFG(int32 zoneid, uint16 instance_id, NewZone_Struct* zd){
    ...
    char *query = 0;
	char additionalData[32];
	
	memset(&additionalData,0,32);
	if ( instance_id > 10000 )
		_snprintf(additionalData,32,"and id=%i",instance_id);
...
// Kings & Bandits query addition  ZoneDatabase::GetZoneCFG input canPVP
bool ZoneDatabase::GetZoneCFG(int32 zoneid, uint16 instance_id, NewZone_Struct *zone_data, bool &can_bind, bool &can_combat, bool &can_levitate, bool &can_castoutdoor, bool &is_city, bool &is_hotzone, int &ruleset, char **map_filename, uint8* canPVP, uint8* canHateSummonFull, uint32* pvpHotZone, float* no_los_pet_distance, bool* deathmods) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	...
	*map_filename = new char[100];
	...
	memset(&additionalData,0,32);
	if ( instance_id > 0 && instance_id < 10000 )
		_snprintf(additionalData,32,"and id=%i",instance_id);

	if (RunQuery(query, MakeAnyLenString(&query, "SELECT ztype,"
		"fog_red,fog_green,fog_blue,fog_minclip,fog_maxclip," // 6
		"fog_red2,fog_green2,fog_blue2,fog_minclip2,fog_maxclip2," // 11
		"fog_red3,fog_green3,fog_blue3,fog_minclip3,fog_maxclip3," // 16
		"fog_red4,fog_green4,fog_blue4,fog_minclip4,fog_maxclip4,fog_density," // 22
		"sky,zone_exp_multiplier,safe_x,safe_y,safe_z,underworld," // 28
		"minclip,maxclip,time_type,gravity,canbind,cancombat,canlevitate,canpvp,hatesummonfull," // 36, Kings & Bandits query addition  ZoneDatabase::GetZoneCFG - gravity and canpvp
		"castoutdoor,hotzone,pvphotzone,no_los_pet_distance,ruleset,suspendbuffs,map_file_name,short_name,use_death_mods" // 42
		" from zone where zoneidnumber=%i %s",zoneid, additionalData), errbuf, &result)) {
		safe_delete_array(query);
		...
		if(row) 
		{
			...
			// Kings & Bandits Addition
			if ( canPVP )
			*canPVP = (uint8)atoul(row[r++]);
			else
				r++;

			if ( canHateSummonFull )
				*canHateSummonFull = (uint8)atoul(row[r++]);
			else
				r++;
			...
			if ( pvpHotZone )
				*pvpHotZone = (uint32)atoul(row[r++]);
			else
				r++;

			if ( no_los_pet_distance )
				*no_los_pet_distance = atof(row[r++]);
			else
				r++;
			...
			char *file = row[r++];
			LogFile->write(EQEMuLog::Debug, "Map file name passed: %s",file);
			if(file && (strcasecmp(file,"(NULL)")))
			{
			...
			LogFile->write(EQEMuLog::Debug, "Map file name set to: %s",*map_filename);

			// if we want to honor the sped up respawn times / loot values
			if ( deathmods )
				*deathmods = atoi(row[r++])==0?false:true;
			else
				r++;
	...
	{
		LogFile->write(EQEMuLog::Debug, "Error in GetZoneCFG query %s: %s", query, errbuf);
...
bool ZoneDatabase::GetAccountInfoForLogin_result(MYSQL_RES* result, sint16* admin, char* account_name, int32* lsaccountid, int8* gmspeed, bool* revoked,bool* gmhideme, uint32* account_creation, bool* tellsoff, bool* replayEnabled) {
    MYSQL_ROW row;
...
int8 *class_, int8 *level, bool *LFP, bool *LFG) {
	...
	if (character_id && *character_id) {
		...
		querylen = MakeAnyLenString(&query, "SELECT id,profile,zonename,x,y,z,guild_id,rank,extprofile,class,level,lfp,lfg,instanceid,pvpflag FROM character_ LEFT JOIN guild_members ON id=char_id WHERE id=%i", *character_id);
	}
	else {
		querylen = MakeAnyLenString(&query, "SELECT id,profile,zonename,x,y,z,guild_id,rank,extprofile,class,level,lfp,lfg,instanceid,pvpflag FROM character_ LEFT JOIN guild_members ON id=char_id WHERE name='%s'", name);
	}
...
bool ZoneDatabase::GetCharacterInfoForLogin_result(MYSQL_RES* result, 
	...
	if (mysql_num_rows(result) == 1) {
		...
		if ( pp )
		{
			pp->pvp = (uint8)atol(row[14]);
			_log(CLIENT__ERROR, "Setting player profile flag to %d!\n",pp->pvp);
		}
		...
		return GetInventory(char_id, inv, zoneid);
	}
...
const NPCType* ZoneDatabase::GetNPCType (uint32 id) {
   ...
   // Otherwise, get NPCs from database.
      ...
      const char *basic_query = "SELECT "
            ...
            "npc_types.merchant_level_requirement,"
		...
		if (RunQuery(query, strlen(query), errbuf, &result)) {
         // Process each row returned.
			while((row = mysql_fetch_row(result))) {
				...
				tmpNPCType->MerchantMinLevel = atoi(row[r++]);
				...
				tmpNPCType->randomNameGroupID = (uint32)atoul(row[r++]);
 ...
 */
bool ZoneDatabase::GetAccountInfoForLogin(int32 account_id, sint16* admin, char* account_name, int32* lsaccountid, int8* gmspeed, bool* revoked,bool* gmhideme, bool* tellsoff, bool* replayEnabled) {
	char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
    MYSQL_RES *result;
	
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT status, name, lsaccount_id, gmspeed, revoked, hideme, time_creation, tellsoff, replay FROM account WHERE id=%i", account_id), errbuf, &result)) {
		safe_delete_array(query);
		bool ret = GetAccountInfoForLogin_result(result, admin, account_name, lsaccountid, gmspeed, revoked,gmhideme,0,tellsoff,replayEnabled);
		mysql_free_result(result);
		return ret;
	}
	else
...
bool ZoneDatabase::getZoneHotZone(char* shortName)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
    MYSQL_RES *result;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT hotzone from zone where short_name='%s'", shortName), errbuf, &result))
	{
		if (mysql_num_rows(result) > 0) {
			row = mysql_fetch_row(result);
			int retVal = atoi(row[0]);
			...
			cerr << "Error in getZoneHotZone no result '" << query << "' " << errbuf << endl;
			mysql_free_result(result);
			safe_delete_array(query);
			return false;
		}
		...
		cerr << "Error in getZoneHotZone query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
	}
	return false;
}
...
int32 ZoneDatabase::getPVPHotZone(char* shortName)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
    char *query = 0;
    MYSQL_RES *result;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT pvphotzone from zone where short_name='%s'", shortName), errbuf, &result))
	{
		if (mysql_num_rows(result) > 0) {
			row = mysql_fetch_row(result);
			int retVal = atoi(row[0]);
			...
			cerr << "Error in getPVPHotZone no result '" << query << "' " << errbuf << endl;
			mysql_free_result(result);
			safe_delete_array(query);
			return 0;
		}
		...
		cerr << "Error in getPVPHotZone query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
	}
	return 0;
}

bool ZoneDatabase::DBLoadAchievements(int32 zoneid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES *result;
	...
	if (RunQuery(query,MakeAnyLenString(&query, "SELECT id,name,description,reqkills,reqdeaths,reqkillstreak,reqpoints,reqlevel,reqitemid,earnpoints,earncoin,broadcast,earnitemid,reqzonekills,reqzonedeaths,reqteamid,reqtime,reqtimezonedeath,isrepeatable,cooldown_seconds from achievements WHERE reqzoneid=%i OR reqzoneid=0", zoneid),errbuf,&result))
	{
		safe_delete_array(query);
		while(row = mysql_fetch_row(result))
		{
			...
			//row[1] = name
			//row[2] = desc
			int32 reqkills = atoul(row[3]);
			int32 reqdeaths = atoul(row[4]);
			int32 reqkillstreak = atoul(row[5]);
			int32 reqpoints = atoul(row[6]);
			int32 reqlevel = atoul(row[7]);
			int32 reqitemid = atoul(row[8]);
			
			int32 pts = atoul(row[9]);
			int32 coin = atoul(row[10]);
			int32 bcast = atoul(row[11]);
			int32 earnitemid = atoul(row[12]);

			int32 reqzonekills = atoul(row[13]);
			int32 reqzonedeaths = atoul(row[14]);
			int8 reqteamid = atoul(row[15]);
			int32 reqtime = atoul(row[16]);
			int32 reqtimezonedeath = atoul(row[17]);
			int32 isrepeatable = atoul(row[18]);
			int32 cooldown_seconds = atoul(row[19]);

			KBAchievement* kbachiev = new KBAchievement(id,(char*)row[1],(char*)row[2],reqkills,reqdeaths,reqkillstreak,reqpoints,reqlevel,reqitemid,pts,coin,bcast,earnitemid,
				reqzonekills,reqzonedeaths,reqteamid, reqtime, reqtimezonedeath, isrepeatable, cooldown_seconds);
			zone->achievements.Insert(kbachiev);
		}
...
bool ZoneDatabase::GetMessageQueue(MYSQL_RES* result, Client* sendto)
{
	MYSQL_ROW row;
	while((row = mysql_fetch_row(result))) {
		char* fromName = row[0];
		int16 channelID = atoi(row[1]);
		int8 language = atoi(row[2]);
		int8 langskill = atoi(row[3]);
		char* message = row[4];
		char* timestamp = row[5];
		int32 command = atoi(row[6]);
		int32 cmdvalue = atoi(row[7]);
		int32 guildid = atoi(row[8]);
		char* lastAttacker = row[9];
		int32 fromgroupid = atoi(row[10]);
		char* accountName = row[11];
		int32 optionalfield1 = atoi(row[12]);
		int32 optionalfield2 = atoi(row[13]);

		switch(command)
			{
			case 1: // summon item
				{ 
					sendto->SummonItem(cmdvalue);
					break;
				}
			case 2: // nuke one of item id
				{
					sendto->NukeItem(cmdvalue,1);
					break;
				}
			case 3: // nuke all of item id
				{
					sendto->NukeItem(cmdvalue);
					break;
				}
			case 4: // move to safe spot
				{
					sendto->MovePC(zone->GetZoneID(), zone->GetInstanceID(), zone->safe_x(), zone->safe_y(), zone->safe_z(), 0.0f);
					break;
				}
			case 5: // cross zone kill update
				{
					bool legitKillPtr = false;
					bool addDeathKillPtr = false;
					bool isBountySet = (channelID || sendto->IsBountySet());

					uint32 pts = sendto->UpdatePVPKillStats(fromName, language, 0, database.GetZoneID(message), 0, guildid, langskill, cmdvalue, lastAttacker, 0, isBountySet, database.GetCharacterID(fromName), fromgroupid, accountName, optionalfield1, optionalfield2, &legitKillPtr, &addDeathKillPtr);
					
					sendto->UpdateAchievements();

					if ( addDeathKillPtr )
					{
						database.InsertMessageQueue(fromName, (char*)sendto->GetName(), 0, 0, 0, "", 0, true, 6, 1, 0);
					}

					break;
				}
			case 6: // death cross zone update
				{
					if ( cmdvalue == 1 )
					{		
						sendto->SetPVPDeaths(sendto->GetPVPDeaths()+1);
						sendto->SetCurrentKillStreak(0);
						sendto->UpdateAchievements();

						PVPLeaderBoardEntry_Struct lbevictim;
						if ( RuleB(PVPLeaderBoard, TrackDeaths ) )
							lbevictim.Deaths = sendto->GetPP().PVPDeaths;
						else
							lbevictim.Deaths = 0;

						lbevictim.Kills = sendto->GetPP().PVPKills;
						strcpy(lbevictim.Name,sendto->GetName());
						lbevictim.TotalPoints = sendto->GetPP().PVPCareerPoints;
						lbevictim.Infamy = sendto->GetInfamyLevel();
			...
			case 7: // set pvp team
				{
					sendto->SetPVP(cmdvalue);
					break;
				}
			case 8: // add pvp points (not career pts)
				{
					if ( sendto->AddPVPPoints(cmdvalue,false) )
						sendto->Message(13, "You have received %i pvp points.", cmdvalue);
					break;
				}
			case 9: // add pvp points (career pts)
				{
					if ( sendto->AddPVPPoints(cmdvalue) )
						sendto->Message(13, "You have received %i pvp points.", cmdvalue);
					break;
				}
			case 10: // add group pvp points (career pts)
				{
					int32 zoneID = atoi(message);

					if ( zoneID != 0 && zoneID != zone->GetZoneID() && !RuleB(PVPCombat,GroupPVPPointsCrossZone) )
					{
						sendto->Message(13, "You missed out on %i PVP points since you were not in the origin zone it happened in (%s).", cmdvalue, database.GetZoneName(zoneID));
					}
					else
					{
						if ( sendto->AddPVPPoints(cmdvalue) )
		...
		if ( command < 5 && strlen(message) > 0 )
			sendto->ChannelMessageSend(fromName,sendto->GetName(),(int8)channelID, language, message);
	}

	return true;
}

bool ZoneDatabase::SetStartingItems(Client* client, PlayerProfile_Struct* pp, Inventory* inv, uint32 si_race, uint32 si_class, uint32 si_deity, uint32 si_current_zone, char* si_name, int admin_level)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES *result;
	...
	while((row = mysql_fetch_row(result))) {
		...
		client->PutItemInInventory(slot,*myinst,true);
		safe_delete(myinst);
	}
