...
void PerlembParser::PurgeDeadMob(Mob* mob) {
	eventQueueProcessing = true;
	queue<EventRecord> newQueue;
	...
	while(!eventQueue.empty()) {
		...
		if ( e.mob == mob )
		{
			//printf("Dead mob target found, purging %s\n",mob->GetName());
			e.mob = NULL;
		}
		
		if ( e.npcmob != mob )
		{
			//printf("%s is ok not purging.\n",e.npcmob->GetName());
			newQueue.push(e);
		}
		/*else if ( e.npcmob != NULL )
			printf("Dead mob found, purging %s\n",e.npcmob->GetName());*/
	}
	
	while(!newQueue.empty()) {
		EventRecord e = newQueue.front();
		newQueue.pop();
		eventQueue.push(e);
	}
...
void PerlembParser::HandleQueue() {
	...
	while(!eventQueue.empty()) {
		...
		/*try
		{
		printf("QueueHandle: Event: %i\n",e.event);
		printf("Mob: %s\n",e.npcmob->GetName());
		printf("MobTarget: %s\n",e.mob ? e.mob->GetName() : "N/A");
		}catch(...)
		{
...
void PerlembParser::EventCommon(QuestEventID event, int32 objid, const char * data, NPC* npcmob, ItemInst* iteminst, Mob* mob, int32 extradata)
{
	...
	if(perl->InUse()) {
		//printf("Perl in use %i queued for %s, target: %s.\n", event,npcmob->GetName(),mob ? mob->GetName() : "N/A");
	...
	if(!isPlayerQuest && !isItemQuest && !isSpellQuest)
	{
		if (npcmob)
		{
			...
			ExportVar(packagename.c_str(), "moborigname", npcmob->GetOrigName());
...
// $oncursor
	...
	switch (event) {
		...
		case EVENT_ITEM: {
			...
			ExportVar(packagename.c_str(), "item1_name", GetVar("item1.name", objid).c_str());
			ExportVar(packagename.c_str(), "item2_name", GetVar("item2.name", objid).c_str());
			ExportVar(packagename.c_str(), "item3_name", GetVar("item3.name", objid).c_str());
			ExportVar(packagename.c_str(), "item4_name", GetVar("item4.name", objid).c_str());
		...
		case EVENT_DEATH: {
			ExportVar(packagename.c_str(), "killedby_charid", data);
			ExportVar(packagename.c_str(), "legitkill", extradata);
			break;
		}
