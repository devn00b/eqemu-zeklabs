	...
	if(petBuffs && botPetSaveId > 0) {
		...
		if(!database.RunQuery(Query, MakeAnyLenString(&Query, "SELECT SpellId, CasterLevel, Duration FROM botpetbuffs WHERE BotPetsId = %u;", botPetSaveId), TempErrorMessageBuffer, &DatasetResult)) {
			...
			while(DataRow = mysql_fetch_row(DatasetResult)) {
				...
				petBuffs[BuffCount].dropOnCombat = IsEffectInSpell(petBuffs[BuffCount].spellid,SE_NegateIfCombat);
...
bool Bot::BotRangedAttack(Mob* other) {
	...
	if(!this->CalcBotHitChance(other, ARCHERY, SLOT_RANGE)) {
		...
		other->Damage(this, 0, SPELL_UNKNOWN, ARCHERY, RangedWeapon->ItemType);
	}
	else {
		...
		if(!TryHeadShot(other, ARCHERY)) {
			...
			if((WDmg > 0) || (ADmg > 0)){
				...
				other->Damage(this, TotalDmg, SPELL_UNKNOWN, ARCHERY, RangedWeapon->ItemType);
			}
			else {
				other->Damage(this, -5, SPELL_UNKNOWN, ARCHERY, RangedWeapon->ItemType);
			}
		}
	}
...
bool Bot::TryFinishingBlow(Mob *defender, SkillType skillinuse, uint8 itemType)
{
	int8 aa_item = 0;
	// Deathblow
	if(GetLevel() >= 68) {
...
void Bot::DoSpecialAttackDamage(Mob *who, SkillType skill, uint8 itemType, sint32 max_damage, sint32 min_damage, sint32 hate_override) {
