...
// Kings & Bandits - perl npc implementations

XS(XS_NPC_AddLootGroup); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_AddLootGroup)
{
	dXSARGS;
	if (items < 2 || items > 4)
		Perl_croak(aTHX_ "Usage: NPC::AddLootGroup(THIS, groupid)");
	{
		NPC *		THIS;
		int32		itemid = (int32)SvUV(ST(1));
		...
		if ( THIS->AddedLootGroupInPerl )
			XSRETURN_EMPTY;

		RandomLootDropResult_Struct rlds;
		memset(&rlds,0,sizeof(RandomLootDropResult_Struct));
		char* npcName = (char*)THIS->GetOrigName();
		if ( database.GenerateRandomLootGroupResult(&rlds, itemid, npcName) )
		{
			for(int i=0;i<rlds.itemDropCount;i++)
			{
				uint32 inItemID = rlds.items[i].itemID;
				if ( inItemID > 0 )
					THIS->AddItem(inItemID, -1, false);
			}

			THIS->AddCash((uint64)rlds.coinDropValue);

			THIS->AddedLootGroupInPerl = true;
		}
		else
...
XS(XS_NPC_AddLootGroupEx); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_AddLootGroupEx)
{
	dXSARGS;
	if (items < 2 || items > 4)
		Perl_croak(aTHX_ "Usage: NPC::AddLootGroupEx(THIS, groupid, override:true:false)");
	{
		NPC *		THIS;
		int32		itemid = (int32)SvUV(ST(1));
		bool		overrideField = false;
		...
		if ( items > 2 )
			overrideField = (bool)SvTRUE(ST(1));

		if ( THIS->AddedLootGroupInPerl && !overrideField )
			XSRETURN_EMPTY;
...
XS(XS_NPC_OpenPlayerShroudWindow); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_OpenPlayerShroudWindow)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::OpenPlayerShroudWindow(THIS, plyr)");
	{
		NPC *		THIS;
		bool		RETVAL;
		Client*		player;
		...
		if (sv_derived_from(ST(1), "Client")) {
			...
			player = INT2PTR(Client *,tmp);
		}
		else
			Perl_croak(aTHX_ "p is not of type Client");
		if(player == NULL)
			Perl_croak(aTHX_ "p is NULL, avoiding crash.");

	    player->OpenShroudWindow((Mob*)THIS);
		ST(0) = boolSV(RETVAL);
		sv_2mortal(ST(0));
	}
	XSRETURN(1);
...
XS(XS_NPC_HasItem)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::HasItem(THIS, itemid)");
	{
		NPC *		THIS;
		int32		RETVAL;
		dXSTARG;
...
XS(XS_NPC_AddCash)
{
		...
		Perl_croak(aTHX_ "Usage: NPC::AddCash(THIS, in_copper, in_silver, in_gold, in_platinum)");
	{
		...
		if ( in_silver < 1 && in_gold < 1 && in_platinum < 1 )
			THIS->AddCash((uint64)in_copper);
		else
...
XS(XS_NPC_SpawnFix); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_SpawnFix)
{
	dXSARGS;
	if (items != 5)
		Perl_croak(aTHX_ "Usage: NPC::SpawnFix(THIS,x,y,z,heading)");
	{
		NPC *		THIS;
		int32		RETVAL;
		dXSTARG;
		...
		{
			RETVAL = (int32)database.RunQuery(query, MakeAnyLenString(&query, "UPDATE spawn2 SET x='%f', y='%f', z='%f', heading='%f' WHERE id='%i'",mtx,mty,mtz,mth,s2->GetID()), errbuf);
			safe_delete_array(query);
			database.ReloadSpawn2(s2);
		}
		XSprePUSH; PUSHu((UV)RETVAL);
	}
	XSRETURN(1);
...
XS(XS_NPC_Repop); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_Repop)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::Repop(THIS, timedelay)");
	{
		NPC *		THIS;
		int32		repopTimeMS = (int32)SvUV(ST(1));
		...
		if ( THIS->respawn2 != NULL )
		{
			if ( !THIS->respawn2->isTimerEnabled() )
					THIS->respawn2->Disable(true);
				
			THIS->respawn2->Repop(repopTimeMS);
		}
	}
	XSRETURN_EMPTY;
}
...
XS(XS_NPC_DisplayWaypointInfo)
{
		...
		Perl_croak(aTHX_ "Usage: NPC::DisplayWaypointInfo(THIS, to)");
	{
		...
		THIS->DisplayWaypointInfo(to,true);
	}
	XSRETURN_EMPTY;
}
...
XS(XS_NPC_GetNPCTypeID); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_GetNPCTypeID)
{
	dXSARGS;
	if (items != 1)
		Perl_croak(aTHX_ "Usage: NPC::GetNPCTypeID(THIS)");
	{
		NPC *		THIS;
		int32		RETVAL;
		dXSTARG;
...
XS(XS_NPC_HasSpell); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_HasSpell)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::HasSpell(THIS, spellid)");
	{
		NPC *		THIS;
		bool		RETVAL;
		...
		int32 spellid = SvUV(ST(1));

		RETVAL = THIS->HasSpell(spellid);
		ST(0) = boolSV(RETVAL);
		sv_2mortal(ST(0));
	}
	XSRETURN(1);
...
XS(XS_NPC_HasSpellList); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_HasSpellList)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::HasSpellList(THIS, spell_list_id)");
	{
		NPC *		THIS;
		bool		RETVAL;
		...
		int32 spell_list_id = SvUV(ST(1));

		RETVAL = THIS->HasSpellList(spell_list_id);
		ST(0) = boolSV(RETVAL);
		sv_2mortal(ST(0));
	}
	XSRETURN(1);
...
XS(XS_NPC_HasSpellGroupOmit); /* prototype to pass -Wmissing-prototypes */
XS(XS_NPC_HasSpellGroupOmit)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: NPC::HasSpellGroupOmit(THIS, effect_group)");
	{
		NPC *		THIS;
		bool		RETVAL;
		...
		int32 effect_group = SvUV(ST(1));

		RETVAL = THIS->HasSpellGroupOmit(effect_group);
		ST(0) = boolSV(RETVAL);
		sv_2mortal(ST(0));
	}
	XSRETURN(1);
...
XS(boot_NPC)
{
	...
	// Kings & Bandits - implementations
	newXSproto(strcpy(buf, "AddLootGroup"), XS_NPC_AddLootGroup, file, "$$");
	newXSproto(strcpy(buf, "AddLootGroupEx"), XS_NPC_AddLootGroupEx, file, "$$;$");
	newXSproto(strcpy(buf, "AddLootTable"), XS_NPC_AddLootTable, file, "$");
	newXSproto(strcpy(buf, "OpenPlayerShroudWindow"), XS_NPC_OpenPlayerShroudWindow, file, "$$");
	...
	newXSproto(strcpy(buf, "SpawnFix"), XS_NPC_SpawnFix, file, "$$$$$");
	newXSproto(strcpy(buf, "Repop"), XS_NPC_Repop, file, "$$");
	...
	newXSproto(strcpy(buf, "GetNPCTypeID"), XS_NPC_GetNPCTypeID, file, "$");
	newXSproto(strcpy(buf, "HasSpell"), XS_NPC_HasSpell, file, "$$");
	newXSproto(strcpy(buf, "HasSpellList"), XS_NPC_HasSpellList, file, "$$");
	newXSproto(strcpy(buf, "HasSpellGroupOmit"), XS_NPC_HasSpellGroupOmit, file, "$$");
