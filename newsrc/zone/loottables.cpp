	...
	*plat = 0;
	...
	for (int32 i=0; i<lts->NumEntries; i++) {
		for (int32 k = 1; k <= lts->Entries[i].multiplier; k++) {
			...
			uint16 probability = lts->Entries[i].probability;
		
			if ( npc->respawn2 && RuleB(Zone, LootMod) && lts->Entries[i].probability < RuleI(Zone,LootModChanceBelow) )
		{
			probability = (int)((float)probability + (float)probability * (float)log((float)npc->respawn2->getDeathCount() + (float)RuleR(Zone,LootModLogBase)) - (float)RuleR(Zone,LootModGraphBase));
		}

			if ( MakeRandomInt(0,99) < probability) {
...
// in another table.
	...
	int high = lds->NumEntries-1;

	k = MakeRandomInt(0, high);
	...
	while(!found) {
		...
		if ( loops == 0 && lds->NumEntries > 1 && npc->HasItem(lds->Entries[k].item_id) )
		{
			k++;
			continue;
		}
		...
		if ( npc->respawn2 && RuleB(Zone, LootMod) )
		{
			
			int rndChance = MakeRandomInt(0,RuleI(Zone,CorrectLootMaxRandom));
			if ( rndChance < RuleI(Zone,CorrectLootDropChance) )
			{
				if ( thischance < RuleI(Zone,LootModChanceBelow) )
				{
					uint32 fourTimesOrig = lds->Entries[k].chance*RuleI(Zone,LootModMaxMultiplier);
					thischance = (uint32)((float)thischance + (float)thischance * (float)log((float)npc->respawn2->getDeathCount() + (float)RuleR(Zone,LootModLogBase)) - (float)RuleR(Zone,LootModGraphBase));
					if ( thischance > 100 )
						thischance = 99;
					else if ( thischance > fourTimesOrig )
						thischance = fourTimesOrig;
				}
				else if ( thischance >= RuleI(Zone,LootModChanceAbove) )
				{
					uint32 halfOrig = (uint32)((float)lds->Entries[k].chance* RuleR(Zone,LootModMinMultiplier));
					thischance = (uint32)((float)thischance * (1 / (float)log((float)npc->respawn2->getDeathCount() + (float)RuleR(Zone,LootModAboveLogBase))));
					if ( thischance < halfOrig )
						thischance = halfOrig;
				}
			}
		}										
		...
		unsigned int drop_chance = MakeRandomInt(0, totalchance-1);
			//printf("Drop chance for npc: %s, total chance:%i this chance:%i, drop roll:%i, item %i \n", npc->GetName(), totalchance, thischance, drop_chance, lds->Entries[k].item_id);
#if EQDEBUG>=11
	...
	if (equipit) {
		...
		*/
		...
		if(eslot != 0xFF) {
			...
			// Nov 4 2017 - Image EQPVP - Fixed equipSlot being set correctly for loot (this not being set was breaking the display of wear change)
			item->equipSlot = Inventory::CalcSlotFromMaterial(eslot);	
		}
	}
...
void NPC::AddItem(int32 itemid) {
	//slot isnt needed, its determined from the item.
	const Item_Struct * i = database.GetItem(itemid);
	if(i == NULL)
		return;
	AddLootDrop(i, &itemlist, i->MaxCharges, true, true);
}
...
void NPC::AddRandomLootTable() {
	if (npctype_id != 0) { // check if it's a GM spawn
		
		for(int i=0;i<RandomLootGroups.size();i++)
		{
			uint32 lootGroupID = RandomLootGroups[i];
			RandomLootDropResult_Struct rld;
			memset(&rld,0,sizeof(RandomLootDropResult_Struct));
			if ( database.GenerateRandomLootGroupResult(&rld,lootGroupID,(char*)GetOrigName() ) )
			{
			copper += rld.coinDropValue; // Kings & Bandits - TODO: Breakdown to copper/silver/gold/platinum?  This may not work automatically.
			for(int z=0;z<rld.itemDropCount;z++)
			{
				uint32 itemID = rld.items[z].itemID;
				if ( itemID > 0 )
				{
					const Item_Struct* itm = database.GetItem(itemID);
					AddItem(itm,itm->MaxCharges,true);
				}
			}
			}
		}
