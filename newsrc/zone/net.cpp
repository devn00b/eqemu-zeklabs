...
extern volatile bool ZoneLoading;
#ifdef SHAREMEM
...
#include "../KingsBandits/KBFunctions.h"
...
int32				numclients = 0;
uint32				staticZones = 0;
uint32				bootedZones = 0;
uint32				availZones = 0;
#ifdef CATCH_CRASH
int8			error = 0;
#endif
...
int main(int argc, char** argv) {
	// Kings & Bandits - fix loot corpses decay being empty in variables
	memset(&npcCorpseDecayTimes,0,sizeof(npcDecayTimes_Struct));
	...
	*/
	...
	_log(ZONE__INIT, "Loading Random Drop Rates");
	if (!database.LoadRandomDropRates()) {
		_log(ZONE__INIT_ERR, "Loading Random Drop Rates FAILED!");
		_log(ZONE__INIT, "Failed.  But ignoring error and going on...");
	}

	_log(ZONE__INIT, "Loading Random Loot Drops & Items");
	if (!database.LoadRandomLootDrops()) {
		_log(ZONE__INIT_ERR, "Loading Random Loot Drops FAILED!");
		_log(ZONE__INIT, "Failed.  But ignoring error and going on...");
	}

	_log(ZONE__INIT, "Loading Random Name Groups");
	if (!database.LoadRandomNameGroups()) {
		_log(ZONE__INIT_ERR, "Loading Random Name Groups FAILED!");
		_log(ZONE__INIT, "Failed.  But ignoring error and going on...");
	}

	/*
	RandomLootDropResult_Struct rldr;
	memset(&rldr,0,sizeof(RandomLootDropResult_Struct));
	database.GenerateRandomLootGroupResult(&rldr,7);*/
#ifdef ADDONCMD	
...
#endif
#endif
	const ZoneShortName_Struct* zsn = NULL;

	if (!strlen(zone_name) || !strcmp(zone_name,".")) {
		_log(ZONE__INIT, "Entering sleep mode");
	} else 
		zsn = database.GetZoneInfo(zone_name);

	if (zsn != NULL && !Zone::Bootup(zsn->zoneID, zsn->instanceID, true)) { //todo: go above and fix this to allow cmd line instance
	...
	while(RunLoops) {
		{	//profiler block to omit the sleep from times
			...
			if (!eqsf.IsOpen() && Config->ZonePort!=0) {
				...
				if (!eqsf.Open((char*)ipaddr.c_str(), Config->ZonePort)) {
		...
		{
			_log(ZONE__INIT, "DBASync Failure in net.cpp...");
		}

		if ( ZoneLoaded || ZoneLoading )
			Sleep(ZoneTimerResolution);
		else
			Sleep(LongZoneTimerResolution);
	}
...
void BringZoneDown()
{
		...
		{
			char msg[256];
			_snprintf(msg,256,"-e \"UPDATE zone set isup=0,static_zone=0 where short_name='%s'\"",zone->GetShortName());
			execl("mysql.exe", "-uzonecrash", "-pdkl2lLZ", msg, "rzserver");
		}
}
