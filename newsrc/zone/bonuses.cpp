...
{
	memset(&aabonuses, 0, sizeof(StatBonuses));	//start fresh
	...
	if ( IsNPC() )
		CastToNPC()->CalcNPCRegen(spellbonuses.HPRegen,spellbonuses.ManaRegen);
...
void Client::AddItemBonuses(const ItemInst *inst, StatBonuses* newbon, bool isAug, bool isTribute) {
	...
	if (item->Focus.Effect>0 && (item->Worn.Type == ET_Focus)) { // focus effects
...
void Mob::CalcSpellBonuses(StatBonuses* newbon)
{
	...
	for(i = 0; i < buff_count; i++) {
		if(buffs[i].spellid != SPELL_UNKNOWN)
			ApplySpellsBonuses(buffs[i].spellid, buffs[i].casterlevel, newbon, buffs[i].casterid, false, buffs[i].ticsremaining); // Kings & Bandits - EQEMu R1880 - spell additions + ticsremaining
	}
...
// Kings & Bandits - EQEMu R1880 - spell additions + ticsremaining ApplySpellsBonuses (new ticsremaining field)
void Mob::ApplySpellsBonuses(int16 spell_id, int8 casterlevel, StatBonuses* newbon, int16 casterId, bool item_bonus, int32 ticsremaining)
{
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		// Kings & Bandits - hardcode a caster level check on spell bonuses
		if ( casterlevel > 70 )
			casterlevel = 70;
		else if ( casterlevel < 1)
			casterlevel = 1;

		 // Kings & Bandits - EQEMu R1880 - CalcSpellEffectValue + ticsremaining
		effect_value = CalcSpellEffectValue(spell_id, -1, i, casterlevel, caster, ticsremaining);
		...
		switch (spells[spell_id].effectid[i])
		{
			...
			{
				newbon->amplificationMod += effect_value;
				break;
			}
