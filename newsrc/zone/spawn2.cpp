...
: timer(100000), resetDeathTimer(21600000)
{
	...
	resetDeathTimer.Start(RuleI(Zone,WipeSpawnDeathCountMinutes)*60000); // 60000 ms in one minute
}
...
bool Spawn2::Process() {
	...
	if ( resetDeathTimer.Check() ) {
		setDeathCount(getDeathCount()/2);
	}
	...
	if (timer.Check())	{
		...
		int32 npcid = sg->GetNPCType(getDeathCount());
		if (npcid == 0) {
		...
		// now that we set the spawngroup id we can check to load all the proper loot groups
		npc->LoadRandomLootGroups();
...
void Spawn2::Disable(bool depoponly)
{
	if(npcthis)	
	{
		npcthis->Depop();
...
void Spawn2::DeathReset(float modRespawnTime)
{
	...
	if ( modRespawnTime > 0.0f )
		cur = (int)((float)cur * modRespawnTime);

	//set our timer to our reset local
	timer.Start(cur);
	
	resetDeathTimer.Start(RuleI(Zone,WipeSpawnDeathCountMinutes)*60000, true); // 60000 ms in one minute
...
bool ZoneDatabase::PopulateZoneSpawnList(int32 zoneid, int32 instance_id, LinkedList<Spawn2*> &spawn2_list, int16 version, int32 repopdelay) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	
	const char *zone_name = database.GetZoneName(zoneid, instance_id);
...
bool ZoneDatabase::ReloadSpawn2(Spawn2* sp2) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;

	if (RunQuery(query, MakeAnyLenString(&query, "SELECT x, y, z, heading FROM spawn2 WHERE id=%i", sp2->GetID()), errbuf, &result))
	{
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
	...
	LogFile->write(EQEMuLog::Error, "Error in ReloadSpawn2 query '%s': %s", query, errbuf);
	safe_delete_array(query);
	return false;
}
...
int32 Zone::DumpSpawn2(ZSDump_Spawn2* spawn2dump, int32* spawn2index, Spawn2* spawn2) {
	...
	while(iterator.MoreElements())
	{
		if (iterator.GetData() == spawn2) {
			...
			if ( iterator.GetData()->Enabled() && !iterator.GetData()->isTimerEnabled() )
				spawn2dump[*spawn2index].time_left = 0xFFFFFFFF;
			else
