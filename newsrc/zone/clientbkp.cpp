...
bool Client::Save(int8 iCommitNow, bool online, bool saveBackup) {
	...
	if ( saveBackup )
	{
		...
		database.UpdateCharacterStats(CharacterID(), m_pp.STR, m_pp.STA, m_pp.DEX, m_pp.AGI, m_pp.INT, m_pp.WIS, m_pp.CHA,m_pp.cur_hp,GetMaxHP(), m_pp.mana, GetMaxMana(), m_pp.endurance, GetMaxEndurance(), m_pp.timePlayedMin, (int)online);
	}
	m_pp.x = x_pos;
	m_pp.y = y_pos;
	m_pp.z = z_pos;
	...
	for (int i=0; i < buff_count; i++) {
		if (buffs[i].spellid != SPELL_UNKNOWN) {
			...
			if ( buffs[i].player_id == 0 )
				m_pp.buffs[i].player_id = 1000000;
...
float Client::CalcCityMod(Mob* iOther, bool reverse)
{
	...
	if (iOther && iOther->IsNPC())
	{
		...
		case 8: //freportn
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(FREEPORTTEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == FREEPORTTEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 44 ) ) )
		...
		case 29: // halas
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(HALASTEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == HALASTEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 90 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
		...
		case 2: // qeynos2
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(QEYNOSTEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == QEYNOSTEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 71 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
		...
		case 19: // rivervale
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(RIVERVALETEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == RIVERVALETEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 81 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
		...
		case 129: // icewell keep
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(THURGADINTEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == THURGADINTEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 183 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
		...
		case 62: // felwitheb
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(FELWITHETEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == FELWITHETEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 5 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
		...
		case 24: // erudnext
			{
				TeamSettings_Struct* tss = database.GetTeamSettings(ERUDINTEAMID);

				if ( !tss )
					break;
				// Class check, 20+ is GM Class/Banker/Merchant/etc.					// hardcode race check, no better way right now, may be missing a fewn pc's
				if ( ( tss->guildid > 0 && tss->guildid == GuildID() ) || ( GetPVPValue() == ERUDINTEAMID && ( iOther->CastToNPC()->GetClass() > 19 || iOther->CastToNPC()->GetRace() == 3 ) ) )
					formulamod = (float)tss->citizenTaxRate;
				else
					formulamod = (float)tss->newbieTaxRate;
...
uint32 Client::UpdatePVPKillStats(Client* deadplayer, bool* legitKillPtr)
{
	...
	strncpy(m_pp.PVPLastKill.Name,deadplayer->GetName(),64);
	m_pp.PVPLastKill.Level = deadplayer->GetLevel();
	m_pp.PVPLastKill.Race = deadplayer->GetRace();
	...
	m_pp.PVPLastKill.Zone = zone->GetZoneID();
	m_pp.PVPLastKill.Class = deadplayer->GetClass();

	char tmp[20];
	database.GetVariable("ServerType", tmp, 9);
	int pvpFlag = atoi(tmp);

	if ( (deadplayer->GuildID() > 0 && GuildID() > 0 && deadplayer->GuildID() < GUILD_NONE && GuildID() < GUILD_NONE &&
		deadplayer->GuildID() == GuildID()) || ( ( pvpFlag == SERVERTYPE_KAB || pvpFlag == SERVERTYPE_ZEK ) && GetPVPValue() < 5 && GetPVPValue() == deadplayer->GetPVPValue()) )
	{
		if ( legitKillPtr )
			*legitKillPtr = false;
	...
	else if ( deadplayer->CastToClient()->GetIP() == GetIP() )
	{
		if ( legitKillPtr )
			*legitKillPtr = false;
	...
	for ( int i=0;i<50;i++)
	{
		if ( !strcasecmp(m_pp.PVPRecentKills[i].Name,deadplayer->GetName()) )
		{
			if ( time > 0 && ( time -  m_pp.PVPRecentKills[i].Time ) < EARNKILLS_DELAY )
				foundDeadPlayer++;
		}
	...
	{
		Message(13,"You recently killed %s and thus will not earn a pvp kill.",deadplayer->GetName());
		legitKill = false;
		addDeathKill = false;
	...
	else
	{
		...
		sint32 lvlDiff = deadplayer->GetLevel() - GetLevel();
		uint32 lowChance = 0;
		uint32 highChance = 0;
		...
		if ( lvlDiff > 0 )
		{
			if ( lvlDiff > 10 )
			{
				Message(13,"No points will be earned as %s was too high of level.",deadplayer->GetName());
				highChance = 0;
				lowChance = 0;
				lvlDiff = 0;
				legitKill = false;
		...
		else
		{
			...
			if ( lvlDiff < 1 )
			{
				...
				uint32 lvlChance = GetLevel()/5;
				highChance = ((uint32)(float)lvlChance * randChance) + 1;
				lowChance = ((uint32)(float)lvlChance * randChance * randomFormula) + 1;
			}
			else if ( lvlDiff > 10 )
			{
				Message(13,"No points will be earned as %s was too low of level.",deadplayer->GetName());
				highChance = 0;
				lowChance = 0;
				legitKill = false;
				addDeathKill = false;
			...
			{
				uint32 base = (deadplayer->GetLevel() + GetLevel())/4;
				if ( lvlDiff < 4 )
					lvlDiff = base/3;
				else if ( lvlDiff < 2 )
					lvlDiff = base/2;
		...
		// Unless this is Kings & Bandits (server type 2) we reject points.
	...
	{
		// write the info!
		strcpy(m_pp.PVPRecentKills[bestReplaceID].Name,deadplayer->GetName());
		m_pp.PVPRecentKills[bestReplaceID].Level = deadplayer->GetLevel();
		m_pp.PVPRecentKills[bestReplaceID].Race = deadplayer->GetRace();
		m_pp.PVPRecentKills[bestReplaceID].Time = time;
		m_pp.PVPRecentKills[bestReplaceID].Zone = zone->GetZoneID();
		m_pp.PVPRecentKills[bestReplaceID].Class = deadplayer->GetClass();
...
void Client::UpdatePVPStats(Client* attacker, bool* legitKill)
{
	...
	uint32 time = (uint32)now;
	strcpy(m_pp.PVPLastDeath.Name,attacker->GetName());
	m_pp.PVPLastDeath.Level = attacker->GetLevel();
	m_pp.PVPLastDeath.Race = attacker->GetRace();
	m_pp.PVPLastDeath.Time = time;
	m_pp.PVPLastDeath.Zone = zone->GetZoneID();
	m_pp.PVPLastDeath.Class = attacker->GetClass();
	...
	if (worldserver.Connected())
	{
		...
		{
			loserName = KingsBandits::GetKingdomName(this->GetPVPValue());
			winnerName = KingsBandits::GetKingdomName(attacker->GetPVPValue());
			snprintf(msg,256,"<%s> %s of The %s has been slain by %s of The %s!",zone->GetLongName(),this->GetName(),loserName,attacker->GetName(),winnerName);
		}
		else
		{
...
void Client::WipeCharacter()
{
	...
	for(SkillType skill_num=_1H_BLUNT;skill_num <= HIGHEST_SKILL;skill_num=(SkillType)(skill_num+1)) {
		CastToClient()->SetSkill(skill_num, 1);
	}
	...
	memset(&cc, 0, sizeof(CharCreate_Struct));
