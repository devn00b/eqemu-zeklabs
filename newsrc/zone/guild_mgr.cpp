...
void ZoneGuildManager::ProcessWorldPacket(ServerPacket *pack) {
	...
	if(ZoneLoaded && zone && zone->IsGuildManagementDisabled()) {
		return;
	}
...
void ZoneGuildManager::ZoneCancelByOwner(Client* owner)
{
	LinkedListIterator<GuildApproval*> iterator(list);
	...
	while(iterator.MoreElements())
	{
		for(int i=0;i<6;i++)
		{
			if ( iterator.GetData()->GetMember(i) == owner )
				iterator.GetData()->SetMember(i,NULL);
		}
		iterator.Advance();
	}
...
void GuildApproval::GuildApproved()
{
	...
	if ( owner->GuildID() != GUILD_NONE )
	{
		owner->Message(13,"You are already in a guild.");
		return;
	}
	...
	guild_mgr.SetGuild(owner->CharacterID(),tmpeq,0); // set them into the guild
	guild_mgr.SetGuildRank(owner->CharacterID(), GUILD_LEADER); // put the guild rank to leader
