...
public:
	...
	virtual void Damage(Mob* from, sint32 damage, int16 spell_id, SkillType attack_skill, uint8 itemType, 
		bool avoidable = true, sint8 buffslot = -1, bool iBuffTic = false, bool isDmgShield = false, bool isCritHit = false, int32 player_id=0);
	...
	void ApplyPVPMitigation(Client* attacker, SkillType attack_skill, uint8 itemType, sint32 &damage, int16 spell_id=SPELL_UNKNOWN, bool isDmgShield=false);
...
//	void	Discipline(ClientDiscipline_Struct* disc_in, Mob* tar);
	...
	void	AddAchievement(int32 achievementid);
	bool	CheckAchievements(int32 achievementID);
	...
	void PutItemInOpenSlot(int32 itemID, sint32 charges);
	...
	*/
	...
	void CheatDetected(CheatTypes CheatType, float x, float y, float z, float destx=0, float desty=0, float destz=0, int32 destzoneid=0, int32 zonereasonid=0, int32 targetzoneid=0);
	...
	Mob* GetAttackTarget() { if ( flushattacktarget_timer.Check() ) attackTarget = NULL; return attackTarget; }
	void SetAttackTarget(Mob* target) { flushattacktarget_timer.Start(30000, true); attackTarget = target; }
