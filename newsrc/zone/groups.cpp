...
bool Group::AddMember(Mob* newmember) {
	...
	//see if they are allready in the group
	 for (i = 0; i < MAX_GROUP_MEMBERS; i++) {
		if(!strcasecmp(membername[i], newmember->GetName()))
		{
			// KaB / Red69 / Zek - Oct 1st changed in attempt to try to resolve grouping issues
			members[i] = newmember;
			skipAdd = true;
			break;
		}
...
void Group::SendHPPacketsTo(Mob *member)
{
	if(member && member->IsClient())
	{
		...
		for (uint32 i = 0; i < MAX_GROUP_MEMBERS; i++)
		{
			...
			{
				EQApplicationPacket* hpapp = new EQApplicationPacket();
...
bool Group::UpdatePlayer(Mob* update){
	...
	if(update->IsClient()) {
		...
		for (i = 0; i < MAX_GROUP_MEMBERS; i++) {
			LogFile->write(EQEMuLog::Status, "%s: UpdatePlayer for group id %i, player %s position %i.", update->GetName(), GetID(), membername[i],i);
			if(membername[i][0] == '\0')
	...
	LogFile->write(EQEMuLog::Error, "Failed to UpdatePlayer for group %lu, character: %s", (unsigned long)GetID(), update->GetName());
	return false;
}
...
bool Group::DelMember(Mob* oldmember,bool ignoresender, bool learnMembers)
{
	if (oldmember == NULL){
		return false;
	}
...
bool Group::LearnMembers() {
	...
	char ln[64];
	memset(ln, 0, 64);
	strcpy(ln, database.GetGroupLeadershipInfo(GetID(), ln));
	strn0cpy(membername[0],ln,64);
	members[0] = entity_list.GetClientByName(ln);

	// Image (bug #2017-1): Addressing issues with group boolean flag not being properly assigned
	if ( members[0] )
		members[0]->SetGrouped(true);
	...
	if (database.RunQuery(query,MakeAnyLenString(&query, "SELECT name FROM group_id WHERE groupid=%lu", (unsigned long)GetID()),
		...
		std::list<string> newMembers;
		bool group_fields[MAX_GROUP_MEMBERS] = {true,false,false,false,false,false};
		while((row = mysql_fetch_row(result))) {
			if(!row[0] || !strcasecmp(membername[0],row[0]))
				continue;

			bool memberExists = false;
			for(int z=1;z<MAX_GROUP_MEMBERS;z++)
			{
				if ( !strcasecmp(membername[z],row[0]) )
				{
					memberExists = true;
					group_fields[z] = true;
					break;
			...
			newMembers.push_back((string)name);
		}
		...
		for(int z=1;z<MAX_GROUP_MEMBERS;z++)
		{
			if ( !group_fields[z] )
			{
				memset(membername[z],0,64);
				members[z] = NULL;
			}
		}

		if(!newMembers.empty()) {
			for(list<string>::iterator itr = newMembers.begin(); itr != newMembers.end(); itr++) {
				for(int z=1;z<MAX_GROUP_MEMBERS;z++)
				{
					if ( membername[z][0] == '\0' )
					{
						strn0cpy(membername[z], (*itr).c_str(), 64);
						members[z] = NULL;
						break;
					}
				}
