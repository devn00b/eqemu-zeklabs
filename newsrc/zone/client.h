...
#define GROUND_CLICK_RANGE 20	// range for ground pickup
extern Zone* zone;
extern TaskManager *taskmanager;


#define INFAMY_LIMIT  999998
#define MAX_PLAYERID 1000000
class CLIENTPACKET
{
public:
    CLIENTPACKET();
...
typedef enum {
	...
	MQBuffExploitRoot=8,
	MQBuffExploitStun=9,
	MQBuffExploitSeeInvis=10,
	MQBuffExploitBindSight=11,
	MQBuffExploitTargetRange=12
} CheatTypes;
...
public:
	...
	virtual void Damage(Mob* from, sint32 damage, int16 spell_id, SkillType attack_skill, uint8 itemType, 
		bool avoidable = true, sint8 buffslot = -1, bool iBuffTic = false, bool isDmgShield = false, bool isCritHit = false, int32 player_id=0, bool isDoT=false);
	...
	// Image (bug #2017-1): Addressing issues with group boolean flag not being properly assigned
	...
	void ApplyPVPMitigation(int8 level, int8 player_class, SkillType attack_skill, uint8 itemType, sint32 &damage, int16 spell_id=SPELL_UNKNOWN, bool isDmgShield=false, bool isDoT=false);
...
//	void	Discipline(ClientDiscipline_Struct* disc_in, Mob* tar);
	...
	void	GiveSoulBoundItems();

	void	LoadAchievements();
	void	AddAchievement(int32 achievementid, int32 zoneid, int32 isrepeatable=0, int32 cooldown_seconds=0, int8 incooldown=0);
	bool	CheckAchievements(KBAchievement* kb, int32 curTime);
	void	UpdateAchievements();

	virtual bool IsClient() const { return true; }

	// Kings & Bandits - shroud implementation
	virtual bool IsShroudNPC() const { return shroudnpc; }

	virtual void DBAWComplete(int8 workpt_b1, DBAsyncWork* dbaw);
	bool	FinishConnState2(DBAsyncWork* dbaw);
	void	CompleteConnect();
	bool	AsyncHandleAsyncInsert(DBAsyncWork* dbaw);
	bool	AsyncLoadMsgQueue(DBAsyncWork* dbaw);
	bool	AsyncLoadCharacterLevelUpdate(DBAsyncWork* dbaw);
	...
	void	MakeDyePacket(int r, int g, int b);
	void	DyeArmor(DyeStruct* dye, bool skipItemRequirement=false);
	...
	void    SendZonePoints();
	void	SendEmptyMerchantResponse();
	// KaB - Red69 - Zek / Aug 22 2012.  Rework on trade/quest turn in of items
	bool	HandleTradeGuardCards(Mob* with);
	bool	StoreTurnInItems(Mob* with, bool successTurnin=true);
	...
	void	LogLDONMerchant(Client* player, Mob* merchant, int32 quantity, sint32 price, const Item_Struct* item, bool buying);
	...
	void	FastQueuePacket(EQApplicationPacket** app, bool ack_req = true, bool deletePacket=true, CLIENT_CONN_STATUS = CLIENT_CONNECTINGALL);
	...
	void	SendQuestSound(uint16 entity_id, uint32 copper, uint32 silver, uint32 gold, uint32 platinum, int32 item_id=0, uint32 exp=0);
	void	PlayMP3(const char* fname);

	void	LearnRecipe(uint32 recipeID);

	int		sanitizeCharArray(char* inArray, char* outData, int maxLength);
	...
	void			SetHideMe(bool hm, bool isPlayer=false);
	void			SetGMToggle(bool turntellsoff);
	...
	virtual bool	Save() { return Save(0); }
			bool	Save(int8 iCommitNow, bool online=true, bool saveBackup=true); // 0 = delayed, 1=async now, 2=sync now
			void	SaveBackup();

	inline bool ClientDataLoaded() const { return client_data_loaded; }
	inline bool ZonedInLinkdead() const { return linkdead_zonein; }
	...
	void	WorldKick();
	void	RefreshPVPFlag(Client* to=NULL);

	void GarbleMessage(char *message, uint32 variance);
	inline uint32	GetIntoxication()		const {  return m_pp.intoxication; }
	...
	void	SetPVP(uint8 value);
	
	//inline bool	GetPVP()	const { return (bool)m_pp.pvp; }
	inline bool	GetGM()		const { return (bool) m_pp.gm; }
	
	// Kings & Bandits
	// Use the core PVP value for a team based system
	// 1 = pvp free for all - no alignment
	// 2 = kingdom 1
	// 3 = kingdom 2, ... etc
	uint8	GetPVPValue() { return m_pp.pvp; }
	uint8	GetPVP() { return m_pp.pvp; }
	bool	IsKing() { return isKing; }
	void	SetIsKing(bool value) { isKing = value; }

	inline uint32	GetPVPKills() const { return m_pp.PVPKills; }
	inline uint32	GetPVPDeaths() const { return m_pp.PVPDeaths; }
	inline uint32	GetPVPCareerPoints() const { return m_pp.PVPCareerPoints; }
	inline uint32	GetPVPCurrentPoints() const { return m_pp.PVPCurrentPoints; }
	inline uint32	GetBestKillStreak() const { return m_pp.PVPBestKillStreak; }
	inline uint32	GetWorstDeathStreak() const { return m_pp.PVPWorstDeathStreak; }
	inline uint32	GetCurrentKillStreak() const { return m_pp.PVPCurrentKillStreak; }
	
	void SetPVPKills(uint32 newValue) { m_pp.PVPKills = newValue; }
	void SetPVPDeaths(uint32 newValue) { m_pp.PVPDeaths = newValue; }
	void SetPVPCareerPoints(uint32 newValue) { m_pp.PVPCareerPoints = newValue; }
	void SetPVPCurrentPoints(uint32 newValue) { m_pp.PVPCurrentPoints = newValue; }
	void SetBestKillStreak(uint32 newValue) { m_pp.PVPBestKillStreak = newValue; }
	void SetWorstDeathStreak(uint32 newValue) { m_pp.PVPWorstDeathStreak = newValue; }
	void SetCurrentKillStreak(uint32 newValue) { m_pp.PVPCurrentKillStreak = newValue; }
	

	int PutItemInOpenSlot(int32 itemID, sint32 charges, int32 augslot1=0, int32 augslot2=0, int32 augslot3=0, int32 augslot4=0, int32 augslot5=0);
	
	void SetLastKillInfo(uint32 time, char* defenderName, uint32 defenderLevel, uint32 defenderRace, uint32 zoneID, uint32 defenderClass);
	void SetLastDeathInfo(uint32 time, char* attackerName, uint32 attackerLevel, uint32 attackerRace, uint32 zoneID, uint32 attackerClass, uint32 ptsEarned);
	bool DoPVPKillSanityChecks(uint32 attackGuildID, uint32 defendGuildID, uint8 attackPVPValue, uint8 defendPVPValue, uint32 defendIP, uint32 attackIP, 
	...
	void UpdatePVPStats(Client* attacker, bool* legitKill);
		uint32 UpdatePVPKillStats(	char* deadName, uint8 deadLevel, uint32 deadRace, uint32 zoneID, uint8 deadClass_, int32 deadGuildID, int8 deadPVPValue, int32 deadPlayerIP, 
								char* lastAttackerName, int32 deadLastTime, int8 bountySet, uint32 deadCharID, int32 groupID, char* deadAccountName, int32 deadPlayerKillCount, int32 deadPlayerDeathCount, 
								bool* legitKillPtr, bool* addDeathKillPtr);
	uint32 UpdatePVPKillStats(Client* deadplayer, bool* legitKillPtr=0);

	inline void	SetAnon(uint8 value) { m_pp.anon = value; }
	...
	*/
	...
	sint32 GetActSpellCasttime(int16 spell_id, sint32, sint32&);
	...
	void	SetPVPPoints(uint32 Points) { m_pp.PVPCurrentPoints = Points; SendPVPStats(); }
	int32	GetPVPPoints() { return m_pp.PVPCurrentPoints; }
	bool	AddPVPPoints(sint32 Points, bool modifyCareerPts=true);
	...
	void	Gate();
	void	SetBindPoint(int to_zone = -1, float new_x = 0.0f, float new_y = 0.0f, float new_z = 0.0f, bool zonepointCheck=true);
	...
	bool	CheckLoreConflict(uint32 itemID);
	...
	bool	GuildRemoveAllowed(int32 char_id);
	...
	virtual uint16 GetSkill(SkillType skill_id) const { if (skill_id <= HIGHEST_SKILL) { return((itembonuses.skillmod[skill_id] > 0)? m_pp.skills[skill_id]*(100 + (uint16)itembonuses.skillmod[skill_id])/100 : m_pp.skills[skill_id]); } return 0; }
	...
	uint32	NukeItem(uint32 itemnum, uint32 count=0);
	...
	void	SummonItem(uint32 item_id, sint8 charges = 0, uint32 aug1=0, uint32 aug2=0, uint32 aug3=0, uint32 aug4=0, uint32 aug5=0, bool attuned=false, uint16 to_slot=SLOT_CURSOR, bool suppressLore=false);
	...
	void	SendFailPickPocket();

	void	LogPlayerMessage(int32 eventTypeID, char* eventDesc, char* eventText, Mob* target=0);
	...
	// Kings & Bandits additions for city mod
	float	CalcCityMod(Mob* iOther, bool reverse = false);
	float	DetermineCityMod(Mob* iOther, int teamID, int classRangeStart, int raceID);
	void	AddCityRevenue(uint32 value);
	...
	// if they attempt another zoning 'event' while one takes place this will wait and do it in the next process
	std::list<QueuedZoneEvent> zone_event_list;

	//Anti-Cheat Stuff
	int32 m_TimeSinceLastPositionCheck;
	double m_DistanceSinceLastPositionCheck;
	...
	void CheatDetected(CheatTypes CheatType, float x, float y, float z, float destx=0, float desty=0, float destz=0, int32 destzoneid=0, int32 zonereasonid=0, int32 targetzoneid=0, double naturalRunSpeed=0.0, double actualRunSpeed=0.0 );
	...
	void SendBuffDurationPacket(int16 spell_id, int duration, int inlevel, int slotid=0, int slot=2, uint32 counters=0, bool quickQueue=true);
	...
	void ResetDiscTimer(int32 timerID, int32 timeRemaining,int32 unknown);
	...
	QGlobalCache *GetQGlobals() {
		if ( !qGlobals ) {
			QGlobalCache* cache = CreateQGlobals();
		cache->LoadByCharID(CharacterID());	
		}
	return qGlobals;
	}
	...
	// Red69 / Zek / KaB - Sep 28 2012 - Modified SetPendingRezzData to take in the Resurrect_Struct from the other zone to properly set the client destination for a rez.
	void SetPendingRezzData(int XP, int16 SpellID, const char *CorpseName, Resurrect_Struct* resStr) {
		PendingRezzXP = XP;
		PendingRezzSpellID = SpellID;
		PendingRezzCorpseName = CorpseName;
		if ( resStr != NULL )
			memcpy(&resStruct,resStr,sizeof(Resurrect_Struct));
		else
			memset(&resStruct,0,sizeof(Resurrect_Struct));
	}
	...
	// Kings & Bandits - Shroud client functionality

	bool OpenShroudWindow(Mob* npc);
	int16 GetAttackTargetID() { return attackTargetID; }
	bool IsInitiatorOfCombat() { return initiatedCombat; }
	Mob* GetAttackTarget() { if ( flushattacktarget_timer.Check() ) {  attackTarget = NULL; attackTargetID = 0; initiatedCombat = false; } return attackTarget; }
	void SetAttackTarget(Mob* target, bool repeat=true, bool initiator=true) {
		if ( target != NULL )
		{
			// both sides are engaged now!
			if ( repeat && target->IsClient() && target->CastToClient()->GetAttackTarget() == NULL )
				target->CastToClient()->SetAttackTarget(this,false,false);

			flushattacktarget_timer.Start(30000, true);
	...
	ZoneMode GetZoneMode() { return zone_mode; }
	void SetLevitateFlag(uint8 value) { Levitate = value; }
	uint8 GetLevitateFlag() { return Levitate; }
	void RecordReplayFrame();


	bool isDespawningFromZone() { return isDespawning; } // if the client is leaving the zone this is true

	void LoadMessageQueue(bool petitionCheck=false);

	void LoadCharacterLevelUpdates(int16 old_level);

	void InsertQueryAsync(char* query, int timeMS=0);

	void UpdateInfamyLevel(sint32 amount)
	{
		if ( GetGM() )
			amount = 0;

		prevInfamyLevel = infamyLevel;

		if ( IsBountySet() )
			infamyLevel = 999999;
		else
		{
			if ( ((sint32)infamyLevel + amount) < 0 )
				infamyLevel = 0;
			else
			{
				infamyLevel += amount;

				if ( IsBountySet() && infamyLevel > INFAMY_LIMIT)
					infamyLevel = INFAMY_LIMIT;
			}
		}
	}

	void CheckInfamyLevel(bool forceMessage=false)
	{
		if ( infamyDecayTimer.Check() )
			UpdateInfamyLevel(-RuleI(PVPCombat, InfamyDecayRate));

		if ( prevInfamyLevel != infamyLevel && infamyUpdateTimer.Check() )
		{
			prevInfamyLevel = infamyLevel;
			UpdateLeaderBoard();
			Message(13,"Your current infamy level is at %i.",infamyLevel);
		}
		...
		if ( !infamyReached )
		{
			...
			{
				// pvp all levels gets enabled here
				Message(13,"You have exceeded the infamy level allowed and are now flagged to be attacked by higher levels (Current Level: %i).",infamyLevel);
				infamyReached = true;
			}
		}
		else
		{
			if ( infamyLevel < RuleI(PVPCombat, InfamyAttackLevel) )
			{
				Message(13,"Your infamy level has dropped below the limit and is no longer flagged for attack from higher levels (Current Level: %i).",infamyLevel);
				infamyReached = false;
			}
		}
	}

	bool IsInfamyReached() { return infamyReached; }
	uint32 GetInfamyLevel() { return infamyLevel; }
	bool IsBountySet() { return bountySet; }
	void SetBounty(bool bounty) { bountySet = bounty; }

	void SetZoneLockoutID(uint32 id) { zoneLockoutID = id; }

	void SetShadowStepUpdate(bool v) { position_timer_counter = 0; m_ShadowStepUpdate = v; position_timer.Start(100, true); }
	bool IsShadowStepUpdate() { return m_ShadowStepUpdate; }

	bool IsCampTimerEnabled() { camp_timer.Enabled(); }
	void DisableCampTimer() { camp_timer.Disable(); }
	
	void SetItemEditID(int32 id) { editSelectedItem = id; }
	int32 GetItemEditID() { return editSelectedItem; }

	void AddClientCache(uint32 entityid) { cacheClientEntities.push_back(entityid); }
	void MakeBuffFadePacket(int16 spell_id, int slot_id, bool send_message = true);
	void SetGMSpeedFlag(uint8 val) { gmspeed = val; }

	void CheatSaveBuffs(CheatTypes CheatType);
	void ClearTarget();

	void SetExtProfileNPC(bool enableNPC, char* npcName, uint8 npcClass, uint8 npcRace);

	LinkedList<BulkZoneSpawnPacket*> GetBulkSpawnQueue() { return BzspQueue; }
	void AddToBulkQueue(BulkZoneSpawnPacket* ptr) { BzspQueue.Insert(ptr); }
protected:
	friend class Mob;
	...
	// OpenShroudWindow
	// true - opened shroud window for client
	// false - shroud window could not be opened (player is already a monster)

	void ResetUnderAttack() { underattack_timer.Start(UnderAttackTimerDelay,true); }
private:
	LinkedList<BulkZoneSpawnPacket*> BzspQueue;
	...
	std::list<KBAchievement> achievements;
	...
	bool				RemoteLoginServer;
	bool	Trader;
	bool	Buyer;
	string	BuyerWelcomeMessage;
	bool	AbilityTimer;

	// replay track 6/15 Image (Red69/LoZ/KaB)
	bool	ReplayEnabled;
	uint32	ReplayID;
	uint32  FrameID;
	Timer	ReplayTimer;
	float	RecordedSpeed;
	uint8	Levitate;
	uint32	infamyLevel; // 8/31
	uint32	prevInfamyLevel;
	bool	infamyReached; // level of infamy reached to be FFA LVL
	Timer	infamyDecayTimer;
	Timer	infamyUpdateTimer;
	bool	bountySet;
	uint32 zoneLockoutID; // zone lockout id for messaging updates
	...
	Timer	clientinzone_timer;
	Timer	forceposupdate_timer;
	Timer	checktellqueue_timer;
	Timer	cheattrigger_timer;
	...
	// Kings & Bandits
	bool isKing; // False unless they are king of their own flag group

	Mob* attackTarget; // last attack target
	int16 attackTargetID;
	bool initiatedCombat;
	int32 editSelectedItem;

	// client is leaving zone
	bool isDespawning;

	// client entities that were not available as the player zoned in
	std::list<uint32> cacheClientEntities;
