...
Corpse::Corpse(NPC* in_npc, ItemList* in_itemlist, int32 in_npctypeid, const NPCType** in_npctypedata, int32 in_decaytime, bool overrideDecay)
	...
	loot_cooldown_timer(10)
{
	...
	if(IsEmpty() && !overrideDecay)
	{
		corpse_decay_timer.SetTimer(RuleI(NPC,EmptyNPCCorpseDecayTimeMS)+1000);
	} // Kings & Bandits - decay override for time to add items
	else if (overrideDecay )
	{
		corpse_decay_timer.SetTimer(600000);
	}
...
// Mongrel: added see_invis and see_invis_undead
Corpse::Corpse(Client* client, sint32 in_rezexp, bool dropItems)
	// vesuvias - appearence fix
	: Mob
	(
	"Unnamed_Corpse",
	...
	loot_cooldown_timer(10)
{
	...
	strcpy(name, pp->name);
	corpselevel		= client->GetLevel();

	//become_npc was not being initialized which led to some pretty funky things with newly created corpses
	become_npc = client->IsBecomeNPC();

	SetPKItem(0);

	bool setCash = false;
	if ( GetLevel() >= RuleI(Character, DeathItemLossLevel) && !dropItems )
	{
		setCash = true;
	...
	if(!RuleB(Character, LeaveNakedCorpses) || 
		(( RuleB(Character, LeaveCorpses) || dropItems ) && GetLevel() >= RuleI(Character, DeathItemLossLevel)) ) {
		// cash
			if ( !setCash )
			{
				SetCash(pp->copper, pp->silver, pp->gold, pp->platinum);
		...
		for(i = 0; i <= 30; i++)
		{
			...
			if ( item )
			{
				// Kings & Bandits - remove soulbound items from corpses
				if((!strcasecmp(item->GetItem()->CharmFile,"SoulboundItem") ) )
				{
					if ( !client->KeyRingCheck(item->GetItem()->ID) )
						client->KeyRingAdd(item->GetItem()->ID);

					client->DeleteItemInInventory(i,1);
					removed_list.push_back(i);
				}
				else if ( become_npc && !strcasecmp(item->GetItem()->CharmFile,"DoNotDrop") )
				{
					continue;
				}
				else if ( item )
		...
		if(cursor) {
			list<ItemInst*>::const_iterator s=client->GetInv().cursor_begin(),e=client->GetInv().cursor_end();
...
void Corpse::RandomItemToCorpse(Client* client)
{
	RandomItemSteal(client,this);
}

uint32 Mob::RandomItemSteal(Client *client, Mob* receiver)
{
	int bagindex;
	ItemInst *item = NULL;
	sint16 equipslot = 0;
	...
	// non supported cases to the top
	if ( receiver && receiver->IsNPC() && !receiver->IsCorpse() )
	{
		client->Message(13,"[Mob::RandomItemSteal] Random item steal to NPC is not supported.");
		return 0;
	}
	if ( receiver && ( receiver->IsClient() || receiver->IsCorpse() ) )
		receiverExists = true; // supported cases would be triggered here
	else
	{	// no receiver provided, can't continue
		client->Message(13,"[Mob::RandomItemSteal] No correct receiver was defined, must be client or corpse.");
		return 0;
	}

	do
	{
		equipslot = MakeRandomInt(SLOT_PERSONAL_BEGIN,SLOT_PERSONAL_END);
		item = client->GetInv().GetItem(equipslot);
		retries++;

		if ( item != NULL )
		{		
			bool isWeapon = false;
				switch(item->GetItem()->ItemType){
				case ItemType1HS:
				case ItemType2HS:
				case ItemTypePierce:
				...
				// we won't drop weapons
				if ( isWeapon || (item && receiver && receiver->IsClient() && receiver->CastToClient()->CheckLoreConflict(item->GetItem()->ID)) )
					item = NULL;
				// make sure its not augmented or nodrop
				else if ( item && item->GetItem()->NoDrop && !item->IsAugmented() )
					break;
				else
					item = NULL;
		}

	}while(item == NULL && retries < 5);

	if ( item == NULL )
	{
		for(int i=SLOT_PERSONAL_BEGIN;i<SLOT_PERSONAL_END;i++)
		{
			item = client->GetInv().GetItem(i);
			equipslot = i;
			...
			if ( isWeapon || (item && receiver && receiver->IsClient() && receiver->CastToClient()->CheckLoreConflict(item->GetItem()->ID)) )
				item = NULL;
			else if ( item != NULL && item->GetItem()->NoDrop && !item->IsAugmented() )
				break;
			else
				item = NULL;
		}
	...
	if ( item == NULL )
	{
		mlog(INVENTORY__SLOTS, "%s: No appropriate random item found to put on corpse.",client->GetName());
		return 0;
	}

	sint16 interior_slot;
	ItemInst *interior_item = NULL;
	
	retries = 0;

	if(item->IsType(ItemClassContainer))
	{
		mlog(INVENTORY__SLOTS, "%s: Random container selected for corpse %s (%d) in slot %d charges %i", client->GetName(), item->GetItem()->Name, item->GetItem()->ID, equipslot, item->GetCharges());
		do
		{
			for(bagindex = MakeRandomInt(0,10); bagindex <= 10; bagindex++)
			{
				...
				bool isWeapon = false;
				if ( interior_item != NULL )
				{
					switch(interior_item->GetItem()->ItemType){
					case ItemType1HS:
					case ItemType2HS:
					case ItemTypePierce:
					case ItemType2HPierce:
				...
				// we won't drop weapons
				if ( isWeapon || (interior_item && receiver && receiver->IsClient() && receiver->CastToClient()->CheckLoreConflict(interior_item->GetItem()->ID)))
					interior_item = NULL;
				else if(interior_item && interior_item->GetItem()->NoDrop && !interior_item->IsAugmented())
				{
					char msg[256];
					char* link = 0;
					if ( (( receiver->IsClient() && RuleB(PVPCombat,DisplayPlayerStealItem) ) || ( receiver->IsCorpse() && RuleB(PVPCombat,DisplayCorpseDropItem) )) && 
						client->MakeItemLink(link, interior_item) )
					{
						snprintf(msg,256,"<%s> %s has lost [%c%s%s%c]!",zone->GetLongName(),client->GetName(), 0x12, link, interior_item->GetItem()->Name, 0x12);
						worldserver.SendEmoteMessage(0, 0, 13, msg);
					}
					mlog(INVENTORY__SLOTS, "%s: Putting random item on corpse %s (%d) into slot %d charges %i", client->GetName(), interior_item->GetItem()->Name, interior_item->GetItem()->ID, interior_slot, interior_item->GetCharges());
					if ( receiver->IsCorpse() )
						receiver->CastToCorpse()->AddItem(interior_item->GetItem()->ID, interior_item->GetCharges(), equipslot, interior_item->GetAugmentItemID(0), interior_item->GetAugmentItemID(1), interior_item->GetAugmentItemID(2), interior_item->GetAugmentItemID(3), interior_item->GetAugmentItemID(4));
					else if ( receiver->IsClient() )
						receiver->CastToClient()->SummonItem(interior_item->GetItem()->ID, interior_item->GetCharges(), interior_item->GetAugmentItemID(0), interior_item->GetAugmentItemID(1), interior_item->GetAugmentItemID(2), interior_item->GetAugmentItemID(3), interior_item->GetAugmentItemID(4));
					itemID = interior_item->GetItem()->ID;
					client->DeleteItemInInventory(interior_slot, 0, true, true);
					break;
				}
				else
					interior_item = NULL;
	...
	else
	{
		...
		char* link = 0;
		if ( (( receiver->IsClient() && RuleB(PVPCombat,DisplayPlayerStealItem) ) || ( receiver->IsCorpse() && RuleB(PVPCombat,DisplayCorpseDropItem) )) && 
			client->MakeItemLink(link, item))
		{
			snprintf(msg,256,"<%s> %s has lost [%c%s%s%c]!",zone->GetLongName(),client->GetName(), 0x12, link, item->GetItem()->Name, 0x12);
			worldserver.SendEmoteMessage(0, 0, 13, msg);
		}
		mlog(INVENTORY__SLOTS, "%s: Random item selected for corpse %s (%d) in slot %d charges %i", client->GetName(), item->GetItem()->Name, item->GetItem()->ID, equipslot, item->GetCharges());
		if ( receiver->IsCorpse() )
			receiver->CastToCorpse()->AddItem(item->GetItem()->ID, item->GetCharges(),  equipslot, item->GetAugmentItemID(0), item->GetAugmentItemID(1), item->GetAugmentItemID(2), item->GetAugmentItemID(3), item->GetAugmentItemID(4));
		else if ( receiver->IsClient() )
			receiver->CastToClient()->SummonItem(item->GetItem()->ID, item->GetCharges(), item->GetAugmentItemID(0), item->GetAugmentItemID(1), item->GetAugmentItemID(2), item->GetAugmentItemID(3), item->GetAugmentItemID(4));
		itemID = item->GetItem()->ID;
		client->DeleteItemInInventory(equipslot, 0, true, true);
	}
...
void Corpse::MoveItemToCorpse(Client *client, ItemInst *item, sint16 equipslot)
{
	...
	mlog(INVENTORY__SLOTS, "Putting item on corpse %s (%d) into slot %d charges %i", item->GetItem()->Name, item->GetItem()->ID, equipslot, item->GetCharges());
	...
	if(item->IsType(ItemClassContainer))
	{
		for(bagindex = 0; bagindex <= 10; bagindex++)
		{
			...
			if(interior_item)
			{
				if((item && !strcasecmp(item->GetItem()->CharmFile,"SoulboundItem") ) )
				{
					if ( !client->KeyRingCheck(item->GetItem()->ID) )
						client->KeyRingAdd(item->GetItem()->ID);
				}
				...
				{
					mlog(INVENTORY__SLOTS, "Putting item on corpse %s (%d) into slot %d charges %i", interior_item->GetItem()->Name, interior_item->GetItem()->ID, interior_slot, interior_item->GetCharges());
...
void Corpse::RemoveItem(ServerLootItem_Struct* item_data)
{
	...
	for(; cur != end; cur++) {
		...
		if (sitem == item_data)
		{
			pIsChanged = true;
			itemlist.erase(cur);
			sint16 slot = sitem->equipSlot;
			material = Inventory::CalcMaterialFromSlot(slot);
			if(material != 0xFF)
...
// KaB (Red69) - BecomeNPC always give loot 8-5-2011
	if ( become_npc )
		return true;
...
void Corpse::MakeLootRequestPackets(Client* client, const EQApplicationPacket* app) {
	...
	// Kings & Bandits - modify to not allow ppl on the corpse after an item is taken
	if ( IsPlayerCorpse() && !become_npc )
	{
		if (dbid == 0) {
			//		SendLootReqErrorPacket(client, 0);
		...
		else if((charid != client->CharacterID()) && GetPKItem()==0){
			SendLootReqErrorPacket(client, 0);
			// 4 = cannot loot while a hostile creature is  in your presence
			client->Message(13, "You are not allowed to loot from this corpse.");
			return;
		}
	}
	...
	if (pLocked && client->Admin() < 100) {
		SendLootReqErrorPacket(client, 0); // 0 = being looted by someone else
		client->Message(13, "Error: Corpse locked by GM.");
		return;
	}

	int32 curDistance = 0;
	// Kings & Bandits - check distance from corpse!
	int32 dist2 = 30000; // this is slightly higher than it should be, 500 is optimal, this is just in case
	if ((curDistance = DistNoRootNoZ(*client)) > dist2)
	{
		SendLootReqErrorPacket(client, 3); // too far away

		if ( curDistance > dist2*dist2 )
		{
			char text[128];
			_snprintf(text,128,"Bad Corpse Loot Atempt: A loot attempt at a distance of %i (%i is max).",curDistance,dist2);
			database.logevents(client->AccountName(),client->AccountID(),0,client->GetName(),this->GetName(),"Long Corpse Loot Attempt",text,17,client->GetX(),
				client->GetY(),client->GetZ(), (char*)database.GetZoneName(GetZoneID(), client->GetPP().zoneInstance, true),GetX(),GetY(),GetZ());
		}
		return;
	}
	...
	if (this->BeingLootedBy != 0xFFFFFFFF) {
		// lets double check....
		Client* looter = entity_list.GetClientByID(this->BeingLootedBy);
		if (looter == 0)
	...
	else if(!become_npc && IsPlayerCorpse() && charid == client->CharacterID())
		tCanLoot = 2;
...
void Corpse::LootItem(Client* client, const EQApplicationPacket* app)
{
	...
	if(!become_npc && IsPlayerCorpse() && (charid != client->CharacterID())){

		// Kings & Bandits level check for playercorpse
		if ( corpselevel > 0 )
		{
			int32 diff = client->GetLevel()-corpselevel;
			int playerLevelDiff = abs((int)diff);
			if ( playerLevelDiff > 10 )
			{
				client->Message(13, "You are too high of level to interact with this player's corpse.");
				SendEndLootErrorPacket(client);
				BeingLootedBy = NULL;
				return;
			}
	...
	{
		// Image 2018 - todo: if we want to add item loot restrictions back need to add them here, lootitem slot_id does NOT match player slot
	...
	if (client && inst)
	{
		if (client->CheckLoreConflict(item))
		{
			...
			client->Message(13,"Item %s (%i) conflicts with a lore item in your inventory.",item->Name,item->ID);
			SendEndLootErrorPacket(client);
			BeingLootedBy = 0;
			safe_delete(inst);
			return;
		...
		else
		{
			...
			if ( bag_item_data != NULL )
			{
				for ( int i=0;i<10;i++)
				{
					if ( bag_item_data[i] != NULL && bag_item_data[i]->item_id > 0 )
					{
						if ( client->CheckLoreConflict(bag_item_data[i]->item_id) )
						{
							item = database.GetItem(bag_item_data[i]->item_id);
							client->Message_StringID(0,LOOT_LORE_ERROR);
							client->Message(13,"Bag Item %s %i, slot %i conflicts with a lore item in your inventory.",item->Name, bag_item_data[i]->item_id, i);
							SendEndLootErrorPacket(client);
							BeingLootedBy = 0;
							safe_delete(inst);
							return;
		...
		// Kings & Bandits - important fix to make sure no bags are being exchanged and also that the item is droppable.
		// Kings & Bandits - PlayerCorpse lore/nodrop/bag checks
		if ( !become_npc && GetPKItem() == 0 && IsPlayerCorpse() && client->CharacterID() != GetCharID() &&
			((  item != NULL && item->LoreFlag /* loreFlag loot items*/ || (!item->NoDrop || item->ItemClass == ItemClassContainer) ) ||
		(!inst->GetItem()->NoDrop || inst->IsAugmented() || inst->IsType(ItemClassContainer))) )
		{
			client->Message(0,"You cannot loot a lore, no-drop or bag item that does not belong to you.");
			SendEndLootErrorPacket(client);
			BeingLootedBy = 0;
			safe_delete(inst);
			return;
	...
	// Kings & Bandits - save updates after an item is looted from a corpse
	Save();
	client->Save();
...
void Corpse::EndLoot(Client* client, const EQApplicationPacket* app) {
	...
	if ( client->GetID() == this->BeingLootedBy )
	{
		this->BeingLootedBy = 0xFFFFFFFF;
		if (this->IsEmpty())
			Delete();
...
{
	// Kings & Bandits - corpse summoning distance
	int32 dist2 = 5000; // this is the same as LootCorpse

	if ( spell )
		CheckDistance = false; // this isn't necessary yet since spells dont do anything like dist checks

	// Kings & Bandits - this has been fixed to allow GM's to summon corpses.
	if (!spell) {
		if (this->GetCharID() == client->CharacterID() || client->Admin() > 100) {
			if (IsLocked() && client->Admin() < 100) {
				client->Message(13, "That corpse is locked by a GM.");
				return false;
			}
			if (client->Admin() > 100 || !CheckDistance || (DistNoRootNoZ(*client) <= dist2))
			{
...
int32 ZoneDatabase::GetNextPlayerBuryDecayTime(int32 char_id) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	int32 CorpseDecayTime = 0;

	if (RunQuery(query, MakeAnyLenString(&query, "SELECT (UNIX_TIMESTAMP() - UNIX_TIMESTAMP(timeofdeath)) FROM player_corpses WHERE charid='%i' AND (IsBurried=1 OR (IsBurried=0 AND timeofdeath!=0)) ORDER BY timeofdeath asc limit 1", char_id), errbuf, &result)) {
		row = mysql_fetch_row(result);
		if ( row != NULL )
		{
			CorpseDecayTime = atoi(row[0]);
		}
		
		mysql_free_result(result);
	}
	else {
		cerr << "Error in GetNextPlayerBuryDecayTime query '" << query << "' " << errbuf << endl;
	}

	safe_delete_array(query);

	return CorpseDecayTime;
}
...
Corpse* ZoneDatabase::SummonBurriedPlayerCorpse(int32 char_id, int32 dest_zoneid, int16 dest_instanceid, float dest_x, float dest_y, float dest_z, float dest_heading) {
	...
	if (RunQuery(query, MakeAnyLenString(&query, 
		"SELECT id, charname, data, timeofdeath, rezzed, zoneid, instanceid, IsBurried FROM player_corpses WHERE charid='%i' AND (IsBurried=1 OR (IsBurried=0 AND timeofdeath!=0 AND (UNIX_TIMESTAMP() - UNIX_TIMESTAMP(timeofdeath)) > %d)) ORDER BY timeofdeath",
		char_id, (RuleI(Character, CorpseDecayTimeMS) / 1000)), errbuf, &result)) {
		safe_delete_array(query);
		while((row = mysql_fetch_row(result))!=NULL)
		{
			lengths = mysql_fetch_lengths(result);
			if(row) {
				int8 burried = atoul(row[7]);
				int32 zoneid = atoul(row[5]);
				int32 instanceid = atoul(row[6]);
				if ( !burried )
				{
					int32 instID = 0;
					if ( instanceid > 10000 )
					...
					const char* zoneName = database.GetZoneName(zoneid,instID);
					
					if ( strlen(zoneName) > 0 )
					{
						char zone[64];
						snprintf(zone,64,"%s",zoneName);
						if ( database.IsZoneUp(zone) )
						{
							continue;
						}
					}
...
void Corpse::AddLooter(uint32 charid)
{
	for (int i=0; i<MAX_LOOTERS; i++)
	{
		if (looters[i] == 0)
