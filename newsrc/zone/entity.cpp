...
#ifdef EMBPERL
	((PerlembParser *)parse)->Event(EVENT_LOGOUT, 0, "", (NPC*)NULL, mob->CastToClient());
#endif
#ifdef WIN32
				struct in_addr	in;
...
NPC* EntityList::FindNearbyNPC(float x, float y, float z, float radius, NPC* originator)
{
	LinkedListIterator<NPC*> iterator(npc_list);
	iterator.Reset();
	...
	while(iterator.MoreElements())
	{
		...
		ox = (x < npc->GetX()) ? (npc->GetX() - x) : (x - npc->GetX());
		oy = (y < npc->GetY()) ? (npc->GetY() - y) : (y - npc->GetY());
		oz = (z < npc->GetZ()) ? (npc->GetZ() - z) : (z - npc->GetZ());
...
void EntityList::SendZoneSpawns(Client* client)
{
	...
	while(iterator.MoreElements()) {
		...
		{
			if(ent->IsClient() && 
			( ent->CastToClient()->GMHideMe(client) || ent->CastToClient()->IsHoveringForRespawn() || (ent->CastToClient()->improved_hidden && ent->IsInvisible(client)) )
			)
			{
				iterator.Advance();
				continue;
...
void EntityList::SendZoneSpawnsBulk(Client* client)
{
	...
	EQApplicationPacket* app = NULL;

	float dist = RuleR(Combat, CreateSpawnNPCDistance);
	...
	int maxNPCsPerQueue = RuleI(Combat, MaxNPCQueueCount);
	int queueCount = 0;
	BulkZoneSpawnPacket* bzspQueue = new BulkZoneSpawnPacket(client, maxNPCsPerQueue+1);
	client->AddToBulkQueue(bzspQueue);
	...
	for(iterator.Reset(); iterator.MoreElements(); iterator.Advance())
	{
		...
		{
				if(spawn->IsClient() && (spawn->CastToClient()->isDespawningFromZone() || spawn->CastToClient()->GMHideMe(client) || spawn->CastToClient()->IsHoveringForRespawn() || 
				(spawn->CastToClient()->improved_hidden && spawn->IsInvisible(client)) )
				)
					continue;
			...
			// December 2018, delay spawns not nearby and non clients
			if ( !spawn->IsClient() )
			{
				if ( (client->DistNoRoot(*spawn) > dist2)) {
					queueCount++;
					if ( queueCount > maxNPCsPerQueue )
					{
						bzspQueue = new BulkZoneSpawnPacket(client, maxNPCsPerQueue+1);
						client->AddToBulkQueue(bzspQueue);
						queueCount = 0;
					}
					bzspQueue->AddSpawn(&ns);
					continue;
				}
			}

			if ( spawn->IsClient() && ((!spawn->IsGrouped()) || (spawn->GetGroup() != client->GetGroup() )) )
			{
				if ( (client->DistNoRoot(*spawn) > dist2)) {
					ns.spawn.x = -99999;
					ns.spawn.y = -99999;
		...
		else if ( !spawn->InZone() && spawn->IsClient() && client != spawn )
		{
			LogFile->write(EQEMuLog::Status, "%s: SpawnPlayerCache for client %s (%i) has been added.", client->GetName(), spawn->GetName(), spawn->GetID());
			client->AddClientCache(spawn->GetID());
		}
	}
	safe_delete(bzsp);
}
...
void EntityList::RemoveFromTargets(Mob* mob)
{
	...
	iterator.Reset();
	while(iterator.MoreElements()) {
		if ( iterator.GetData()->IsClient() && iterator.GetData()->CastToClient()->GetAttackTarget() == mob )
			iterator.GetData()->CastToClient()->SetAttackTarget(NULL);
		if ( iterator.GetData()->IsCharmed() && iterator.GetData()->GetOwner() == mob )
			iterator.GetData()->SetOwnerID(0);
	...
	if ( parse )
	{
		PerlembParser* parser = (PerlembParser*)parse;
		parser->PurgeDeadMob(mob);
	}	
}
...
void EntityList::QueueCloseClients(Mob* sender, const EQApplicationPacket* app, bool ignore_sender, float dist, Mob* SkipThisMob, bool ackreq, eqFilterType filter) {
	...
	while(iterator.MoreElements()) {
		...
		if ((!ignore_sender || ent != sender) && (ent->CastToMob() != SkipThisMob)) {
...
//sender can be null
void EntityList::QueueClients(Mob* sender, const EQApplicationPacket* app, bool ignore_sender, bool ackreq, bool posUpdates) {
	LinkedListIterator<Client*> iterator(client_list);
	...
	while(iterator.MoreElements())
	{
		...
		{
			if ( !ent->GetGM() && posUpdates && RuleB(Zone,RestrictPositionUpdates) )
			{
				bool appropriateClass = false;
				...
				case BARD:
					{
						appropriateClass = true;
						break;
					}
				}

				if ( !appropriateClass || !ent->IsTracking() )
				{
					// advance since we are continuing here and not queueing the packet
					iterator.Advance();
					continue;
				}
			}
...
bool EntityList::RemoveMob(Mob *delete_mob) {
	...
	while(iterator.MoreElements())
	{
		...
		{
			if ( iterator.GetData()->CastToClient()->GetAttackTarget() == delete_mob )
				iterator.GetData()->CastToClient()->SetAttackTarget(NULL);

			if ( iterator.GetData()->CastToClient()->GetBindSightTarget() == delete_mob )
				iterator.GetData()->CastToClient()->SetBindSightTarget(NULL);
		}

		if(iterator.GetData()==delete_mob){
			iterator.RemoveCurrent();
			if ( !delete_mob->IsClient() ) // we need to iterate through the entire list if its a client we are deleting
				return true;
			else
				continue;
		}
...
bool EntityList::RemoveClient(int16 delete_id){
	...
	while(iterator.MoreElements())
	{
		if(iterator.GetData()->CastToClient()->GetAttackTarget() != NULL 
			&& iterator.GetData()->CastToClient()->GetAttackTargetID() == delete_id )
		{
			iterator.GetData()->CastToClient()->SetAttackTarget(NULL);
		}

		if ( iterator.GetData()->CastToClient()->GetBindSightTarget() != NULL && iterator.GetData()->CastToClient()->GetBindSightTarget()->GetID() == delete_id )
			iterator.GetData()->CastToClient()->SetBindSightTarget(NULL);

		if ( iterator.GetData()->CastToMob()->GetBindWoundTarget() != NULL && iterator.GetData()->CastToMob()->GetBindWoundTarget()->GetID() == delete_id )
			iterator.GetData()->CastToMob()->SetBindWoundTarget(NULL);
...
bool EntityList::RemoveClient(Client *delete_client){
	...
	while(iterator.MoreElements())
	{
		if(iterator.GetData()->CastToClient()->GetAttackTarget() == delete_client )
		{
			iterator.GetData()->CastToClient()->SetAttackTarget(NULL);
		}
		if ( iterator.GetData()->CastToClient()->GetBindSightTarget() == delete_client )
			iterator.GetData()->CastToClient()->SetBindSightTarget(NULL);
		
		if ( iterator.GetData()->CastToMob()->GetBindWoundTarget() == delete_client )
			iterator.GetData()->CastToMob()->SetBindWoundTarget(NULL);
...
void EntityList::RemoveFromPlayerPetHateLists(Mob* mob, bool settoone) {
	LinkedListIterator<Client*> iterator(client_list);

	iterator.Reset();
	while(iterator.MoreElements()) {
		Mob* pet = iterator.GetData()->GetPet();
		if (pet != NULL && 
			pet->CheckAggro(mob)) {
			if (!settoone)
			{
				pet->RemoveFromHateList(mob);
			}
			else
			{
				pet->SetHate(mob,1);
...
void EntityList::SendPositionUpdates(Client* client, int32 cLastUpdate, float range, Entity* alwayssend, bool iSendEvenIfNotChanged) {
	...
	while(iterator.MoreElements()) {
		...
		if (mob && !mob->IsCorpse() && (iterator.GetData() != client)
			...
			if(mob->IsClient() && 
			( (mob->CastToClient()->improved_hidden && mob->IsInvisible(client)) )
			)
			{
				iterator.Advance();
				continue;
				...
				bool distCheckAttempted = false;
				if (range == 0 || (iterator.GetData() == alwayssend) || mob->IsClient() || ( distCheckAttempted = true && (distCheck = (mob->DistNoRoot(*client) <= range))) ) {

					if ( range != 0 && !distCheckAttempted && (distCheck = (mob->DistNoRoot(*client) <= range)) );

					if ( ( !distCheck && range != 0 ) && !client->GetGM() && mob->IsClient() && RuleB(Zone,RestrictPositionUpdates) )
					{
						bool appropriateClass = false;

						switch(client->GetClass())
						{
						case RANGER:
						case DRUID:
						case BARD:
						...
						if ( !appropriateClass || !client->IsTracking() )
						{
							// advance since we are continuing here and not queueing the packet
							iterator.Advance();
							continue;
...
bool EntityList::MakeTrackPacket(Client* client)
{
	...
	while(iterator.MoreElements())
	{
		...
		{
			if((iterator.GetData() != client) && iterator.GetData()->IsTrackable() && !iterator.GetData()->IsInvisible(client)) {
...
void EntityList::AddHealAggro(Mob* target, Mob* caster, int16 thedam)
{
	...
	std::list<NPC*>::iterator iter = aggroList.begin();
	while(iter != aggroList.end())
	{
		cur = *iter;
		...
		if (!cur->IsMezzed() && !cur->IsStunned() && !cur->IsFeared())
		{
			if(cur->IsPet()){
				...
				if(caster){
					...
					{
						if ( caster->IsNPC() && caster->CastToNPC()->teamID > 0 && cur->IsNPC() )
						{
							// don't do anything if we are a guard don't get the npc to aggro the guard for no good reason
						}
						else
...
int16 EntityList::CreateGroundObject(int32 itemid, float x, float y, float z, float heading, int32 decay_time, uint32 requiredlevel, char* qglobal)
{
	...
	if(is)
	{
		...
		if(i)
		{
			Object* object = new Object(i,x,y,z,heading,decay_time,requiredlevel,qglobal);
...
void EntityList::ListGroundSpawns(Client* c)
{
	LinkedListIterator<Object*> iterator(object_list);
	iterator.Reset();
	c->Message(13,"Ground Spawn List");
	while(iterator.MoreElements())
	{
		Object* object=iterator.GetData();
		if ( object->IsGroundSpawn() )
		{
			c->Message(0,"GroundSpawn (%s): X: %f, Y: %f, Z: %f, ItemID: %i", object->GetName(),object->GetX(),object->GetY(),object->GetZ(),object->GetObjectDataDropID());
		}

		iterator.Advance();
	}
	c->Message(13,"End List");
}