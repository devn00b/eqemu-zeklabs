...
void NPC::DisplayWaypointInfo(Client *c, bool dispWaypoints) {
...
void NPC::UpdateWaypoint(int wp_index)
{
	...
	if(Waypoints.empty() || Waypoints.size() < 1 || wp_index >= static_cast<int>(Waypoints.size())) {
...
void NPC::CalculateNewWaypoint()
{
	...
	case 6: // Kings & Bandits: Image's random waypoint code, make it worth a damn!
		{
			list<wplist> randwps;
			GetImageWaypoint(randwps);
			
			if ( randwps.size() < 1 )
				return;

			int rndIndx = MakeRandomInt(0,randwps.size() - 1);
			list<wplist>::iterator iter = randwps.begin();
			std::advance(iter,rndIndx);
			cur_wp = (*iter).index;
			
			break;
		}
			case 7: // KaB/Red69/Zek (Sep 22): Image's patrol to end wp (and stop)
			{
				if(!reached_end)
				{
					cur_wp = cur_wp + 1;
...
void NPC::GetImageWaypoint(list<wplist> &wp_list)
{
	wp_list.clear();
	if(Waypoints.size() <= 2)
	{
	...
	list<wp_distance> distances;
	int low = MakeRandomInt(0,Waypoints.size()-1);
	int high = MakeRandomInt(low,Waypoints.size()-1);
...
{ 
   ...
   else 
   {
      ...
      case 0: //Random Half
		  {
		  int timeDelay = (cur_wp_pause - MakeRandomInt(0, cur_wp_pause-1)/2)*1000;
		  if ( timeDelay < 1 )
			  timeDelay = 100;

         AIwalking_timer->Start(timeDelay); 
         break; 
		  }
      case 1: //Full 
		  {
      ...
      case 2: //Random Full 
		  {
         AIwalking_timer->Start(MakeRandomInt(1, cur_wp_pause-1)*1000); 
         break; 
		  }
	  default:
		  {
         AIwalking_timer->Start(cur_wp_pause*1000+100);
		  break;
		  }
      } 
   } 
...
void NPC::SaveGuardSpot(bool iClearGuardSpot) {
	if (iClearGuardSpot) {
		...
		// yeah this causes problems Image Red69/KaB 4/23/2013
		/*guard_x = 0;
		guard_y = 0;
		guard_z = 0;
		guard_heading = 0;*/
...
void NPC::NextGuardPosition() {
	if (!Replaying && 
		!CalculateNewPosition2(guard_x, guard_y, guard_z, GetMovespeed())) {
...
// mob move fix
	...
	if (numsteps<20)
	{
	    ...
	    else
		{
			...
			y_pos = y;
			z_pos = z;
			// Kings  & Bandits - uh we 'hop' but heading might be good?
			//heading = CalculateHeadingToTarget(x, y);
...
void NPC::AssignWaypoints(sint32 grid) {
	...
	pausetype = 0; // KaB / Red69 / Zek / Need a default value somewhere for grid pausetype.
...
int32 ZoneDatabase::GetFreeGrid(int16 zoneid) {
	...
	if (RunQuery(query, MakeAnyLenString(&query,"SELECT max(id) from grid_entries where zoneid = %i",zoneid),errbuf,&result)) {
	safe_delete_array(query);
	if (mysql_num_rows(result) == 1) {
			row = mysql_fetch_row(result);
			int32 tmp=0;
