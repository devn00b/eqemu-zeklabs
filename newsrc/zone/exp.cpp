...
void Client::AddEXP(int32 in_add_exp, int8 conlevel, bool resexp) {
	...
	if ( GetPVPValue() > 0 )
	{
		...
		if ( pvpFlag == SERVERTYPE_RED69 || RuleB(Kingdoms,GuildXP) )
		{
			uint8 bonusXP = KingsBandits::GuildCityXPBonus(GuildID());
			add_exp += (int32)(((double)add_exp* (double)bonusXP/100.0f));
		}
		else if ( pvpFlag == SERVERTYPE_KAB || pvpFlag == SERVERTYPE_ZEK )
		{
			if ( RuleB(PVPCombat,TeamGuildWars) )
			{
				// todo: loop through each team and determine xp
			}
			else
			{
				TeamSettings_Struct* settings = database.GetTeamSettings(GetPVPValue());
				if ( settings )
				{
					add_exp += (int32)(((double)add_exp* (double)settings->teamXPBonus/100.0f));
				}
			}
		}
	}

	// Kings & Bandits 50+ is reduced in exp rate
	if ( GetLevel() > 49 && RuleB(Kingdoms,HellExpansionLevels) )
	{
		add_exp /= 2;
	}
	...
	if(resexp) {
		...
		if(RuleB(Character,UseXPConScaling) && conlevel != 0xFF && !resexp)
		{
	// Kings & Bandits 50+ is reduced in exp rate
	if ( GetLevel() > 49 )
	{
				switch (conlevel)
				{
					case CON_GREEN:
						...
						return;
					case CON_LIGHTBLUE:
							add_exp = add_exp * RuleI(Character, LightBlueModifier)/200;
							add_aaxp = add_aaxp * RuleI(Character, LightBlueModifier)/200;
						break;
					case CON_BLUE:
...
void Client::SetEXP(int32 set_exp, int32 set_aaxp, bool isrezzexp) {
	...
	if ( !this->IsOfficialClient() && Admin() < 1 )
		maxlevel = RuleI(World,MinLevelCheckBypass);

	if ( RuleB(PVPCombat, TeamGuildWars) && !GetPVP() )
	{
		teamRequired = true;
		maxlevel = RuleI(PVPCombat,LevelPickTeam);
	}
	...
	if ((GetLevel() != check_level) && !(check_level >= maxlevel)) {
		if ( !this->IsOfficialClient() && GetLevel() >= maxlevel && Admin() < 80 )
		{
			Message(13,"You will need to install the official client at eqpvp.com to continue levelling.");
		}
		else
		{
				char val1[20]={0};
	...
	if(GetLevel() == maxlevel - 1){
		...
		if(set_exp > expneeded)
		{
			...
			if ( teamRequired )
				Message(13,"You will need to port to Nexus and select a team to continue levelling.");

			if ( !this->IsOfficialClient() && Admin() < 80 )
				Message(13,"You will need to install the official client at eqpvp.com to continue levelling.");
	}
...
void Client::SetLevel(int8 set_level, bool command)
{
	...
	switch(atoi(tmp))
	{
		case 1:
			{
				// Normal PVP
				if ( set_level > 9 && !m_pp.pvp)
				{
					this->SetPVP((uint8)BANDITTEAMID);
					Message(13,"You have been inducted to the ways of discord.");
				}
				break;
			}
		case 2:
			{
				// Kings & Bandits - SetLevel check, if higher than 29 we enable pvp if it is not.  Set to bandits team.
				if ( set_level > 29 && !m_pp.pvp)
				{
					this->SetPVP((uint8)BANDITTEAMID);
					Message(13,"You have been inducted to the ways of discord.  You are currently a bandit, if you wish to join a kingdom and repent your sins against the kings you should visit the Nexus.");
				}
			break;
			}
	}
        ...
        SetMana(CalcMaxMana());
        UpdateWho();
	if ( !RuleB(Character,AutoAssignSpells) || GetLevel() > RuleI(Character,AutoAssignSpellMaxLevel) )
		Save();
	else
		LoadCharacterLevelUpdates(prevLevel);
}
...
bool Group::GivePVPPoints(Client* killer, uint32 amount, int32 origZoneID) {
	unsigned int i; 
	
	char msg[64];
	_snprintf(msg,64,"%d",origZoneID);

	const char* deathZone = database.GetZoneName(origZoneID);
	...
	for (i = 0; i < MAX_GROUP_MEMBERS; i++)  {
		if (members[i] != NULL && members[i]->IsClient()) // If Group Member is Client
		{
			...
			if ( origZoneID != 0 && !RuleB(PVPCombat,GroupPVPPointsCrossZone) && origZoneID != zone->GetZoneID() )
			{
				cmember->Message(13, "You missed out on %i PVP points since you were not in the origin zone it happened in (%s).", amount, deathZone);
				continue;
			}

			uint32 maxPts = cmember->GetLevel() * RuleI(PVPCombat,MaxPointsMultiplier);
			if ( cmember->GetPVPCareerPoints() >= maxPts )
			{
				cmember->Message(13,"You have capped your career points at this level (%i points at level %i)",maxPts,cmember->GetLevel());
				continue;
			}

			ret = true;
			cmember->AddPVPPoints(amount);
		}
		else if(membername[i][0])
		{
			// if we allow cross zone group pvp points, then assign
			database.InsertMessageQueue(membername[i],"",0,0,0,msg,0,true,10,amount);
		}
	}
...
void Group::GivePVPCoin(uint32 itemID,sint32 charges) {
	unsigned int i; 
	...
	for (i = 0; i < MAX_GROUP_MEMBERS; i++)  {
		...
		{
			Client *cmember = members[i]->CastToClient();
			cmember->PutItemInOpenSlot(itemID,charges);
			cmember->Message(4,"You have been awarded a PVP Coin for the battle!");
		} 
	}
}
