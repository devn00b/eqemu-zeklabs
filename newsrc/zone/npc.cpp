...
NPC::NPC(const NPCType* d, Spawn2* in_respawn, float x, float y, float z, float heading, int iflymode, bool IsCorpse, bool isTempPet)
	: Mob(d->name,
	d->lastname,
	d->max_hp,
	d->max_hp,
...
{	
	TemporaryPet = isTempPet;
	AIautocastspell_timer = NULL;
	// Kings & Bandits
	AddedLootGroupInPerl = false;
	RandomLootGroups.clear();
	teamID = 0;
	npcID = 0;
	isGuildNPC = false;
	...
	// Kings & Bandits - need these to set default regens from db
	hp_regen = d->hp_regen;
	...
	MerchantMinLevel = d->MerchantMinLevel;
	...
	// Kings & Bandits - create random name for npc
	if ( d->randomNameGroupID > 0 )
	{
		string newName = database.GenerateRandomName(d->randomNameGroupID);
		//cout<<"NPC Has Random Name Group: "<< d->randomNameGroupID << ", Name Established: " << newName <<endl;
		if ( newName.length() > 0 )
		{
			strn0cpy(name,newName.c_str(),64);
			strn0cpy(orig_name,newName.c_str(),64);
		}
	}
...
void NPC::SetTarget(Mob* mob) {
	...
	if (mob) {
		/* Image(2014): if statement block added so we don't call the attack_timer 
		// less the mob has triggered it with check previously or the timer is not enabled
		*/
		if ( !attack_timer.Enabled() || attacked )
			SetAttackTimer();
	} else {
...
void NPC::AddCash(uint64 copper){
	uint64 tmp;
	uint64 tmp2;
	tmp = copper;
	...
	LogFile->write(EQEMuLog::Debug, "NPC::AddMoneyToPP() %s should have:  plat:%i gold:%i silver:%i copper:%i", GetName(), platinum, gold, silver, copper);
}
...
NPC* NPC::SpawnNPC(const char* spawncommand, float in_x, float in_y, float in_z, float in_heading, Client* client) {
	if(spawncommand == 0 || spawncommand[0] == 0) {
		...
		NPC* npc = new NPC(npc_type, 0, in_x, in_y, in_z, in_heading, FlyMode3);
...
int32 ZoneDatabase::NPCSpawnDB(int8 command, const char* zone, uint32 zone_version, Client *c, NPC* spawn, int32 extra) {
	...
	case 0: { // add spawn with new npc_type - khuong
		...
		if (!RunQuery(query, MakeAnyLenString(&query, "INSERT INTO npc_types (name, level, race, class, hp, gender, texture, helmtexture, size, loottable_id, merchant_id, face, runspeed, prim_melee_type, sec_melee_type, bodytype) values(\"%s\",%i,%i,%i,%i,%i,%i,%i,%f,%i,%i,%i,%f,%i,%i,%i)", tmpstr, spawn->GetLevel(), spawn->GetRace(), spawn->GetClass(), spawn->GetMaxHP(), spawn->GetGender(), spawn->GetTexture(), spawn->GetHelmTexture(), spawn->GetSize(), spawn->GetLoottableID(), spawn->MerchantType, 0, spawn->GetRunspeed(), 28, 28, spawn->GetBodyType()), errbuf, 0, 0, &npc_type_id)) {
			safe_delete(query);
			return false;
		}
		if(c) c->LogSQL(query);
...
void NPC::SetEquipment(int8 material_slot, int32 itemid)
{
	if ( material_slot < MAX_MATERIALS )
		equipment[material_slot] = itemid;
}
		...
		if (x > 0)
		{
			...
			if (inst)
			{
				...
				if (item)
				{
					...
					{
						thief->PutItemInOpenSlot(item->ID, charges[random]);
						RemoveItem(item->ID);
...
void NPC::CalcNPCRegen(sint16 hpRegenBonus,sint16 manaRegenBonus,bool addDefault) {
	sint16 defaultmanaregen = CalcNPCManaRegen();
	sint16 defaulthpregen = CalcNPCHPRegen();

	// we want to override the defaults and add additional hp/mana
	if ( addDefault )
	{
		default_mana_regen = defaultmanaregen + manaRegenBonus;
		default_hp_regen = defaulthpregen + hpRegenBonus;
	}

	// this will the actual value we use to increment the mana regen
	mana_regen = default_mana_regen;

	// we aren't overriding defaults so add the mana regen bonus
	if ( !addDefault )
		mana_regen += manaRegenBonus;
	
	// this will the actual value we use to increment the hp regen
	hp_regen = default_hp_regen;
	
	// we aren't overriding defaults so add the hp regen bonus
	if ( !addDefault )
...
void NPC::LoadRandomLootGroups()
{
	return;

	// Kings & Bandits - LoadRandomLootGroups on spawn2 new NPC()  - Disabled for now

	if ( RandomLootGroups.size() > 0 )
		return; // we already loaded

	const RandomLootDrop_Struct* rld = 0;
	uint32 it = 0;

	while ((rld = database.IterateRandomLootDrops(&it))) {
		if ( rld->npcID > 0 && rld->npcID == this->GetNPCTypeID() )
		{
			RandomLootGroups.push_back(rld->dropRateID);
			continue;
		}
		if ( rld->zoneID  > 0 && rld->zoneID == zone->GetZoneID() )
		{
			RandomLootGroups.push_back(rld->dropRateID);
			continue;
		}
		if ( rld->npcGroupID  > 0 && rld->npcGroupID == spawn_group )
		{
			RandomLootGroups.push_back(rld->dropRateID);
			continue;
		}
...
void NPC::DisplayLootInfo(Client* to, bool condenseList)
{
	// Kings & Bandits LootDrop Data
	if ( GetLoottableID() > 0 )
	{
		const LootTable_Struct* loottbl = database.GetLootTable(GetLoottableID());
		
		char lootDropData[128];
		if ( loottbl != NULL )
		{
			for(int i=0;i<loottbl->NumEntries;i++)
			{

				if ( loottbl->Entries[i].lootdrop_id > 0 )
				{
					const LootDrop_Struct* lds = database.GetLootDrop(loottbl->Entries[i].lootdrop_id);

					sint32 entries = -1;
					uint16 msgColor = 15; // yellow default
					if ( lds )
					{
						entries = (sint32)lds->NumEntries;
					}
					else
						msgColor = 13; // red for warning

					if ( entries < 0 )
						_snprintf(lootDropData,128,"[DropStructNotFound] LootDropID: %i, Multiplier: %i, Probability: %i",loottbl->Entries[i].lootdrop_id,loottbl->Entries[i].multiplier,loottbl->Entries[i].probability);
					else
						_snprintf(lootDropData,128,"LootDropID: %i, Multiplier: %i, Probability: %i NumItmEntries: %i",loottbl->Entries[i].lootdrop_id,loottbl->Entries[i].multiplier,loottbl->Entries[i].probability,entries);

					to->Message(msgColor,lootDropData);
					
					int position = 0;
					if ( lds )
					{
						for(int itm=0;itm<lds->NumEntries;itm++)
						{
							if ( condenseList )
							{
								if ( position > 80 )
								...
								if ( position == 0 )
									position = _snprintf(lootDropData,128,"ItmID/Charges/Chance/EquipItm: %i / %i / %i / %i",lds->Entries[itm].item_id,lds->Entries[itm].item_charges,lds->Entries[itm].chance,lds->Entries[itm].equip_item);
								else
									position += _snprintf(&lootDropData[position],128-position," | %i / %i / %i / %i",lds->Entries[itm].item_id,lds->Entries[itm].item_charges,lds->Entries[itm].chance,lds->Entries[itm].equip_item);
							}
							else
							{
								_snprintf(lootDropData,128,"ItmID: %i ItmCharges: %i ItmDropChance: %i EquipItem: %i",lds->Entries[itm].item_id,lds->Entries[itm].item_charges,lds->Entries[itm].chance,lds->Entries[itm].equip_item);
								to->Message(msgColor,lootDropData);
							}
						}
							if ( position > 0 ) // we have data and it should be ok since it was from a condensed list
							to->Message(15,lootDropData);
					}
					else if ( entries > -1 )
					{
						to->Message(13,"No entries found.");
					}
				}
			} // end of for loop
		}
	else
		to->Message(13,"Could not locate loottable data for LootTableID %i NPCID %i (EntityID %i)",GetLoottableID(),this->GetNPCTypeID(),this->GetID());
	}
	else
		to->Message(13,"No loot table data for NPCID %i (EntityID %i)",this->GetNPCTypeID(),this->GetID());
}
...
void NPC::ReloadSpellList()
{
	ClearSpellGroupOmitList();
	ClearSpellOmitList();
		...
		MYSQL_RES *result;
		MYSQL_ROW row;
		// retrieve team guard omitted spells (By effect id)
		if(database.RunQuery(query,MakeAnyLenString(&query,"SELECT effect_type, spell_id  FROM team_guard_omit_spells where npcid=%i and teamid=%i and (spawngroupid=0 or spawngroupid=%i)",GetNPCTypeID(),teamID,GetSp2()),errbuf,&result))
		{
			while((row = mysql_fetch_row(result)))
			{
				int effect_type = atoi(row[0]);
				int spell_id = atoi(row[1]);
				if ( effect_type > 0 )
					AddSpellGroupOmit(effect_type);
				
				if ( spell_id > 0 )
					AddSpellOmit(spell_id);
			}
			mysql_free_result(result);
		}
		safe_delete_array(query);
		
		// retrieve team guard spells
		if(database.RunQuery(query,MakeAnyLenString(&query,"SELECT npc_spell_list FROM team_guard_spells where npcid=%i and teamid=%i and (spawngroupid=0 or spawngroupid=%i)",GetNPCTypeID(),teamID,GetSp2()),errbuf,&result))
		{
			while((row = mysql_fetch_row(result)))
			{
				AddSpellList(atoi(row[0]));
			}
			mysql_free_result(result);
		}
		safe_delete_array(query);
		...
		for(std::list<int32>::iterator iter = new_list.begin();
			iter != new_list.end();
			++iter)
		{
			if ( this->teamID > 0 )
				AI_AddNPCSpells(*iter, false, RuleI(Kingdoms,MaxLevelGuardSpells));
			else
				AI_AddNPCSpells(*iter, false);
		}
}