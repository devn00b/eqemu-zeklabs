...
struct StatBonuses {
	...
	int16	singingMod;
	int16	amplificationMod; // SingingSkill
	int16	brassMod;
	int16	percussionMod;
	int16	windMod;
	int16	stringedMod;
...
bool logpos;
	...
	enum eStandingPetOrder { SPO_Follow, SPO_Sit, SPO_Guard, SPO_ClientControl };

	// added to determine if we triggered attack_timer for npcs
	bool	attacked;
...
//	static	int		CheckEffectIDMatch(int8 effectindex, int16 spellid1, int8 caster_level1, int16 spellid2, int8 caster_level2);
	...
	bool	BehindMob(Mob* other = 0, float playerx = 0.0f, float playery = 0.0f, float angleRequirement=0.0f) const;
	...
	// abstract methods
	virtual void Death(Mob* killerMob, sint32 damage, int16 spell_id, SkillType attack_skill, int32 player_id=0) = 0;
	virtual void Damage(Mob* from, sint32 damage, int16 spell_id, SkillType attack_skill, uint8 itemType, 
		bool avoidable = true, sint8 buffslot = -1, bool iBuffTic = false, bool damageShield = false, bool isCritHit = false, int32 player_id = 0, bool isDoT=false) = 0;
	...
	void SetMobMaxHP(uint32 value=0) { if ( value < 1 ) cur_hp = max_hp; else base_hp = max_hp = cur_hp = value; }
	void SetMobBaseMana(uint32 value=0) { if ( value > 0 )  base_mana += value; }
	uint32 GetMobBaseMana() { return base_mana; }
	void SetMobMaxMana(uint32 value=0) { if ( value < 1 ) cur_mana = max_mana; else max_mana = cur_mana = value; }
	...
	virtual void AddToHateList(Mob* other, sint32 hate = 0, sint32 damage = 0, bool iYellForHelp = true, bool bFrenzy = false, bool iBuffTic = false, bool skipEventAggro = false);
	
	inline virtual void SetHP(sint32 hp) { if (hp >= max_hp) cur_hp = max_hp; else cur_hp = hp;}

	void SetBaseAC(sint32 newAC) { AC = newAC; }
	...
	void ChangeSize(float in_size, int effect_mod=0, bool bNoRestriction = false);
	...
	void CreateSpawnPacket(EQApplicationPacket* app, Mob* ForWho = 0, bool omitPlayerLocation=false);
	virtual void FillSpawnStruct(NewSpawn_Struct* ns, Mob* ForWho);
	void CreateHPPacket(EQApplicationPacket* app);
	void SendHPUpdate(bool skipSelf=false);
	...
	bool AddProcToWeapon(int16 spell_id, bool bPerma = false, int16 iChance = 3, bool isPoison=false);
	...
	virtual bool TryCriticalHit(Mob *defender, int16 skill, uint8 itemType, sint32 &damage);
	void TryPetCriticalHit(Mob *defender, int16 skill, sint32 &damage);
	virtual bool TryFinishingBlow(Mob *defender, SkillType skillinuse, uint8 itemType);
	virtual bool TryHeadShot(Mob* defender, SkillType skillInUse, uint8 itemType);
	virtual void DoRiposte(Mob* defender);
	void ApplyMeleeDamageBonus(int16 skill, sint32 &damage, Mob* defMob=0);
	...
	virtual void MakePet(int16 spell_id, const char* pettype, const char *petname = NULL, float pet_power = 0.0f);
...
//	int8	flag[60];		//this is for quests or something...
	...
	virtual void SetLastName(const char *new_name = NULL) { new_name ? strn0cpy(lastname, new_name, 64) : strn0cpy(lastname, GetName(), 64); return;};
	...
	virtual bool IsAttackLevelPermitted(Mob* c1, Mob* c2, bool ignoreInfamy=false);
	bool IsBeneficialAllowed(Mob *target);

	inline sint32	GetHP()			const { return cur_hp; }
	inline sint32	GetMaxHP()		const { return max_hp; }
	virtual sint32	CalcMaxHP();
	// Kings & Bandits - modified walk speed to be slower on server calc, this makes it look better on the client
	float GetWalkspeed() const { return(_GetMovementSpeed(-65)); }
	...
	inline virtual sint16	GetBaseAC()		const { return AC; }
	...
	virtual sint32 GetActSpellCasttime(int16 spell_id, sint32 casttime, sint32& diff);
	...
	inline const int	GetSizeMod() const		{ return effect_size; }
	...
	void SetSpellRecastTimer(int16 spell_id);
	...
	//Buff related
	void BuffProcess();
	virtual void DoBuffTic(int16 spell_id, int32 ticsremaining, int8 caster_level, int32 player_id, int buffID, Mob* caster = 0); // player_id = global char id for caster
	bool IsBuffToFade(int16 spell_id);
	...
	virtual void Stun(int duration, bool randInterrupt=false);
	virtual void HandleStunInterrupt(bool randInt);
	virtual void UnStun();
	virtual void BreakMobInvis(bool breakOwnerInvis=true);
	virtual void BreakMobHide(bool breakOwnerHide=true);

	inline void Silence(bool newval) { silenced = newval; }
	inline void Amnesia(bool newval) { amnesiad = newval; }
	void TemporaryPets(int16 spell_id, Mob *target, const char *name_override = NULL, uint32 duration_override = 0, bool spawnAtPlayer=false);
	void WakeTheDead(int16 spell_id, Mob *target, uint32 duration);
	void TryDotCritical(int16 spell_id, int &damage);
	void Spin();
	void Kill();
	bool PassCharismaCheck(Mob* caster, Mob* spellTarget, int16 spell_id, int32 tics_remaining=0);
	bool TryDeathSave();
	...
	bool	CanThisClassSneak(bool ignoreClientSkill=false) const;
	bool	CanThisClassDisarm(bool ignoreClientSkill=false) const;
	bool	CanThisClassBash(bool ignoreClientSkill=false) const;
	bool	CanThisClassDoubleAttack(bool ignoreClientSkill=false) const;
	bool	CanThisClassDualWield(bool ignoreClientSkill=false) const;
	bool	CanThisClassRiposte(bool ignoreClientSkill=false) const;
	bool	CanThisClassDodge(bool ignoreClientSkill=false) const;
	bool	CanThisClassParry(bool ignoreClientSkill=false) const;
	bool	CanThisClassBlock(bool ignoreClientSkill=false) const;
	...
	sint32  AffectMagicalDamage(sint32 damage, int16 spell_id, const bool iBuffTic, Mob* attacker, bool isDoT=false);

	virtual void DoSpecialAttackDamage(Mob *who, SkillType skill, uint8 itemType, sint32 max_damage, sint32 min_damage = 1, sint32 hate_override = -1);
    bool Flurry();
    bool Rampage();
    bool AddRampage(Mob*);
	void ClearRampage();
	...
	uint16 GetLootingEntityID() { return entity_id_being_looted; }
	...
	int CalcSpellEffectValue(int16 spell_id, int buffID, int effect_id, int caster_level = 1, Mob *caster = NULL, int ticsremaining = 0);
	...
	float odd_rewind_x; //Lieka:  Used for storing /rewind values
	float odd_rewind_y; //Lieka:  Used for storing /rewind values
	float odd_rewind_z; //Lieka:  Used for storing /rewind values
	bool rewind_warp;
	...
	QGlobal* CheckGlobal(const char *varname);
	...
	sint16 GetMobHPRegen() { return hp_regen; }
	sint16 GetMobManaRegen() { return mana_regen; }

	void SetLastWeaponSpeed(int16 value) { lastWeaponSpeed = value; }
	int16 GetLastWeaponSpeed() { return lastWeaponSpeed; }

	bool IsGuardRace();

	bool IsPoisonSpellProcPresent() { 	
		for (int j = 0; j < MAX_PROCS; j++)
		{
			if ( SpellProcs[j].spellID != SPELL_UNKNOWN && 
				SpellProcs[j].isPoison )
				return true;
		}
	...
	void StartReplay(uint32 replayID);
	void PlayFrame();
	
	virtual bool MakeNewPositionAndSendUpdate(float x, float y, float z, float speed, bool checkZ);

	void SetBindWoundTarget(Mob* target) { bindwound_target = target; }
	Mob* GetBindWoundTarget() { return bindwound_target; }

	uint32 RandomItemSteal(Client *client, Mob* receiver);
protected:
	void CommonDamage(Mob* other, sint32 &damage, const uint16 spell_id, const SkillType attack_skill, bool &avoidable, const sint8 buffslot, const bool iBuffTic, int32 player_id=0, bool isDoT=false);
	...
	sint16	default_hp_regen;
	sint16	default_mana_regen;
	...
	void ClearCombatNegatedSpells();
	bool HasBuff(int16 spell_id);
	...
	Timer	aggroparser_timer; // delays the time in which aggro is sent

	//spell casting vars
	Timer spellend_timer;
	int16 casting_spell_id;
    float spell_x, spell_y, spell_z;
	double attacked_count;
	bool delaytimer;
...
//	VERTEX fear_vector;
	...
	// Red69/LoZ/KaB: fix for bard songs with recast time sep 7 2012 - need to move outside private to protected for use in client_process.cpp
	void _StopSong();		//this is not what you think it is


	void			LoadExtProcs(ExtendedProfile_Procs* permaprocs, ExtendedProfile_Procs* spellprocs);

	bool	Replaying;
	uint32	ReplayingID;
	uint32	ReplayingFrame;
	Timer	ReplayingTimer;
	Client* ReplayingOwner;
	float	LastReplaySpeed;
private:
	Mob* target;
};
