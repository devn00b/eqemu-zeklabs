...
const char *GetRandPetName()
{
	static const char *petnames[] = { "Gabaner","Gabann","Gabantik","Gabarab","Gabarer","Gabarn","Gabartik",
		...
		"Xebarer","Xebekn","Xebarn","Xebeker","Xeber","Xebober","Xebtik","Xekab","Xeker",
...
void Mob::MakePet(int16 spell_id, const char* pettype, const char *petname, float pet_power) {
	...
	if (this->IsClient())
	{
		if ( pet_power == 0.0f )
		{
			pet_power = (float)CastToClient()->GetFocusEffect(focusPetPower, spell_id) / 100.0f;
			CastToClient()->m_epp.pet_power = pet_power;
		}
	...
	if(strncmp(pettype, "BLpet", 5) == 0) 
	{
		switch(GetBaseRace()) 
		{
//devn00b: added custom pet types for beastlord, because? I can..
		case VAHSHIR: 
			npc_type->race = DEVS_BL_VAHSIR; 
			npc_type->size *= 0.8f; 
			npc_type->gender = 2;
			break;
		case TROLL: 
			npc_type->race = DEVS_BL_TROLL; 
			//npc_type->size *= 2.0f; 
			 npc_type->size *= 0.8f;
                        npc_type->gender = 2;
			break;
		case OGRE: 
			npc_type->race = DEVS_BL_OGRE; 
			npc_type->texture = 3; 
			npc_type->gender = 2; 
			break;
		case BARBARIAN: 
			npc_type->race = DEVS_BL_BARB; 
			npc_type->texture = 2;
                         npc_type->size *= 0.8f;
			break;
		//devn00b changed gender from 1 to 2 to create the correct pet.
		case IKSAR: 
			npc_type->race = DEVS_BL_IKSAR; 
			npc_type->texture = 1; 
			npc_type->gender = 2; 
			//npc_type->size *= 2.0f; 
			 npc_type->size *= 0.8f;
...
void NPC::GetPetProcs(ExtendedProfile_Procs* permaprocs, ExtendedProfile_Procs* spellprocs)
{
	for (int i = 0; i < MAX_PROCS; i++) {		
		if ( permaprocs != NULL )
		{
			if (PermaProcs[i].spellID > 0 && PermaProcs[i].spellID != SPELL_UNKNOWN) {
				LogFile->write(EQEMuLog::Error, "%s: PermaProc %i saved.", GetName(), PermaProcs[i].spellID);
		...
		if ( spellprocs != NULL )
		{
			if (SpellProcs[i].spellID > 0 && SpellProcs[i].spellID != SPELL_UNKNOWN) {
				LogFile->write(EQEMuLog::Error, "%s: SpellProc %i saved.", GetName(), SpellProcs[i].spellID);
...
// solar: fix this if buffs struct is fixed
			...
			pet_buffs[i].player_id = buffs[i].player_id;
...
void NPC::SetPetState(SpellBuff_Struct *pet_buffs, int32 *items, ExtendedProfile_Procs* permaprocs, ExtendedProfile_Procs* spellprocs) {
	//restore their buffs...

	LoadExtProcs(permaprocs,spellprocs);
	...
	for (i = 0; i < BUFF_COUNT; i++) {
		...
		if (pet_buffs[i].spellid <= (int32)SPDAT_RECORDS && pet_buffs[i].spellid != 0 && pet_buffs[i].duration > 0) {
			...
			buffs[i].player_id			= pet_buffs[i].player_id;
			buffs[i].casterid			= 0;
			buffs[i].player_class		= 0;
			...
			buffs[i].dropOnCombat 		= IsEffectInSpell(buffs[i].spellid,SE_NegateIfCombat);
