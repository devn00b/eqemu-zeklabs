...
sint32 Client::GetActSpellDamage(int16 spell_id, sint32 value) {
	...
	if(tt != ST_Self) {
		...
		if(this->itembonuses.SpellDmg && spells[spell_id].classes[(GetClass()%17) - 1] >= GetLevel() - 5) {
		...
		if (chance > 0) {
			...
			if(MakeRandomInt(0,100) <= chance) {
				...
				entity_list.MessageClose(this, false, 100, MT_SpellCrits, "%s delivers a critical blast! (PVE: dm%d)", GetName(), (-value * modifier / 100) + spell_dmg);	
			} else 
				mlog(SPELLS__CRITS, "Spell crit failed. Final Damage Modifier: %d, Final Damage: %d", modifier, (value * modifier / 100) - spell_dmg);
		}
	}
	
	sint32 result = ((value * modifier / 100) - spell_dmg);

	if ( RuleB( EventLog, LogSpellDamage ) )
		LogFile->write(EQEMuLog::Error, "%s cast the spell %i with a value %i.  Modifier: %i.  Result: %i.", GetCleanName(), spell_id,value,modifier,result);
...
sint32 Client::GetActSpellHealing(int16 spell_id, sint32 value) {
	...
	if(spells[spell_id].buffduration < 1) 
	{
		...
		if(this->itembonuses.HealAmt && spells[spell_id].classes[(GetClass()%17) - 1] >= GetLevel() - 5) {
...
sint32 Client::GetActSpellCasttime(int16 spell_id, sint32 casttime, sint32& diff)
{
	diff = 0;
	...
	sint32 tmpCastTime = (casttime*(100 - cast_reducer)/100);

	if (cast_reducer > RuleI(Spells,MaxCastReduction))
		cast_reducer = RuleI(Spells,MaxCastReduction);	//is this just an arbitrary limit?

	sint32 newcasttime = casttime;

	if ( casttime > RuleI(Spells,MinCastTimeReduction) )
	{
		newcasttime = (casttime*(100 - cast_reducer)/100);
		if ( spells[spell_id].buffduration > 0 && IsDetrimentalSpell(spell_id) && IsEffectInSpell(spell_id, SE_CurrentHP))
		{
			diff = newcasttime - tmpCastTime;
		}
		else if ( newcasttime < RuleI(Spells,MinCastTimeReduction))
		{
			diff = RuleI(Spells,MinCastTimeReduction) - tmpCastTime;
			newcasttime = RuleI(Spells,MinCastTimeReduction);
		}
	}
	else
	{
		diff = tmpCastTime - newcasttime;
	}
	
	if ( RuleB( EventLog, LogCastTime ) )
		LogFile->write(EQEMuLog::Error, "%s cast the spell %i with a %i cast time.  Reducer: %i.  Result: %i.", GetCleanName(), spell_id,casttime,cast_reducer,newcasttime);
...
bool Client::UseDiscipline(int32 spell_id, int32 target) {
	...
	if(spell.recast_time > 0)
	{
		...
		p_timers.Start(DiscTimer, reduced_recast);
	}
	else
	{
...
void EntityList::AESpell(Mob *caster, Mob *center, int16 spell_id, bool affect_caster, sint16 resist_adjust, bool clientOnly)
{
	...
	for(iterator.Reset(); iterator.MoreElements(); iterator.Advance())
	{
		...
		if(!clientOnly && curmob == center)	//do not affect center
			continue;
		if(curmob == caster && !affect_caster)	//watch for caster too
			continue;
		// Kings & Bandits - support for AE Client Only spells
		if ( clientOnly && curmob->IsNPC() )
			continue;
		if(center->DistNoRoot(*curmob) > dist2)	//make sure they are in range
			continue;
		if(isnpc && curmob->IsNPC()) {	//check npc->npc casting
			if ( caster->IsNPC() && !caster->IsCharmed() && !curmob->CastToNPC()->IsCharmed() 
				&& curmob->CastToNPC()->teamID > 0 && curmob->CastToNPC()->teamID == caster->CastToNPC()->teamID )
				continue;
