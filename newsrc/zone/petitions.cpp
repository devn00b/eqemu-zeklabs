...
int32 PetitionList::GetPetitionIDByAccountName(const char* acctname) {
	LinkedListIterator<Petition*> iterator(list); 
...
void ZoneDatabase::DeletePetitionFromDB(Petition* wpet) {
	...
	if (!RunQuery(query, MakeAnyLenString(&query, "DELETE from tell_queue where petitionid = %i", wpet->GetID()), errbuf, 0, &affected_rows)) {
		LogFile->write(EQEMuLog::Error, "Error in DeletePetitionFromDB query '%s': %s", query, errbuf);
	}
	safe_delete_array(query);
