...
void Object::HandleAugmentation(Client* user, const AugmentItem_Struct* in_augment, Object *worldo)
{
	...
	if (in_augment->augment_slot == -1)
	{
		...
		{
			char logMsg[512];
			_snprintf(logMsg, 512, "Item %s (%i) augmented with %s (%i)", tobe_auged->GetItem()->Name, tobe_auged->GetItem()->ID, auged_with->GetItem()->Name, auged_with->GetItem()->ID);
			database.logevents(user->AccountName(),user->AccountID(),user->Admin(),user->GetName(),(worldo != NULL) ? worldo->GetName() : "AugmentPool","Augmenting Item",logMsg,32,user->GetX(),
			user->GetY(),user->GetZ(), (char*)database.GetZoneName(user->GetZoneID(), user->GetPP().zoneInstance, true),(worldo != NULL) ? worldo->GetX() : 0,(worldo != NULL) ? worldo->GetY() : 0,(worldo != NULL) ? worldo->GetZ() : 0);	
...
void Object::HandleCombine(Client* user, const NewCombine_Struct* in_combine, Object *worldo)
{
	...
	if(worldcontainer){
		...
		for (uint8 i=0; i<10; i++){
			const ItemInst* inst = container->GetItem(i);
			if (inst) {
				// allow augments to be recovered when combining
			for (uint8 a=0; a<4; a++){
					if ( inst->GetAugmentItemID(a) > 0 )
					{
						user->Message(13,"Item %s has augment, pulling it out of the item after combination completes.",inst->GetItem()->Name,inst->GetAugmentItemID(a));
						augments.push_back(inst->GetAugmentItemID(a));
					}
			}
	...
	t1.stop();
	LogFile->write(EQEMuLog::Error, "TradeskillExecute recipe %i took: %f", spec.recipe_id, t1.getDuration());
	
	for(std::list<int32>::iterator it = augments.begin(); it != augments.end(); ++it)
	{
		user->SummonItem(*it);
	}
...
//		spec->onsuccess.push_back(pair<uint32,uint8>::pair(item, num));
		spec->onsuccess.push_back(std::pair<uint32,uint8>(item, num)); //devn00b
	}
	mysql_free_result(result);
...
//			spec->onfail.push_back(pair<uint32,uint8>::pair(item, num));
			spec->onfail.push_back(std::pair<uint32,uint8>(item, num)); //devn00b
		}
		mysql_free_result(result);
	}
	safe_delete_array(query);
