...
{
	return ( ( IsEffectInSpell(spell_id, SE_Invisibility2) || IsEffectInSpell(spell_id, SE_Invisibility) ) ) || ( IsValidSpell(spell_id) && spells[spell_id].goodEffect == 2 );
}
...
bool IsPartialCapableSpell(int16 spell_id, bool casterIsClient, bool defenderIsClient)
{
	bool isPVP = false;
	if ( casterIsClient && defenderIsClient )
		isPVP = true;

	if(IsPureNukeSpell(spell_id) || 
		// Kings & Bandits - limit root, fear and charm to nonpvp on partial
		(!isPVP && (IsEffectInSpell(spell_id, SE_Root) || IsFearSpell(spell_id) || 
...
bool IsResistableSpell(int16 spell_id)
{
	...
	if ( spells[spell_id].effectid[0] == SE_AddFaction )
		return false;
