	...
	if (logtrade == true) {
		...
		if ((cash>0) || (item_count>0)) {
			...
			database.logevents(trader->AccountName(), trader->AccountID(),
				trader->Admin(), trader->GetName(), with->GetName(), "Trade", logtext, 6, trader->GetX(), trader->GetY(),trader->GetZ(),(char*)database.GetZoneName(trader->GetZoneID(),trader->GetPP().zoneInstance),
				with->GetX(),with->GetY(),with->GetZ());
		}
	}
}
...
bool Client::HandleTradeGuardCards(Mob* tradingWith)
{
			bool structChange = false;
		// Kings & Bandits - team guard settings
		if ( tradingWith->CastToNPC()->teamID > 0 && tradingWith->CastToNPC()->npcID> 0)
		{
			GuardSettings_Struct* gss = database.GetGuardSettings(tradingWith->CastToNPC()->teamID,tradingWith->CastToNPC()->npcID,tradingWith->CastToNPC()->GetSp2());
			GuardSettings_Struct updateStruct;
			memset(&updateStruct,0,sizeof(GuardSettings_Struct));
			if ( gss != NULL )
			{
				memcpy(&updateStruct,gss,sizeof(GuardSettings_Struct));
				bool reloadSpells = false;
				for (sint16 i=3000; i<=3003; i++) {
					const ItemInst* inst = m_inv[i];
					if (inst) {
						_log(TRADING__CLIENT, "Item to trade: %i - %s (%s).",inst->GetItem()->ID,inst->GetItem()->Name,inst->GetItem()->CharmFile);
						if ( strstr(inst->GetItem()->CharmFile,"guard card") != NULL || (inst->GetItem()->ItemType == ItemTypeSpell && inst->GetItem()->Scroll.Effect > 0 && inst->GetItem()->Scroll.Effect != SPELL_UNKNOWN) )
						{
							_log(TRADING__CLIENT, "Guard related item - calculating new stats.");
							bool returnItmContinue = false;

							if ( inst->GetItem()->HP > 0 && ( tradingWith->GetMaxHP() + inst->GetItem()->HP ) > GUARDHPLIMIT )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max HP limit of %i.",inst->GetItem()->Name,GUARDHPLIMIT);
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->ReqLevel > 0 && ( tradingWith->GetLevel() + inst->GetItem()->ReqLevel ) > RuleI(Kingdoms,GuardLevelLimit) )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max level limit of %i.",inst->GetItem()->Name,RuleI(Kingdoms,GuardLevelLimit));
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->AC > 0 && ( tradingWith->GetAC() + inst->GetItem()->AC ) > GUARDACLIMIT )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max AC limit of %i.",inst->GetItem()->Name,GUARDACLIMIT);
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->Mana > 0 && ( tradingWith->GetMaxMana() + inst->GetItem()->Mana ) > GUARDMANALIMIT )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max mana limit of %i.",inst->GetItem()->Name,GUARDMANALIMIT);
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->Regen > 0 && ( tradingWith->GetMobHPRegen() + inst->GetItem()->Regen ) > GUARDMAXHPREGEN )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max hp regen limit of %i.",inst->GetItem()->Name,GUARDMAXHPREGEN);
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->ManaRegen > 0 && ( tradingWith->GetMobManaRegen() + inst->GetItem()->ManaRegen ) > GUARDMAXMANAREGEN )
							{
								Message(13,"I was unable to apply the guard upgrade %s.  This would exceed the max mana regen limit of %i.",inst->GetItem()->Name,GUARDMAXMANAREGEN);
								returnItmContinue = true;
							}
							
							if ( inst->GetItem()->BaneDmgRaceAmt > 0 )
							{
								if ( tradingWith->CastToNPC()->GetClass() == BANKER || 
									tradingWith->CastToNPC()->GetClass() == MERCHANT )
								{
									Message(13,"I was unable to apply the guard upgrade %s.  This NPC cannot be assigned guard spell upgrades.",inst->GetItem()->Name);
									returnItmContinue = true;
								}
								else if ( tradingWith->CastToNPC()->HasSpellList(inst->GetItem()->BaneDmgRaceAmt) )
								{
									Message(13,"I was unable to apply the guard upgrade %s.  The NPC already has this spell list.",inst->GetItem()->Name);
									returnItmContinue = true;
								}
								else if ( tradingWith->CastToNPC()->CountSpellListInRange(1,12) >= RuleI(Kingdoms,MaxSpellClassUpgrades) && inst->GetItem()->BaneDmgRaceAmt < 13 )
								{
									Message(13,"I was unable to apply the guard upgrade %s.  The guard already contains the max number of class spell lists possible (%i max).",inst->GetItem()->Name, RuleI(Kingdoms,MaxSpellClassUpgrades));
									returnItmContinue = true;
								}
							}
							
							if ( inst->GetItem()->Scroll.Effect > 0 && inst->GetItem()->Scroll.Effect != SPELL_UNKNOWN )
							{
								if ( tradingWith->CastToNPC()->HasSpellOmit(inst->GetItem()->Scroll.Effect) )
								{
									Message(13,"I was unable to apply the guard omission %s.  The guard already contains the omission of this spell.",inst->GetItem()->Name);
									returnItmContinue = true;
								}
							}
							
							if ( inst->GetItem()->Focus.Type > 0 )
							{
								if ( tradingWith->CastToNPC()->HasSpellGroupOmit(inst->GetItem()->Focus.Type) )
								{
									Message(13,"I was unable to apply the guard omission list %s.  The guard already contains the omission of this spell group.",inst->GetItem()->Name);
									returnItmContinue = true;
								}
							}


							if ( returnItmContinue )
							{
								PushItemOnCursor(*inst, true);
								continue; // item would make guard too powerful, skip
							}
							
							database.logdatabase("",0,0,"","Upgrade","GuardStatus",LOG_GUARDSTATUS,(char*)zone->GetShortName(),"%s (%i) has been upgraded with %s (City: %s).",tradingWith->GetCleanName(),tradingWith->GetNPCTypeID(),inst->GetItem()->Name,
								KingsBandits::GetKingdomName(tradingWith->CastToNPC()->teamID,false,true));

							updateStruct.hpBonus += inst->GetItem()->HP;
							updateStruct.manaBonus += inst->GetItem()->Mana;
							updateStruct.hpRegenBonus += inst->GetItem()->Regen;
							updateStruct.manaRegenBonus += inst->GetItem()->ManaRegen;
							updateStruct.AC += inst->GetItem()->AC;
							updateStruct.level += inst->GetItem()->ReqLevel;

							// class needs a special setup
							
							// HP and Mana regen rates are not set in the Mob class
							tradingWith->SetMobMaxHP(tradingWith->GetMaxHP() + inst->GetItem()->HP);
							tradingWith->SetMobBaseMana(tradingWith->GetMobBaseMana() + inst->GetItem()->Mana);
							tradingWith->SetMobMaxMana(tradingWith->GetMaxMana() + inst->GetItem()->Mana);
							tradingWith->SetBaseAC(tradingWith->GetBaseAC() + inst->GetItem()->AC);
							tradingWith->SetLevel(tradingWith->GetLevel() + inst->GetItem()->ReqLevel,true);
							
							// update npc with all regen stats
							if ( inst->GetItem()->ManaRegen > 0 || inst->GetItem()->Regen > 0 )
								tradingWith->CastToNPC()->CalcNPCRegen((sint16)inst->GetItem()->Regen,(sint16)inst->GetItem()->ManaRegen, true);

							if ( inst->GetItem()->BaneDmgRaceAmt > 0 )
							{
								database.AddGuardSpellList(tradingWith->CastToNPC()->teamID,tradingWith->CastToNPC()->GetNPCTypeID(), inst->GetItem()->BaneDmgRaceAmt, tradingWith->CastToNPC()->GetSp2());
								tradingWith->CastToNPC()->AI_AddNPCSpells(inst->GetItem()->BaneDmgRaceAmt, false, RuleI(Kingdoms,MaxLevelGuardSpells));
								Message(4,"Spell List added from card %s",inst->GetItem()->Name);
							}
							
							if ( inst->GetItem()->Scroll.Effect > 0 && inst->GetItem()->Scroll.Effect != SPELL_UNKNOWN )
							{
								database.AddGuardSpellOmitList(tradingWith->CastToNPC()->teamID,tradingWith->CastToNPC()->GetNPCTypeID(), 0, inst->GetItem()->Scroll.Effect, tradingWith->CastToNPC()->GetSp2());
								tradingWith->CastToNPC()->AddSpellOmit(inst->GetItem()->Scroll.Effect);
								Message(4,"Spell Scroll(%i) added from %s",inst->GetItem()->Scroll.Effect,inst->GetItem()->Name);
								reloadSpells = true;
							}
							
							if ( inst->GetItem()->Focus.Type > 0 )
							{
								database.AddGuardSpellOmitList(tradingWith->CastToNPC()->teamID,tradingWith->CastToNPC()->GetNPCTypeID(), inst->GetItem()->Focus.Type, 0, tradingWith->CastToNPC()->GetSp2());
								tradingWith->CastToNPC()->AddSpellGroupOmit(inst->GetItem()->Focus.Type);
								Message(4,"Spell Omission Group(%i) added from card %s",inst->GetItem()->Focus.Type,inst->GetItem()->Name);
								reloadSpells = true;
							}

							if ( inst->GetItem()->ReqLevel > 0 )
							{
								reloadSpells = true;
							}
				...
				if ( structChange )
				{
					Message(4,"%s has been upgraded with provided cards.  The guard has been provided a total of an additional %i hp, %i mana, %i ac, %i levels (including previous upgrades).",tradingWith->GetOrigName(),updateStruct.hpBonus,updateStruct.manaBonus,updateStruct.AC,updateStruct.level);
					database.UpdateGuardSettings(updateStruct.teamID, tradingWith->CastToNPC()->npcID, updateStruct.classID, updateStruct.hpBonus, updateStruct.manaBonus , updateStruct.hpRegenBonus ,
					updateStruct.manaRegenBonus , updateStruct.AC , updateStruct.level, tradingWith->CastToNPC()->GetSp2() );
					gss->hpBonus = updateStruct.hpBonus;
					gss->manaBonus = updateStruct.manaBonus;
					gss->level = updateStruct.level;
					gss->AC = updateStruct.AC;
					gss->hpRegenBonus = updateStruct.hpRegenBonus;
					gss->manaRegenBonus = updateStruct.manaRegenBonus;
					//todo implement others here when they work
				}
				
				if ( reloadSpells )
					tradingWith->CastToNPC()->ReloadSpellList();
			}
...
// KaB - Red69 - Zek / Aug 22 2012.  Rework on trade/quest turn in of items
bool Client::StoreTurnInItems(Mob* tradingWith, bool successTurnin) {
	...
	if ( !tradingWith || !tradingWith->IsNPC() )
		return false;
				...
				for (sint16 i=3000; i<=3003; i++) {
					const ItemInst* inst = m_inv[i];
					if (inst) {
						string turnIn("Quest Turn In Attempt");
						if ( !successTurnin )
						{
							turnIn.assign("NPC Turn In");
						}

						char itemData[128];
						_snprintf(itemData,128,"%s (%i)", inst->GetItem()->Name,inst->GetItem()->ID);
						database.logevents(AccountName(),AccountID(),admin,GetName(),tradingWith->GetName(),(char*)turnIn.c_str(),itemData,22,GetX(),
	...
	if(tradingWith && tradingWith->IsClient()) {
		...
		if(other) {
			...
			for (sint16 i=3000; i<=3007; i++){
				...
				if ( (!isAugmented && inst->GetItem()->NoDrop != 0) || other == this) {
					...
					if (other->PutItemInInventory(slot_id, *inst, true)) {
						...
						PushItemOnCursor(*inst, true);
						mlog(TRADING__ERROR, "Unable to give item %d (%d) to %s, returning to giver, is augmented: %i.", inst->GetItem()->Name, inst->GetItem()->ID, other->GetName(), isAugmented);
					}
		...
		HandleTradeGuardCards(tradingWith);
#ifdef EMBPERL
		...
		if ( !quest_npc )
			StoreTurnInItems(tradingWith, false);
		...
		for (sint16 i=3000; i<=3003; i++) {
			...
			if (inst) {
				...
				strcpy(itemNames[i-3000],inst->GetItem()->Name);
				...
				if (item2 && quest_npc == false) {
					...
					if(GetGM() || ((strstr(inst->GetItem()->CharmFile,"guard card") == NULL) && !(inst->GetItem()->ItemType == ItemTypeSpell && inst->GetItem()->Scroll.Effect > 0 && inst->GetItem()->Scroll.Effect != SPELL_UNKNOWN) && !inst->GetItem()->NoTransfer)) {
						xy++;
						if (xy <= 20) { // 20 items max in an NPC loot table
							...
							if ( tradingWith->CastToNPC()->teamID > 0 )
							{
								database.AddGuardItem(tradingWith->CastToNPC()->teamID,tradingWith->CastToNPC()->npcID,zone->GetZoneID(),tradingWith->CastToNPC()->GetSp2(), item2->ID, charges[i-3000]);
							}
						}
					}
					// Return NO DROP and Attuned items being handed into a non-quest NPC if the rule is true
					else if (RuleB(NPC, ReturnNonQuestNoDropItems) && (strstr(inst->GetItem()->CharmFile,"guard card") == NULL)) {
						PushItemOnCursor(*inst, true);
					}
				}
				DeleteItemInInventory(i);
		...
		if (quest_npc) {
			...
			for ( int z=0; z < 4; z++ ) {
				...
				snprintf(temp1, 100, "item%d.name.%d", z+1,tradingWith->GetNPCTypeID());
				snprintf(temp2, 100, "%s",itemNames[z]);
				parse->AddVar(temp1,temp2);
				//					  memset(temp1,0x0,100);
				//					  memset(temp2,0x0,100);
...
void Client::NukeTraderItem(int16 Slot,sint16 Charges,int16 Quantity,Client* Customer,int16 TraderSlot, int SerialNumber) {
	...
	database.SaveInventory(CharacterID(), Inst, Slot, &m_inv, zone->GetZoneID());
...
void Client::SellToBuyer(const EQApplicationPacket *app) {
	...
	if(!item->Stackable) {
		...
		for(uint32 i = 0; i < Quantity; i++) {
			...
			if(!ItemToTransfer || (Buyer->PutItemInOpenSlot(ItemToTransfer->GetItem()->ID,ItemToTransfer->GetCharges(),ItemToTransfer->GetAugmentItemID(0),ItemToTransfer->GetAugmentItemID(1),
						ItemToTransfer->GetAugmentItemID(2),ItemToTransfer->GetAugmentItemID(3),ItemToTransfer->GetAugmentItemID(4)) == SLOT_INVALID)) {
				_log(TRADING__BARTER, "Unexpected error while moving item from seller to buyer.");
	   			Message(13, "Internal error while processing transaction.");
			...
			database.SaveInventory(CharacterID(), 0, SellerSlot, &m_inv, zone->GetZoneID());
		...
		while(QuantityMoved < Quantity) {
			...
			if(Buyer->PutItemInOpenSlot(ItemToTransfer->GetItem()->ID,ItemToTransfer->GetCharges(),ItemToTransfer->GetAugmentItemID(0),ItemToTransfer->GetAugmentItemID(1),
						ItemToTransfer->GetAugmentItemID(2),ItemToTransfer->GetAugmentItemID(3),ItemToTransfer->GetAugmentItemID(4)) == SLOT_INVALID) {
					_log(TRADING__BARTER, "Unexpected error while moving item from seller to buyer.");
	 	  			Message(13, "Internal error while processing transaction.");
					safe_delete(ItemToTransfer);
				...
				database.SaveInventory(CharacterID(), ItemToTransfer, SellerSlot, &m_inv, zone->GetZoneID());
