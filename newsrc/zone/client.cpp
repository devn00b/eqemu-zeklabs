...
Client::Client(EQStreamInterface* ieqs)
	...
	cheattrigger_timer(1000)
{
	levelUpdateRunning = false;
	linkdead_zonein = false;
	editSelectedItem = 0;
	zoneLockoutID = 0;
...
bool Client::Save(int8 iCommitNow, bool online, bool saveBackup) {
	...
	for (int i=0; i < buff_count; i++) {
		if (buffs[i].spellid != SPELL_UNKNOWN) {
			memcpy(&m_epp.buffs[i],&buffs[i],sizeof(Buffs_Struct));
			...
			// extended profile tracking of buffs
	...
	if (GetPet() && !GetPet()->IsClient() && !GetPet()->IsFamiliar() && GetPet()->CastToNPC()->GetPetSpellID() && !dead) {
		...
		pet->GetPetProcs(m_epp.PetPermaProcs, m_epp.PetSpellProcs);
	} else {
		m_epp.pet_id = 0;
		m_epp.pet_hp = 0;
	}
	...
	uint32 teamID = KingsBandits::LeaderInCityBank(CharacterID(),zone->GetZoneID());
...
void Client::ChannelMessageReceived(int8 chan_num, int8 language, int8 lang_skill, const char* orig_message, const char* targetname) {
	char message[4096];
	...
	if(RuleB(Chat, EnableAntiSpam))
	{
		if(strcmp(targetname, "discard") != 0)
		{
			if(chan_num == 3 || chan_num == 4 || chan_num == 5 || chan_num == 7)
			{
				...
				if ( chan_num == 7 )
					AllowedMessages = RuleI(Chat, MaxTellsPerInterval);
	...
	case 8: { // /say
		...
		if ( RuleB(Kingdoms,OldQuestMessageHandling) )
			entity_list.ChannelMessage(sender, chan_num, language, lang_skill, message);
			...
			if(!GetTarget()->CastToNPC()->IsEngaged()) {
				...
				if(((PerlembParser *)parse)->HasQuestSub(GetTarget()->GetNPCTypeID(),"EVENT_SAY")){
					...
					if (DistNoRootNoZ(*GetTarget()) <= 200) {
						if ( !RuleB(Kingdoms,NewQuestMessagingForTeamNPCOnly) || (RuleB(Kingdoms,NewQuestMessagingForTeamNPCOnly) && GetTarget()->CastToNPC()->teamID > 0) )
							questTriggered = true;
		...
		{
			if ( questTriggered )
				ChannelMessageSend(sender->GetName(), 0, chan_num, language, lang_skill, message);
			else
				entity_list.ChannelMessage(sender, chan_num, language, lang_skill, message);
		}
...
void Client::FillSpawnStruct(NewSpawn_Struct* ns, Mob* ForWho)
{
	...
	if ( GetEPP().isBecomeNPC && strnlen(GetEPP().npcName,64) > 0 && (ForWho == NULL || ( ForWho->IsClient() && ForWho != this ) ) )
		strncpy(ns->spawn.name, this->CastToClient()->GetEPP().npcName, 64);

	if ( IsClient() && GetEPP().isBecomeNPC )
	{
		ns->spawn.race		= GetEPP().npcRace;
		ns->spawn.class_	= GetEPP().npcClass;
	}
	...
	if (IsBecomeNPC() == true || GetEPP().isBecomeNPC)
		ns->spawn.NPC = 1;
	else if (ForWho == this)
		ns->spawn.NPC = 10;
	else
...
if(GetPVP())
this->Message_StringID(13,PVP_ON);
else if ( GetLevel() < RuleI(PVPCombat,LevelPickTeam) ) // Kings & Bandits Client::SetPVP hardcode check for less than 30, they cannot leave pvp if they are higher than 30
{
Message(13, "You no longer follow the ways of discord.");
m_pp.pvp = 0;
}
...
SendAppearancePacket(AT_PVP, GetPVPValue() ? 1 : 0);
Save();
}*/
...
void Client::RefreshPVPFlag(Client* to)
{
	int32 pvpValue = 0;
	if ( RuleB(PVPCombat,PVPFlagForced) && GetPVPValue() )
		pvpValue = 1;
	else
	{
		switch(GetPVPValue())
	...
	SendAppearancePacket(AT_PVP, pvpValue, (to == NULL) ? true : false, false, to);
}
...
void Client::LogLoot(Client* player, Corpse* corpse, const Item_Struct* item){
	...
	if (item!=0){
		...
		strcat(logtext,") Looted EntID: ");
		char corpseID[100];
		_snprintf(corpseID,100,"%i",corpse->GetID());
		strcat(logtext,corpseID);
		...
		if ((corpse->GetPlatinum() + corpse->GetGold() + corpse->GetSilver() + corpse->GetCopper())>0) {
			...
			sprintf(coinloot,"%i PP %i GP %i SP %i CP EntID: %i",corpse->GetPlatinum(),corpse->GetGold(),corpse->GetSilver(),corpse->GetCopper(),corpse->GetID());
			logtext=coinloot;
...
bool Client::BindWound(Mob* bindmob, bool start, bool fail){
	...
	// KaB / Red69 / Zek BindWound: sitting bindwound fix 8/21/12
	if ( ( start && IsSitting() ) || bindmob == NULL )
	{
		CancelBindWound();
		return(true);
	}
...
void Client::SetHideMe(bool flag, bool isPlayer)
{
	...
	uint8 adminval = Admin();
	if ( adminval > 0 )
		adminval -= 1;
	
	entity_list.QueueClientsStatus(this, app, true, 0, adminval);
...
void Client::LoadAchievements()
{
	...
	sprintf(query, "select achievementid,UNIX_TIMESTAMP(ts),cooldown_seconds,incooldown,zoneid from character_achievements where charid=%i",character_id);
	if (database.RunQuery(query, strlen(query), errbuf, &result))
	{
		safe_delete_array(query);
		while(0 != (row = mysql_fetch_row(result))){
			KBAchievement achieve(atoi(row[0]),atoi(row[1]),atoi(row[2]),atoi(row[3]),atoi(row[4]));
...
void Client::AddAchievement(int32 achievementid, int32 zoneid, int32 isrepeatable, int32 cooldown_seconds, int8 incooldown)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	...
	sprintf(query, "INSERT INTO character_achievements(charid,achievementid,zoneid,cooldown_seconds,incooldown) VALUES(%i,%i,%i,%i,%i)",character_id,achievementid,zoneid,cooldown_seconds,incooldown);
	if(database.RunQuery(query, strlen(query), errbuf, 0, &affected_rows))
	{
		KBAchievement achieve(achievementid,time,cooldown_seconds,incooldown,zoneid);
		achievements.push_back(achieve); // track against other achievement events
		safe_delete_array(query);
		return;
	}
	else
...
bool Client::CheckAchievements(KBAchievement* achievement, int32 curTime)
{
	for(std::list<KBAchievement>::iterator iter = achievements.begin();
		iter != achievements.end();
		++iter)
	{
		KBAchievement achieve = *iter;
		if ( achieve.achievementid == achievement->achievementid)
		{
			if ( !achievement->isrepeatable && !achievement->cooldown )
				return true;
			else
			{
				if ( achievement->cooldown_seconds > 1 && ( curTime - achieve.timestamp ) < achievement->cooldown_seconds )
					return true;
			}
		}
	}
...
void Client::UpdateAchievements()
{
	...
	uint32 zoneKillsLastSeconds = 0;
	uint32 zoneDeathsLastSeconds = 0;
	uint32 prevRequiredTime = 0; // reset if we don't have the right one
	...
	while(iterator.MoreElements())
	{
		...
		if ( kb && !CheckAchievements(kb,time) )
		{
			uint16 killsLastSeconds = 0;
			if ( kb->reqtime > 0 )
			{
				for(int i=0;i<50;i++)
				{
					...
					else if ( kb->reqtime > 0 && 
						m_pp.PVPRecentKills[i].Time > 0 && time > 0 && ( time -  m_pp.PVPRecentKills[i].Time ) < kb->reqtime )
					{
						killsLastSeconds++;
					}
				}
			...
			if ( kb->reqtime > 0 && ( kb->reqtime != prevRequiredTime || (kb->reqtimezonedeath > 0 && kb->reqtime != kb->reqtimezonedeath) ) )
			{
				prevRequiredTime = kb->reqtime;
				zoneKillsLastSeconds = 0;
				zoneDeathsLastSeconds = 0;
				database.GetPlayerZoneKills(CharacterID(),(char*)shortName,&zoneKillsLastSeconds, kb->reqtime);
				uint32 deathTime = kb->reqtime;
				if ( kb->reqtimezonedeath > 0 )
					deathTime = kb->reqtimezonedeath;
				database.GetPlayerZoneDeaths(CharacterID(),(char*)shortName,&zoneDeathsLastSeconds, deathTime);
			}
			else if ( kb->reqtime == 0 )
			{
				zoneKillsLastSeconds = 0;
			...
			bool passesNormalScenarios = (kb->reqtime == 0 && m_pp.PVPDeaths >= kb->reqdeaths && m_pp.PVPKills >= kb->reqkills && m_pp.PVPCurrentKillStreak >= kb->reqkillstreak && 
				m_pp.PVPCareerPoints >= kb->reqpoints && GetLevel() >= kb->reqlevel && zoneKills >= kb->reqzonekills && zoneDeaths >= kb->reqzonedeaths);

			bool passesTimeScenarios = (kb->reqtime > 0 && killsLastSeconds >= kb->reqkills && m_pp.PVPDeaths >= kb->reqdeaths && m_pp.PVPCurrentKillStreak >= kb->reqkillstreak && 
				m_pp.PVPCareerPoints >= kb->reqpoints && GetLevel() >= kb->reqlevel && zoneKillsLastSeconds >= kb->reqzonekills && zoneDeathsLastSeconds >= kb->reqzonedeaths);
			
			if ( passesNormalScenarios || passesTimeScenarios )
			{
				...
				case 3:
					{
						Message(5,"Congratulations you have earned the %s achievement (%s)!",kb->name,kb->description);
						break;
					}
				case 4:
					{
						entity_list.Message(0, 5,"%s has earned the %s achievement (%s)!",GetName(),kb->name,kb->description);
						break;
					}
				case 5:
					{
						worldserver.SendEmoteMessage(0, 0, 0, 5,"%s has earned the %s achievement (%s)!",GetName(),kb->name,kb->description);
						break;
					}
				}
				if ( kb->earnpoints > 0 ) { if ( this->AddPVPPoints(kb->earnpoints) ) Message(4,"You have been awarded %i PVP points!",kb->earnpoints); }
				...
				AddAchievement(kb->achievementid, zone->GetZoneID(), kb->isrepeatable, kb->cooldown_seconds, (kb->cooldown_seconds > 0) ? 1 : 0);
			}
		}
...
bool Client::DoPVPKillSanityChecks(uint32 attackGuildID, uint32 defendGuildID, uint8 attackPVPValue, uint8 defendPVPValue, uint32 defendIP, uint32 attackIP, 
		char* lastAttacker, int32 fromgroupid, char *deadName, char* deadAccountName, char* attackerName, char* attackerAccountName, uint8 attackLevel, uint8 deadLevel)
{
	char tmp[20];
	database.GetVariable("ServerType", tmp, 9);
	int pvpFlag = atoi(tmp);
	...
	sint16 lvlDiff = (sint16)abs((sint16)attackLevel-(sint16)deadLevel);

	if ( RuleB(PVPCombat, DisablePVPCreditLevelRange) && ( (RuleI(PVPCombat, PVPLevelRange)>0 && lvlDiff > RuleI(PVPCombat, PVPLevelRange)) || (RuleI(PVPCombat, PVPLevelRange) == 0 && lvlDiff > 4) ) )
	{
		LogFile->write(EQEMuLog::Error, "%s Failed LEVEL check.",GetName());
		Message(13,"You are out of level range with the player, no PVP credit will be given.");
		return false;
	}
	else if ( RuleB(PVPCombat, DisableGroupCredit) && group && fromgroupid > 0 && group->GetID() == fromgroupid )
	{
	...
	else if ( RuleB(PVPCombat,DisablePVPCreditNonPVPFlagged) && ( attackPVPValue == 0 || defendPVPValue == 0 ) )
	{
		LogFile->write(EQEMuLog::Error, "%s Failed PVP DisablePVPCreditNonPVPFlagged.",GetName());
		Message(13,"One player is not assigned to a PVP team, no PVP credit will be given.");
		return false;
	}
...
uint32 Client::UpdatePVPKillStats(char* deadName, uint8 deadLevel, uint32 deadRace, uint32 zoneID, uint8 deadClass_, int32 deadGuildID, 
	int8 deadPVPValue, int32 deadPlayerIP, char* lastAttacker, int32 deadLastTime, int8 bountySet, uint32 deadCharID, int32 fromgroupid, char* deadAccountName, 
	int32 deadPlayerKillCount, int32 deadPlayerDeathCount, bool* legitKillPtr, bool* addDeathKillPtr)
{
	time_t now;
	time(&now);
	...
	if ( !DoPVPKillSanityChecks(GuildID(), deadGuildID, GetPVPValue(), deadPVPValue, deadPlayerIP, GetIP(), lastAttacker, fromgroupid, (char*)deadName, deadAccountName, (char*)GetName(), (char*)AccountName(), GetLevel(), deadLevel) )
	{
		if ( legitKillPtr )
			*legitKillPtr = false;
	...
	uint32 maxPts = GetLevel() * RuleI(PVPCombat,MaxPointsMultiplier);
	...
	else
	{
		...
		if ( lvlDiff > RuleI(PVPCombat,PVPKillMaxLvlDiffPoints) )
		{
			Message(13,"No points will be earned as %s was too high of level.",deadName);
			legitKill = false;
			addDeathKill = false;
		}
		else if ( lvlDiff < -RuleI(PVPCombat,PVPKillMaxLvlDiffPoints) )
		{
		...
		else
		{
			...
			ptsEarned = (uint32)(float)ptsEarned * ratioDiff; // use ratio diff to make ultimate amount
		...
		if ( this->GetPVPCareerPoints() >= maxPts )
		{
			Message(13,"You have capped your career points at this level (%i points at level %i)",maxPts,GetLevel());
			ptsEarned = 0;
		}
		else if ( this->GetPVPCareerPoints()+ptsEarned > maxPts )
		{
			Message(13,"You have reached your career points cap at this level (%i points at level %i)",maxPts,GetLevel());
			if ( this->GetPVPCareerPoints() > maxPts )
				ptsEarned = 0;
			else
				ptsEarned = maxPts - this->GetPVPCareerPoints();
		}
	...
	if ( zone->GetInstanceID() > 9999 )
		instanced = true;
	database.AddCharacterPVPKill(this->CharacterID(),deadCharID,m_pp.PVPLastKill.Points,(char*)this->GetName(),deadName,(char*)database.GetZoneName(zoneID), GetLevel(), deadLevel, GuildID(), deadGuildID, GetPVPValue(), deadPVPValue, instanced);
...
uint32 Client::UpdatePVPKillStats(Client* deadplayer, bool* legitKillPtr)
{
	...
	deadplayer->GetPVPValue(), deadplayer->GetIP(), deadplayer->GetPP().PVPLastDeath.Name, deadplayer->GetPP().PVPLastDeath.Time, isBountySet,deadplayer->CharacterID(), groupID, (char*)deadplayer->AccountName(), deadplayer->GetPP().PVPKills, deadplayer->GetPP().PVPDeaths, legitKillPtr, &addDeathKillPtr);
...
int Client::PutItemInOpenSlot(int32 itemID, sint32 charges, int32 augSlot1, int32 augSlot2, int32 augSlot3, int32 augSlot4, int32 augSlot5)
{

	ItemInst* inst = database.CreateItem(itemID, charges);
	if ( augSlot1 > 0 )
		inst->PutAugment(&database, 0, augSlot1);
	if ( augSlot2 > 0 )
		inst->PutAugment(&database, 1, augSlot2);
	if ( augSlot3 > 0 )
		inst->PutAugment(&database, 2, augSlot3);
	if ( augSlot4 > 0 )
		inst->PutAugment(&database, 3, augSlot4);
	if ( augSlot5 > 0 )
		inst->PutAugment(&database, 4, augSlot5);
	...
	if (worldserver.Connected())
	{
		...
		else
		{
			...
			if ( !RuleB(PVPCombat, PureRed) && ( pvpFlag == SERVERTYPE_KAB || pvpFlag == SERVERTYPE_ZEK ) )
			{
				...
				if ( ZonedInLinkdead() )
				{
					snprintf(msg,256,"<%s-LinkDead> %s [%s] of The %s has been slain by %s [%s] of The %s!",zone->GetLongName(),this->GetName(),loserGuildName,loserName,attacker->GetName(),winnerGuildName, winnerName);
			...
			else
			{
				...
				if ( ZonedInLinkdead() )
					snprintf(msg,256,"<%s-LinkDead> %s of %s has been slain by %s of %s!",zone->GetLongName(),this->GetName(),loserName,attacker->GetName(),winnerName);
				else
...
void Client::ResetDiscTimer(int32 timerID, int32 timeRemaining,int32 unknown)
{
	MemorizeSpell(timerID, timeRemaining, unknown);
}
			...
			if(speed > (runs * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))
			{
				...
				if(!GetGMSpeed() && (runs >= GetBaseRunspeed() || (speed > (GetBaseRunspeed() * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))))
				{
					if(IsShadowStepExempted())
					{
						...
						{
							CheatDetected(MQWarpShadowStep, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), speed);
						}
					}
					else if(IsKnockBackExempted())
					{
						...
						if(speed > 30.0f)
						{
							CheatDetected(MQWarpKnockBack, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), speed);
						}
					}
					else if(!IsPortExempted())
					{
						if(!IsMQExemptedArea(zone->GetZoneID(), GetX(), GetY(), GetZ()))
						{
							if(speed > (runs * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))
							{
								CheatDetected(MQWarp, GetX(), GetY(), GetZ(),rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), speed);
							...
							{
								CheatDetected(MQWarpLight, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), speed);
							}
						}
					}
				}
...
void Client::SuspendMinion()
{
	...
	if(!CurrentPet)
	{
		if(m_pp.SuspendedMinion.SpellID > 0)
		{
			...
			if(AALevel >= 2)
			{
				CurrentPet->SetPetState(m_pp.SuspendedMinion.Buffs, m_pp.SuspendedMinion.Items, NULL, NULL);
...
bool Client::AddPVPPoints(sint32 Points, bool modifyCareerPts)
{
	sint32 newPoints = (sint32)m_pp.PVPCurrentPoints + (sint32)Points;

	sint32 newCareerPts = (sint32)m_pp.PVPCareerPoints + (sint32)Points;
	
	sint32 maxPts = GetLevel() * RuleI(PVPCombat,MaxPointsMultiplier);
	if ( modifyCareerPts && GetPVPCareerPoints() >= maxPts && newCareerPts > maxPts )
	{
		Message(13,"You have capped your career points at this level (%i points at level %i)",maxPts, GetLevel());
		return false;
	}
	else if ( modifyCareerPts && newCareerPts > maxPts )
	{
		// reset points to be whatever the max is at this level since we are about to cap out career pts
		Points = maxPts - (sint32)m_pp.PVPCareerPoints;
		newPoints = (sint32)m_pp.PVPCurrentPoints + Points;
		newCareerPts = maxPts;
	}
	...
	if ( modifyCareerPts )
	{
		if ( newCareerPts < 0 )
			...
			m_pp.PVPCareerPoints = (uint32)(sint32)m_pp.PVPCareerPoints + Points;
	}
	
	if ( newPoints < 0 )
		m_pp.PVPCurrentPoints = 0;
	else
		m_pp.PVPCurrentPoints = (uint32)(sint32)m_pp.PVPCurrentPoints + Points;
...
void Client::SendEmptyMerchantResponse()
{
	EQApplicationPacket* outapp = new EQApplicationPacket(OP_AdventureMerchantResponse,2);
	outapp->pBuffer[0] = 0;
	FastQueuePacket(&outapp);
}
...
void Client::LogPlayerMessage(int32 eventTypeID, char* eventDesc, char* eventText, Mob* target)
{
	if ( target != NULL )
	{
		...
		if ( target->IsObject() )
		{
			snprintf(name,64,"ItemID: %i",target->CastToObject()->GetItemID());
		}
		else
		{
			snprintf(name,64,"%s",target->GetCleanName());
		}
		database.logevents(AccountName(),AccountID(),admin,GetName(),(char*)name,eventDesc,
...
void Client::WipeCharacter()
{
	...
	m_pp.zone_id = m_pp.binds[4].zoneId;
	m_pp.binds[0] = m_pp.binds[4];
...
void Client::LoadCharacterLevelUpdates(int16 old_level)
{
	QGlobal res;
	if ( QGlobalCache::GetQGlobal(res,GetQGlobals()->GetBucket(),"disabledlevelupdates",0,CharacterID(),zone->GetZoneID()) ) {
		Message(13, "You have disabled your automatic level updates, you must go to Nexus to re-affirm in or manually update.");
		return;
	}
	
	levelUpdateRunning = true;
	...
	if ( old_level == 0 )
		old_level = 1;
	else if ( old_level > GetLevel())
		old_level = GetLevel();
	
	uint32_breakdown workpt;
	workpt.b4() = DBA_b4_Entity;
	workpt.w2_3() = GetID();
	workpt.b1() = DBA_b1_Entity_Client_LevelUpdate;
	DBAsyncWork* dbaw = new DBAsyncWork(&database, &MTdbafq, workpt, DBAsync::Read);
	
	char itemName[64];
	_snprintf(itemName,64,"Spell:");
	if ( GetClass() == BARD )
		_snprintf(itemName,64,"Song:");
	
	dbaw->AddQuery(0, &query, MakeAnyLenString(&query, "select merchantlist.item,items.scrolleffect,spells_new.classes%i,items.price,items.sellrate from ((merchantlist inner join items on merchantlist.item = items.id) inner join spells_new on items.scrolleffect = spells_new.id and spells_new.classes%i >= %i and spells_new.classes%i <= %i) where merchantlist.isactive = 1 and items.ldonprice = 0 and items.name like '%s%%' group by items.scrolleffect", GetClass(),GetClass(),old_level,GetClass(), GetLevel(), itemName), true);
	
	dbasync->AddWork(&dbaw, 0);
}

bool Client::AsyncHandleAsyncInsert(DBAsyncWork* dbaw)
{
	DBAsyncQuery* dbaq = 0;
	MYSQL_RES* result = 0;
	char errbuf[MYSQL_ERRMSG_SIZE];
	...
	dbaq = dbaw->PopAnswer();
	if (!dbaq) {
		cout << "Error in AsyncHandleAsyncInsert(): dbaq==0" << endl;
		return false;
	}
	if (!dbaq->GetAnswer(errbuf, &result)) {
		// successful insert has no result
		return true;
	}
...
bool Client::AsyncLoadCharacterLevelUpdate(DBAsyncWork* dbaw)
{
	DBAsyncQuery* dbaq = 0;
	MYSQL_RES* result = 0;
	char errbuf[MYSQL_ERRMSG_SIZE];
	...
	if (!dbaq) {
		levelUpdateRunning = false;
		cout << "Error in AsyncLoadCharacterLevelUpdate(): dbaq==0" << endl;
		return false;
	}
	if (!dbaq->GetAnswer(errbuf, &result)) {
		levelUpdateRunning = false;
		cout << "Error in AsyncLoadCharacterLevelUpdate(): !dbaq[" << dbaq->QPT() << "]->GetAnswer(): " << errbuf << endl;
		return false;
	}
	...
	if (dbaq->QPT() == 0) {
		MYSQL_ROW row;
		while((row = mysql_fetch_row(result))) {
			int32 itemToScribe = atoi(row[0]);
			int16 scrollEffect = atoi(row[1]);
			int16 bookSlot = GetNextAvailableSpellBookSlot();
			sint32 price = atoi(row[3]);
			float sellRate = atof(row[4]);
			if(!IsDiscipline(scrollEffect) && !HasSpellScribed(scrollEffect)) {	//isn't a discipline & we don't already have it scribed
			
				if ( RuleB(Character,AutoAssignSpellCost) )
				{
					sint32 SinglePrice = (price * (RuleR(Merchant, SellCostMod)) * sellRate);

					if(SinglePrice > 0 && !TakeMoneyFromPP(SinglePrice)) {
							Message(13, "You do not have enough money to obtain the rest of your spells at this time, visit Heimdallr in Nexus with more cash on hand.");
							break;
					}
				...
				ScribeSpell(scrollEffect, bookSlot);
				count++;
			}
		}
	}
	...
	if ( CanThisClassDoubleAttack(true) && GetSkill(DOUBLE_ATTACK) == 0 )
		SetSkill(DOUBLE_ATTACK, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassDualWield(true) && GetSkill(DUAL_WIELD) == 0 )
		SetSkill(DUAL_WIELD, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassRiposte(true) && GetSkill(RIPOSTE) == 0 )
		SetSkill(RIPOSTE, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassDodge(true) && GetSkill(DODGE) == 0 )
		SetSkill(DODGE, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassParry(true) && GetSkill(PARRY) == 0 )
		SetSkill(PARRY, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassBlock(true) && GetSkill(BLOCKSKILL) == 0 )
		SetSkill(BLOCKSKILL, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassBash(true) && GetSkill(BASH) == 0 )
		SetSkill(BASH, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassDisarm(true) && GetSkill(DISARM) == 0 )
		SetSkill(DISARM, RuleI(Character,AutoAssignSkillLevel));
	if ( CanThisClassSneak(true) && GetSkill(SNEAK) == 0 )
		SetSkill(SNEAK, RuleI(Character,AutoAssignSkillLevel));

	if ( GetClass() == MONK )
	{
		// round kick at lvl 5
		if ( GetLevel() > 4 && GetSkill(ROUND_KICK) == 0 )
			SetSkill(ROUND_KICK, RuleI(Character,AutoAssignSkillLevel));
		// tiger claw at lvl 10
		if ( GetLevel() > 9 && GetSkill(TIGER_CLAW) == 0 )
			SetSkill(TIGER_CLAW, RuleI(Character,AutoAssignSkillLevel));
		// double attack at lvl 15
		if ( GetLevel() > 14 && GetSkill(DOUBLE_ATTACK) == 0 )
			SetSkill(DOUBLE_ATTACK, RuleI(Character,AutoAssignSkillLevel));
		// feign death at lvl 17
		if ( GetLevel() > 16 && GetSkill(FEIGN_DEATH) == 0 )
			SetSkill(FEIGN_DEATH, RuleI(Character,AutoAssignSkillLevel));
		// intimidation at lvl 18
		if ( GetLevel() > 17 && GetSkill(INTIMIDATION) == 0 )
			SetSkill(INTIMIDATION, RuleI(Character,AutoAssignSkillLevel));
		// eagle strike at lvl 20
		if ( GetLevel() > 19 && GetSkill(EAGLE_STRIKE) == 0 )
			SetSkill(EAGLE_STRIKE, RuleI(Character,AutoAssignSkillLevel));
		// dragon punch at lvl 25
		if ( GetLevel() > 24 && GetSkill(DRAGON_PUNCH) == 0 )
			SetSkill(DRAGON_PUNCH, RuleI(Character,AutoAssignSkillLevel));
		// flying kick at lvl 30
		if ( GetLevel() > 29 && GetSkill(FLYING_KICK) == 0 )
			SetSkill(FLYING_KICK, RuleI(Character,AutoAssignSkillLevel));
	}

	if ( GetClass() == RANGER || GetClass() == BARD )
	{
		// hide at lvl 25
		if ( GetLevel() > 24 && GetSkill(HIDE) == 0 )
			SetSkill(HIDE, RuleI(Character,AutoAssignSkillLevel));
	}
	
	if ( GetClass() == SHADOWKNIGHT )
	{
		// hide at lvl 35
		if ( GetLevel() > 34 && GetSkill(HIDE) == 0 )
			SetSkill(HIDE, RuleI(Character,AutoAssignSkillLevel));
	}

	if ( GetClass() == BERSERKER )
	{
		if ( GetLevel() > 5 && GetSkill(FRENZY) == 0 )
			SetSkill(FRENZY, RuleI(Character,AutoAssignSkillLevel));
	}
	
	if ( GetClass() == ROGUE )
	{
		// hide at lvl 3
		if ( GetLevel() > 2 && GetSkill(HIDE) == 0 )
			SetSkill(HIDE, RuleI(Character,AutoAssignSkillLevel));
		// pick lock at lvl 5
		if ( GetLevel() > 4 && GetSkill(PICK_LOCK) == 0 )
			SetSkill(PICK_LOCK, RuleI(Character,AutoAssignSkillLevel));
		// pick pocket at lvl 7
		if ( GetLevel() > 6 && GetSkill(PICK_POCKETS) == 0 )
			SetSkill(PICK_POCKETS, RuleI(Character,AutoAssignSkillLevel));
		// backstab at lvl 10
		if ( GetLevel() > 9 && GetSkill(BACKSTAB) == 0 )
			SetSkill(BACKSTAB, RuleI(Character,AutoAssignSkillLevel));
		// disarm traps at lvl 15
		if ( GetLevel() > 14 && GetSkill(DISARM_TRAPS) == 0 )
			SetSkill(DISARM_TRAPS, RuleI(Character,AutoAssignSkillLevel));
		// make poison at lvl 18
		if ( GetLevel() > 17 && GetSkill(APPLY_POISON) == 0 )
			SetSkill(APPLY_POISON, RuleI(Character,AutoAssignSkillLevel));
		// make poison at lvl 20
		if ( GetLevel() > 19 && GetSkill(MAKE_POISON) == 0 )
			SetSkill(MAKE_POISON, RuleI(Character,AutoAssignSkillLevel));
		// intimidation at lvl 22
		if ( GetLevel() > 21 && GetSkill(INTIMIDATION) == 0 )
			SetSkill(INTIMIDATION, RuleI(Character,AutoAssignSkillLevel));
	}
...
void Client::LoadMessageQueue(bool petitionCheck)
{
	...
	if ( petitionCheck )
	{
		dbaw->AddQuery(0, &query, MakeAnyLenString(&query, "Select fromname, chanid, language, langskill, message, inserted, command, commandvalue, guildid, lastattacker, fromgroupid, accountname, optional1, optional2 from tell_queue where toname='%s' and updatereceived=0 and petitionid!=0 order by inserted", this->GetName()), true);
	...
	{
		dbaw->AddQuery(0, &query, MakeAnyLenString(&query, "Select fromname, chanid, language, langskill, message, inserted, command, commandvalue, guildid, lastattacker, fromgroupid, accountname, optional1, optional2 from tell_queue where toname='%s' and updatereceived=0 order by inserted", this->GetName()), true);
...
void Client::InsertQueryAsync(char* query, int timeMS)
{
	if (!RunLoops)
		return;
	char* newquery = 0;
	...
	workpt.b4() = DBA_b4_Entity;
	workpt.w2_3() = GetID();
	workpt.b1() = DBA_b1_Entity_Client_AsyncInsert;
	DBAsyncWork* dbaw = new DBAsyncWork(&database, &MTdbafq, workpt, DBAsync::Write);
	dbaw->AddQuery(0, &newquery, MakeAnyLenString(&newquery, "%s", query), false, false);
	dbasync->AddWork(&dbaw, timeMS);
}
...
void Client::MakeDyePacket(int r, int g, int b)
{
	DyeStruct dyes;
	memset(&dyes, 0, sizeof(dyes));
	int rgb = ((r&0x0ff)<<16)|((g&0x0ff)<<8)|(b&0x0ff);
	for(int i=0;i<MAX_MATERIALS;i++)
	{
		dyes.dye[i].rgb.red = r;
		dyes.dye[i].rgb.green = g;
		dyes.dye[i].rgb.blue = b;
		dyes.dye[i].rgb.use_tint = 1;
	}
	
	DyeArmor(&dyes,true);
}

void Client::GarbleMessage(char *message, uint32 variance)
{
...
void Client::SetStartingItems()
{
	LogFile->write(EQEMuLog::Status, "%s: SetStartingItems race: %i, class: %i, deity: %i, zoneid: %i.", GetName(), m_pp.race, m_pp.class_, m_pp.deity, m_pp.binds[4].zoneId);
	database.SetStartingItems(this, &m_pp, &m_inv, m_pp.race, m_pp.class_, m_pp.deity, m_pp.binds[4].zoneId, m_pp.name, Admin());
}

void Client::CheatSaveBuffs(CheatTypes CheatType) {
	if (!spells_loaded)
		return;
	uint32 i;

	char targetInfo[512];
	snprintf(targetInfo,512,"");
	...
	for (i=0; i < buff_count; i++) {
		if (buffs[i].spellid != SPELL_UNKNOWN) {
			...
			if (buffs[i].durationformula == DF_Permanent)
				snprintf(msg,64,"%i: %s (perm),",buffs[i].spellid,spells[buffs[i].spellid].name);
			else
				snprintf(msg,64,"%i: %s (%i tics),",buffs[i].spellid,spells[buffs[i].spellid].name,buffs[i].ticsremaining);

			strcat(targetInfo, msg);

			length = strnlen(targetInfo,512);

			if ( length > 400 )
			{
				snprintf(msg,64," cut off length too long.");
				strcat(targetInfo,msg);
				length += strnlen(targetInfo,512);
				break;
			}
	...
	if ( buffsCounted < 1 )
		...
		char msg[64];
		snprintf(msg,64," end of list.  CheatType: %i",CheatType);
		strcat(targetInfo,msg);
		char* query = 0;
		MakeAnyLenString(&query, 
			"Insert into eventlog (accountname,accountid,status,charname,target,descriptiontype,description,event_nid,charxpos,charypos,charzpos,charzonename) " 
			"values('%s',%i,%i,'%s','%s','%s','%s','%i','%f','%f','%f','%s')", 
			this->account_name,this->AccountID(), 0, this->name, GetTarget() ? GetTarget()->GetName() : "N/A", "BuffsStatus", targetInfo, 15,GetX(),
				GetY(),GetZ(), (char*)zone->GetShortName());
		InsertQueryAsync(query,500);
		safe_delete_array(query);
}
...
void Client::SetExtProfileNPC(bool enableNPC, char* npcName, uint8 npcClass, uint8 npcRace)
{
	if ( enableNPC )
	{
		m_epp.isBecomeNPC = true;
		strncpy(m_epp.npcName,npcName,64);
		m_epp.npcClass = npcClass;
		m_epp.npcRace = npcRace;
		Message(13,"BecomeNPC *ON*, Race: %s, Class: %s, Gender: %i",GetRaceName(m_epp.npcRace), GetEQClassName(m_epp.npcClass), m_epp.npcGender);
	}
	else
	{
		m_epp.isBecomeNPC = false;
		memset(m_epp.npcName,0,64);
		m_epp.npcName[0] = '/0';
		m_epp.npcClass = 0;
		m_epp.npcRace = 0;
		Message(13,"BecomeNPC *OFF*");
	}
