				...
				switch (AIspells[i].type) {
					...
					case SpellType_Buff: {
						if (
							...
							&& tar->CanBuffStack(AIspells[i].spellid, GetLevel(), true) >= 0
							&& !(this->teamID == 0 && tar->IsPet() && tar->GetOwner()->IsClient() && this != tar)	//no buffing PC's pets, but they can buff themself
							) {
								int32 tempTime = 0;
							AIDoSpellCast(i, tar, mana_cost, &tempTime);
...
bool EntityList::AICheckCloseBeneficialSpells(NPC* caster, int8 iChance, float iRange, int16 iSpellTypes) {
	...
	bool isGuard = (caster->teamID > 0) ? true : false;
    ...
    for(iterator.Reset(); iterator.MoreElements(); iterator.Advance()) {
		...
		if (   t1 > iRange
			|| t2 > iRange
			|| (!isGuard && t3 > iRange)
			|| (!isGuard && mob->DistNoRoot(*caster) > iRange2)
			|| (isGuard && mob->DistNoZ(*caster) > iRange2)
	...
	if ( isGuard )
	{
		TeamSettings_Struct* team = database.GetTeamSettings(caster->teamID);
		if ( team == NULL )
			return false;

		LinkedListIterator<Client*> itr(client_list);
		for(itr.Reset(); itr.MoreElements(); itr.Advance()) {
			Client* client = itr.GetData();
		
			//Since >90% of mobs will always be out of range, try to
			//catch them with simple bounding box checks first. These
			//checks are about 6X faster than DistNoRoot on my athlon 1Ghz
			t1 = client->GetX() - caster->GetX();
			t2 = client->GetY() - caster->GetY();
			//cheap ABS()
			if(t1 < 0)
				t1 = 0 - t1;
			if(t2 < 0)
			...
			if (   t1 > iRange
				|| t2 > iRange
				|| (!teamGuildWars && client->GuildID() != team->guildid)
				|| (teamGuildWars && team->ownerteamid == 0 ) 
							// Image (Jul 29, 2019): rewritten if statement to include bandit team id to check on guildid
				|| (teamGuildWars && team->ownerteamid != BANDITTEAMID && client->GetPVPValue() != team->ownerteamid) 
				|| (teamGuildWars && team->ownerteamid == BANDITTEAMID && client->GuildID() != team->guildid) 
				|| caster->IsOnHatelist(client->CastToMob())
				|| !client->Connected()
				|| client->DistNoZ(*caster) > iRange2
				|| !caster->CheckLosFN(client)
					//this call should seem backwards:
		) {
				continue;
		}
			int32 rndInt = MakeRandomInt(0,RuleI(Kingdoms,RandomMaxBuffPercentage));

			if ( rndInt > RuleI(Kingdoms,BuffChooseClientToCheck) )
				continue;
			else if ( client->GetLevel() < RuleI(Kingdoms,MinLevelBuff) )
				continue;
			...
			Mob* pet = NULL;
			if ( client->GetPetID() != 0 && MakeRandomInt(0,1) == 0 && (pet = client->GetPet()) != NULL) // if we cant help the client then we buff the pet
			{
				if ( caster->AICastSpell(pet, 100, iSpellTypes ) )
					return true;
			}
			else if (caster->AICastSpell(client, 100, iSpellTypes))
				return true;
		}
	}
...
void Mob::AI_Start(int32 iMoveDelay) {
	...
	heading_timer = new Timer(AImovement_duration*5);
	heading_timer->Start();
...
void NPC::AI_Start(int32 iMoveDelay) {
	...
	if (AIspells.size() == 0) {
		...
		AIautocastspell_timer->Start(RandomTimer(1000, 15000), false);
	}
		...
		else
		{
			if(!IsRooted() && !IsMezzed())
			{
				...
				else
				{
					...
					SetHeading(CalculateHeadingToTarget(Goal.x, Goal.y));
				}				
			}
			else if(IsMoving())
			{
...
void Mob::AI_Process() {
	...
	if ( !flee_mode && IsEngaged() && GetTarget() && heading_timer->Check()) {
		
		bool is_combat_range = CombatRange(target);

		if ( ( !IsPet() || GetPetOrder() > SPO_Follow ) && is_combat_range )
	...
	// KaB - Red69 - Zek / Bard Casting fix for AI
	// Bards should be able to cast and do other things!! DUH!
	...
	if (engaged) 
	{
		...
		if (!target)
		{ // KaB / Red69 / Zek - Fix hate list wiping.  If we don't have a target but we make it down this code
			// prevent it from happening again, clear the hate list!
			
			// changing this caused temporary pets to not get aggro on spawn
			// removed hate list wipe
			return;
		}
...
void NPC::AI_DoMovement() {
	...
	else if (roamer) 
	{
		...
		sint32 gridno = CastToNPC()->GetGrid(); 

		//printf("%s: DistractedGrid %i, gridno %i, curwp: %i, movetimercomplete: %i, walkingtimerenabled: %i\n",GetName(),DistractedFromGrid,gridno,cur_wp, movetimercompleted,AIwalking_timer->Enabled());
		if (gridno > 0 || cur_wp==-2)  {
			if (movetimercompleted==true && !DistractedFromGrid) {  // time to pause at wp is over
			...
			{	// currently moving
				...
				{	// not at waypoint yet, so keep moving
					...
					{
						//printf("%s: DistractedGrid %i, gridno %i, curwp: %i, x: %f, y: %f, z: %f\n",GetName(),DistractedFromGrid,gridno,cur_wp, cur_wp_x, cur_wp_y, cur_wp_z);
	...
	if (AIautocastspell_timer->Check(false)) {
		...
		if (!AICastSpell(this, 100, SpellType_Heal | SpellType_Buff | SpellType_Pet)) {
			if(!entity_list.AICheckCloseBeneficialSpells(this, 33, MobAISpellRange, SpellType_Heal | SpellType_Buff)) {
				...
				if ( this->teamID > 0 )
					AIautocastspell_timer->Start(RandomTimer(RuleI(Kingdoms, GuardBeneficialCheckMinMS), RuleI(Kingdoms, GuardBeneficialCheckMaxMS)), false);
				else
...
void Mob::AreaRampage()
{
	...
	// Image 2015: Added null check so we don't call Attack unless there is a target
	if(index_hit == 0 && GetTarget() != NULL )
		Attack(GetTarget());
}
...
bool NPC::AI_AddNPCSpells(int32 iDBSpellsID, bool resetSpellList, int32 levelCap) {
	...
	if ( levelCap == 0 || levelCap > GetLevel() )
		levelCap = GetLevel();
	...
	if (parentlist) {
		...
		for (i=0; i<parentlist->numentries; i++) {
			if (levelCap >= parentlist->entries[i].minlevel && levelCap <= parentlist->entries[i].maxlevel && parentlist->entries[i].spellid > 0) {
				if (!IsSpellInList(spell_list, parentlist->entries[i].spellid))
				{
					if ( parentlist->entries[i].type == SpellType_Buff && (IsSpellGroupOmit(parentlist->entries[i].spellid) || HasSpellOmit(parentlist->entries[i].spellid)) )
						continue;
	...
	for (i=0; i<spell_list->numentries; i++) {
		if (levelCap >= spell_list->entries[i].minlevel && levelCap <= spell_list->entries[i].maxlevel && spell_list->entries[i].spellid > 0) {
			
			if ( spell_list->entries[i].type == SpellType_Buff && (IsSpellGroupOmit(spell_list->entries[i].spellid) || HasSpellOmit(spell_list->entries[i].spellid)) )
				continue;
...
bool NPC::HasSpell(sint16 spellID)
{	
...
void NPC::RemoveAllSpellsFromNPCList()
{
	std::vector<AISpells_Struct>::iterator iter = AIspells.begin();
	while(iter != AIspells.end())
	{
