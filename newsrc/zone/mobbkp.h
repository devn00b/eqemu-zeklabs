...
//LOS Parameters:
#define HEAD_POSITION 1.1f	//ratio of GetSize() where NPCs see from
#define SEE_POSITION 0.75f	//ratio of GetSize() where NPCs try to see for LOS
...
struct Buffs_Struct {
	...
	int32	player_id; // global id for character ("character id")
	//todo: dot stacking?
};
...
//	static	int		CheckEffectIDMatch(int8 effectindex, int16 spellid1, int8 caster_level1, int16 spellid2, int8 caster_level2);
	...
	virtual void Damage(Mob* from, sint32 damage, int16 spell_id, SkillType attack_skill, uint8 itemType, 
		bool avoidable = true, sint8 buffslot = -1, bool iBuffTic = false, bool damageShield = false, bool isCritHit = false, int32 player_id = 0) = 0;
	...
	void SetMobMaxHP(uint32 value=0) { if ( value < 1 ) cur_hp = max_hp; else max_hp = cur_hp = value; }
...
//	int8	flag[60];		//this is for quests or something...
	...
	virtual bool IsAttackLevelPermitted(Mob* c1, Mob* c2);
	...
	float GetWalkspeed() const { return(_GetMovementSpeed(-60)); }
	...
	//Buff related
	void BuffProcess();
	virtual void DoBuffTic(int16 spell_id, int32 ticsremaining, int8 caster_level, int32 player_id, Mob* caster = 0); // player_id = global char id for caster
...
protected:
	void CommonDamage(Mob* other, sint32 &damage, const uint16 spell_id, const SkillType attack_skill, bool &avoidable, const sint8 buffslot, const bool iBuffTic, int32 player_id=0);
