...
XS(XS_Mob_Damage)
{
		...
		Perl_croak(aTHX_ "Usage: Mob::Damage(THIS, from, damage, spell_id, attack_skill, avoidable= true, buffslot= -1, iBuffTic= false)");
	{
		...
		THIS->Damage(from, damage, spell_id, attack_skill, 0, avoidable, buffslot, iBuffTic);
	}
	XSRETURN_EMPTY;
}
...
XS(XS_Mob_ChangeSize)
{
		...
		Perl_croak(aTHX_ "Usage: Mob::ChangeSize(THIS, in_size, bNoRestriction= false)");
	{
		...
		THIS->ChangeSize(in_size, THIS->GetSizeMod(), bNoRestriction);
	}
	XSRETURN_EMPTY;
}
...
XS(XS_Mob_GetMobHPRegen); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_GetMobHPRegen)
{
	dXSARGS;
	if (items != 1)
		Perl_croak(aTHX_ "Usage: Mob::GetMobHPRegen(THIS)");
	{
		Mob *		THIS;
		sint16		RETVAL;
		dXSTARG;
		...
		RETVAL = THIS->GetMobHPRegen();
		XSprePUSH; PUSHi((IV)RETVAL);
	}
	XSRETURN(1);
}

XS(XS_Mob_GetMobManaRegen); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_GetMobManaRegen)
{
	dXSARGS;
	if (items != 1)
		Perl_croak(aTHX_ "Usage: Mob::GetMobManaRegen(THIS)");
	{
		Mob *		THIS;
		sint16		RETVAL;
		dXSTARG;
...
XS(XS_Mob_GetActSpellCasttime)
{
		...
		Perl_croak(aTHX_ "Usage: Mob::GetActSpellCasttime(THIS, spell_id, casttime)");
	{
		...
		sint32 diff;
		RETVAL = THIS->GetActSpellCasttime(spell_id, casttime,diff);
		XSprePUSH; PUSHi((IV)RETVAL);
	}
	XSRETURN(1);
}
...
XS(XS_Mob_AddToHateList)
{
		...
		Perl_croak(aTHX_ "Usage: Mob::AddToHateList(THIS, other, hate= 0, damage= 0, iYellForHelp= true, bFrenzy= false, iBuffTic= false)");
	{
		...
		THIS->AddToHateList(other, hate, damage, iYellForHelp, bFrenzy, iBuffTic, true);
	}
	XSRETURN_EMPTY;
}
...
XS(XS_Mob_DoSpecialAttackDamage)
{
		...
		Perl_croak(aTHX_ "Usage: Mob::DoSpecialAttackDamage(THIS, target, skill, max_damage, min_damage = 1, hate_override = -1)");
	{
		...
		THIS->DoSpecialAttackDamage(target, attack_skill, 0, max_damage, min_damage, hate_override);
	}
	XSRETURN_EMPTY;
}
...
XS(XS_Mob_GetGlobal); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_GetGlobal)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: Mob::GetGlobal(THIS, GlobalString)");
	{
		Mob *		THIS;
		Const_char *		RETVAL = NULL;
		dXSTARG;
		...
		QGlobal* qglobal = THIS->CheckGlobal(parse);
		if ( qglobal != NULL )
		{
			RETVAL = qglobal->value.c_str();
		}
		sv_setpv(TARG, RETVAL); XSprePUSH; PUSHTARG;
	}
	XSRETURN(1);
...
XS(XS_Mob_CheckGlobal); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_CheckGlobal)
{
	dXSARGS;
	if (items != 2)
		Perl_croak(aTHX_ "Usage: Mob::GetGlobal(THIS, GlobalString)");
...
XS(XS_Mob_CheckRule); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_CheckRule)
{
	dXSARGS;
	if (items < 2 || items > 2)
		Perl_croak(aTHX_ "Usage: Mob::CheckRule(THIS, rulename)");
	{
		Mob *		THIS;
		char*		rulename = (char *)SvPV_nolen(ST(1));
		dXSTARG;
		sint32 RETVAL = 0;
		if (sv_derived_from(ST(0), "Mob")) {
			IV tmp = SvIV((SV*)SvRV(ST(0)));
		...
		if ( !strcmp(rulename,"epicsenabled") )
			RETVAL = RuleB(ExpanionSettings, EpicsEnabled);
		else if ( !strcmp(rulename,"mobexpansion") )
			RETVAL = RuleI(ExpanionSettings, MobExpansion);
...
XS(boot_Mob)
{
	...
	XS_VERSION_BOOTCHECK ;
		...
		newXSproto(strcpy(buf, "GetMobHPRegen"), XS_Mob_GetMobHPRegen, file, "$");
		newXSproto(strcpy(buf, "GetMobManaRegen"), XS_Mob_GetMobManaRegen, file, "$");
		...
		newXSproto(strcpy(buf, "CheckGlobal"), XS_Mob_CheckGlobal, file, "$$");
		...
		newXSproto(strcpy(buf, "CheckRule"), XS_Mob_CheckRule, file, "$$");
	XSRETURN_YES;
}
