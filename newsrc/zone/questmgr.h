...
public:
	...
	void gmsay(const char *str, int32 color, bool send_to_world);
...
//  bool isflagset(int flag_num);
	...
	void unscribespells();
	void untraindiscs();
	void givecash(int copper, int silver, int gold, int platinum, bool convertCopper=false);
	void pvp(const char *mode);
	// Kings & Bandits questmgr.h additions
	void pvpvalue(const char *mode);
	void summonlootgroup(int groupid);
	...
	void facenpctype(int npc_type_id);
	// Kings & Bandits kingdom additions
	void setcitycitizentaxrate(uint32 kingdomid, uint8 taxratepct);
	void setcitynewbietaxrate(uint32 kingdomid, uint8 taxratepct);
	uint8 getcitycitizentaxrate(uint32 kingdomid);
	uint8 getcitynewbietaxrate(uint32 kingdomid);
	
	void setcitybankreserve(uint32 kingdomid, uint32 reservevalue);
	uint32 getcitybankreserve(uint32 kingdomid);
	uint32 getcityupkeepcost(uint32 kingdomid);
	
	uint8 getteamexpbonus(uint32 kingdomid);
	uint8 setteamexpbonus(uint32 kingdomid, uint8 value);
	
	uint32 findcampteamid(uint32 guildid);
	uint32 findguardteamid(uint32 npctypeid, uint32 spawngroupid);
	uint8 loadguardtype(uint32 kingdomid, uint32 npctypeid);
	uint8 addguardtype(uint32 kingdomid, uint32 npcid, uint16 classid, uint32 spawngroupid);
	
	uint32 addteamid(uint32 guildid, uint32 ownerteamid, uint32 charid, float x, float y, float z, float radius, uint32 zoneid);
	uint32 updateteamid(uint32 guildid, uint32 ownerteamid, uint32 charid, float x, float y, float z, float radius, uint32 zoneid);
	
	uint32 getguardfield(uint32 kingdomid, uint32 npctypeid, const char *fieldname);
	uint32 resetguardfields(uint32 kingdomid, uint32 npctypeid, int16 killentityid=0);
	
	uint32 spawninnpc(uint32 kingdomid, char* spawnName, uint32 npc_type_id, float x, float y, float z, float heading, int respawntime);
	uint8 getisupkeeppaid(uint32 kingdomid);
	
	uint8 zonepointnearby(float x, float y, float z, float maxdist);
	uint8 findnpcnear(float x, float y, float z, float maxdist);
	
	// Aded April 29/2013 KaB/Red69: Support for setting guildid,charid of owner

	void logdatabase(const char* accountname,int32 accountid,int8 status,const char* charname,const char* target, const char* descriptiontype,int event_nid, const char* description, ...);

	bool setkingdomowner(uint32 kingdomid, int32 guildid, int32 charid, int32 teamID, int32 charLevel=0);
	uint32 getkingdomownercharid(uint32 kingdomid);
	uint32 getkingdomownerguildid(uint32 kingdomid);
	bool setkingdomownerlastvisit(uint32 kingdomid, int32 timestamp);
	uint32 getkingdomownerlastvisit(uint32 kingdomid);
	
	void setcitymode(uint32 kingdomid, uint8 citymode, bool updateDB=false);
	void setcitylocation(uint32 kingdomid, float x, float y, float z, float radius, int32 zoneid);
	uint8 getcitymode(uint32 kingdomid);
	TeamSettings_Struct* getteamsettings(uint32 kingdomid);

	void save();
	void faction(int faction_id, int faction_value);
	void setsky(uint8 new_sky, uint8 new_red=0, uint8 new_green=0, uint8 new_blue=0, bool override_=false);
	...
	int32 getnextcorpsedecaytime(int32 char_id);
