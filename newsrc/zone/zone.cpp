...
volatile bool ZoneLoading = false;
extern Parser* parse;
...
bool Zone::Bootup(int32 iZoneID, int32 iInstanceID, bool iStaticZone) {
	...
	const char* zonename = database.GetZoneName(iZoneID,iInstanceID);
	...
	if (!zone->Init(iStaticZone)) {
		...
		// Kings & Bandits - Notify db zone is down
		char msg[256];
		snprintf(msg,256,"UPDATE zone set isup=0 where short_name='%s'",zonename);
		database.UpdateDatabaseQuery(msg);
		ZoneLoading = false;
		return false;
	}
	...
	if (database.RunQuery(query, len_query, errbuf, &result)) {
		...
		while ((row = mysql_fetch_row(result))) {
			if (atoi(row[9]) == 0)
			{
				...
				const char* shortname = database.GetZoneName(atoi(row[1]), instanceid, false); // zoneid -> zone_shortname
			...
			if (itemid == 0) {
				...
				// Groundspawn object
				// KaB / Red69 / Zek - Fixed charges so we can load back up items ppl dropped
				inst = database.CreateItem(itemid, charges);
				if ( inst != NULL && inst->GetItem()->MaxCharges == -1 )
			...
			Object* object = new Object(id, type, icon, data, inst);
				...
				// KaB / Red69 / Zek - Fixed adding objects so we can load back up items ppl dropped
...
int Zone::SaveTempItem(int32 merchantid, int32 npcid, int32 item, sint32 charges, bool sold){
	...
	if(update_charges){
		...
		if(sold && ml.charges != -1)
			ml.charges = ml.charges + charges;
		else if ( ml.charges < 0 )
			ml.charges = -1;
		else if ( charges > 0 )
			ml.charges = charges;
		else
		...
		if ( ml.charges != -1 && ml.charges < 1 )
			ml.charges = 0;

		if(!ml.origslot)
			ml.origslot = ml.slot;
		if((sold && charges > -1) || (!sold && charges!=0)){
			if ( ml.charges < 0 )
				charges = -1;
...
sint32 Zone::GetTempMerchantQuantity(int32 NPCID, uint32 Slot) {
...
void Zone::LoadTempMerchantData_result(MYSQL_RES* result) {
    MYSQL_ROW row;
	std::map<uint32,std::list<TempMerchantList> >::iterator cur = tmpmerchanttable.end();
	int32 npcid = 0;
...
void Zone::Shutdown(bool quite) 
{
	...
	// Notify db zone is up
	char msg[256];
	_snprintf(msg,256,"UPDATE zone set isup=0,static_zone=0 where short_name='%s'",zone->GetShortName());
	database.UpdateDatabaseQuery(msg);
	...
	std::map<uint32,EQApplicationPacket *>::iterator itrmerch;
	while(zone->merchantbuffer.size()) {
		itrmerch=zone->merchantbuffer.begin();
		safe_delete(itrmerch->second);
		zone->merchantbuffer.erase(itrmerch);
	}
...
{
	SetServerPlayerCounts(0,0,0,0,0,0);
	zoneid = in_zoneid;
	...
	can_pvp = 0; // Kings & Bandits - default pvp setting 0
	...
	pQueuedMerchantsWorkID = 0;
	pvpzone = false;
	guildmanagedisabled = false;
	use_death_mods = false;
	/* Kings & Bandits - bad dont need this! - still don't need this because we don't force people here, only when they level
	if(database.GetServerType() == 1)
		pvpzone = true;
	*/
	database.GetZoneLongName(short_name, &long_name, file_name, &psafe_x, &psafe_y, &psafe_z, &pgraveyard_id, &pMaxClients, &guildmanagedisabled);
	if(graveyard_id() > 0)
	{
...
Zone::~Zone() {
	...
	// clear temp merchant list
	tmpmerchanttable.clear();
	merchanttable.clear();
	...
	ClearZonePoints(&zone_point_list);
	entity_list.Clear();
	ClearBlockedSpells();
	...
	// Kings & Bandits cleanup
	safe_delete(Hour_Timer);
	if(aas != NULL) {
		int r;
...
bool Zone::Init(bool iStaticZone) {
	...
	//KaB (Red69) - Load persistent state into spawn2 list, 8-5-2011
	bool zoneLoadState = false;

	char pzs[3] = "";
	if (!iStaticZone && database.GetVariable("PersistentZoneState", pzs, 2)) {
		if (pzs[0] == '1') {
			try
			{
			LogFile->write(EQEMuLog::Normal, "Loading zone state");
			int resp = database.LoadZoneState(short_name, spawn2_list);
			if ( resp > 0 && spawn2_list.Count() > 5)
				zoneLoadState = true;
			else
				spawn2_list.Clear();
			}catch(...) { }
	...
	// failed to load spawns from persistence, load a fresh list
	if ( !zoneLoadState )
	{
		if (!database.PopulateZoneSpawnList(zoneid, instanceid, spawn2_list, GetInstanceVersion()))
		{
	...
	// Kings & Bandits - Notify db zone is up
	char msg[256];
	snprintf(msg,256,"UPDATE zone set isup=1,static_zone=%i where short_name='%s'",iStaticZone, short_name);
	database.UpdateDatabaseQuery(msg);
	...
	// K&B Achievements
	zone->LoadAchievements(zone->GetZoneID());
	...
	if (!LoadZoneCFG(zone->GetShortName(), zone->GetInstanceID(), true)) // try loading the zone name...
		LoadZoneCFG(zone->GetFileName(), 0); // if that fails, try the file name, then load defaults
...
void Zone::ReloadStaticData() {
	...
	// K&B Red69 zone point mem leak fix
	ClearZonePoints(&zone_point_list);
...
bool Zone::LoadZoneCFG(const char* filename, uint16 instance_id, bool DontLoadDefault) 
{
	...
	{
		map_name = NULL;
		// Kings & Bandits - added can_pvp field
		if(!database.GetZoneCFG(database.GetZoneID(filename), 0, &newzone_data, can_bind, 
			can_combat, can_levitate, can_castoutdoor, is_city, is_hotzone, default_ruleset, &map_name, &can_pvp, &canhatesummonfull, &is_pvphotzone, &no_pet_los_distance, &use_death_mods)) 
		{
...
bool Zone::SaveZoneCFG() {
	return database.SaveZoneCFG(GetZoneID(), GetInstanceID(), &newzone_data);
}
...
void Zone::AddAuth(ServerZoneIncommingClient_Struct* szic) {
	...
	zca->officialClient = szic->officialClient;
	zca->stale = false;
...
bool Zone::GetAuth(int32 iIP, const char* iCharName, int32* oWID, int32* oAccID, int32* oCharID, sint16* oStatus, char* oLSKey, bool* oTellsOff, bool* officialClient) {
	LinkedListIterator<ZoneClientAuth_Struct*> iterator(client_auth_list);
	...
	while (iterator.MoreElements()) {
		...
		if (strcasecmp(zca->charname, iCharName) == 0) {
				...
				*oTellsOff = zca->tellsoff;
				if (officialClient)
					*officialClient = zca->officialClient;
				zca->stale = true;
			return true;
		}
		iterator.Advance();
...
bool Zone::Process() {
	...
	// Kings & Bandits
	if ( Hour_Timer && Hour_Timer->Check()) {
		is_hotzone = database.getZoneHotZone((char*)GetShortName());
		is_pvphotzone = database.getPVPHotZone((char*)GetShortName());
	}
...
void Zone::Repop(int32 delay) {
	...
	if (!database.PopulateZoneSpawnList(zoneid, instanceid, spawn2_list, GetInstanceVersion(), delay))
...
ZonePoint* Zone::GetClosestZonePoint(float x, float y, float z, int32 to, Client* client, float max_distance) {
	...
	float max_dist_to_use = max_distance2;
	float closest_dist_to_use = max_distance2;
	iterator.Reset();
	while(iterator.MoreElements())
	{
		max_dist_to_use = max_distance2;
		...
		if (zp->target_zone_id == to || to == 0)
		{
			if ( zp->radius > 0 )
			{
				max_dist_to_use = zp->radius*zp->radius;
			}
			...
			if (dist < closest_dist)
			{
				closest_zp = zp;
				closest_dist = dist;
				closest_dist_to_use = max_dist_to_use;
			}
		}
		iterator.Advance();
	}
	
	if(closest_dist>(closest_dist_to_use))
	{
		float send_x = 0;
		float send_y = 0;
		float send_z = 0;
		...
		if(client)
			client->CheatDetected(MQZoneUnknownDest, client->GetX(),client->GetY(),client->GetZ(),send_x,send_y,send_z,to); //[Paddy] Someone is trying to use /zone
		...
		if (mysql_num_rows(result) == 1) {
			...
			if (lengths[4] != (sizeof(ZSDump_Spawn2) * spawn2_count)) {
				...
				for (i=0; i < spawn2_count; i++) {
					if ( spawn2_dump[i].spawn2_id == 0 )
					{
						//cerr << "SpawnID for: spawn2_dump[" << i << "] == 0" << endl;
						continue;
					}
			...
			if (lengths[5] != (sizeof(ZSDump_NPC) * npc_count)) {
				...
				for (i=0; i < npc_count; i++) {
					...
					else if ( npc_dump[i].spawn2_dump_index == 0 || npc_dump[i].corpse )
						continue; // need to fix corpses later, they might be spawning as npcs
					...
					if (npc_dump[i].npctype_id == 0) {
						if (npc_dump[i].gmspawntype_index == 0xFFFFFFFF) {
							...
							float x = npc_dump[i].x;
							float y = npc_dump[i].y;
							float z = npc_dump[i].z;
							float heading = npc_dump[i].heading;
							...
							if ( tmp != NULL )
							{
								...
								y = tmp->GetY();
								z = tmp->GetZ();
								heading = tmp->GetHeading();
							}

							npc_loaded[i] = new NPC(&gmspawntype_dump[npc_dump[i].gmspawntype_index], tmp, x,y,z,heading, FlyMode3, npc_dump[i].corpse);
							if ( tmp != NULL )
							{
								npc_loaded[i]->SetSp2(tmp->SpawnGroupID());
						...
						if (crap != 0)
						{
							...
							npc_loaded[i] = new NPC(crap, tmp, x,y,z,heading, FlyMode3, npc_dump[i].corpse);
							if ( tmp != NULL )
							{
								npc_loaded[i]->SetSp2(tmp->SpawnGroupID());
								if ( zone->InstantGrids() )
...
void Zone::weatherSend(Client* clientToSend)
{
	/*switch(zone_weather)
	{
	case 0:
	...
	if ( clientToSend != NULL )
		clientToSend->QueuePacket(outapp);
	else
...
// Load Kings & Bandits achievements
void Zone::LoadAchievements(int32 zoneid)
{
	if ( !database.DBLoadAchievements(zoneid) )
		LogFile->write(EQEMuLog::Error, "... Failed to load Kings and Bandits achievements.");
}
...
KBAchievement* Zone::GetAchievementID(int32 achievementid) {
	LinkedListIterator<KBAchievement*> iterator(achievements);
	iterator.Reset();
	while(iterator.MoreElements())
	{
		KBAchievement* kb = iterator.GetData();
		if ( kb && kb->achievementid == achievementid )
			return kb;
		iterator.Advance();
	}
...
// Kings & Bandits (Red69) Fix mem leak on zone points
void Zone::ClearZonePoints(LinkedList<ZonePoint*>* zone_points)
{
	if ( zone_points == NULL )
		return;

	LinkedListIterator<ZonePoint*> iterator(*zone_points);
	iterator.Reset();
	while(iterator.MoreElements())
	{
		iterator.RemoveCurrent();
...
void ZoneDatabase::logdatabase(const char* accountname,int32 accountid,int8 status,const char* charname,const char* target, const char* descriptiontype,int event_nid, char* zonename, const char* description, ...)
{
	va_list argptr;
	char buffer[256];
	
	va_start(argptr, description);
	vsnprintf(buffer, 256, description, argptr);
	va_end(argptr);
	database.logevents(accountname,accountid,status,charname,target,descriptiontype,(char*)buffer,event_nid,0,0,0,zonename);
}