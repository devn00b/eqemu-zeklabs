...
string TitleManager::GetPrefix(int TitleID, Client* c)
{
	vector<TitleEntry>::iterator Iterator;
	...
	while(Iterator != Titles.end())
	{
		...
		{
			if ( c == NULL || IsClientEligibleForTitle(c,Iterator) ) // check and see if it is ok for the client to take this title
...
string TitleManager::GetSuffix(int TitleID, Client* c)
{
	vector<TitleEntry>::iterator Iterator;
