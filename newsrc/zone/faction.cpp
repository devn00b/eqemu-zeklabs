...
FACTION_VALUE Client::GetReverseFactionCon(Mob* iOther) {
	...
	// Kings & Bandits force kindly faction when gm flag is on
	if ( GetGM() )
	...
	if (iOther->GetPrimaryFaction() == 0 && iOther->IsNPC() && iOther->CastToNPC()->teamID == 0)
		return FACTION_INDIFFERENT;
...
FACTION_VALUE Client::GetFactionLevel(int32 char_id, int32 npc_id, int32 p_race, int32 p_class, int32 p_deity, sint32 pFaction, Mob* tnpc)
{
	...
	{    	//pFaction == 0
		if ( IsNPC() && tnpc->CastToNPC()->teamID == 0 )
			return(FACTION_INDIFFERENT);
		/*
	...
	// Kings & Bandits - Client::GetFactionLevel
	if ( tnpc->IsNPC() )
	{
		char tmp[20];
		database.GetVariable("ServerType", tmp, 9);
		int pvpFlag = atoi(tmp);
		if ( tnpc->CastToNPC()->teamID > 0 )
		{
			switch(pvpFlag)
			{
			case SERVERTYPE_RED69:
				{
					TeamSettings_Struct* ts = database.GetTeamSettings(tnpc->CastToNPC()->teamID);
					if ( ts && ts->guildid > 0 && ts->guildid == GuildID() )
					{
						if ( fac >= FACTION_KINDLY )
							return FACTION_KINDLY;
					}
					else if ( ts && ts->guildid > 0 && GuildID() > 0 && GuildID() != GUILD_NONE && ts->guildid != GuildID() ) // only apply people in guilds to low faction
					{
						return FACTION_SCOWLS;
					}
					break;
			...
			case SERVERTYPE_KAB:
				{
					bool passStatus = false; // if true we hate them!
					bool overrideRed = RuleB(PVPCombat,PureRed);
					bool teamGuildWars = RuleB(PVPCombat,TeamGuildWars);
					
					TeamSettings_Struct* team = database.GetTeamSettings(tnpc->CastToNPC()->teamID);

					if ( !team )
						break;
					
					uint8 cityMode = team->citymode;
				if ( cityMode == CITYENABLED || cityMode == CAMPCONSTRUCTED )
					...
					if ( !overrideRed )
					{
						if ( teamGuildWars )
						{
								...
								{
									Mob* actualTarget = NULL;

									if ( GetAttackTarget()->HasOwner() )
										actualTarget = GetAttackTarget()->GetOwner();
									else
										actualTarget = GetAttackTarget();

									if ( actualTarget != NULL && actualTarget->IsClient() && 
							// Image (Jul 29, 2019): rewritten if statement to include bandit team id to check on guildid
								( (team->ownerteamid == BANDITTEAMID && actualTarget->CastToClient()->GuildID() == team->guildid && team->guildid != GuildID() )
								|| ( team->ownerteamid != BANDITTEAMID && actualTarget->CastToClient()->GetPVPValue() == team->ownerteamid && GetPVPValue() != team->ownerteamid ))
								)
										passStatus = true;
								}
								if ( GetAttackTarget() == NULL && 
									GetLevel() < RuleI(Kingdoms,GuardAttacksLevel) )
								{
									// do nothing
								}
							// Image (Jul 29, 2019): rewritten if statement to include bandit team id to check on guildid
								else if ( cityMode == HOSTILE && GetPVPValue() > 0 && (( team->ownerteamid != BANDITTEAMID && GetPVPValue() != team->ownerteamid) || 
									( team->ownerteamid == BANDITTEAMID && GuildID() != team->guildid )))
								{
									passStatus = true;
								}
							// Image (Jul 29, 2019): rewritten if statement to include bandit team id to check on guildid
								else if ( cityMode == RESTRICTIVE && GetPVPValue() > 0 && GuildID() > 0 && GuildID() != GUILD_NONE && ((team->ownerteamid != BANDITTEAMID && GetPVPValue() != team->ownerteamid) || 
									( team->ownerteamid == BANDITTEAMID && GuildID() != team->guildid )) )
								{
									GuildHate_Struct* ghs = NULL;
									ghs = database.GetGuildHateList(tnpc->CastToNPC()->teamID,GuildID());
						...
						{
							if ( cityMode == HOSTILE && GetPVPValue() > 0 && ( tnpc->CastToNPC()->teamID != GetPVPValue() && (team->guildid == 0 || team->guildid != GuildID()) ) )
								passStatus = true;
							else if ( cityMode == RESTRICTIVE && ( tnpc->CastToNPC()->teamID != GetPVPValue() && (team->guildid == 0 || team->guildid != GuildID()) ) )
							{	
								GuildHate_Struct* ghs = NULL;
								ghs = database.GetGuildHateList(tnpc->CastToNPC()->teamID,GuildID());
					...
					else if ( GetAttackTarget() != NULL && GetAttackTarget()->IsClient() && 
						GetAttackTarget()->CastToClient()->GuildID() == team->guildid && 
						GuildID() != team->guildid )
						passStatus = true;
					if ( overrideRed && GetAttackTarget() == NULL && 
						GetLevel() < RuleI(Kingdoms,GuardAttacksLevel) && ( GuildID() == 0 || GuildID() == GUILD_NONE ) )
					{
						// do nothing
					}
					else if ( overrideRed && GetPVPValue() > 0 && cityMode == HOSTILE && ( ( team->guildid == 0 ) || ( team->guildid > 0 && GuildID() != team->guildid ) ) )
					{
						passStatus = true;
					}
					else if ( overrideRed && cityMode == RESTRICTIVE && GuildID() > 0 && GuildID() != GUILD_NONE && GuildID() != team->guildid )
					{
						GuildHate_Struct* ghs = NULL;
						ghs = database.GetGuildHateList(tnpc->CastToNPC()->teamID,GuildID());
					...
					//printf("Data: %i %i %i %i\n",overrideRed, team->citymode, GuildID(), passStatus);
					...
					if ( GetPVPValue() > 0 )
					{
						...
						{
							TeamSettings_Struct* team = database.GetTeamSettings(tnpc->CastToNPC()->teamID);
							if ( fac >= FACTION_KINDLY && (( RuleB(PVPCombat,TeamGuildWars) && team != NULL && team->ownerteamid == GetPVPValue() ) ||
								( !overrideRed && tnpc->CastToNPC()->teamID == GetPVPValue() ) || 
								( overrideRed && team->guildid == GuildID() )) )
								return FACTION_KINDLY;
						}
						else
						{

							if ( ( team && team->upkeepPaid ) || ( tnpc->CastToNPC()->GetFactionCount() < 1 ) )
								return FACTION_SCOWLS;
						}
					}
					break;
		...
		bool guildwarsTeams = RuleB(PVPCombat,TeamGuildWars);
		if ( pvpFlag == SERVERTYPE_KAB || guildwarsTeams )
		{
			TeamSettings_Struct* team = NULL;
			...
			case 8: //freportn
				{
					team = database.GetTeamSettings(FREEPORTTEAMID);
					break;
				}
			case 29: // halas
				{
					team = database.GetTeamSettings(HALASTEAMID);
					break;
				}
			case 1: // qeynos
			case 2: // qeynos2
				{
					team = database.GetTeamSettings(QEYNOSTEAMID);
					break;
				}
			case 19: // rivervale
				{
					team = database.GetTeamSettings(RIVERVALETEAMID);
					break;
				}
			case 61: // felwithea
			case 62: // felwitheb
				{
					team = database.GetTeamSettings(FELWITHETEAMID);
					break;
				}
			case 23: // erudnint
			case 24: // erudnext
				{
					team = database.GetTeamSettings(ERUDINTEAMID);
					break;
				}
			case 115: // thurgadina
			case 129: // thurgadinb
				{
					team = database.GetTeamSettings(THURGADINTEAMID);
					break;
				}
			}

			if ( team != NULL && ( (pvpFlag == SERVERTYPE_KAB && GetPVPValue() == team->teamID) || (guildwarsTeams && team->ownerteamid == GetPVPValue()) )  )
			{
				if ( tnpc->CastToClient()->GetClass() == MERCHANT )
				{
					if ( team && !team->upkeepPaid && fac >= FACTION_INDIFFERENT )
						return FACTION_INDIFFERENT;
					else if ( team && team->upkeepPaid && fac >= FACTION_KINDLY )
						return FACTION_KINDLY;
				}
				else
					return FACTION_INDIFFERENT;
			}
			else if ( team != NULL && pvpFlag == SERVERTYPE_KAB && GetPVPValue() > 0 && tnpc->CastToNPC()->GetClass() != MERCHANT && team->upkeepPaid )
				return FACTION_SCOWLS;
		}
	}
...
void  Client::SetFactionLevel(int32 char_id, int32 npc_id, int8 char_class, int8 char_race, int8 char_deity)
{
	...
	for(int i = 0;i<MAX_NPC_FACTIONS;i++)
	{
		...
		if(database.GetFactionData(&fm,char_class,char_race,char_deity,faction_id[i]))
		{
			...
			if(this->itembonuses.CHA) {
				int faction_mod = itembonuses.CHA / 5;
