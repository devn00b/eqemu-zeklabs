	...
	switch (event) {
		...
		case EVENT_AGGRO_SAY: {
			...
			break;
							  ...
							  // Kings & Bandits - EVENT_ADD_LOOT call
		case EVENT_ADD_LOOT: {
			
			MakeVars(data, npcid);
			SendCommands("event_add_loot", qstID, npcmob, mob);
			break;
							 }
		case EVENT_LOGOUT: {
			SendCommands("event_logout", qstID, npcmob, mob);
			break;
							 }
		default: {
...
//end Myra
	...
	else if (!strcmp(command,"unique_spawn")) 
	{
		...
		quest_manager.givecash(atoi(arglist[0]), atoi(arglist[1]), atoi(arglist[2]), atoi(arglist[3]), atoi(arglist[4]));
	}
