...
public:
	...
	void	SetOwner(Client* newOwner) { owner = newOwner; }
	Client*	GetMember(int pos) { if (pos < 6) return members[pos]; return NULL; }
	void	SetMember(int pos, Client* member) { if (pos < 6) members[pos] = member; }
private:
...
public:
	...
	void	ZoneCancelByOwner(Client* owner);
