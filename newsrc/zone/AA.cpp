...
void Mob::TemporaryPets(int16 spell_id, Mob *targ, const char *name_override, uint32 duration_override, bool spawnAtPlayer) {
	...
	while(summon_count > 0) {
		...
		if ( !spawnAtPlayer )
		{
			spawnX += swarm_pet_x[summon_count];
			spawnY += swarm_pet_y[summon_count];
		}
		...
		NPC* npca = new NPC(
				...
				0, 
				spawnX, spawnY, 
				GetZ(), GetHeading(), FlyMode3, false, true);
		...
		if ( npca->GetNPCTypeID() == RuleI(Zone,EyeOfZommNPCID) )
		{
			npca->CastToMob()->SetPetOrder(SPO_ClientControl);
			npca->CastToMob()->SetFollowID(0);
		}
		summon_count--;
	}
...
int32 ZoneDatabase::CountAAs(){
    ...
    MYSQL_ROW row;
	int count=0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT count(title_sid) from altadv_vars where disabled=0"), errbuf, &result)) {
...
void ZoneDatabase::LoadAAs(SendAA_Struct **load){
	...
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT skill_id from altadv_vars where disabled=0 order by skill_id"), errbuf, &result)) {
		int skill=0,ndx=0;
		while((row = mysql_fetch_row(result))!=NULL) {
			skill=atoi(row[0]);
...
SendAA_Struct* ZoneDatabase::GetAASkillVars(int32 skill_id)
{
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SET @row = 0"), errbuf)) {	//initialize "row" variable in database for next query
		...
		if (RunQuery(query, MakeAnyLenString(&query, 
			...
			" FROM altadv_vars a WHERE disabled=0 and skill_id=%i", skill_id), errbuf, &result)) {
			safe_delete_array(query);
			if (mysql_num_rows(result) == 1) {
