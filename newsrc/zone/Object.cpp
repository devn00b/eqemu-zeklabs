...
// KaB / Red69 / Zek - Addition for item ground spawns to be set without checking the map z coord 9/19/2012 (nomapz var)
Object::Object(const ItemInst* inst, char* name,float max_x,float min_x,float max_y,float min_y,float z,float heading,int32 respawntimer, bool nomapz)
 : respawn_timer(respawntimer), decay_timer(300000)
{
	...
	m_nomapz = nomapz; // KaB / Red69 / Zek - Addition for item ground spawns to be set without checking the map z coord 9/19/2012
	RandomSpawn(false);
...
Object::Object(const ItemInst *inst, float x, float y, float z, float heading, int32 decay_time, int32 levelrequirement, char* qglobalflag)
 : respawn_timer(0), decay_timer(decay_time)
{
	...
	m_levelrequirement = levelrequirement;
	strncpy(m_qglobal,qglobalflag,64);
}
	...
	if(user != NULL)
	{
		...
		if(container != NULL)
		{
			for (uint8 i = 0; i < MAX_ITEMS_PER_BAG; i++)
			{
				...
				if(inst != NULL)
				{	
					user->PutItemInOpenSlot(inst->GetItem()->ID,inst->GetCharges(),inst->GetAugmentItemID(0),inst->GetAugmentItemID(1),
						inst->GetAugmentItemID(2),inst->GetAugmentItemID(3),inst->GetAugmentItemID(4));
						safe_delete(inst);
				}
			}
		}
...
void Object::RandomSpawn(bool send_packet) {
	...
	// Red69 / KaB / Zek - Fix for randomized spawn locations having a proper Z location on the map.
	//printf("Original pos for object randomized to %4.3f, %4.3f, %4.3f\n", m_data.x, m_data.y, m_data.z);

	if ( !m_nomapz && zone->HasMap() )
	{
			VERTEX dest(m_data.x, m_data.y, m_data.z);

			float newz = zone->zonemap->FindClosestZ(dest);

		//	printf("BestZ returned %4.3f at %4.3f, %4.3f, %4.3f\n", newz, m_data.x, m_data.y, m_data.z);
...
void Object::RejectClick(Client* sender)
{
	EQApplicationPacket* outapp = new EQApplicationPacket(OP_ClickObject, sizeof(ClickObject_Struct));
	...
	clickobj->player_id = sender->GetID();
...
bool Object::HandleClick(Client* sender, const ClickObject_Struct* click_object)
{
	...
	if (m_type == OT_DROPPEDITEM) {
		if (m_inst && sender) {
			...
			QGlobal res;
			if ( strlen(GetQGlobalFlag()) > 0 && 
				!QGlobalCache::GetQGlobal(res,sender->GetQGlobals()->GetBucket(),GetQGlobalFlag(),0,sender->CharacterID(),zone->GetZoneID()) ) {
				RejectClick(sender);
				sender->Message(13,"You may not pick up this item as you are missing a requirement (%s)",GetQGlobalFlag());
				return false;
			}
			if(GetLevelRequirement() > 0 && sender->GetLevel() > GetLevelRequirement() && !sender->GetGM()) {
				RejectClick(sender);
				sender->Message(13,"You may not pick up this item you are too high level (must be level %i or lower)",GetLevelRequirement());
				return false;
			}
			...
			if(sender->CheckLoreConflict(m_inst->GetItem())) {
				_snprintf(logMsg,256,"(LoreConflict) Player attempted pick up of item %d %s",m_inst->GetItem()->ID,m_inst->GetItem()->Name);
				sender->LogPlayerMessage(23,"Player Attempted Pickup Lore Conflict", logMsg, CastToMob()); // KaB / Red69 / Zek - 9/18 addition for target info from obj
				sender->Message_StringID(0, DUP_LORE);
			...
			_snprintf(logMsg,256,"Player Picked Up item %d %s",m_inst->GetItem()->ID,m_inst->GetItem()->Name);
			sender->LogPlayerMessage(21,"Player Picked Up Item", logMsg, CastToMob()); // KaB / Red69 / Zek - 9/18 addition for target info from obj
#ifdef EMBPERL
			char buf[10];
		...
		if(user == NULL || user == sender)
			...
			coa->open		= 0x00;
			sender->Message(13, "Somebody is already using that container.");
		}
		m_inuse			= true;
		coa->type		= m_type;
		coa->unknown16	= 0x0a;	
...
Ground_Spawns* ZoneDatabase::LoadGroundSpawns(int32 zone_id, int16 version, Ground_Spawns* gs){
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT max_x,max_y,max_z,min_x,min_y,heading,name,item,max_allowed,respawn_timer,nomapz from ground_spawns where zoneid=%i and version=%u limit 50", zone_id, version), errbuf, &result))
	{
		safe_delete_array(query);
		int i=0;
