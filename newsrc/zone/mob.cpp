...
attack_timer(2000),
	...
	GravityTimer(1000),
	ViralTimer(0),
	aggroparser_timer(1000),
	ReplayingTimer(RuleI(Replay,SnapshotDelay)*1000)	
{
	effect_size = 100;
	ownerptr = NULL;
	// setting default for attack_timer not being triggered
	attacked = false;
	// Kings & Bandits - lastweaponspeed default
	SetLastWeaponSpeed(0);
	...
	cur_mana = 0;
	max_mana = 0;
	default_hp_regen = in_hp_regen;
	default_mana_regen = in_mana_regen;
	...
	SetPetType(petNone); //default to none
	held = false;
...
Mob::~Mob()
{
	...
	// Kings & Bandits - despawnpacket required
	safe_delete(app);
...
void Mob::SetInvisible(bool state)
{
	...
	if (GetPet() &&  ( invisible || hidden || improved_hidden) )
	{
...
bool Mob::IsInvisible(Mob* other) const
{
	...
	//handle sneaking
	if(sneaking) {
		// KaB - Zek - Red69 - Non-rogue sneaking, not behind a mob will not make you 'invisible'
		if ( GetClass() == ROGUE && hidden )
		{
			if ( other->IsClient() )
			{
				if ( !improved_hidden && other->SeeInvisible() )
					return false;
				else if ( improved_hidden && other->CastToClient()->GetGM() )
					return false;
				else if ( improved_hidden && other->see_improved_hide )
					return false;
			}
			
			return true;
		}
		else if ( !BehindMob(other, GetX(), GetY(), 45.0f ) ) // block 270 degrees of view (they can see 90 degrees, so 45*2)
			return false;
		else
			return true;
	}
...
sint32 Mob::CalcMaxHP() 
{
	...
	max_hp += (sint32)((double)max_hp * (double)(spellbonuses.MaxHPChange + itembonuses.MaxHPChange) / 10000.0);
...
void Mob::CreateSpawnPacket(EQApplicationPacket* app, Mob* ForWho, bool omitPlayerLocation) {
	...
	if ( IsClient() && ForWho != NULL && ForWho->IsClient() )
	{
			float dist = RuleR(PVPCombat, ClientViewSpawnDistance);
			float dist2 = dist * dist; //pow(dist, 2);
			if ( (DistNoRoot(*ForWho) > dist2)) {
				ns->spawn.x = zone->safe_x();
				ns->spawn.y = zone->safe_y();
				ns->spawn.z = zone->safe_z();
			}
	}

	if ( omitPlayerLocation )
	{
		ns->spawn.x = zone->safe_x();
		ns->spawn.y = zone->safe_y();
		ns->spawn.z = zone->safe_z();
...
void Mob::FillSpawnStruct(NewSpawn_Struct* ns, Mob* ForWho)
{
	...
	ns->spawn.walkspeed	= runspeed * 0.4f;
	...
	if ( IsNPC() && CastToNPC()->IsTemporaryPet() )
	{
	...
	// Kings & Bandits - CLIENT COLOR SCHEME FLAG SETTING USING MODIFIED CLIENT
	if ( IsClient() )
	{
		switch(CastToClient()->GetPVPValue() )
		{
		case NEUTRALPLAYER:
			ns->spawn.bodytype = 1; // blue
			break;
		case QEYNOSTEAMID:
			ns->spawn.bodytype = 27; // dark purple
			break;
		case HALASTEAMID:
			ns->spawn.bodytype = RuleI(PVPCombat,HalasBodyType); // formerly orange (29) bugs out spells
			break;
		case FREEPORTTEAMID:
			ns->spawn.bodytype = 30; // light blue
			break;
		case RIVERVALETEAMID:
			ns->spawn.bodytype = 31; // bright yellow
			break;
		case BANDITTEAMID:
			ns->spawn.bodytype = 1;
			// SET AT_PVP flag to 1 in Mob::SendAppearancePacket
			break;
		}

		if ( RuleB(PVPCombat, InfamySpawnColor) && CastToClient()->IsInfamyReached() )
			ns->spawn.bodytype = RuleI(PVPCombat, InfamySpawnValue); // bright yellow
	}

	memset(ns->spawn.set_to_0xFF, 0xFF, sizeof(ns->spawn.set_to_0xFF));
	
	if ( IsNPC() && CastToNPC()->isGuildNPC )
		ns->spawn.guildID = 0;
}
...
void Mob::SendHPUpdate(bool skipSelf)
{
...
void Mob::SendPosition() 
{
	...
	bool eyeZomm = false;
	Mob* zommOwner = NULL;
	if ( GetOwnerID() != 0 && IsNPC() && CastToNPC()->GetNPCTypeID() == RuleI(Zone,EyeOfZommNPCID) )
	{
		eyeZomm = true;
		Mob* owner = entity_list.GetMob(GetOwnerID());
		if ( owner != NULL && owner->IsClient() )
			zommOwner = owner;
	}

	float range = 800;

	if ( IsClient() && CastToClient()->IsShadowStepUpdate() )
		range = RuleI(Zone,ShadowStepPositionUpdateDistance);

	entity_list.QueueCloseClients(this, app, true, range, zommOwner, true);
	safe_delete(app);
}
...
void Mob::SendPosUpdate(int8 iSendToSelf) {
	...
	else
	{
		...
		{
			if ( zommOwner )
				entity_list.QueueCloseClients(this, app, true, range, zommOwner, false);
			else
				entity_list.QueueClients(this, app, (iSendToSelf==0), false, true);
			move_tic_count = 0;
		}
		else
		{
			if ( zommOwner )
				...
				entity_list.QueueCloseClients(this, app, (iSendToSelf==0), range, NULL, false);
			move_tic_count++;
		}
#endif
	}
...
void Mob::ShowStats(Client* client) {
	...
	if(c && RuleB(Character, UseNewStatsWindow)) {
		...
		if (c) {
			client->Message(2, "  Level: %i  AC: %i  Class: %i  Size (%i/100 mod): %1.1f  Weight: %.1f/%d  Haste: %i DS: %i DS Cap: %i", GetLevel(), GetAC(), GetClass(), GetSizeMod(), GetSize(), (float)c->CalcCurrentWeight() / 10.0f, c->GetSTR(), GetHaste(), c->GetDS(), RuleI(Character, ItemDamageShieldCap));
			client->Message(2, "  HP: %i  Max HP: %i  HP Regen: %i HP Regen Cap: %i",GetHP(), GetMaxHP(), c->CalcHPRegen(), c->CalcHPRegenCap());
			client->Message(2, "  Mana: %i  Max Mana: %i  Mana Regen: %i Mana Regen Cap: %i", GetMana(), GetMaxMana(), c->CalcManaRegen(), c->CalcManaRegenCap());
			client->Message(2, "  End.: %i, Max End.: %i  End. Regen: %i End. Regen Cap: %i",c->GetEndurance(), c->GetMaxEndurance(), c->CalcEnduranceRegen(), c->CalcEnduranceRegenCap());
			client->Message(2, "  Bind Point: Zone (%i): %s X: %f Y: %f Z: %f",c->CastToClient()->GetBindZoneID(),database.GetZoneName(c->CastToClient()->GetBindZoneID(),0,true), c->CastToClient()->GetBindX(), c->CastToClient()->GetBindY(), c->CastToClient()->GetBindZ());
		}
		else {
			client->Message(2, "  Level: %i  AC: %i  Class: %i  Size (%i/100 mod): %1.1f  Haste: %i", GetLevel(), GetAC(), GetClass(), GetSizeMod(), GetSize(), GetHaste());
			client->Message(2, "  HP: %i  Max HP: %i",GetHP(), GetMaxHP());
			client->Message(2, "  Mana: %i  Max Mana: %i", GetMana(), GetMaxMana());
		}

		client->Message(12, "  Total ATK: %i  Worn/Spell ATK (Cap %i): %i  Server Used ATK: %i", this->CastToClient()->GetTotalATK(), RuleI(Character, ItemATKCap), GetATKBonus(), attackRating);
		client->Message(12, "  STR: %i  STA: %i  DEX: %i  AGI: %i  INT: %i  WIS: %i  CHA: %i Haste: %i", GetSTR(), GetSTA(), GetDEX(), GetAGI(), GetINT(), GetWIS(), GetCHA(), GetHaste());
		if (c)
			client->Message(12, "  hSTR: %i  hSTA: %i  hDEX: %i  hAGI: %i  hINT: %i  hWIS: %i  hCHA: %i", c->GetHeroicSTR(), c->GetHeroicSTA(), c->GetHeroicDEX(), c->GetHeroicAGI(), c->GetHeroicINT(), c->GetHeroicWIS(), c->GetHeroicCHA());
		client->Message(12, "  MR: %i  PR: %i  FR: %i  CR: %i  DR: %i Corruption: %i", GetMR(), GetPR(), GetFR(), GetCR(), GetDR(), GetCorrup());
		if (c)
			client->Message(12, "  hMR: %i  hPR: %i  hFR: %i  hCR: %i  hDR: %i hCorruption: %i", c->GetHeroicMR(), c->GetHeroicPR(), c->GetHeroicFR(), c->GetHeroicCR(), c->GetHeroicDR(), c->GetHeroicCorrup());
		// Mod2s
		if (c)
			client->Message(14, "  Shielding: %i  Spell Shield: %i  DoT Shielding: %i Stun Resist: %i  Strikethrough: %i  Avoidance: %i  Accuracy: %i  Combat Effects: %i", c->GetShielding(), c->GetSpellShield(), c->GetDoTShield(), c->GetStunResist(), c->GetStrikeThrough(), c->GetAvoidance(), c->GetAccuracy(), c->GetCombatEffects());
		// Mod3s
		if (c)
			client->Message(14, "  Heal Amt.: %i  Spell Dmg.: %i  Clairvoyance: %i DS Mitigation: %i", c->GetHealAmt(), c->GetSpellDmg(), c->GetClair(), c->GetDSMit());
		if(c && GetClass() == BARD)
			client->Message(14, "  Singing: %i  Brass: %i  String: %i Percussion: %i Wind: %i", c->GetSingMod(), c->GetBrassMod(), c->GetStringMod(), c->GetPercMod(), c->GetWindMod());
	}
	client->Message(14, "  Race: %i  BaseRace: %i  Texture: %i  HelmTexture: %i  Gender: %i  BaseGender: %i", GetRace(), GetBaseRace(), GetTexture(), GetHelmTexture(), GetGender(), GetBaseGender());
	if (client->Admin() >= 100) {
		client->Message(14, "  EntityID: %i  PetID: %i  OwnerID: %i  AIControlled: %i  Targetted: %i LastAtkWepSpeed: %i", 
			this->GetID(), this->GetPetID(), this->GetOwnerID(), this->IsAIControlled(), this->targeted, GetLastWeaponSpeed() );
		if (c) {
			client->Message(14, "  CharID: %i  PetID: %i OfficialClient: %i", c->CharacterID(), this->GetPetID(), c->IsOfficialClient());
		}
		else if (this->IsCorpse()) {
			if (this->IsPlayerCorpse()) {
				Corpse *corpse = this->CastToCorpse();
				client->Message(14, "  CharID: %i  PlayerCorpse: %i", corpse->GetCharID(), corpse->GetDBID());
			}
			else {
				client->Message(14, "  NPCCorpse", this->GetID());
			}
		}
		else if (this->IsNPC()) {
			...
			if(n->respawn2 != 0)
			{
				...
				deathcount = n->respawn2->getDeathCount();
			}
			client->Message(14, "  NPCID: %u  SpawnGroupID: %u LootTable: %u  FactionID: %i  SpellsID: %u MerchantID: %i", this->GetNPCTypeID(),spawngroupid, n->GetLoottableID(), n->GetNPCFactionID(), n->GetNPCSpellsID(), n->MerchantType);
			client->Message(14, "  Accuracy: %i  MeleeTex1: %i  MeleeTex2: %i DeathCount: %i", n->GetAccuracyRating(),n->GetMeleeTexture1(),n->GetMeleeTexture2(), deathcount);
			n->DisplayLootInfo(client); // display condensed loot list
			// Kings & Bandits display grid info minus grid waypoints
			n->DisplayWaypointInfo(client,false);
	...
	for (i=0; i < buff_count; i++) {
		if (buffs[i].spellid != SPELL_UNKNOWN) {
			if (buffs[i].durationformula == DF_Permanent)
				client->Message(0, "  %i: %s(%i): Permanent", i, spells[buffs[i].spellid].name,buffs[i].spellid);
			else if ( buffs[i].player_id > 0 && buffs[i].player_id != MAX_PLAYERID )
				client->Message(0, "  %i: %s(%i): %i tics left, caster_player_id: %i", i, spells[buffs[i].spellid].name,buffs[i].spellid, buffs[i].ticsremaining, buffs[i].player_id);
			else
				client->Message(0, "  %i: %s(%i): %i tics left", i, spells[buffs[i].spellid].name,buffs[i].spellid, buffs[i].ticsremaining);
...
void Mob::SendIllusionPacket(int16 in_race, int8 in_gender, int8 in_texture, int8 in_helmtexture, int8 in_haircolor, int8 in_beardcolor, int8 in_eyecolor1, int8 in_eyecolor2, int8 in_hairstyle, int8 in_luclinface, int8 in_beard, int8 in_aa_title, int32 in_drakkin_heritage, int32 in_drakkin_tattoo, int32 in_drakkin_details, float in_size) {
	...
	if ( GetSizeMod() > 0 )
		this->size = this->size * (GetSizeMod() / 100.0);
...
void Mob::SendAppearancePacket(int32 type, int32 value, bool WholeZone, bool iIgnoreSelf, Client *specific_target) {
	...
	// sent if we want to force to a single flag so buffs work correctly between all players
	if ( type == AT_PVP && RuleB(PVPCombat,PVPFlagForced) && value > 0 )
		value = 1;
	// Kings & Bandits Modification - send different color depending on team
	else if ( type == AT_PVP && RuleB(PVPCombat,TeamSpawnColor) )
	{
		// NOTE: NEVER EVER SendAppearancePacket to AT_PVP, INFINITE LOOP!
		switch(value)
		{
		case BANDITTEAMID: // team 5, set AT_PVP to true "Red" flag.
			{
				value = 1;
				break;
			}
		...
		if ( zone->CanPVPValue() == PVPFFAVALUE )
			value = 1;

		// Don't need this anymore because we handle it in Mob::FillSpawnStruct
		/*	switch(value)
		{
		case 0:
		SendAppearancePacket(AT_NPCName,0);
		SendAppearancePacket(AT_Anon,0);
		break;
		case 1: // first team light-blue (qeynos)
		SendAppearancePacket(AT_Anon,0);
		SendAppearancePacket(AT_NPCName,2);
		type = AT_PVP;
		value = 0; // sets npc name with no 'Trader' tag
		break;
		case 2: // second team color pink (halas)
		SendAppearancePacket(AT_NPCName,0);
		SendAppearancePacket(AT_Anon,2);
		if ( IsClient() )
		CastToClient()->SetAnon(2);
		value = 1;
		break;
		case 3: // second team color is red (freeport)
		SendAppearancePacket(AT_NPCName,0);
		SendAppearancePacket(AT_Anon,0);
		value = 1;
		break;
		case 4: // fourth team is purple (rivervale)
		SendAppearancePacket(AT_NPCName,0);
		SendAppearancePacket(AT_Anon,2);
		if ( IsClient() )
		CastToClient()->SetAnon(2);
		value = 0; // reset pvp flag to 0
		break;
		}*/
	}
	...
	if ( type == AT_NPCName && value && this->IsClient() && this->CastToClient()->GetPVP() > 0 )
		SendAppearancePacket(AT_PVP, 0);
...
void Mob::ChangeSize(float in_size, int effect_mod, bool bNoRestriction) {
	//Neotokyo's Size Code
	
	if ( effect_mod < 1 )
		effect_mod = 100;
	
	if ( effect_mod != effect_size )
	{
		effect_size = effect_mod;
		switch(race){
			case OGRE:
				in_size = 9;
				break;
		...
		this->size = in_size * ((float)effect_size / 100.0f);
	}
	else if ( in_size != 0.0f )
		this->size = in_size;
	...
	SendAppearancePacket(AT_Size, (int32) this->size);
}
...
//heko: for backstab
bool Mob::BehindMob(Mob* other, float playerx, float playery, float angleRequirement) const {
	if (!other)
		return true; // sure your behind your invisible friend?? (fall thru for sneak)

	if ( angleRequirement == 0.0f )
		angleRequirement = 90.0f;
	...
	if (angle > angleRequirement) //not sure what value to use (90*2=180 degrees is front)
		return true;
	else
		return false;
}
...
void Mob::SetAttackTimer() {
	...
	// Image(2014): cpu check, reset since we are triggering
	attacked = false;
...
bool Mob::CanThisClassDualWield(bool ignoreClientSkill) const
{
	// All npcs over level 13 can dual wield
	if (this->IsNPC() && GetPetID() == 0 && !this->IsCharmed() && (this->GetLevel() >= 13))
		return true;
...
bool Mob::CanThisClassSneak(bool ignoreClientSkill) const
{
	if (IsNPC())
		return true;
	...
	case ROGUEGM:
		{
			// get it at lvl 1 no check required, just skip default
			break;
		}
	case BARD:
	case BARDGM:
	...
	if (IsClient() && !ignoreClientSkill)
		return(CastToClient()->HasSkill(SNEAK));	// No skill = no chance
	else
		return true;	//if we get here, we are the right class
	//and are at the right level, and are NPC
}

bool Mob::CanThisClassDisarm(bool ignoreClientSkill) const
{
	if (IsNPC())
		return true;
	...
	if (IsClient() && !ignoreClientSkill)
		return(CastToClient()->HasSkill(BASH));	// No skill = no chance
	else
		return true;	//if we get here, we are the right class
	//and are at the right level, and are NPC
}

bool Mob::CanThisClassBash(bool ignoreClientSkill) const
{
	if (IsNPC())
		return true;
...
bool Mob::CanThisClassDoubleAttack(bool ignoreClientSkill) const
{
...
bool Mob::CanThisClassParry(bool ignoreClientSkill) const
{
	// Trumpcard
	switch(GetClass()) // Lets make sure they are the right level! -image
	{
	...
	if (this->IsClient() && !ignoreClientSkill)
		return(this->CastToClient()->HasSkill(PARRY));	// No skill = no chance
	else
		return true;
}

bool Mob::CanThisClassDodge(bool ignoreClientSkill) const
{
	// Trumpcard
	switch(GetClass()) // Lets make sure they are the right level! -image
	{
...
bool Mob::CanThisClassRiposte(bool ignoreClientSkill) const //Could just check if they have the skill?
{
	// Trumpcard
	switch(GetClass()) // Lets make sure they are the right level! -image
	{
...
bool Mob::CanThisClassBlock(bool ignoreClientSkill) const
{
	switch(GetClass())
	{
	case BEASTLORDGM:
...
bool Mob::IsGuardRace()
{
	switch(GetBaseRace())
	{
	case 71: // qeynos guards ( texture 1 )
	case 44: // freeport guards ( texture 1 )
		{
			if ( GetTexture() == 1 || GetTexture() == 2 ) // 1 = west, 2 = north
				return true;

			break;
		}
	case 77: // neriak guards (texture anything but 1) - 1 is merchant
	case 81: // rivervale guards (texture anything but 1) - 1 is merchant
	case 90: // halas guards (texture anything but 1) - 1 is merchant
	case 78: // erudite guards (texture anything but 1) - 1 is merchant
	case 93: // ogre guards (texture anything but 1) - 1 is merchant
	case 92: // troll guards (texture anything but 1) - 1 is merchant
	case 94: // kaladim guards (texture anything but 1) - 1 is merchant
	case 112: // wood elf guards (texture anything but 1) - 1 is merchant
	case 106: // high elf guards (texture anything but 1) - 1 is merchant? blue outfit
		{
			if ( GetTexture() != 1 )
				return true;
	...
	case 12: // gnome guards are just texture 3 plate armored ... assume there arent any others in world
		{
			if ( GetTexture() == 3 )
				return true;
	...
	case 139: // iksar citizens (no other texture)
	case 67: // highkeep/highpass guards
		{
			return true;
			break;
		}
...
bool Mob::HateSummon() {
	...
	// KaB / Red69 / Zek - clients cannot have pets charmed that summon
	if ( GetOwner() != NULL && GetOwner()->IsClient() && ( IsCharmed() || IsPet() ) )
		return false;

	if(GetOwnerID())
		mob_owner = entity_list.GetMob(GetOwnerID());
		// ppl were exploiting training mobs
	if ( ( GetHPRatio() >= 98 && !zone->CanHateSummonFull() ) || SpecAttacks[SPECATK_SUMMON] == false || !GetTarget() || 
	...
	// get summon target
	SetTarget(GetHateRandom());
	bool targFound = false;
	int count = 0;
	while( GetTarget() && count < 5 )
	{
		...
		else if ( GetTarget()->CastToClient()->GetZoneMode() == ZoneUnsolicited )
		{
			targFound = true;
			break;
		}
	...
	if ( !targFound )
	{
		LogFile->write(EQEMuLog::Error, "HateSummon for %s failed because no target existed to be summoned at this time, attempted %i times.\n", GetName(), count);
		return false;
	}
	...
	// check if NPC is in a good place to summon people and not out of world
		if (zone->zonemap == NULL) {
			LogFile->write(EQEMuLog::Error, "%s: Zonemap unavailable cannot HateSummon.", 
			GetName());
		return false;
	}
	
	NodeRef pnode;
	pnode = zone->zonemap->SeekNode( zone->zonemap->GetRoot(), GetX(), GetY() );
	if (pnode == NODE_NONE) {
		LogFile->write(EQEMuLog::Error, "%s: Zonemap SeekNode failed, cannot HateSummon.", 
			GetName());
		return false;
	}
	...
	me.x = GetX();
	me.y = GetY();
	me.z = GetZ() + (target->GetSize()==0.0?6:target->GetSize()) * HEAD_POSITION;
	...
	if (best_z == -999999)
	{
		LogFile->write(EQEMuLog::Error, "%s: Zonemap FindBestZ failed, cannot HateSummon.", 
			GetName());
		return false;
	}
	// end new map checks to avoid out of world summon
...
sint32 Mob::GetEquipmentMaterial(int8 material_slot) const
{
	...
	if(item != 0)
	{
		if	// for primary and secondary we need the model, not the material
			...
			material_slot == MATERIAL_HEAD
			// 10/26/2014 (Image) - change for allowing head material to use idfile
			)
		{
...
sint32 Mob::GetActSpellCasttime(int16 spell_id, sint32 casttime, sint32& diff) {
...
void Mob::SetFlyMode(int8 flymode)
{
	if(IsClient() && flymode >= 0 && flymode < 3)
	{
		...
		CastToClient()->SetLevitateFlag(flymode);
	}
...
QGlobal* Mob::CheckGlobal(const char* varname)
{
	if ( IsClient() )
	{
		QGlobal res;
		if ( QGlobalCache::GetQGlobal(res,CastToClient()->GetQGlobals()->GetBucket(),(char*)varname,0,CastToClient()->CharacterID(),zone->GetZoneID()) )
		{
			return &res;
		}
	}
...
void Mob::DoKnockback(Mob *caster, uint32 pushback, uint32 pushup)
{
	if(IsClient())
	{
		// if one of these is true we can't perform a proper knockback
		if ( CastToClient()->IsShadowStepExempted() || CastToClient()->IsPortExempted() )
			return;
...
sint16 Mob::GetCrippBlowChance()
{
	...
	{
		if ( IsClient() )
			crip_chance += RuleI(Combat, WarBaseCripplingBlowChance);
		else
			crip_chance += RuleI(Combat, WarBaseCripplingBlowChanceNPC);
	}
...
void Client::SendClientStatWindow(Client* client)
{
	...
	client->SendWindow(0,0,0,1,this,"", " "
		...
		// Kings & Bandits - EQEmu R1882 - Fixed issue with statswindow haste fields
...
void Mob::StartReplay(uint32 replayID)
{
	Replaying = true;
	ReplayingID = replayID;
	ReplayingFrame = 0;
	cur_wp = 0;
	ReplayingTimer.Start((RuleI(Replay,SnapshotDelay)*1000),true);
	LastReplaySpeed = runspeed;
	if ( IsClient() )
		CastToClient()->Message(13,"Starting replay %i.", replayID);
	else
		this->Shout("Starting replay %i.", replayID);
}
...
void Mob::PlayFrame()
{
	...
	float x,y,z,heading,speed,recordedspeed,targx,targy,targz,targheading,targspeed = 0.0f;
	uint32 animation = 0;
	uint8 levitate = 0;
	char name[64];
	_snprintf(name,64,"");

	bool replayActive = database.GetReplayFrame(ReplayingID, ReplayingFrame, &x, &y, &z, &heading, &speed, &recordedspeed, &animation, &levitate, &targx, &targy, &targz, &targheading, &targspeed, name);

	if ( speed > runspeed )
		LastReplaySpeed = speed;
	else
		LastReplaySpeed = runspeed;
	...
	{
		Replaying = false;
		ReplayingTimer.Disable();
		if ( IsClient() )
			CastToClient()->Message(13,"End of replay %i.", ReplayingID);
		else
			this->Shout("End of replay %i.", ReplayingID);
	...
	if ( IsClient() )
	{
		if ( CastToClient()->GetLevitateFlag() != levitate )
		{
			SendAppearancePacket(AT_Levitate, levitate);
			CastToClient()->SetLevitateFlag(levitate);
		}
	}

	if ( IsClient() )
		CastToClient()->Message(13,"Replay Move (Frame %i): (X: %f, Y: %f, Z: %f, Heading: %f) - RunSpeed: %f, RecordedSpeed: %f - (Target: %s).",ReplayingFrame,x,y,z,heading,speed,recordedspeed,name);
	else
		this->Shout("Replay Move (Frame %i): (X: %f, Y: %f, Z: %f, Heading: %f) - RunSpeed: %f, RecordedSpeed: %f - (Target: %s).",ReplayingFrame,x,y,z,heading,speed,recordedspeed,name);

	ReplayingFrame++;
	if ( IsClient() )
		CastToClient()->MovePC(x,y,z,heading,1,ZoneSolicited);
	else if ( IsNPC() )
	{
		CastToNPC()->SetRunning(true);

		if ( ReplayingFrame == 1 )
			CastToNPC()->GMMove(x,y,z,heading,true);
		else
		CastToNPC()->MoveTo(x,y,z,heading,true);
	}
}
...
void Mob::ClearCombatNegatedSpells()
{
...
void Mob::LoadExtProcs(ExtendedProfile_Procs* permaprocs, ExtendedProfile_Procs* spellprocs) {
	...
	if ( permaprocs != NULL )
	{
		if (permaprocs[i].spellID > 0 && permaprocs[i].spellID != SPELL_UNKNOWN) {
			LogFile->write(EQEMuLog::Error, "%s: PermaProc %i loaded.", GetName(), permaprocs[i].spellID);
			PermaProcs[i].spellID = permaprocs[i].spellID;
			PermaProcs[i].chance = permaprocs[i].chance;
			PermaProcs[i].pTimer = NULL;
			PermaProcs[i].isPoison = permaprocs[i].isPoison;
		}
	}
	...
	if ( spellprocs != NULL )
	{
		if (spellprocs[i].spellID > 0 && spellprocs[i].spellID != SPELL_UNKNOWN) {
			LogFile->write(EQEMuLog::Error, "%s: SpellProc %i loaded.", GetName(), spellprocs[i].spellID);
			SpellProcs[i].spellID = spellprocs[i].spellID;
			SpellProcs[i].chance = spellprocs[i].chance;
			SpellProcs[i].pTimer = NULL;
			SpellProcs[i].isPoison = spellprocs[i].isPoison;
		}
	}
}
