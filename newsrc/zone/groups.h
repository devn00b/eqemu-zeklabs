...
public:
	...
	bool	DelMember(Mob* oldmember,bool ignoresender = false, bool learnMembers = true);
	void	DisbandGroup();
	...
	// Kings & Bandits pvp coin to group
	bool	GivePVPPoints(Client* killer, uint32 amount, int32 originZoneID);
	void	GivePVPCoin(uint32 itemID,sint32 charges);
	...
	inline	void SetLeader(Mob* newleader){
		int oldMemberPos = -1;
		if ( newleader != NULL )
		{
			for(int i=0;i<MAX_GROUP_MEMBERS;i++)
			{
				if ( !strcasecmp(membername[i],newleader->GetName()) )
				{
					oldMemberPos = i;
					break;
				}
				else if ( membername[i][0] == '\0' )
				{
					oldMemberPos = i;
					break;
				}
			...
			if ( oldMemberPos > -1 )
			{
				members[oldMemberPos] = members[0];
				strcpy(membername[oldMemberPos],membername[0]);
			}
		}
