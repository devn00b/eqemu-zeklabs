	...
	uint32 timer, uint32 timer_duration, uint32 type, sint16 *resist_adjust)
{
	...
	if(casting_spell_id == spell_id)
		ZeroCastingVars();
	else if ( casting_spell_id != spell_id && IsBardSong(spell_id) )
		ZeroCastingVars();
	...
	{
		LogFile->write(EQEMuLog::Error, "%s: Spell casting canceled (%i): not able to cast now. Valid? %d, casting %d, waiting? %d, spellend? %d, stunned? %d, feared? %d, mezed? %d, silenced? %d, amnesiad? %d", 
			GetName(), spell_id, IsValidSpell(spell_id), casting_spell_id, delaytimer, spellend_timer.Enabled(), IsStunned(), IsFeared(), IsMezzed(), IsSilenced(), IsAmnesiad() );
	...
	sint16 resist_adjust)
{
	...
	if ( IsClient() )
	{
		if ( (IsEffectInSpell(spell_id, SE_YetAnotherGate)
						|| IsEffectInSpell(spell_id, SE_Teleport)
						|| IsEffectInSpell(spell_id, SE_Teleport2)) )
						{
							QGlobal res;
							if(QGlobalCache::GetQGlobal(res,CastToClient()->GetQGlobals()->GetBucket(),"noteleportallowed",0,CastToClient()->CharacterID(),zone->GetZoneID()))
							{
								Message(0, "You are unable to teleport or translocate at this time.");
								InterruptSpell();
								return(false);
							}
						}
		else if ( IsEffectInSpell(spell_id, SE_Gate) )
						{
							QGlobal res;
							if(QGlobalCache::GetQGlobal(res,CastToClient()->GetQGlobals()->GetBucket(),"nogateallowed",0,CastToClient()->CharacterID(),zone->GetZoneID()))
							{
								Message(0, "You are unable to teleport or translocate at this time.");
								InterruptSpell();
								return(false);
	...
	if ( spell.targettype == ST_TargetOptional && target_id > 0 )
	{
		Mob* tmpMob = entity_list.GetMob(target_id);
		if ( tmpMob != NULL && !CheckLosFN(tmpMob) ) {
	...
	if(cast_time <= -1) {
		...
		// KaB / Red69 / Zek - mod to see if gate spells will not bug out
		if ( ( IsTeleportSpell(spell_id) || IsGateSpell(spell_id) ) && cast_time > 500 )
			cast_time -= 350;

		// if there's a cast time, check if they have a modifier for it
		if(cast_time) {
			cast_time = GetActSpellCasttime(spell_id, cast_time, castdiff);	
		}
	}
	else
	...
	// Break Invis & Hide since we are casting
	BreakMobInvis(false);
	BreakMobHide();
	...
	begincast->cast_time = orgcasttime+castdiff; // client calculates reduced time by itself
	outapp->priority = 3;
	...
	int16 mana_used, int32 inventory_slot, sint16 resist_adjust)
{
	...
	Mob *spell_target = spell_target = entity_list.GetMob(target_id);
	...
	{
		if (IsBardSong(spell_id)) {
				// Red69/LoZ/KaB: fix for bard songs with recast time sep 7 2012 (Removed recasttime != 0 check)
	...
	else // not bard, check movement
	{
		...
		// Kings & Bandits - interrupt if player zones out
		if ( IsDetrimentalSpell(spell_id) )
		{
			...
			else if ( spell_target != NULL && spell_target->IsClient() && IsClient() && spell_target != this )
				this->CastToClient()->SetAttackTarget(spell_target);
		}
			...
			)
		{
			...
			if(attacked_count > RuleR(Combat,InterruptMaxAttacks)) attacked_count = RuleR(Combat,InterruptMaxAttacks);

			float channelchance, distance_moved, d_x, d_y, distancemod;

			if(IsClient())
			{
				if ( GetClass() == BARD || GetClass() == ROGUE || GetClass() == WARRIOR || GetClass() == MONK || GetClass() == BERSERKER )
				{
					channelchance = 0.0f;
				}
				else
				{
					...
					channelchance = 30.0f + (float)GetSkill(CHANNELING) / 400.0f * 100.0f;
					channelchance -= attacked_count * 2.0f;
					channelchance += channelchance * (float)(GetAA(aaChanellingFocus)*5) / 100.0f; 
					channelchance += channelchance * (float)(GetAA(aaInternalMetronome)*5) / 100.0f;
				}
			} else {
			...
			// Kings & Bandits, make sure we ALWAYS have a chance to be interrupted, at least 5%.
			int randChance = channelchance;
			if ( randChance > 95 )
				randChance = 95;

			if(MakeRandomFloat(0, 100) > randChance) {
	...
	if(bard_song_mode)
	{
		if(IsClient())
		{
			...
			// KaB - Zek - Red69 - this should disable bards ability to use Melody
...
bool Mob::DetermineSpellTargets(uint16 spell_id, Mob *&spell_target, Mob *&ae_center, CastAction_type &CastAction) {
	...
	*/
	...
	case ST_TargetsTarget:
		...
		// Kings & Bandits - support for AE Client Only spells (clientOnly)
	case ST_AreaClientOnly:
		{	
			if ( this->GetTarget () == NULL || !this->GetTarget()->IsClient() )
			{
				spell_target = this;
			}
			else
				spell_target = this->GetTarget();

			CastAction = AEClientOnly;
			break;
		}
	default:
		{
	...
	int16 mana_used, int32 inventory_slot, sint16 resist_adjust)
{
	...
	if ( spell_id == 2039 && IsClient() )
	{
		improved_hidden = true;
		CastToClient()->SetHideMe(1,true);
	}

	if( mana_used > 0 && zone != NULL && zone->CanPVPValue() == DISABLEACTIONVALUE ){
		if(IsClient()){
	...
	// Kings & Bandits - we already checked their distance for AreaClientOnly in the AESpell.
	if ( GetSpellTargetType(spell_id) != ST_AreaClientOnly )
	{
	...
	case AETarget:
		{
			...
			if(ae_center->IsBeacon()) {
				...
				{
					// 10/26/2014 Image - fix for weapon AE spell casts
					bool affectcaster = true;
					if ( slot == 10 && !IsBeneficialSpell(spell_id) )
						affectcaster = false;

					// affect the target too
					SpellOnTarget(spell_id, spell_target, false, affectcaster, resist_adjust);
				}
	...
	case AEClientOnly:
		{
			bool affect_caster = IsClient();	//NPC AE spells do not affect the NPC caster
			entity_list.AESpell(this, spell_target, spell_id, affect_caster, resist_adjust, true);
			break;
		}
	case DirectionalAE:
		{
...
void Mob::SetSpellRecastTimer(int16 spell_id)
{
	...
	CastToClient()->GetPTimers().Clear(&database, pTimerSpellStart + spell_id);
...
void Mob::BardPulse(uint16 spell_id, Mob *caster) {
	...
	// Kings & Bandits - Mob::BardPulse bard fix for continuous spells, this will make it so they do not cast on a corpse (an npc/client after death).
	if ( caster->IsClient() )
	{
		if ( this->IsCorpse() && IsDetrimentalSpell(spell_id) )
		{
			caster->InterruptSpell(0, 0, spell_id);
			caster->SendSpellBarEnable(spell_id);
			return;
		}
	}
...
int Mob::CalcBuffDuration(Mob *caster, Mob *target, int16 spell_id, sint32 caster_level_override)
{
	...
	LogFile->write(EQEMuLog::Debug, "Spell %d: Casting level %d, formula %d, base_duration %d: result %d",
		spell_id, castlevel, formula, duration, res);

	// enchanter mesmerization mastery aa
	if (caster != NULL && caster->IsClient() && caster->CastToClient()->GetAA(aaMesmerizationMastery) > 0)
		res *= 1.35;
...
int Mob::CheckStackConflict(int16 spellid1, int caster_level1, int16 spellid2, int caster_level2, Mob* caster1, Mob* caster2)
{
	...
	*/
	...
	else if ( IsBardSong(spellid1) && IsBardSong(spellid2) )
	{
		// Chords stack with disruptive discord
		/*	703 - chords of dissonance
			730 - Denon`s Disruptive discord
			746 - Chords of Cessation
			*/
		if ( ( spellid1 == 703 || spellid1 == 746 ) && spellid2 == 730 )
			return 0;
		else if ( ( spellid2 == 703 || spellid2 == 746 ) && spellid1 == 730 )
			return 0;
	}
	...
	for(i = 0; i < EFFECT_COUNT; i++)
	{
		...
		if(effect1 == SE_StackingCommand_Block)
		{
			...
			*/
			...
			{
				sp2_value = CalcSpellEffectValue(spellid2, -1, blocked_slot, caster_level2);
	...
	for(i = 0; i < EFFECT_COUNT; i++)
	{
		...
		if(effect2 == SE_StackingCommand_Overwrite)
		{
			...
			{
				sp1_value = CalcSpellEffectValue(spellid1, -1, overwrite_slot, caster_level1);
	...
	for(i = 0; i < EFFECT_COUNT; i++)
	{
		...
		*/
		...
		sp1_value = CalcSpellEffectValue(spellid1, -1, i, caster_level1);
		sp2_value = CalcSpellEffectValue(spellid2, -1, i, caster_level2);
		...
		// (2014) Image: resolves bard charm/mez spell not overriding tash and other magic resistance lowering spells
		bool isSecondCharm = IsCharmSpell(spellid2);
		bool isSecondMez = IsMezSpell(spellid2);

		if ( (!isSecondCharm && !isSecondMez ) )
		{
			bool isFirstCharm = IsCharmSpell(spellid1);
			bool isFirstMez = IsMezSpell(spellid1);
			if( ( isFirstCharm || isFirstMez ) || ( sp2_value < sp1_value ) ) {
...
int Mob::AddBuff(Mob *caster, int16 spell_id, int duration, sint32 level_override)
{
	...
	buffs[emptyslot].player_id = ( caster && caster->IsClient() ) ? caster->CastToClient()->CharacterID() : 0;
	buffs[emptyslot].player_class = caster  ? caster->GetClass() : 0;
	buffs[emptyslot].dropOnCombat = IsEffectInSpell(spell_id,SE_NegateIfCombat);
	buffs[emptyslot].UpdateClient = true;
	if ( IsPoisonCounterSpell(spell_id) || IsDiseaseCounterSpell(spell_id) )
		buffs[emptyslot].CountersReady = false;
	else
		buffs[emptyslot].CountersReady = true;
	...
	// April 13 2016 - Zek - Image: Fixes clients having incorrect buff durations against other clients
	if ( RuleB(PVPCombat, UpdateClientBuffInZone) && IsClient() && caster->IsClient() && IsMezSpell(spell_id) )
		buffs[emptyslot].UpdateClient = true;
...
bool Mob::SpellOnTarget(int16 spell_id, Mob* spelltar, bool reflect, bool use_resist_adjust, sint16 resist_adjust)
{
	...
	// Image 2018: This fixes spells not properly setting on their target such as scale of wolf (424)
	if ( IsBeneficialSpell(spell_id) && spelltar && spelltar->IsClient() )
	{
		// Image 2019: disable workaround for selos
		if ( spell_id == 717 || spell_id == 1330 || spell_id == 4395 || spell_id == 2605 )
			action->source = GetID();
		else
			action->source = spelltar->GetID();
	}
	else if(IsClient() && CastToClient()->GMHideMe())
	{
		action->source = spelltar->GetID();
	}
	else
	...
	// Image 11/11/2014 eqpvp: THIS NEEDS TO BE UPDATED FOR ANY BIND SIGHT SPELLS OR OTHERWISE IT WILL BIND TARGET TO CASTER SIGHT!!
	// select target
	if	// Bind Sight line of spells 
		(
		spell_id == 500 || 	// bind sight
		spell_id == 407 ||		// cast sight
		spell_id == 721 ||		// lyssa's solidarity
		spell_id == 6592 		// gm bind sight
		)
	{ 
		action->target = GetID(); 
	} 
	...
	if(!(IsClient() && CastToClient()->GetGM()) && !IsHarmonySpell(spell_id))	// GMs can cast on anything
	{
		...
		if(IsBeneficialSpell(spell_id))
		{
			QGlobal res;
			if ( this != spelltar && 
				spelltar->IsClient() && QGlobalCache::GetQGlobal(res,spelltar->CastToClient()->GetQGlobals()->GetBucket(),"nobeneficialbuffsallowed",0,spelltar->CastToClient()->CharacterID(),zone->GetZoneID()) ) {
					spelltar->CastToClient()->Message(13,"You cannot receive buffs at this time as you are currently flagged to not allow buffs.");
					safe_delete(action_packet);
					return false;
			}
			if
				...
				spelltar != this &&
				(
				// 2/12/2014 - Rolfron Zek - Image
				// modified to allow poison counter and disease counter to bypass beneficial check in spells
				( !IsBeneficialAllowed(spelltar) && ( !IsPoisonCounterSpell(spell_id) && !IsDiseaseCounterSpell(spell_id) ) ) ||
				(
		...
		} // Kings & Bandits - CancelMagic (Nullify Magic) (Cancel Magic) fix for beneficial targets
		else if	( !(IsEffectInSpell(spell_id, SE_CancelMagic) && IsBeneficialAllowed(target)) && 
			!IsAttackAllowed(spelltar, true) && !IsResurrectionEffects(spell_id)) // Detrimental spells - PVP check
		{
	...
	int buffslot = FindBuffSlot(spell_id);
	if ( IsClient() && buffslot >= 0 && buffs[buffslot].UpdateClient && buffs[buffslot].CountersReady )
	{
		uint32 counters = 0;
		if ( IsPoisonCounterSpell(buffs[buffslot].spellid) )
			counters = buffs[buffslot].poisoncounters;
		else if ( IsDiseaseCounterSpell(buffs[buffslot].spellid) )
			counters = buffs[buffslot].diseasecounters;
		CastToClient()->SendBuffDurationPacket(buffs[buffslot].spellid, buffs[buffslot].ticsremaining, buffs[buffslot].casterlevel, buffslot, 2, counters, false);
	}
...
void Corpse::CastRezz(int16 spellid, Mob* Caster){
	...
	// we pass the caster name below so we need to check if the character is present
	if ( !Caster )
	{
		return;
	}
...
// Image: check if the buff is about to go anyway
bool Mob::IsBuffToFade(int16 spell_id)
{
	uint32 buff_count = GetMaxTotalSlots();
	for (int j = 0; j < buff_count; j++)
	{
	...
	return true; // no buff so yea its faded
}
...
void Mob::BuffFadeByEffect(int effectid, int skipslot)
{
	...
	if ( effectid == SE_Charm )
	{
		charmed = false;
		WipeHateList();
		entity_list.RemoveFromHateLists(this);
		// (2014) Image: Fixes charm on npc target and resetting target so it cons/can be casted on etc.
...
bool Mob::IsImmuneToSpell(int16 spell_id, Mob *caster)
{
	...
	if(IsMezSpell(spell_id))
	{
		...
		// KaB - Red69 - Zek / client modification, we are not immune due to level restrictions that may exist for npc's
		...
		else
		{
			...
			if(effect_index > -1 && GetLevel() > spells[spell_id].max[effect_index])
			{
	...
	if(IsCharmSpell(spell_id))
	{
		...
		if(!caster->IsNPC())
		{
			// Kings & Bandits - disabled flag for charm - we want to allow charm on players
			if (IsClient() && ( (!RuleB(PVPCombat,CharmAllowed)) || (RuleB(PVPCombat,CharmAllowed) && caster->GetClass() != ENCHANTER))) {
				...
				return true;
			} else if(GetOwner() != NULL && GetOwner()->IsClient()) {
...
float Mob::ResistSpell(int8 resist_type, int16 spell_id, Mob *caster, bool use_resist_override, int resist_override)
{
	...
	if(IsEffectInSpell(spell_id, SE_Fear) && CalcFearResistChance() == 100  )
	{
		mlog(SPELLS__RESISTS, "Target is fearless to spell %d", spell_id);
		return(0);
	}
	...
	// Kings & Bandits - cannot resist cancel magic unless immune to magic
	if ( IsEffectInSpell(spell_id, SE_CancelMagic) )
	{
		// KaB Red(69) - BecomeNPC always resists dispell 8-5-2011
		if ( IsClient() && CastToClient()->IsBecomeNPC() )
			return 0;
		else
			return 100;
	}
	...
	if ( caster && caster->IsClient() && ( IsEffectInSpell(spell_id, SE_MovementSpeed) || IsEffectInSpell(spell_id, SE_Root) ) && 
	( HasBuff(717) || HasBuff(4395) || HasBuff(2605) || HasBuff(1330) || HasBuff(1750) || HasBuff(2007) ) )
	{
		Message(13, "Bard speed song has reduced your resistance to a movement debuff!");
		target_resist = (target_resist * RuleI(PVPCombat,BardResistReductionPct) ) / 100;
	}
	...
	bool allowBaseResist = false;

	if(IsClient() && GetLevel() >= 21 && temp_level_diff > 20)
	{
		// March 17 2016 removed
		//temp_level_diff = 20;
		allowBaseResist = true;
	}
	...
	if ( caster->IsClient() && IsClient() )
	{
		allowBaseResist = false;
		level_mod /= 1.5;
	}
	...
	if ( RuleB(Combat,SpellResistDebugMessaging) )
	{
		if ( caster && caster->IsClient() )
			caster->Message(0,"TargetResistData %s(spellid:%i): LevelMod: %i, ResistMod: %i, TargetResist: %i.  ResultResistChance: %i",GetName(),spell_id, level_mod,resist_modifier,target_resist,resist_chance);
	}
	...
	if(resist_chance > spells[spell_id].MaxResist && spells[spell_id].MaxResist != 0 && IsEffectInSpell(spell_id, SE_Root ))
	{
		if ( caster && caster->IsClient() && IsNPC() )
	...
	// Kings & Bandits - spell resist modification to client versus client, set a base minRoll
	if ( IsClient() && caster && caster->IsClient() )
	{
		int lvl_diff = caster->GetLevel() - GetLevel();
		if ( lvl_diff < 0 )
		{
			if ( lvl_diff == -1 )
				lvl_diff = -2;

			// caster is weaker - we still increase the minRoll to be whatever the level difference
			minRoll = ( ( GetLevel() / 3 ) * ( lvl_diff * -1 ) );
		}
		else if ( lvl_diff < 1 )
		{
			// caster is equal
		...
		{
			// caster is greater - we still increase the minRoll to be whatever the level difference
			minRoll = ( ( GetLevel() ) / 4 );
		}
		...
		if ( minRoll >= resist_chance )
			minRoll = resist_chance - 1;
	...
	// Kings & Bandits - modification for levels less than 20, resets the maxRoll of resistance to 125 instead of 200.
	if ( GetLevel() < 21 ) // lets try to give the lowbies a chance.
		maxRoll = 200;
	else if ( allowBaseResist )
		maxRoll = resist_chance + 10;
	else
	{
		if ( resist_chance == 0 )
			resist_chance = 1; // log can't take 0

		double rLog = log((double)resist_chance);
		double logDivider = RuleR(Combat,ResistanceLogDivider);
		if ( logDivider == 0.0 )
			logDivider = 1.0;

		resist_chance = resist_chance + ((int)(double)resist_chance*(rLog/logDivider));

		if ( resist_chance > RuleI(Combat,ResistanceCap))
			resist_chance = RuleI(Combat,ResistanceCap);

		maxRoll = RuleI(Combat,ResistanceMaxRoll);
	}


	/*else if ( resist_chance > 230 )
	{
		maxRoll = 300;
	}
	else if ( resist_chance > 220 )
		maxRoll = 240;
	else if ( resist_chance > 200 )
		maxRoll = 235;
	else if ( resist_chance > 170 )
		maxRoll = 215;*/

	//Finally our roll
	int roll = MakeRandomInt(minRoll, maxRoll);
	
	
	if ( RuleB(Combat,SpellResistDebugMessaging) )
	{
		if ( caster && caster->IsClient() )
			caster->Message(0,"TargetResistDataCont %s(spellid:%i): MinRoll: %i, MaxRoll: %i.  ResultResistChance(roll>resist): %i > %i",GetName(),spell_id, minRoll, maxRoll, roll, resist_chance);
	}

	LogFile->write(EQEMuLog::Debug,
		"Spell %d is being casted against %s, we rolled %i (min %i max %i).  This target has a %i chance of resist. (if %i > %i, we hit)", spell_id, this->GetName(),roll, minRoll, maxRoll, resist_chance, roll, resist_chance);
	...
	else
	{
		...
		if(!IsPartialCapableSpell(spell_id,caster->IsClient(),IsClient()))
		{
			return 0;
		}
		else
		{
			...
			LogFile->write(EQEMuLog::Debug, "Spell %d is being casted against %s, partial modifier returning %i (if %i < 0, always hit, if %i > 100, always resist, otherwise return %i as partial mod.", spell_id, this->GetName(), partial_modifier, partial_modifier, partial_modifier, partial_modifier);
...
void Mob::BreakMobHide(bool breakOwner)
{
	if ( improved_hidden && IsClient() )
			CastToClient()->SetHideMe(0,true);

	// Red69 - Zek - KaB / Jul 26 - fixed sneaking not breaking
	if(sneaking || hidden || improved_hidden){
		sneaking = false;
		hidden = false;
	...
	if ( breakOwner && GetOwner() != NULL )
	{
		GetOwner()->BreakMobHide();
	}
}

void Mob::BreakMobInvis(bool breakOwner)
{	
	if(invisible) {
	...
	if ( breakOwner && GetOwner() != NULL )
	{
		GetOwner()->BreakMobInvis();
	}
}

void Mob::HandleStunInterrupt(bool randInterrupt)
{
	if(!casting_spell_id) {
		return;
	}
	...
	else
	{
		...
		int rand = MakeRandomInt(1,100) + persistent_casting;

		if ( rand > 100 )
			rand = 100;

		if(rand < RuleI(Combat, RandomInterruptSpellChance))
			InterruptSpell();
	}
}

void Mob::Stun(int duration, bool randInterrupt)
{
	//make sure a shorter stun does not overwrite a longer one.
	if(stunned && stunned_timer.Enabled())
		return;

	HandleStunInterrupt(randInterrupt);
...
// Hogie - Stuns "this"
void Client::Stun(int duration, bool randInterrupt)
{
	if ( stunned ) // can't double stun 10/13/2016 - image
		return;

	Mob::Stun(duration, randInterrupt);
...
void NPC::Stun(int duration, bool randInterrupt) {
	Mob::Stun(duration);
	SetRunAnimSpeed(0);
	SendPosition();
}
...
void Mob::Mesmerize()
{
	...
	entity_list.RemoveFromPlayerPetHateLists(this);
...
void Client::MakeBuffFadePacket(int16 spell_id, int slot_id, bool send_message)
{
	...
	sbf->effect=2; // used to be slot until I changed from SpellBuffFadeOld_Struct ->SpellBuffFade_Struct 
...
void Client::MemSpell(int16 spell_id, int slot, bool update_client)
{
	...
	if ( slot == (MAX_PP_MEMSPELL-1) && !GetAA(aaMnemonicRetention) )
		return;
	...
	SetSpellRecastTimer(spell_id); // Image: added 6/25/2013 as when you mem a spell the recast time is triggered
...
bool Mob::AddProcToWeapon(int16 spell_id, bool bPerma, int16 iChance, bool isPoison) {
	...
	if (bPerma) {
		for (i = 0; i < MAX_PROCS; i++) {
			if (PermaProcs[i].spellID == SPELL_UNKNOWN) {
				...
				PermaProcs[i].isPoison = isPoison;
		...
		for (i = 0; i < MAX_PROCS; i++) {
			if (SpellProcs[i].spellID == SPELL_UNKNOWN) {
				...
				SpellProcs[i].isPoison = isPoison;
...
bool Mob::AddDefensiveProc(int16 spell_id, int16 iChance)
{
	...
	for (i = 0; i < MAX_PROCS; i++) {
		if (DefensiveProcs[i].spellID == SPELL_UNKNOWN) {
			...
			DefensiveProcs[i].isPoison = false;
...
void Client::SendBuffDurationPacket(int16 spell_id, int duration, int inlevel, int slotid, int slot, uint32 counters, bool quickQueue)
{
	...
	//sbf->effect = 2; // 2 makes durations update on client for certain spells, other spells like resist / strength reduction are 1
	sbf->effect = 1;
	
	if ( !IsBardSong(spell_id) && ( IsEffectInSpell(spell_id, SE_MovementSpeed) || IsEffectInSpell(spell_id, SE_Root) ) )
	{
		sbf->effect = 2;
		sbf->bard_modifier = -1;
	}
	// Image 2018: spell categories, typically effect 2 overrides as required to give proper buff duration
	switch(spells[spell_id].spell_category)
	{
	case 7: // dot magic
	case 8: // dot undead
	case 9: // dot lifetaps
	case 142: // target ae fire
	case 143: // target ae ice
	case 99: // mesmerize (Image: added May 2018)
		{
			sbf->effect = 2;
			break;
		}
...
void Client::InitializeBuffSlots()
{
	...
	for(int x = 0; x < max_slots; ++x)
	{
		...
		buffs[x].casterid = 0;
		buffs[x].casterlevel = 0;
		buffs[x].durationformula = 0;
		buffs[x].ticsremaining = 0;
		buffs[x].numhits = 0;
		buffs[x].persistant_buff = 0;
		buffs[x].player_id = 0;
		buffs[x].player_class = 0;
		buffs[x].dropOnCombat = false;
...
bool Mob::HasBuff(int16 spell_id)
{
	uint32 buff_count = GetMaxBuffSlots() + GetMaxSongSlots();
