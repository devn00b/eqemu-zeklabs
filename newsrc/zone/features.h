...
enum {	//timer settings, all in milliseconds
	...
	LongZoneTimerResolution = 100,			//dynamic sleeping zone timer wait
	UnderAttackTimerDelay = 30000, // for clients timing out after no attack
	LinkDeadDelayNotAttacked = 30000 // timer delay for not being attacked (then dropped after time frame)
};
