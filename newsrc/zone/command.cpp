...
int command_init(void) {
	...
	(
		...
		command_add("testsound","",200,command_testsound) ||
		command_add("setdeathcount","",200,command_setdeathcount) ||
		command_add("testspawnkill","- Sends an OP_Death packet for spawn made with #testspawn",200,command_testspawnkill) ||
		command_add("testopcode","- sends opcode",200,command_testopcode) ||
		...
		command_add("endurance","- Fill your or your target's endurance",50,command_endurance) ||
		...
		command_add("resetgmspeedflag","[on/off] - Turn GM speed flag off or on for speed cheat testing",200,command_resetgmspeedflag) ||
		...
		command_add("killclientbuff","- nukes the buff client side only",50,command_killclientbuff) ||
		...
		command_add("pvpreset","[kills/deaths/killstreak/bestkillstreak] [value] - Reset targets kills or deaths to the set value",100,command_pvpreset) ||
		command_add("infamy"," [addsubtract value] - Update character infamy to add or subtract",200,command_infamy) ||
		command_add("setxp","[value] - Set your or your player target's experience",100,command_setxp) ||
		// Kings & Bandits - changed setpvppoints function to add instead in the Client->SetPVPPoints call
		command_add("pvpachievements"," - updates your pvp achievements test command",80,command_achievements) ||
		command_add("addpvppoints","[value] - Add or subtract you or your player target's PVP points",100,command_setpvppoints) ||
		command_add("addpvppointsnocareer","[value] - Add or subtract you or your player target's PVP points, only adds points to current, does not modify career",100,command_setpvppointsnocareer) ||
		...
		command_add("itemedit","[column] [value] - Mega item editing command",100,command_itemedit) ||
		command_add("givestartitems","Give player starting items",100,command_givestartingitems) ||
		command_add("nukeinventory","nuke targets inventory",100,command_nukeinventory) ||
		command_add("qglobal","[on/off/view] - Toggles qglobal functionality on an NPC",100,command_qglobal) ||
		command_add("checkglobal","[globalname] [npcid] [charid] [zoneid] - Check if a client has a qglobal",100,command_checkglobal) ||
		...
		command_add("uptime","[zone server id] - Geuptime of worldserver, or zone server if argument provided",10,command_uptime) ||
		...
		// Kings & Bandits command additions
		command_add("summonlootgroup","[groupid] - Summons a loot group for testing.",200,command_summonlootgroup) ||
		command_add("setpvpflag","[flagid] - Sets the pvp flag value of a specific target and updates appearance, uint8 value.",200,command_setpvpflag) ||
		command_add("setzonepvpflag","[value] 0=nopvp,1=teampvp,2=ffapvp - sets current zone pvp flag.",200,command_setzonepvpflag) ||
		command_add("setisking","[value] 0=notking,1=king - sets king player flag.",200,command_setisking) ||
		command_add("syncacct","[accountname] [synckey] - Syncs a KaB forum and login account to a eqemulator.net login account.",0,command_syncacct) ||
		command_add("reloadzones","[reloadtype] - Reloads values across all zones, reloadtype: quests|hotzones|rules.",200,command_reloadzones) ||
		...
		command_add("listguards","[kingdomid] - shows a list of guards, specify kingdom id number.",200,command_listguards) ||
		command_add("deleteguard","[kingdomid] [guardid] - removes guard from kingdom list.",200,command_deleteguard) ||
		command_add("listguildhate","[kingdomid] - shows a list of hated guilds for that kingdom.",200,command_listguildhate) ||
		command_add("burycorpse","Buries a corpse that is in the world.",200,command_burycorpse) ||
		command_add("replay","replay capabilities, #replay help for more info.",200,command_replay) ||
		command_add("guildnpc","flag npc with a guild id of 0.",200,command_flagnpcguildid) ||
		command_add("teamcount","- Get team counts",200,command_teamcount) ||
		command_add("consider","[pvpcon] [faction] [level].",200,command_consider) ||
		// End of Kings & Bandits additions
...
#endif
		command_add("untraindisc","Untrains all the disciplines usable by the target.",150,command_untraindisc) ||
		...
		command_add("dyeall","[red] [green] [blue] - Dye all armor slots with the color integer value 0-255.",200,command_dyeall) ||
		command_add("resetdisctimer","[discid] [timeleft] [unknown] - Reset the disc timer client side for testing.",200,command_resetdisctimer) ||
		...
		command_add("groundspawns","List - groundspawns command to show all available spawns",100,command_groundspawns) ||
		...
		command_add("ignoretimer","Disables the periodic zone lockout timer information provided when zoning.",0,command_ignoretimer)||
		command_add("becomenpc","[on/off] [npcname] [class] [race]",200,command_becomenpc)
		)
	{
		command_deinit();
		return -1;
...
void command_testsound(Client *c, const Seperator *sep)
{
	if ( !sep->IsNumber(1) )
	{
		c->PlayMP3(sep->arg[1]);
		...
		c->SendQuestSound(atoi(sep->arg[1]),atoi(sep->arg[2]),atoi(sep->arg[3]),atoi(sep->arg[4]),atoi(sep->arg[5]));
	c->Message(0,"Issued Sound");
}
...
void command_testopcode(Client *c, const Seperator *sep)
{
	EQApplicationPacket* outapp = new EQApplicationPacket(OP_PlayMP3,0);
	c->FastQueuePacket(&outapp);
}
		...
		if (c->GetTarget()->IsNPC() && sep->IsNumber(2))
		{
			...
			if (database.GetItem(item))
			{
				if (sep->arg[3][0] != 0 && sep->IsNumber(3))
					...
					c->GetTarget()->CastToNPC()->AddItem(item);
...
void command_zone(Client *c, const Seperator *sep)
{
	...
	else if (sep->arg[1][0] == 0)
	{
		...
		const ZoneShortName_Struct* zns = database.GetZoneInfo(sep->arg[1]);
		if(zns == 0 || zns->zoneID < 1) {
			c->Message(0, "Unable to locate zone '%s'", sep->arg[1]);
			return;
		}
		zoneid = zns->zoneID;
		instanceID = zns->instanceID;
	}
		
	if (sep->IsNumber(2) || sep->IsNumber(3) || sep->IsNumber(4)){
		//zone to specific coords
		c->MovePC(zoneid,instanceID, (float)atof(sep->arg[2]), atof(sep->arg[3]), atof(sep->arg[4]), 0.0f, 0);
		}
	else
		//zone to safe coords
...
void command_ai(Client *c, const Seperator *sep)
{
	...
	else if (strcasecmp(sep->arg[1], "extendspelllist") == 0) {
		if (target && sep->IsNumber(2) && atoi(sep->arg[2]) >= 0) {
			if (target->IsNPC())
				target->CastToNPC()->AI_AddNPCSpells(atoi(sep->arg[2]),false);
			else
				c->Message(0, "%s is not an NPC.", target->GetName());
		}
		else
			c->Message(0, "Usage: (targeted) #ai extendspelllist [npc_spells_id]");
	}
...
void command_setpass(Client *c, const Seperator *sep)
{
	if(sep->argnum != 2)
		...
		sint16 tmpstatus = 0;
		int32 tmpid = database.GetAccountIDByName(false,sep->arg[1], &tmpstatus);
		if (!tmpid)
...
void command_size(Client *c, const Seperator *sep)
{
	...
	if (!sep->IsNumber(1))
		...
		if (newsize > 255)
			...
			if ( c->GetTarget() )
				c->GetTarget()->ChangeSize(newsize,100);
			else
				c->ChangeSize(newsize,100);
			c->Message(0,"Size = %f", newsize);
		}
	}
}
...
void command_flymode(Client *c, const Seperator *sep)
{
  Client *t=c;
	...
	if (strlen(sep->arg[1]) == 1 && !(sep->arg[1][0] == '0' || sep->arg[1][0] == '1' || sep->arg[1][0] == '2'))
		...
		if(c->GetTarget() && c->GetTarget()->IsClient())
			t=c->GetTarget()->CastToClient();
		uint8 val = atoi(sep->arg[1]);
		t->SendAppearancePacket(AT_Levitate, val);
		c->SetLevitateFlag(val);
		if (sep->arg[1][0] == '1')
...
void command_race(Client *c, const Seperator *sep)
{
  Mob *t=c->CastToMob();
	...
	if (sep->IsNumber(1) && atoi(sep->arg[1]) >= 0 && atoi(sep->arg[1]) <= 667) {
		if ((c->GetTarget()) && c->Admin() >= commandRaceOthers)
			t=c->GetTarget();
		t->SendIllusionPacket(atoi(sep->arg[1]));
	}
	else
		c->Message(0, "Usage: #race [0-667]  (0 for back to normal)");
}
...
void command_resetgmspeedflag(Client *c, const Seperator *sep)
{
		bool state=atobool(sep->arg[1]);
	Client *t=c;
	...
	if(sep->arg[1][0] != 0) {
		t->CastToClient()->SetGMSpeedFlag(state);
		c->Message(0, "Turning GMSpeedFlag %s for %s", state?"On":"Off",t->GetName());
	}
	else
		c->Message(0, "Usage: #resetgmspeedflag [on/off]");
}
...
void command_killclientbuff(Client* c, const Seperator *sep)
{
	if ( !sep->IsNumber(1) || !sep->IsNumber(2) || (c->GetTarget() != NULL && !c->GetTarget()->IsClient()) )
	{
		c->Message(0,"Usage (client target/self): #killclientbuff <spellid> <slot>");
	}
		int spellid = atoi(sep->arg[1]);
		int slot = atoi(sep->arg[2]);
	if (c->GetTarget() == 0)
		c->MakeBuffFadePacket(spellid, slot);	
	else
		c->GetTarget()->CastToClient()->MakeBuffFadePacket(spellid, slot);
}
...
void command_pvpreset(Client *c, const Seperator *sep)
{
	Client *t=c;
	...
	int32 value=atoul(sep->arg[2]);
	if ( strcasecmp( sep->arg[1], "kills" ) == 0 )
	{
		t->GetPP().PVPKills = value;
		c->Message(13,"Kills for %s have been set to %i",t->GetName(),t->GetPP().PVPKills);
		t->UpdateLeaderBoard();
		t->Save();
	}
	else if ( strcasecmp( sep->arg[1], "deaths" ) == 0 )
	{
		t->GetPP().PVPDeaths = value;
		c->Message(13,"Deaths for %s have been set to %i",t->GetName(),t->GetPP().PVPDeaths);
		t->UpdateLeaderBoard();
		t->Save();
	}
	else if ( strcasecmp( sep->arg[1], "killstreak" ) == 0 )
	{
		t->GetPP().PVPCurrentKillStreak = value;
		c->Message(13,"CurrentKillStreak for %s have been set to %i",t->GetName(),t->GetPP().PVPCurrentKillStreak);
		t->Save();
	}
	else if ( strcasecmp( sep->arg[1], "bestkillstreak" ) == 0 )
	{
		t->GetPP().PVPBestKillStreak = value;
		c->Message(13,"BestKillStreak for %s have been set to %i",t->GetName(),t->GetPP().PVPBestKillStreak);
		t->Save();
	}
	else
		c->Message(13,"Bad Command. Usage: $pvpreset [kills/deaths/killstreak/bestkillstreak] [value]");
}

void command_infamy(Client *c, const Seperator *sep)
{
	Client *t=c;
	...
	sint32 value=atoi(sep->arg[1]);
	if ( value != 0 )
	{
		t->CastToClient()->UpdateInfamyLevel(value);
		c->Message(13,"%s infamy set to %i (value passed: %i)",t->GetName(),t->GetInfamyLevel(), value);
	}
	else
		c->Message(13,"Bad Command. Usage: #infamy [addsubtract value]");
}


void command_achievements(Client *c, const Seperator *sep)
{
	if ( c->GetTarget() && c->GetTarget()->IsClient() )
	{
		c->Message(4,"Updating %s achievements...",c->GetTarget()->GetName());
		c->GetTarget()->CastToClient()->Message(4,"Updating achievements...");
		c->GetTarget()->CastToClient()->UpdateAchievements();
	}
	else
	{
		c->Message(4,"Updating achievements...");
		c->UpdateAchievements();
	}
}
...
void command_setpvppoints(Client *c, const Seperator *sep)
{
	...
	if (sep->IsNumber(1)) {
		...
		{
			if ( t->AddPVPPoints(atoi(sep->arg[1])) )
				c->Message(0, "Client SetPVPPoints %i successful.",atoi(sep->arg[1]));
			else
				c->Message(13, "Client SetPVPPoints %i FAILED.",atoi(sep->arg[1]));
		}
	}
	else
		c->Message(0, "Usage: #addpvppoints number");
}

void command_setpvppointsnocareer(Client *c, const Seperator *sep)
{
	Client *t=c;
	...
	if (sep->IsNumber(1)) {
		...
		{
			if ( t->AddPVPPoints(atoi(sep->arg[1]), false) )
				c->Message(0, "Client SetPVPPointsNoCareer %i successful.",atoi(sep->arg[1]));
			else
				c->Message(13, "Client SetPVPPointsNoCareer %i FAILED.",atoi(sep->arg[1]));
		}
	}
	else
		c->Message(0, "Usage: #addpvppointsnocareer number");
}
...
void command_damage(Client *c, const Seperator *sep)
{
	if (c->GetTarget()==0)
		...
		if (nkdmg > 2100000000)
			...
			c->GetTarget()->Damage(c, nkdmg, SPELL_UNKNOWN, HAND_TO_HAND, 0, false);
	}
}
...
//impossible, but i'll leave it in anyways
		...
		if (atoi(sep->arg[1]) > c->Admin())
			...
			*((int32*) pack->pBuffer) = database.GetAccountIDByName(false,sep->argplus[2]);
...
void command_guild(Client *c, const Seperator *sep)
{
	...
	*/
	else if (strcasecmp(sep->arg[1], "set") == 0 || ( (strcasecmp(sep->arg[1], "forceset") == 0) && ( forceSet = true ) ) ) {
		if (!sep->IsNumber(3))
			c->Message(0, "Usage: #guild set charname guildgbid (0 = clear guildtag)");
		else {
			uint32 guild_id = atoi(sep->arg[3]);
			
			if(guild_id == 0 && !forceSet)
				guild_id = GUILD_NONE;
			...
			database.SetCharacterStatsStatic(charid,"joinedguild","UNIX_TIMESTAMP()");
		}
	}
		...
		if (c->DistNoRootNoZ(*target) > 200)
			...
			if(cur_level == 1) {
				if(c->IsAttackAllowed(target))
				{
					...
					if (nukedmg>0)
					{
						target->Damage(c, nukedmg, 2751, ABJURE/*hackish*/, 0);
...
void command_npcedit(Client *c, const Seperator *sep)
{
	...
	if ( strcasecmp( sep->arg[1], "help" ) == 0 ) {
		...
		c->Message(0, "#npcedit Face - Sets an NPCs face id (use #face for testing)");
		...
		c->Message(0, "#npcedit merchantminlevel - Sets the min level to use the merchant for an NPC");
		...
		c->Message(0, "#npcedit purgemerchantlist - Purges merchant list from memory (forcing a reload from DB)");
	   
	}
	else if ( strcasecmp( sep->arg[1], "purgemerchantlist" ) == 0 )
	{
		if ( c->GetTarget()->CastToNPC()->MerchantType < 1 )
			c->Message(13,"%s does not have a merchant id.",c->GetTarget()->GetCleanName());
		else
		{
			std::list<MerchantList> merlist = zone->merchanttable[c->GetTarget()->CastToNPC()->MerchantType];
			c->Message(13,"%s had %i entries on the merchant list which were purged.",c->GetTarget()->GetCleanName(),merlist.size());
			if ( merlist.size() > 0 )
				zone->merchanttable[c->GetTarget()->CastToNPC()->MerchantType].clear();
		}
	}
	...
	// Kings & Bandits modify face in npcedit
	else if ( strcasecmp( sep->arg[1], "face" ) == 0 )
	{
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		c->Message(15,"NPCID %u now has the face %i",c->GetTarget()->CastToNPC()->GetNPCTypeID(),atoi(sep->arg[2]));
		database.RunQuery(query, MakeAnyLenString(&query, "update npc_types set face=%i where id=%i",atoi(sep->argplus[2]),c->GetTarget()->CastToNPC()->GetNPCTypeID()), errbuf);
		c->LogSQL(query);
		safe_delete_array(query);
	}
	// Kings & Bandits modify namegroupid in npcedit
	else if ( strcasecmp( sep->arg[1], "namegroupid" ) == 0 )
	{
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		c->Message(15,"NPCID %u now has the namegroupid %i",c->GetTarget()->CastToNPC()->GetNPCTypeID(),atoi(sep->arg[2]));
		database.RunQuery(query, MakeAnyLenString(&query, "update npc_types set namegroupid=%i where id=%i",atoi(sep->argplus[2]),c->GetTarget()->CastToNPC()->GetNPCTypeID()), errbuf);
		c->LogSQL(query);
		safe_delete_array(query);
	}
	...
	else if ( strcasecmp( sep->arg[1], "merchantminlevel" ) == 0 )
	{
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		c->Message(15,"NPCID %u now is merchant_level_requirement %i",c->GetTarget()->CastToNPC()->GetNPCTypeID(),atoi(sep->arg[2]));
		database.RunQuery(query, MakeAnyLenString(&query, "update npc_types set merchant_level_requirement=%i where id=%i",atoi(sep->argplus[2]),c->GetTarget()->CastToNPC()->GetNPCTypeID()), errbuf);
		c->LogSQL(query);
		safe_delete_array(query);
	}
...
void command_checkglobal(Client *c, const Seperator *sep) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if(sep->arg[1][0] == 0) {
		c->Message(0, "Syntax: #checkglobal [globalname] [npcid] [charid] [zoneid] - Check if a client has a qglobal.");
		return;
	}
	Client* t = c;
	if ( c->GetTarget() && c->GetTarget()->IsClient() )
	...
	int32 npcid = atoi(sep->arg[2]);
	int32 charid = atoi(sep->arg[3]);
	int32 zoneid = atoi(sep->arg[4]);
	
	if(sep->arg[3][0] == 0) {
		charid = t->CharacterID();
	}
	c->Message(0, "QGlobal Values passed: npcid: %i, charid: %i, zoneid: %i",npcid,charid,zoneid);

	QGlobal res;
	if ( t->GetQGlobals() == NULL )
	{
		c->Message(13,"QGlobals was NULL, creating and loading qglobal data for character %s",t->GetName());
		QGlobalCache* cache = t->CreateQGlobals();
		cache->LoadByCharID(t->CharacterID());
	}

	if ( QGlobalCache::GetQGlobal(res,t->GetQGlobals()->GetBucket(),(char*)sep->arg[1],npcid,charid,zoneid) )
	{
		c->Message(0,"QGlobal (%s) success return for %s",sep->arg[1],t->GetName());
		c->Message(0,"NPCID: %i, CharID: %i, ZoneID: %i, Value: %s, Expiration Date: %i",res.npc_id,res.char_id,res.zone_id,res.value.c_str(),res.expdate);
	}
	else
	{
		c->Message(13,"QGlobal (%s) failed return for %s",sep->arg[1],t->GetName());
	}
}
...
void command_dyeall(Client *c, const Seperator *sep)
{
	
if(!sep->arg[3][0] || !c->GetTarget() || !c->GetTarget()->IsClient())
	{
		c->Message(0, "FORMAT: #dyeall [red] [green] [blue] - make sure you have a target!");
		return;
	}
	int r = atoi(sep->arg[1]);
	int g = atoi(sep->arg[2]);
	int b = atoi(sep->arg[3]);
	c->GetTarget()->CastToClient()->MakeDyePacket(r,g,b);
	c->Message(13, "Dye applied to all slots on target.");
}

void command_resetdisctimer(Client *c, const Seperator *sep)
{
	
if(!sep->arg[3][0] || !c->GetTarget() || !c->GetTarget()->IsClient())
	{
		c->Message(0, "FORMAT: #resetdisctimer [discid] [timer] [unknown] - make sure you have a target!");
		return;
	}
	int discid = atoi(sep->arg[1]);
	int timer = atoi(sep->arg[2]);
	int unknown = atoi(sep->arg[3]);
	c->GetTarget()->CastToClient()->ResetDiscTimer(discid,timer,unknown);
	c->Message(13, "Discipline reset on client targetted.");
}
...
void command_guildcreate(Client *c, const Seperator *sep)
{
	...
	{
		c->Message(13,"Guild management is disabled.");
		return;
	}

	if ( c->GuildID() != GUILD_NONE )
	{
		c->Message(13,"You are already in a guild.");
		return;
	}
	...
	{
		if (strstr(sep->argplus[1],"<") || strstr(sep->argplus[1],">") )
		{
			c->Message(13,"The characters '<' and '>' cannot be used.");
			return;
		}

		if(strlen(sep->argplus[1])>4 && strlen(sep->argplus[1])<24)
		{
			guild_mgr.AddGuildApproval(sep->argplus[1],c);
		}
		else
		{
			c->Message(0,"Guild name must be more than 4 characters and less than 24.");
		}
	}
}
...
void command_rules(Client *c, const Seperator *sep) {
	...
	if(sep->arg[1][0] == '\0' || !strcasecmp(sep->arg[1], "help")) {
		...
		c->Message(0, "...reloadallzones - Reload all zones with the selected ruleset");
...
//		c->Message(0, "...wload (ruleset name) - Load a ruleset in all zones without changing the selected set");
	...
	} else if ( !strcasecmp(sep->arg[1], "reloadallzones") )
	{
		worldserver.SendCommandRequest(RELOAD_ZONE_RULES, 0, (char*)c->GetName());
		c->Message(0, "Calling worldserver to reload all zone rulesets");
	}
	else if(!strcasecmp(sep->arg[1], "reload")) {
...
void command_untraindisc(Client *c, const Seperator *sep)
{
	...
	{
		c->GetTarget()->CastToClient()->UntrainDiscAll();
		c->Message(13,"Reset %s disciplines.",c->GetTarget()->GetName());
	}
	else
	{
		c->Message(13,"You must select a target and it must be a client.");
	}
}
...
void command_groundspawns(Client *c, const Seperator *sep)
{
	if (!c)
	{
	...
	entity_list.ListGroundSpawns(c);
}
...
// KaB - Red69 - Zek / reloadzones addon with new commands
void command_reloadzones(Client *c, const Seperator *sep)
{
	if(sep->arg[1][0] == '\0' || !strcasecmp(sep->arg[1], "help")) {
		c->Message(0, "Syntax: #reloadzones [subcommand].");
		c->Message(0, "...rules - Reload all zones with the current selected ruleset");
		c->Message(0, "...quests - Reload all zones quests file");
		c->Message(0, "...hotzones - Reload all zones with the current selected ruleset");
		c->Message(0, "...spellonclients [spellid] - Cast a spell on all clients on the server (non-gm)");
		c->Message(0, "...bounty [on/off] [name] - Sends a request to turn a bounty on/off a player");
	}
	else if ( !strcasecmp(sep->arg[1], "rules") )
	{
		worldserver.SendCommandRequest(RELOAD_ZONE_RULES, 0, (char*)c->GetName());
		c->Message(0, "Calling worldserver to reload all zone rulesets");
	}
	else if ( !strcasecmp(sep->arg[1], "quests") )
	{
		worldserver.SendCommandRequest(RELOAD_ZONE_QUESTS, 0, (char*)c->GetName());
		c->Message(0, "Calling worldserver to reload all quest files.");
	}
	else if ( !strcasecmp(sep->arg[1], "hotzones") )
	{
		worldserver.SendCommandRequest(RELOAD_HOTZONES, 0, (char*)c->GetName());
		c->Message(0, "Calling worldserver to reload all hot zone values.");
	}
	else if ( !strcasecmp(sep->arg[1], "pvphotzones") )
	{
		worldserver.SendCommandRequest(RELOAD_PVPHOTZONES, 0, (char*)c->GetName());
		c->Message(0, "Calling worldserver to reload all PVP hot zone values.");
	}
	else if ( !strcasecmp(sep->arg[1], "spellonclients") )
	{
		if ( !sep->IsNumber(2) )
			c->Message(0, "Bad spell id.");
		else
		{
			int32 spellID = atoi(sep->arg[2]);
			worldserver.SendCommandRequest(CAST_SELF_ON_CLIENTS, spellID, (char*)c->GetName());
			c->Message(0, "Calling worldserver to cast spell %i on the clients.", spellID);
		}
	}
	else if ( !strcasecmp(sep->arg[1], "bounty") )
	{
		if ( !strcasecmp(sep->arg[2], "on") )
		{
			worldserver.SendCommandRequest(UPDATE_CLIENT_SETBOUNTY, 1, sep->arg[3]);
			c->Message(13,"Tried to set bounty ON for %s",sep->arg[3]);
		}
		else
		{
			worldserver.SendCommandRequest(UPDATE_CLIENT_SETBOUNTY, 0, sep->arg[3]);
			c->Message(13,"Tried to set bounty OFF for %s",sep->arg[3]);
		}
	}
}
...
void command_listguards(Client *c, const Seperator *sep)
{
	if(sep->arg[1][0] == '\0' || !strcasecmp(sep->arg[1], "help")) {
		c->Message(0, "Syntax: #listguards [kingdomid].");
	}
	else
	{
		uint32 kingdomID = atoi(sep->arg[1]);
		int32 guardsAlive = 0;
		int32 guardsDead = 0;

		if ( kingdomID > 0 && kingdomID < MAXKINGDOMS )
		{
			TeamSettings_Struct* team = database.GetTeamSettings(kingdomID);
			if ( team != NULL )
			{
				c->Message(0,"Guard List for Team ID: %i",team->teamID);
				for(int i=0;i<team->totalGuardsCount;i++)
				{
					NPC* npc = entity_list.GetNPCByNPCTypeID(team->TeamGuards[i].guardNPCTypeID);

					if ( npc )
					c->Message(0,"(%i) %s : NPCID: %i, GuardAlive: %i, KilledByLastCharID: %i, KilledByLastGuildID: %i, KilledByTeamID: %i, SpawnGroupID: %i",i,npc->GetCleanName(), team->TeamGuards[i].guardNPCTypeID,team->TeamGuards[i].didMobSpawn,
						team->TeamGuards[i].lastkilledcharid,team->TeamGuards[i].lastkilledguildid,team->TeamGuards[i].lastkilledteamid,team->TeamGuards[i].spawngroupid);
					else
					c->Message(0,"(%i) NPCID: %i, GuardAlive: %i, KilledByLastCharID: %i, KilledByLastGuildID: %i, KilledByTeamID: %i, SpawnGroupID: %i",i,team->TeamGuards[i].guardNPCTypeID,team->TeamGuards[i].didMobSpawn,
						team->TeamGuards[i].lastkilledcharid,team->TeamGuards[i].lastkilledguildid,team->TeamGuards[i].lastkilledteamid,team->TeamGuards[i].spawngroupid );
				...
				c->Message(0,"Guards Alive: %i, Guards Dead: %i",guardsAlive,guardsDead);
			}
			else
				c->Message(0,"Team %i does not exist.",kingdomID);
		}
		else
		c->Message(0,"Invalid kingdom ID value.");
	}
}


void command_deleteguard(Client *c, const Seperator *sep)
{
	if(sep->arg[1][0] == '\0' || sep->arg[2][0] == '\0' || !strcasecmp(sep->arg[1], "help")) {
		c->Message(0, "Syntax: #deleteguard [kingdomid] [guardid].");
	}
	else
	{
		uint32 kingdomID = atoi(sep->arg[1]);
		uint32 guardID = atoi(sep->arg[2]);
		int32 guardsAlive = 0;
		int32 guardsDead = 0;
		...
		if ( kingdomID > 0 && kingdomID < MAXKINGDOMS )
		{
			...
			if ( team != NULL )
			{
				c->Message(0,"Attempt guard delete for Team ID: %i Guard ID: %i",team->teamID, guardID);
				if ( guardID < team->totalGuardsCount )
				{
					team->TeamGuards[guardID].didMobSpawn = false;
					team->TeamGuards[guardID].spawngroupid = 0;
					team->TeamGuards[guardID].guardNPCTypeID = 0;
					c->Message(13,"GuardID %i deleted.",guardID);
				}
				else
					c->Message(13,"GuardID %i too high, max guards: %i",guardID,team->totalGuardsCount);
			}
			else
				c->Message(0,"Team %i does not exist.",kingdomID);
		}
...
void command_listguildhate(Client *c, const Seperator *sep)
{
	if(sep->arg[1][0] == '\0' || !strcasecmp(sep->arg[1], "help")) {
		c->Message(0, "Syntax: #listguildhate [kingdomid].");
	}
	else
	{
		uint32 kingdomID = atoi(sep->arg[1]);
		...
		if ( kingdomID > 0 && kingdomID < MAXKINGDOMS )
		{
			...
			if ( team != NULL )
			{
				...
				{
					if ( team->guildhate[i].taggedguild == 0 )
						continue;

					c->Message(0,"Guild ID: %i, Timestamp: %i",team->guildhate[i].taggedguild,team->guildhate[i].taggedtimestamp);
					guildcount++;
				}

				c->Message(0,"Guilds Listed: %i",guildcount);
			}
			else
				c->Message(0,"Team %i does not exist.",kingdomID);
		}
...
void command_burycorpse(Client *c, const Seperator *sep)
{
	if (c->GetTarget() != 0)
	{
		if ( c->GetTarget()->IsPlayerCorpse() )
		{
			c->GetTarget()->CastToCorpse()->SetDecayTimer(1);
			c->Message(13,"Setting decay timer to 1ms to call instant decay and bury to database.");
		}
		else
			c->Message(13,"Pick a player corpse target.");
	}
	else
			c->Message(13,"Pick a player corpse target.");
}

void command_replay(Client* c, const Seperator *sep)
{
	if (sep->arg[1][0] == '\0' || strcasecmp(sep->arg[1], "help") == 0) {
			c->Message(0, "Replay Usage:");
			c->Message(0, "  #replay list [charname/charid]");
			c->Message(0, "  #replay run [replayid]");
	}
	else if ( strcasecmp(sep->arg[1], "run") == 0 )
	{
		if ( !sep->IsNumber(2) )
		{
			c->Message(0,"Correct Usage: #replay run [replayid]");
		}
		else if ( c->GetTarget() != NULL && c->GetTarget()->IsNPC() )
		{
			uint32 replayID = atoi(sep->arg[2]);
			c->GetTarget()->StartReplay(replayID);
		}
		else
		{
			uint32 replayID = atoi(sep->arg[2]);
			c->StartReplay(replayID);
		}
	}
	else if ( strcasecmp(sep->arg[1], "list") == 0 )
	{
		if ( sep->arg[2][0] == 0 )
		{
			c->Message(0, "Correct Usage: #replay list [charname/charid]");
		}
		else
		{
			uint32 charID = 0;
			...
			if ( charID > 0 )
			{
				LinkedList<ReplayList_Struct*> replaylist;
				database.GetReplayList(charID,zone->GetZoneID(),&replaylist);
				if ( replaylist.Count() < 1)
					c->Message(0, "No entries recorded for charID: %i, provided value: %s",charID,sep->arg[2]);
				else
				{
					LinkedListIterator<ReplayList_Struct*> iterator(replaylist);
					iterator.Reset();
					c->Message(0, "Replay Results for charID: %i, provided value: %s",charID,sep->arg[2]);
					while(iterator.MoreElements())
					{
						ReplayList_Struct* rls = iterator.GetData();
						
						c->Message(0, "%i: %s - %s",rls->replayid,rls->startTime,rls->endTime);
						iterator.RemoveCurrent(true);
					}
					c->Message(0, "End Replay List");
				}
			}
			else
				c->Message(0, "The character %s could not be found or matched to the character id (%i).", sep->arg[2], charID);
		}
	}
}


void command_flagnpcguildid(Client *c, const Seperator *sep)
{
	if ( c->GetTarget() != NULL && c->GetTarget()->IsNPC() )
	{
		if ( c->GetTarget()->CastToNPC()->isGuildNPC )
		{
		c->GetTarget()->CastToNPC()->isGuildNPC = false;
		c->Message(13,"Removed guild id");
		}
		else
		{
		c->GetTarget()->CastToNPC()->isGuildNPC = true;
		c->Message(13,"Set guild id");
		}
	}
	else
		c->Message(13,"Bad target");
}

void command_consider(Client *c, const Seperator *sep)
{
	if ( !sep->IsNumber(1) || !sep->IsNumber(2) || !sep->IsNumber(3) )
		c->Message(13,"Proper Usage: #consider <pvpcon> <faction> <level>");
	...
	if ( c->GetTarget() != NULL )
	{
		...
		con->playerid = c->GetID();
		con->targetid = c->GetTarget()->GetID();
		con->pvpcon = atoi(sep->arg[1]);
		con->faction = atoi(sep->arg[2]);
		con->level = atoi(sep->arg[3]);
		c->QueuePacket(outapp);
		safe_delete(outapp);
	}
	else
...
void command_setdeathcount(Client *c, const Seperator *sep)
{
	if ( c->GetTarget() == NULL || !c->GetTarget()->IsNPC() )
	{
		c->Message(13,"Target an NPC");
		return;
	}

	if ( !zone->UseDeathMods() )
	{
		c->Message(13,"Zone has use_death_mods off in the zone table, this value cannot be set.");
		return;
	}

	if ( sep->IsNumber(1) )
	{
		if ( c->GetTarget()->CastToNPC()->respawn2 )
		{
			c->GetTarget()->CastToNPC()->respawn2->setDeathCount(atoi(sep->arg[1]));
		}
		else
			c->Message(13,"No respawn2 to set death count on.");
	}
	else
		c->Message(0,"Numerical values only");
}


void command_ignoretimer(Client *c, const Seperator *sep)
{
	c->SetZoneLockoutID(0);
	c->Message(13,"Zone lockout information disabled.");
}


void command_teamcount(Client *c, const Seperator *sep)
{
	c->Message(0, "Server Team Counts:");
	c->Message(0, "Newbie Count: %i", zone->GetPlayerCount(0));
	for (int i=1;i<6;i++)
	{
		c->Message(0, "%s: %i", KingsBandits::GetKingdomName(i,true),zone->GetPlayerCount(i));
	}
}
...
void command_itemedit(Client *c, const Seperator *sep)
{
	...
	const char* supportedFields[] = { "ac"/*0*/, "Name"/*1*/, "aagi"/*2*/, "acha"/*3*/, "adex"/*4*/, "aint"/*5*/, "asta"/*6*/, "astr"/*7*/, "awis"/*8*/, "ldonprice"/*9*/, 
								"ldontheme"/*10*/, "cr"/*11*/, "dr"/*12*/, "fr"/*13*/, "mr"/*14*/, "pr"/*15*/, "augslot1type"/*16*/, "price"/*17*/, "sellrate"/*18*/, "nodrop"/*19*/,
								"norent"/*20*/, "magic"/*21*/, "hp"/*22*/, "regen"/*23*/, "mana"/*24*/, "manaregen"/*25*/, "clicklevel"/*26*/,
								"reclevel"/*27*/, "reqlevel"/*28*/, "proclevel"/*29*/, "wornlevel"/*30*/, "focuslevel"/*31*/,"scrolllevel"/*32*/,"bardlevel"/*33*/, "clicklevel2"/*34*/, "proclevel2"/*35*/,
								"wornlevel2"/*36*/, "focuslevel2"/*37*/,"scrolllevel2"/*38*/,"bardlevel2"/*39*/,
								"focuseffect"/*40*/, "clickeffect"/*41*/, "proceffect"/*42*/, "worneffect"/*43*/, "scrolleffect"/*44*/, "bardeffect"/*45*/, "bardeffecttype"/*46*/,
								"classes"/*47*/, "races"/*48*/, "deity"/*49*/, "itemclass"/*50*/, "lore"/*51*/, "loregroup"/*52*/,
								"haste"/*53*/, "damage"/*54*/, "delay"/*55*/, "clicktype"/*56*/, "proctype"/*57*/, "worntype"/*58*/, "focustype"/*59*/, "scrolltype"/*60*/, "maxcharges"/*61*/, "banedmgraceamt" /*62*/,
								"recasttype" /*63*/, "recastdelay" /*64*/, "augslot2type" /*65*/, "skillmodtype" /*66*/,"skillmodvalue" /*67*/,"backstabdmg" /*68*/, "slots" /*69*/, "damageshield" /*70*/, "dotshielding" /*71*/,
								"attack" /*72*/, "dsmit" /*73*/, "combateffects" /*74*/, "shielding" /*75*/, "stunresist" /*76*/, "strikethrough" /*77*/, "extradmgSkill" /*78*/, "extradmgAmt" /*79*/, "spellshield" /*80*/,
								"Avoidance" /*81*/, "Accuracy" /*82*/, "BardType" /*83*/, "BardValue" /*84*/, "casttime" /*85*/, "healamt" /*86*/, "augtype" /*87*/, "ProcRate" /*88*/, "Range" /*89*/, "MaxCharges" /*90*/, "StackSize" /*91*/,
								"Material" /*92*/, "ItemType" /*93*/, "idfile" /*94*/, "scriptfileid" /*95*/, "charmfile" /*96*/, "icon" /*97*/, "endur" /*98*/, "enduranceregen" /*99*/, "notransfer" /*100*/};

	int fieldCount = (sizeof(supportedFields)/sizeof(const char *));

	_snprintf(fieldName,256,"unknownfield");
	_snprintf(fieldValue,256,"??");
	if ( strcasecmp( sep->arg[1], "getitem" ) == 0 ) {
		int32 newItemId = c->GetItemEditID();

		const Item_Struct* item = database.GetItem(newItemId);
		if ( item != NULL )
		{
				if (c->GetClientVersion() >= EQClientSoF)
				{
					c->Message(0, "Selected Item %i: %c%06X00000000000000000000000000000000000000000000%s%c",(int) item->ID,0x12, item->ID, item->Name, 0x12);
				}
				else
				{
					c->Message(0, "Selected Item %i: %c%06X000000000000000000000000000000000000000%s%c",(int) item->ID,0x12, item->ID, item->Name, 0x12);
				}
		}
		else
		{
			c->Message(15,"No item selected, use #itemedit setitem [id]");
		}
	}
	else if ( strcasecmp( sep->arg[1], "fixitem" ) == 0 )
	{
		Item_Struct* item = database.GetItemNoConst(c->GetItemEditID());
		int32 newItemID = (int32)atoi(sep->arg[2]);
		if (item != NULL && newItemID > 0 )
		{
			item->ID = (int32)newItemID;
			database.FixItem(item, newItemID);
		}
		return;
	}
	else if ( strcasecmp( sep->arg[1], "dupeitem" ) == 0 ) {
		int32 newItemId = atoul(sep->arg[2]);
			Item_Struct* item = database.GetItemNoConst(c->GetItemEditID());
			if ( item != NULL )
			{
				Item_Struct newItem;
				memcpy(&newItem, item, sizeof(Item_Struct));
				uint32 resultID = 0;
				if (!database.AddNewItem(&newItem, 0, "", &resultID) )
				{
					c->Message(13,"Item dupe of %s (%i) failed!",item->Name,item->ID);
				}
				else
				{
					c->Message(15,"Item created: %s (%i)",item->Name,resultID);
					const Item_Struct* item = database.GetItem(resultID);
					if ( item != NULL )
					{
						c->SetItemEditID(resultID);
						if (c->GetClientVersion() >= EQClientSoF)
						{
							c->Message(0, "Selected Item %i: %c%06X00000000000000000000000000000000000000000000%s%c",(int) item->ID,0x12, item->ID, item->Name, 0x12);
						}
					...
					{
						c->Message(15,"Item set failed: %s (%i)",item->Name,resultID);
					}
				}
			}
			else
			{
				c->Message(15,"No item exists with id: %i",newItemId);
			}
			return;
		}
	else if ( strcasecmp( sep->arg[1], "help" ) == 0 || !sep->arg[2][0] ) {

		c->Message(0, "Help File for #itemedit. Syntax for commands are:");
		c->Message(0, "#itemedit setitem [id] - sets the item id to edit");
		c->Message(0, "#itemedit getitem - shows the current item id being edited");
		c->Message(0, "#itemedit dupeitem - make a new item of the current edited item");
		for(int i=0;i<fieldCount;i++)
		{
			c->Message(0, "#itemedit %s [newvalue]", supportedFields[i]);
		}
	}
	else if ( strcasecmp( sep->arg[1], "setitem" ) == 0 ) {
		int32 newItemId = atoul(sep->arg[2]);
		const Item_Struct* item = database.GetItem(newItemId);
		if ( item != NULL )
		{
			c->SetItemEditID(atoi(sep->arg[2]));
						if (c->GetClientVersion() >= EQClientSoF)
						{
							c->Message(0, "Selected Item %i: %c%06X00000000000000000000000000000000000000000000%s%c",(int) item->ID,0x12, item->ID, item->Name, 0x12);
						}
	...
	else if ( c->GetItemEditID() == 0 )
	{
		c->Message(15,"Need to select the item first, use #itemedit setitem [id]");
	}
	else
	{
		for(int i=0;i<fieldCount;i++)
		{
			if ( strcasecmp( sep->arg[1],supportedFields[i]) == 0 ) { 
				_snprintf(fieldName,256,supportedFields[i]);
				_snprintf(fieldValue,256,"%s",sep->argplus[2]);
				field = i;
				runSQL = true;
				break;
			}
		...
		if ( !runSQL ) {
			c->Message(0, "No fields matched, issue [#itemedit help] for more details.");
		}
		else
		{
			Item_Struct* item = database.GetItemNoConst(c->GetItemEditID());
			...
			{
			case 0:
					_snprintf(origValue,256,"AC: %i",item->AC);
					item->AC = atoi(fieldValue);
					break;
			case 1:
					_snprintf(origValue,256,"Name: %s",item->Name);
					strncpy(item->Name,fieldValue,64); 
					break;
			case 2:
					_snprintf(origValue,256,"AGI: %i",item->AAgi);
					item->AAgi = atoi(fieldValue);
					break;
			case 3:
					_snprintf(origValue,256,"CHA: %i",item->ACha);
					item->ACha = atoi(fieldValue);
					break;
			case 4:
					_snprintf(origValue,256,"DEX: %i",item->ADex);
					item->ADex = atoi(fieldValue);
					break;
			case 5:
					_snprintf(origValue,256,"INT: %i",item->AInt);
					item->AInt = atoi(fieldValue);
					break;
			case 6:
					_snprintf(origValue,256,"STA: %i",item->ASta);
					item->ASta = atoi(fieldValue);
					break;
			case 7:
					_snprintf(origValue,256,"STR: %i",item->AStr);
					item->AStr = atoi(fieldValue);
					break;
			case 8:
					_snprintf(origValue,256,"WIS: %i",item->AWis);
					item->AWis = atoi(fieldValue);
					break;
			case 9:
					_snprintf(origValue,256,"LDONPrice: %i",item->LDoNPrice);
					item->LDoNPrice = atoi(fieldValue);
					break;
			case 10:
					_snprintf(origValue,256,"LDONTheme: %i",item->LDoNTheme);
					item->LDoNTheme = atoi(fieldValue);
					break;
			case 11:
					_snprintf(origValue,256,"ColdResistance: %i",item->CR);
					item->CR = atoi(fieldValue);
					break;
			case 12:
					_snprintf(origValue,256,"DiseaseResistance: %i",item->DR);
					item->DR = atoi(fieldValue);
					break;
			case 13:
					_snprintf(origValue,256,"FireResistance: %i",item->FR);
					item->FR = atoi(fieldValue);
					break;
			case 14:
					_snprintf(origValue,256,"MagicResistance: %i",item->MR);
					item->MR = atoi(fieldValue);
					break;
			case 15:
					_snprintf(origValue,256,"PoisonResistance: %i",item->PR);
					item->PR = atoi(fieldValue);
					break;
			case 16:
			{
					_snprintf(origValue,256,"AugSlot1: %i, AugSlotVisible1: %i",item->AugSlotType[1], item->AugSlotVisible[0]);
					int inValue = atoi(fieldValue);
					item->AugSlotType[0] = inValue;
					
					if ( inValue > 0 )
						item->AugSlotVisible[0] = 1;
					else
						item->AugSlotVisible[0] = 0;
					
					break;
			}
			case 17:
					_snprintf(origValue,256,"Price: %i",item->Price);
					item->Price = atoi(fieldValue);
					break;
			case 18:
					_snprintf(origValue,256,"SellRate: %f",item->SellRate);
					item->SellRate = atof(fieldValue);
					break;
			case 19:
					_snprintf(origValue,256,"NoDrop: %i",item->NoDrop);
					item->NoDrop = atoi(fieldValue);
					break;
			case 20:
					_snprintf(origValue,256,"NoRent: %i",item->NoRent);
					item->NoRent = atoi(fieldValue);
					break;
			case 21:
					_snprintf(origValue,256,"Magic: %i",item->Magic);
					item->Magic = atoi(fieldValue);
					break;
			case 22:
					_snprintf(origValue,256,"HitPoints: %i",item->HP);
					item->HP = atoi(fieldValue);
					break;
			case 23:
					_snprintf(origValue,256,"HPRegen: %i",item->Regen);
					item->Regen = atoi(fieldValue);
					break;
			case 24:
					_snprintf(origValue,256,"Mana: %i",item->Mana);
					item->Mana = atoi(fieldValue);
					break;
			case 25:
					_snprintf(origValue,256,"ManaRegen: %i",item->ManaRegen);
					item->ManaRegen = atoi(fieldValue);
					break;
			case 26:
					_snprintf(origValue,256,"ClickLevel: %i",item->Click.Level);
					item->Click.Level = atoi(fieldValue);
					break;
			case 27:
					_snprintf(origValue,256,"Recommended Level: %i",item->RecLevel);
					item->RecLevel = atoi(fieldValue);
					break;
			case 28:
					_snprintf(origValue,256,"Required Level: %i",item->ReqLevel);
					item->ReqLevel = atoi(fieldValue);
					break;
			case 29:
					_snprintf(origValue,256,"ProcLevel: %i",item->Proc.Level);
					item->Proc.Level = atoi(fieldValue);
					break;
			case 30:
					_snprintf(origValue,256,"WornLevel: %i",item->Worn.Level);
					item->Worn.Level = atoi(fieldValue);
					break;
			case 31:
					_snprintf(origValue,256,"FocusLevel: %i",item->Focus.Level);
					item->Focus.Level = atoi(fieldValue);
					break;
			case 32:
					_snprintf(origValue,256,"ScrollLevel: %i",item->Scroll.Level);
					item->Scroll.Level = atoi(fieldValue);
					break;
			case 33:
					_snprintf(origValue,256,"BardLevel: %i",item->Bard.Level);
					item->Bard.Level = atoi(fieldValue);
					break;
			case 34:
					_snprintf(origValue,256,"ClickLevel2: %i",item->Click.Level2);
					item->Click.Level2 = atoi(fieldValue);
					break;
			case 35:
					_snprintf(origValue,256,"ProcLevel2: %i",item->Proc.Level2);
					item->Proc.Level2 = atoi(fieldValue);
					break;
			case 36:
					_snprintf(origValue,256,"WornLevel2: %i",item->Worn.Level2);
					item->Worn.Level2 = atoi(fieldValue);
					break;
			case 37:
					_snprintf(origValue,256,"FocusLevel2: %i",item->Focus.Level2);
					item->Focus.Level2 = atoi(fieldValue);
					break;
			case 38:
					_snprintf(origValue,256,"ScrollLevel2: %i",item->Scroll.Level2);
					item->Scroll.Level2 = atoi(fieldValue);
					break;
			case 39:
					_snprintf(origValue,256,"BardLevel2: %i",item->Bard.Level2);
					item->Bard.Level2 = atoi(fieldValue);
					break;
			case 40:
					_snprintf(origValue,256,"FocusEffect: %i",item->Focus.Effect);
					item->Focus.Effect = atoi(fieldValue);
					break;
			case 41:
					_snprintf(origValue,256,"ClickEffect: %i",item->Click.Effect);
					item->Click.Effect = atoi(fieldValue);
					break;
			case 42:
					_snprintf(origValue,256,"ProcEffect: %i",item->Proc.Effect);
					item->Proc.Effect = atoi(fieldValue);
					break;
			case 43:
					_snprintf(origValue,256,"WornEffect: %i",item->Worn.Effect);
					item->Worn.Effect = atoi(fieldValue);
					break;
			case 44:
					_snprintf(origValue,256,"ScrollEffect: %i",item->Scroll.Effect);
					item->Scroll.Effect = atoi(fieldValue);
					break;
			case 45:
					_snprintf(origValue,256,"BardEffect: %i",item->Bard.Effect);
					item->Bard.Effect = atoi(fieldValue);
					break;
			case 46:
					_snprintf(origValue,256,"BardEffectType: %i",item->Bard.Type);
					item->Bard.Type = atoi(fieldValue);
					break;
			case 47:
					_snprintf(origValue,256,"Classes: %i",item->Classes);
					item->Classes = atoi(fieldValue);
					break;
			case 48:
					_snprintf(origValue,256,"Races: %i",item->Races);
					item->Races = atoi(fieldValue);
					break;
			case 49:
					_snprintf(origValue,256,"Deities: %i",item->Deity);
					item->Deity = atoi(fieldValue);
					break;
			case 50:
					_snprintf(origValue,256,"ItemClass: %i",item->ItemClass);
					item->ItemClass = atoi(fieldValue);
					break;
			case 51:
					_snprintf(origValue,256,"Lore: %s",item->Lore);
					strncpy(item->Lore,fieldValue,80); 
					break;
			case 52:
					_snprintf(origValue,256,"LoreGroup: %i",item->LoreGroup);
					item->LoreGroup = atoi(fieldValue);
					item->LoreFlag = (item->LoreGroup!=0);
					break;
			case 53:
					_snprintf(origValue,256,"Haste: %i",item->Haste);
					item->Haste = atoi(fieldValue);
					break;
			case 54:
					_snprintf(origValue,256,"Damage: %i",item->Damage);
					item->Damage = atoi(fieldValue);
					break;
			case 55:
					_snprintf(origValue,256,"Delay: %i",item->Delay);
					item->Delay = atoi(fieldValue);
					break;
			case 56:
					_snprintf(origValue,256,"ClickEffectType: %i",item->Click.Type);
					item->Click.Type = atoi(fieldValue);
					break;
			case 57:
					_snprintf(origValue,256,"ProcEffectType: %i",item->Proc.Type);
					item->Proc.Type = atoi(fieldValue);
					break;
			case 58:
					_snprintf(origValue,256,"WornEffectType: %i",item->Worn.Type);
					item->Worn.Type = atoi(fieldValue);
					break;
			case 59:
					_snprintf(origValue,256,"FocusEffectType: %i",item->Focus.Type);
					item->Focus.Type = atoi(fieldValue);
					break;
			case 60:
					_snprintf(origValue,256,"ScrollEffectType: %i",item->Scroll.Type);
					item->Scroll.Type = atoi(fieldValue);
					break;
			case 61:
					_snprintf(origValue,256,"MaxCharges: %i",item->MaxCharges);
					item->MaxCharges = atoi(fieldValue);
					break;
			case 62:
					_snprintf(origValue,256,"BaneDmgRaceAmt: %i",item->BaneDmgRaceAmt);
					item->BaneDmgRaceAmt = atoi(fieldValue);
					break;
			case 63:
					_snprintf(origValue,256,"RecastType: %i",item->RecastType);
					item->RecastType = atoi(fieldValue);
					break;
			case 64:
					_snprintf(origValue,256,"RecastDelay: %i",item->RecastDelay);
					item->RecastDelay = atoi(fieldValue);
					break;
                        case 65:
                        {
                                        _snprintf(origValue,256,"AugSlot2: %i, AugSlotVisible2: %i",item->AugSlotType[1], item->AugSlotVisible[1]);
                                        int inValue = atoi(fieldValue);
                                        item->AugSlotType[1] = inValue;

                                        if ( inValue > 0 )
                                                item->AugSlotVisible[1] = 1;
                                        else
                                                item->AugSlotVisible[1] = 0;

                                        break;
                        }
                        case 66:
                        
			                _snprintf(origValue,256,"skilmodtype: %i",item->SkillModType);
                                        item->SkillModType = atoi(fieldValue);
                                        break;
                        ...
                        case 67:
			                ...
			                _snprintf(origValue,256,"skillmodvalue: %i",item->SkillModValue);
                                        item->SkillModValue = atoi(fieldValue);
                                        break;
			

                        case 68:
                       
                                        _snprintf(origValue,256,"BackstabDmg: %i",item->BackstabDmg);
                                        item->BackstabDmg = atoi(fieldValue);
                                        break;
                       

                        case 69:
                        
                                        _snprintf(origValue,256,"Slots: %i",item->Slots);
                                        item->Slots = atoi(fieldValue);
                                        break;
			...
			case 70:
                                        ...
                                        _snprintf(origValue,256,"DamageShield: %i",item->DamageShield);
                                        item->DamageShield = atoi(fieldValue);
                                        break;
                        
			
			case 71:
                        
                                        _snprintf(origValue,256,"DotShielding: %i",item->DotShielding);
                                        item->DotShielding = atoi(fieldValue);
                                        break;
                       

			case 72:
                        
                                        _snprintf(origValue,256,"Attack: %i",item->Attack);
                                        item->Attack = atoi(fieldValue);
                                        break;
                       
			
			case 73:
                        
                                        _snprintf(origValue,256,"DSMitigation: %i",item->DSMitigation);
                                        item->DSMitigation = atoi(fieldValue);
                                        break;
                        

			case 74:
                       
                                        _snprintf(origValue,256,"CombatEffects: %i",item->CombatEffects);
                                        item->CombatEffects = atoi(fieldValue);
                                        break;
                        

			case 75:
                        
                                        _snprintf(origValue,256,"Shielding: %i",item->Shielding);
                                        item->Shielding = atoi(fieldValue);
                                        break;
                        

			case 76:
                        
                                        _snprintf(origValue,256,"StunResist: %i",item->StunResist);
                                        item->StunResist = atoi(fieldValue);
                                        break;
                        
	
			case 77:
                        
                                        _snprintf(origValue,256,"StrikeThrough: %i",item->StrikeThrough);
                                        item->StrikeThrough = atoi(fieldValue);
                                        break;
			...
			case 78:
                                        ...
                                        _snprintf(origValue,256,"ExtraDmgSkill: %i",item->ExtraDmgSkill);
                                        item->ExtraDmgSkill = atoi(fieldValue);
                                        break;
                        

			case 79:
                        
                                        _snprintf(origValue,256,"ExtraDmgAmt: %i",item->ExtraDmgAmt);
                                        item->ExtraDmgAmt = atoi(fieldValue);
                                        break;
                        
			
			case 80:
                        
                                        _snprintf(origValue,256,"SpellShield: %i",item->SpellShield);
                                        item->SpellShield = atoi(fieldValue);
                                        break;
                        
			
			case 81:
                        
                                        _snprintf(origValue,256,"Avoidance: %i",item->Avoidance);
                                        item->Avoidance = atoi(fieldValue);
                                        break;
                        
			
			case 82:
                        
                                        _snprintf(origValue,256,"Accuracy: %i",item->Accuracy);
                                        item->Accuracy = atoi(fieldValue);
                                        break;
                        
			
			case 83:
                        
                                        _snprintf(origValue,256,"BardType: %i",item->BardType);
                                        item->BardType = atoi(fieldValue);
                                        break;
                        
			
			case 84:
                        
                                        _snprintf(origValue,256,"BardValue: %i",item->BardValue);
                                        item->BardValue = atoi(fieldValue);
                                        break;
                        
			
			case 85:
                        
                                        _snprintf(origValue,256,"CastTime: %i",item->CastTime);
                                        item->CastTime = atoi(fieldValue);
                                        item->CastTime_ = atoi(fieldValue);
                                        break;
                        
			
			case 86:
                        
                                        _snprintf(origValue,256,"HealAmt: %i",item->HealAmt);
                                        item->HealAmt = atoi(fieldValue);
                                        break;

			case 87:
                        
                                        _snprintf(origValue,256,"AugType: %i",item->AugType);
                                        item->AugType = atoi(fieldValue);
                                        break;
			case 88:
                        
                                        _snprintf(origValue,256,"ProcRate: %i",item->ProcRate);
                                        item->ProcRate = atoi(fieldValue);
                                        break;
			
			case 89:
                        
                                        _snprintf(origValue,256,"Range: %i",item->Range);
                                        item->Range = atoi(fieldValue);
                                        break;
			case 90:
			...
			case 91:
                                        ...
                                        _snprintf(origValue,256,"StackSize: %i",item->StackSize);
                                        item->StackSize = atoi(fieldValue);
                                        break;
			
			case 92:
                        
                                        _snprintf(origValue,256,"Material: %i",item->Material);
                                        item->Material = atoi(fieldValue);
                                        break;
			

			case 93:
                        
                                        _snprintf(origValue,256,"ItemType: %i",item->ItemType);
                                        item->ItemType = atoi(fieldValue);
                                        break;
										
                        case 94:
			{

                                        _snprintf(origValue,30,"IDFile: %s",item->IDFile);
                                        item->IDFile[30] = fieldValue[30];
                                        break;
			}
                        case 95:
			{

                                        _snprintf(origValue,30,"ScriptFileID: %i",item->ScriptFileID);
                                        item->ScriptFileID = atoi(fieldValue);
                                        break;
			}
                        case 96:
			{

                                        _snprintf(origValue,30,"CharmFile: %s",item->CharmFile);
										strncpy(item->CharmFile,fieldValue,32); 
                                        break;
			}
                        case 97:
			{
				
                                        _snprintf(origValue,30,"Icon: %i",item->Icon);
                                        item->Icon = atoi(fieldValue);
                                        break;
			}
                        case 98:
			{
				
                                        _snprintf(origValue,30,"Endur: %i",item->Endur);
                                        item->Endur = atoi(fieldValue);
                                        break;
			}
                        case 99:
			{
				
                                        _snprintf(origValue,30,"EnduranceRegen: %i",item->EnduranceRegen);
                                        item->EnduranceRegen = atoi(fieldValue);
                                        break;
			}
                        case 100:
			{
				
                                        _snprintf(origValue,30,"NoTransfer: %i",item->NoTransfer);
                                        item->NoTransfer = atoi(fieldValue);
                                        break;
			}

}

			c->Message(15,"Original Value: %s",origValue);
				if (c->GetClientVersion() >= EQClientSoF)
				{
					c->Message(0, "Updating DB of item %c%06X00000000000000000000000000000000000000000000%s%c with FieldName: %s, FieldValue: %s",0x12, item->ID, item->Name, 0x12,fieldName,fieldValue);
				}
				else
				{
					c->Message(0, "Updating DB of item %c%06X000000000000000000000000000000000000000%s%c with FieldName: %s, FieldValue: %s",0x12, item->ID, item->Name, 0x12,fieldName,fieldValue);
				}
			char errbuf[MYSQL_ERRMSG_SIZE];
			char *query = 0;
			database.RunQuery(query, MakeAnyLenString(&query, "update items set %s='%s' where id=%i",fieldName,fieldValue,c->GetItemEditID()), errbuf);
			c->LogSQL(query);
			safe_delete_array(query);

			
			if ( !strcasecmp(fieldName, "casttime") ) // casttime also needs casttime_ set
			{
				int result = 0;
				int fieldVal = atoi(fieldValue);
				if ( fieldVal > 0 )
					result = 1;
				database.RunQuery(query, MakeAnyLenString(&query, "update items set casttime_=%i where id=%i",item->CastTime_,c->GetItemEditID()), errbuf);
				c->LogSQL(query);
				safe_delete_array(query);

			}
			
			if ( !strcasecmp(fieldName, "augslot1type") ) // redo this to match on augslotXtype and handle below appropriately
			{
				int result = 0;
				int fieldVal = atoi(fieldValue);
				if ( fieldVal > 0 )
					result = 1;
				database.RunQuery(query, MakeAnyLenString(&query, "update items set augslot1visible=%i where id=%i",result,c->GetItemEditID()), errbuf);
				c->LogSQL(query);
				safe_delete_array(query);
			...
			//devn00bstuff
			if ( !strcasecmp(fieldName, "augslot2type") ) // redo this to match on augslotXtype and handle below appropriately
                       {
                                int result = 0;
                                int fieldVal = atoi(fieldValue);
                                if ( fieldVal > 0 )
                                       result = 1;
                                database.RunQuery(query, MakeAnyLenString(&query, "update items set augslot2visible=%i where id=%i",result,c->GetItemEditID()), errbuf);
                                c->LogSQL(query);
				safe_delete_array(query);

                        }
			//devn00b if we set item to stacksize, set stackable yes 

			if ( !strcasecmp(fieldName, "stacksize") )                       {
                                int result = 0;
                                int fieldVal = atoi(fieldValue);
                                if ( fieldVal > 0 )
                                       result = 1;
                                database.RunQuery(query, MakeAnyLenString(&query, "update items set stackable=%i where id=%i",result,c->GetItemEditID()), errbuf);
                                c->LogSQL(query);
				safe_delete_array(query);
...
void command_givestartingitems(Client *c, const Seperator *sep)
{
if ( c->GetTarget() != NULL && c->GetTarget()->IsClient() )
{
	Client* targ = c->GetTarget()->CastToClient();
	targ->SetStartingItems();
	c->Message(13,"Gave %s starting items.", targ->GetName());
}
else
	c->Message(13,"No client targetted.");
}


void command_nukeinventory(Client *c, const Seperator *sep)
{
if ( c->GetTarget() != NULL && c->GetTarget()->IsClient() )
{
	Client* targ = c->GetTarget()->CastToClient();
	targ->NukeInventory();
	targ->Message(13,"Nuked %s full inventory.", targ->GetName());
}
else
	c->Message(13,"No client targetted.");
}
...
void command_becomenpc(Client *c, const Seperator *sep)
{
	if(!sep->arg[1][0]) {
		c->Message(13, "Usage: #becomenpc [on/off] [npcname] [class] [race]");
		return;
	}
	...
	{
		if(!sep->arg[4][0]) {
			c->Message(13, "Usage: #becomenpc on [npcname] [class] [race]");
			return;
		}
		c->SetExtProfileNPC(true,sep->arg[2],atoul(sep->arg[3]),atoul(sep->arg[4]));
	}
	else if ( strcasecmp( sep->arg[1], "off" ) == 0 )
	{
		c->SetExtProfileNPC(false,"",0,0);
	}
	else
		c->Message(13,"No command matched, should be #becomenpc [on/off] [name] [class] [race]");
}