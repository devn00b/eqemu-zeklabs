...
uint32 SpawnGroup::GetNPCType(int32 deathCount) {
#if EQDEBUG >= 10
	...
	for(; cur != end; cur++) {
		SpawnEntry *se = *cur;
		int chance = se->chance;
		if ( RuleB(Zone, DeathSpawnNPCTypeMod) && chance < RuleI(Zone,DeathSpawnChanceBelow))
		{
			int rndChance = MakeRandomInt(0,RuleI(Zone,CorrectSpawnMaxRandom));
			if ( rndChance < RuleI(Zone,CorrectSpawnDropChance) )
				chance = (int)((float)chance + (float)chance * (float)log((float)deathCount + (float)RuleR(Zone,DeathSpawnNPCLogBase)) - (float)RuleR(Zone,DeathSpawnNPCGraphBase));
		}
...
void SpawnGroupList::AddSpawnGroup(SpawnGroup* newGroup) {
	...
	if ( groups.count(newGroup->id) != 0 && groups[newGroup->id] != NULL )
		safe_delete(groups[newGroup->id]);
