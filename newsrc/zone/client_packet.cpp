...
void MapOpcodes() {
	...
	ConnectedOpcodes[OP_ShroudUnknown1] = &Client::Handle_OP_ShroudUnknown1;
...
int Client::HandlePacket(const EQApplicationPacket *app)
{
	...
	//cerr << GetName() << " - OPCODE: " << hex << setw(4) << setfill('0') << app->GetOpcode() << dec << ", size: " << app->size << endl;
...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	char charname_entry[64];
	int length = strnlen(cze->char_name,64);

	strncpy(charname_entry,cze->char_name,length);
	charname_entry[length] = '\0';

	// we already passed the zone entry state we should not be here again!
	if ( conn_state > ReceivedZoneEntry )
	{
		LogFile->write(EQEMuLog::Status, "Existing client request received for %s on ZoneEntry, but already received ZoneEntry.", charname_entry);
		return;
	}

	int32 cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) New Client %s received.", cur_time, charname_entry);
	...
	Client* client = entity_list.GetClientByName(charname_entry);
	...
	strncpy(name, charname_entry, length);
	
	mlog(ZONE__WORLD, "Connection arriving from world, name %s (%i) (accountid %i)",name, character_id, account_id);
	...
	dbaw->AddQuery(1, &query, MakeAnyLenString(&query,
		"SELECT status,name,lsaccount_id,gmspeed,revoked,hideme,time_creation,tellsoff,replay FROM account WHERE id=%i",
		account_id));
	...
	dbaw->AddQuery(2, &query, MakeAnyLenString(&query,
		"SELECT id,profile,zonename,x,y,z,guild_id,rank,extprofile,class,level,lfp,lfg,instanceid,pvpflag "
		" FROM character_  LEFT JOIN guild_members ON id=char_id WHERE id=%i limit 1",
		character_id));
...
{
	int32 cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) Client %s charid %i in ReqClientSpawn.", cur_time, GetName(),CharacterID());
...
{
	int32 cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) Client %s charid %i in ReqNewZone.", cur_time, GetName(),CharacterID());
...
void Client::Handle_Connect_OP_SendExpZonein(const EQApplicationPacket *app)
{
	...
	if ( this->CastToClient()->GetEPP().isBecomeNPC )
		this->CastToClient()->SetBecomeNPC(true);
	...
	mlog(CLIENT__ERROR, "SendExpZonein Setting character PVP value to %d",GetPVPValue());
...
void Client::Handle_Connect_OP_WorldObjectsSent(const EQApplicationPacket *app)
{
	...
	LogFile->write(EQEMuLog::Status, "%s: Called WorldObjectsSent (Spawn Packet Sent).", GetName());
	CreateSpawnPacket(outapp,NULL,true);
	outapp->priority = 6;
	if (!GetHideMe()) entity_list.QueueClients(this, outapp, true);
	safe_delete(outapp);
	if(GetPVP())	//force a PVP update until we fix the spawn struct
	...
	if(IsInAGuild() && !zone->IsGuildManagementDisabled()) {
		SendGuildMembers();
		SendGuildURL();
		SendGuildChannel();
	}
...
void Client::Handle_Connect_OP_ClientReady(const EQApplicationPacket *app)
{
	...
	// Image April 2018 fixes clients not appearring cause packet was not sent
	//////////////////////////////////////////////////////
	// Spawn Appearance Packet
	/*EQApplicationPacket* outapp = new EQApplicationPacket(OP_SpawnAppearance, sizeof(SpawnAppearance_Struct));
	SpawnAppearance_Struct* sa = (SpawnAppearance_Struct*)outapp->pBuffer;
	...
	if (!GetHideMe())
	{
		...
		LogFile->write(EQEMuLog::Status, "%s: Called ClientReady (Spawn Packet Sent).", GetName());
		CreateSpawnPacket(outapp,NULL,true);
		outapp->priority = 6;
		entity_list.QueueClients(this, outapp, true);
		
		EQApplicationPacket* posapp = new EQApplicationPacket(OP_ClientUpdate, sizeof(PlayerPositionUpdateServer_Struct));
		PlayerPositionUpdateServer_Struct* spu = (PlayerPositionUpdateServer_Struct*)posapp->pBuffer;
		MakeSpawnUpdate(spu);
		float dist = RuleR(PVPCombat, ClientViewSpawnDistance);
		float dist2 = dist * dist; //pow(dist, 2);
		entity_list.QueueCloseClients(this,posapp,true,dist2);
	...
	{
		LogFile->write(EQEMuLog::Status, "%s: Called ClientReady (GMHidden NO Spawn Packet Sent).", GetName());
	}

	if(GetPVP())	//force a PVP update until we fix the spawn struct
		RefreshPVPFlag();

	LogFile->write(EQEMuLog::Status, "Setting character PVP value to %d",GetPVPValue());
	...
	int32 cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) Client %s charid %i Handle_Connect_OP_ClientReady received, moving to CompleteConnect.", cur_time, GetName(), CharacterID());
...
void Client::CheatDetected(CheatTypes CheatType, float x, float y, float z,
	float destx, float desty, float destz, int32 destzoneid, int32 zonereasonid, int32 targetzoneid, double naturalRunSpeed, double actualRunSpeed)
{ //[Paddy] ToDo: Break warp down for special zones. Some zones have special teleportation pads or bad .map files which can trigger the detector without a legit zone request.
	...
	if ( this->GetTarget() != NULL )
		strncpy(targetName,this->GetTarget()->GetName(),64);
	...
	case MQWarp: //Some zones have serious issues, turning off warp flags for these zones.
			...
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			...
			sprintf(targetInfo,"RewindLoc: %.2f, %.2f, %.2f, TriggerRewindWarp: %i, (If TriggerRewindWarp 1) OddLoc: %.2f, %.2f, %.2f, ProximityLoc: %.2f, %.2f, %.2f, MaxRunSpeed:  %9.2f, ActualRunSpeed:  %9.2f",destx, desty, destz, rewind_warp, odd_rewind_x, odd_rewind_y, odd_rewind_z, proximity_x, proximity_y, proximity_z,naturalRunSpeed,actualRunSpeed);

			char hString[250];
			sprintf(hString, "/MQWarp with location %.2f, %.2f, %.2f zoneid %i (%s)", GetX(), GetY(), GetZ(),GetZoneID(), targetInfo);
			
		char* query = 0;
		MakeAnyLenString(&query, 
			"Insert into eventlog (accountname,accountid,status,charname,target,descriptiontype,description,event_nid,charxpos,charypos,charzpos,charzonename,targxpos,targypos,targzpos) " 
			"values('%s',%i,%i,'%s','%s','%s','%s','%i','%f','%f','%f','%s','%f','%f','%f')", 
			this->account_name,this->AccountID(), 0, this->name, GetTarget() ? GetTarget()->GetName() : "N/A", "MQWarp", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		InsertQueryAsync(query);
		safe_delete_array(query);

			/*database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQWarp", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);*/
		}
		break;
	case MQBuffExploitRoot: //Some zones have serious issues, turning off warp flags for these zones.
	case MQBuffExploitSeeInvis:
	case MQBuffExploitBindSight:
	case MQBuffExploitTargetRange:
		if(RuleB(Zone, EnableMQWarpDetector) 
			&& ((this->Admin() < RuleI(Zone, MQWarpExemptStatus) 
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			char type[32];
			_snprintf(type,32,"MQBuffExploitRoot");
			switch(CheatType)
			{
					case MQBuffExploitSeeInvis:
					_snprintf(type,32,"MQBuffExploitSeeInvis");
					break;
					case MQBuffExploitBindSight:
					_snprintf(type,32,"MQBuffExploitBindSight");
					break;
					case MQBuffExploitTargetRange:
					_snprintf(type,32,"MQBuffExploitTargetRange");
					break;
			}
			...
			char hString[250];
			sprintf(hString, "/%s with location %.2f, %.2f, %.2f zoneid %i (%s)", type, GetX(), GetY(), GetZ(),GetZoneID(), targetInfo);
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, type, hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		}
		break;
	case MQBuffExploitStun: //Some zones have serious issues, turning off warp flags for these zones.
		if(RuleB(Zone, EnableMQWarpDetector) 
			&& ((this->Admin() < RuleI(Zone, MQWarpExemptStatus) 
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			char type[32];
			_snprintf(type,32,"MQBuffExploitStun");
				
			//Message(13, "Large warp detected.");
			char targetInfo[512];

			sprintf(targetInfo,"RewindLoc: %.2f, %.2f, %.2f, TriggerRewindWarp: %i, (If TriggerRewindWarp 1) OddLoc: %.2f, %.2f, %.2f, ProximityLoc: %.2f, %.2f, %.2f, MaxRunSpeed:  %9.2f, ActualRunSpeed:  %9.2f, StunTimeRemains: %i",destx, desty, destz, rewind_warp, odd_rewind_x, odd_rewind_y, odd_rewind_z, proximity_x, proximity_y, proximity_z,naturalRunSpeed,actualRunSpeed,stunned_timer.GetRemainingTime());
	...
	case MQWarpShadowStep:
			...
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			...
			char hString[250];
			sprintf(hString, "/MQWarp(SS) with location %.2f, %.2f, %.2f zoneid %i, the target was shadow step exempt but we still found this suspicious (%s).", GetX(), GetY(), GetZ(), GetZoneID(), targetInfo);

			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQWarpShadowStep", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		}
		break;
	case MQWarpKnockBack:
			...
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			...
			char targetInfo[256];
			sprintf(targetInfo,"RewindLoc: %.2f, %.2f, %.2f, TriggerRewindWarp: %i, (If TriggerRewindWarp 1) OddLoc: %.2f, %.2f, %.2f, ProximityLoc: %.2f, %.2f, %.2f, MaxRunSpeed:  %9.2f, ActualRunSpeed:  %9.2f",destx, desty, destz, rewind_warp, odd_rewind_x, odd_rewind_y, odd_rewind_z, proximity_x, proximity_y, proximity_z,naturalRunSpeed,actualRunSpeed);
			MakeAnyLenString(&hString, "/MQWarp(KB) with location %.2f, %.2f, %.2f zoneid %i, the target was Knock Back exempt but we still found this suspicious (%s).", GetX(), GetY(), GetZ(), GetZoneID(), targetInfo);
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQWarpKnockBack", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
	...
	case MQWarpLight:
			...
			|| (RuleI(Zone, MQWarpExemptStatus)) == -1)))
		{
			if(RuleB(Zone, MarkMQWarpLT))
			{
				...
				char targetInfo[256];
				sprintf(targetInfo,"RewindLoc: %.2f, %.2f, %.2f, TriggerRewindWarp: %i, (If TriggerRewindWarp 1) OddLoc: %.2f, %.2f, %.2f, ProximityLoc: %.2f, %.2f, %.2f, MaxRunSpeed:  %9.2f, ActualRunSpeed:  %9.2f",destx, desty, destz, rewind_warp, odd_rewind_x, odd_rewind_y, odd_rewind_z, proximity_x, proximity_y, proximity_z,naturalRunSpeed,actualRunSpeed);
				MakeAnyLenString(&hString, "/MQWarp(LT) with location %.2f, %.2f, %.2f zoneid %i, possibly: small warp, speed hack, excessive lag, confirm visually.  %s", GetX(), GetY(), GetZ(), GetZoneID(), targetInfo);
				database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQWarpWarning", hString, 15,GetX(),
					GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
				safe_delete_array(hString);
			}
		}
	...
	case MQZone:
		if((RuleB(Zone, EnableMQZoneDetector))&& ((this->Admin() < RuleI(Zone, MQZoneExemptStatus) || (RuleI(Zone, MQZoneExemptStatus)) == -1))) //Lieka:  Exempt these zones from the MQZone detector (This may be depricated now, but were problems in the past)
		{
			char hString[250];
			...
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQZone", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		}
		break;
	case MQZoneUnknownDest:
		...
		{
			char hString[250];
			sprintf(hString, "/MQZoneUnknown used at %.2f, %.2f, %.2f zoneid %i (Dest: %.2f, %.2f, %.2f zone %i)", GetX(), GetY(), GetZ(), GetZoneID(),destx,desty,destz,destzoneid);
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQZoneUnknownDest", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		}
		break;
	case MQGate:
		if (RuleB(Zone, EnableMQGateDetector)&& ((this->Admin() < RuleI(Zone, MQGateExemptStatus) || (RuleI(Zone, MQGateExemptStatus)) == -1))) {
			...
			// KaB / Red69 / Zek - Flag cheaters who use MQ Gate 9/19/2012
			if ( zonereasonid == 0 )
				ChangeLastName("!_I_AM_A_CHEATER_!");

			_snprintf(msg,256,"Bad Gate Request: zonereasonID: %i, TargetZoneID: %i (ZonePoint ZoneID: %i)",zonereasonid,targetzoneid, destzoneid);
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQGate", msg, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
			if(zone)
			{
	...
	case MQGhost: //Lieka:  Not currently implemented, but the framework is in place - just needs detection scenarios identified
		if (RuleB(Zone, EnableMQGhostDetector) && ((this->Admin() < RuleI(Zone, MQGhostExemptStatus) || (RuleI(Zone, MQGhostExemptStatus)) == -1))) {
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "MQGhost", "No data to present, more implementation needed.", 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		}
		break;
	default:
		char *hString = NULL;
		MakeAnyLenString(&hString, "Unhandled HackerDetection flag with location %.2f, %.2f, %.2f zone %i.", GetX(), GetY(), GetZ(), this->GetZoneID());
		database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "CheatDetect", hString, 15,GetX(),
			GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		safe_delete_array(hString);
		break;
	}

	CheatSaveBuffs(CheatType);
	//rewind_warp = false;
}
...
void Client::Handle_OP_ClientUpdate(const EQApplicationPacket *app)
{
	...
	if ( RuleB(Zone,PositionUpdateDebug) )
	{
		Mob* entity = entity_list.GetMob(ppu->spawn_id);
		int32 cur_time = Timer::GetCurrentTime();
		LogFile->write(EQEMuLog::Error, "[%i] %s (%i): OP_ClientUpdate X: %f, Y: %f, Z: %f.  SpawnID: %i TargetID: %s, animation %i",cur_time, GetName(), GetID(), 
			ppu->x_pos, ppu->y_pos, ppu->z_pos, ppu->spawn_id, entity ? entity->GetName() : "N/A", animation);
	}

	if(ppu->spawn_id != GetID()) {
		Mob* npc = entity_list.GetMob(ppu->spawn_id);
		if ( npc && npc->IsNPC() && npc->GetOwner() && npc->GetOwner()->GetID() == GetID() && npc->CastToNPC()->GetNPCTypeID() == RuleI(Zone,EyeOfZommNPCID) )
		{
			npc->SetDeltas(ppu->delta_x, ppu->delta_y, ppu->delta_z, ppu->delta_heading);
...
#endif
			safe_delete(outapp);
			npc->GMMove(ppu->x_pos, ppu->y_pos, ppu->z_pos, EQ19toFloat(ppu->heading), false);
		...
		else if ( ppu->spawn_id == 0 )
			ppu->spawn_id = GetID();
		else
			return; // reject packet
	}

	if ( ppu->delta_x == 0 && ppu->delta_y == 0 )
		moving = false;
	else
	{
		if ( ( IsRooted() || IsStunned() ) )
		{
			if ( !cheattrigger_timer.Enabled() )
			{
				cheattrigger_timer.Start(RuleI(CheatSystem,RootStunTriggerDelayMS), true);
			}
			else if ( cheattrigger_timer.Check() )
			{
				if ( IsRooted() )
					CheatDetected(MQBuffExploitRoot, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
				else if ( IsStunned() )
					CheatDetected(MQBuffExploitStun, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
				cheattrigger_timer.Disable();
			}
		}
		else if ( cheattrigger_timer.Enabled() && cheattrigger_timer.Check() )
			cheattrigger_timer.Disable();
	...
	double dist = 0.0;
	double tmp;
	tmp = (double)x_pos - (double)ppu->x_pos;
	dist += tmp*tmp;
	tmp = (double)y_pos - (double)ppu->y_pos;
	dist += tmp*tmp;
	dist = sqrt(dist);
	...
	if(dist == 0)
	{
		if(m_DistanceSinceLastPositionCheck > 0.0)
		{
			...
			if((cur_time - m_TimeSinceLastPositionCheck) > 0)
			{
				...
				{
					if(!GetGMSpeed() && (runs >= GetBaseRunspeed() || (speed > (GetBaseRunspeed() * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))))
					{
						if(IsShadowStepExempted())
						{
							if(m_DistanceSinceLastPositionCheck > 800)
							{
								CheatDetected(MQWarpShadowStep, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
							}
						}
						else if(IsKnockBackExempted())
						{
							...
							if(speed > 30.0f)
							{
								CheatDetected(MQWarpKnockBack, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
							}
						}
						else if(!IsPortExempted())
						{
							if(!IsMQExemptedArea(zone->GetZoneID(), ppu->x_pos, ppu->y_pos, ppu->z_pos))
							{
								if(speed > (runs * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))
								{
									...
									CheatDetected(MQWarp, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
									//Death(this, 10000000, SPELL_UNKNOWN, _1H_BLUNT);
									if ( RuleB(CheatSystem,DamageWarpingPlayers) )
										Damage(this,RuleI(CheatSystem,DamageWarpingAmount),0,_1H_SLASHING,0,false);
								}
								else
								{
									CheatDetected(MQWarpLight, ppu->x_pos, ppu->y_pos, ppu->z_pos, rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
								}
							}
						}
					}
				...
				m_TimeSinceLastPositionCheck = cur_time;
				m_DistanceSinceLastPositionCheck = 0.0;
	...
	float tmpheading = 0.0f;
	if ( !FCMP(ppu->y_pos, y_pos) || !FCMP(ppu->x_pos, x_pos) || ( (tmpheading = EQ19toFloat(ppu->heading)) && !FCMP(tmpheading, heading) ) || ppu->animation != animation || 
		forceposupdate_timer.Check() )
	{
		x_pos			= ppu->x_pos;
		y_pos			= ppu->y_pos;
		z_pos			= ppu->z_pos;
	...
	if(zone->watermap)
	{
		if(zone->watermap->InLava(x_pos, y_pos, z_pos) || zone->watermap->InWater(x_pos, y_pos, z_pos))
		{
...
void Client::Handle_OP_TargetCommand(const EQApplicationPacket *app)
{
	...
	if (app->GetOpcode() == OP_TargetCommand) {
		...
		if (GetTarget()) {
			...
			if ( origTarget != NULL && origTarget == GetTarget() )
			{
				GetTarget()->IsTargeted(1);
				return;
			}

			// Kings & Bandits OP_Target Line of Sight check. 7/21 - added target distance check back
			// 8-4 - fixed IsInvisible against mob
			if( !GetGM() && !GetTarget()->IsCorpse() && ( 
				(!CheckLosFN(GetTarget()) || (DistNoRoot(*GetTarget()) > TARGETING_RANGE*TARGETING_RANGE)) 
				|| (GetTarget()->GetBodyType() == BT_NoTarget2 || GetTarget()->GetBodyType() == BT_Special 
				|| GetTarget()->GetBodyType() == BT_NoTarget) || GetTarget()->IsInvisible(this) )  )
			{

				if ( GetTarget()->IsInvisible(this) )
					CheatDetected(MQBuffExploitSeeInvis, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
				else
					CheatDetected(MQBuffExploitTargetRange, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
			...
			QueuePacket(app);
			// Kings & Bandits ptr mem leak fix
			EQApplicationPacket* hp_app = new EQApplicationPacket();
			GetTarget()->IsTargeted(1);
			GetTarget()->CreateHPPacket(hp_app);
			FastQueuePacket(&hp_app, false);
		} 
		else 
		{
	...
	else
	{
		if(GetTarget())
		{
			...
			// Kings & Bandits OP_Target Line of Sight check. 7/21 - added target distance check back
			// 8-4 - fixed IsInvisible against mob
			if( !GetTarget()->IsCorpse() && ( (GetTarget()->GetBodyType() == BT_NoTarget2 || GetTarget()->GetBodyType() == BT_Special 
				|| GetTarget()->GetBodyType() == BT_NoTarget) || GetTarget()->IsInvisible(this) )  )
			{
			...
			else if(GetBindSightTarget())
			{
				if(GetBindSightTarget()->DistNoRoot(*GetTarget()) > (zone->newzone_data.maxclip*zone->newzone_data.maxclip))
				{
					...
					{
						CheatDetected(MQBuffExploitBindSight, GetX(), GetY(), GetZ(), rewind_x, rewind_y, rewind_z, 0, 0, 0, GetRunspeed(), RecordedSpeed);
						char *hacker_str = NULL;
						MakeAnyLenString(&hacker_str, "%s (BINDTARGET %s) attempting to target something beyond the clip plane of %.2f units,"
							" from (%.2f, %.2f, %.2f) to %s (%.2f, %.2f, %.2f)", GetName(), GetBindSightTarget()->GetName(),
...
void Client::Handle_OP_AdventureMerchantRequest(const EQApplicationPacket *app)
{
	...
	if ( tmp->CastToNPC()->MerchantMinLevel > 0 && GetLevel() < tmp->CastToNPC()->MerchantMinLevel)
	{
		Message(13,"You are too low level to use this merchant.");
		SendEmptyMerchantResponse();
		return;
	}
	...
	//printf("(Before)MerchList size: %i\n",merlist.size());
	if ( merlist.size() == 0 )
	{		
		zone->LoadNewMerchantData(merchantid);
		merlist = zone->merchanttable[merchantid];
	}
	//printf("(After)MerchList size: %i\n",merlist.size());

		const Item_Struct *item = 0;
		std::list<MerchantList>::const_iterator itr;
	// Thurgadin offers a PVP incentive if you own it.  Might want to put this somewhere else at a later time if cities offered this more freely
	// Thurgadin PVP Incentive Added 5/5/2013 for KaB/Red69/Zek
	TeamSettings_Struct* team = database.GetTeamSettings(THURGADINTEAMID);
		for(itr = merlist.begin();itr != merlist.end() && count<255;itr++){
			const MerchantList &ml = *itr;
			item = database.GetItem(ml.item);
			if(item)
			{
				sint32 curPrice = (sint32)item->LDoNPrice;

				if ( team != NULL && team->guildid > 0 && 
					team->guildid == GuildID() && curPrice > 1 )
				{
					curPrice -= 1;
				}

				float reduction = RuleR(PVPCombat,PVPMerchantDiscount);
				if ( reduction != 0.0f )
				{
					sint32 newPrice = (sint32)((float)curPrice - (float)curPrice*reduction);
					if ( newPrice < 1 )
						curPrice = 1;
					else
						curPrice = newPrice;
	...
	//	printf("String to send: %s\n",ss.str().c_str());
		...
		EQApplicationPacket* outapp = new EQApplicationPacket(OP_AdventureMerchantResponse,ss.str().length()+2);
		outapp->pBuffer[0] = count;
		strn0cpy((char*)&outapp->pBuffer[1],ss.str().c_str(),ss.str().length());
		FastQueuePacket(&outapp);
		//DumpPacket(outapp);
		// we cannot cache these since the npc's have diff values depending on the player
		//zone->merchantbuffer[merchantid] = outapp;
}
...
void Client::Handle_OP_AdventureMerchantPurchase(const EQApplicationPacket *app)
{
	...
	*/
	...
	case DISCORD_MERCHANT:
		{
			if ( aps->Type != DiscordMerchant )
				cheat = true;
			break;
		}
	case NORRATHS_KEEPERS_MERCHANT:
		{
			if ( aps->Type != NorrathsKeepersMerchant )
				cheat = true;
			break;
		}
	case DARK_REIGN_MERCHANT:
		{
			if ( aps->Type != DarkReignMerchant )
				cheat = true;
			break;
		}
	}
	...
	if ( cheat )
	{
		Message(13,"Stop trying to cheat jerk.");
		return;
	}
	...
	if ( RuleB(PVPCombat, DisablePVPItemsByReqLevel) && item->ReqLevel > GetLevel() )
	{
		Message(13,"You are too low level to purchase this item (%s)!  Requires level %i.", item->Name, item->ReqLevel);
		return;
	}
	...
	case LDoNMerchant:
		{		
			...
			if(m_pp.ldon_points_available < sint32(curPrice)) {
			...
			if(item->LDoNTheme <= 16)
			{
				if(item->LDoNTheme & 16)
				{
					if(m_pp.ldon_points_tak < sint32(curPrice))
					{
				...
				{
					if(m_pp.ldon_points_ruj < sint32(curPrice))
					{
				...
				{
					if(m_pp.ldon_points_mmc < sint32(curPrice))
					{
				...
				{
					if(m_pp.ldon_points_mir < sint32(curPrice))
					{
				...
				{
					if(m_pp.ldon_points_guk < sint32(curPrice))
					{
	...
	if(aps->Type == LDoNMerchant)
	{
		sint32 requiredpts = (sint32)curPrice*-1;
	...
	{
		sint32 newPoints = (sint32)GetPVPPoints() - (sint32)curPrice;
		if ( newPoints < 0 )
		{
			Message(13,"Bad bad boy!!");
			return;
		}
		else
		{
			SetPVPPoints((uint32)newPoints);
			//SendPVPStats();
		}
	}
	else if(aps->Type == NorrathsKeepersMerchant)
	{
		SetRadiantCrystals(GetRadiantCrystals() - (sint32)curPrice);
		SendCrystalCounts();
	}
	else if(aps->Type == DarkReignMerchant)
	{
		SetEbonCrystals(GetEbonCrystals() - (sint32)curPrice);
		SendCrystalCounts();
	}
	sint8 charges = 1;
	...
	LogLDONMerchant(this, tmp, 1, curPrice, inst->GetItem(), true);

	if ( RuleI(World,NoTransferZoneExemption) > 0 && RuleI(World,NoTransferZoneExemption) == zone->GetZoneID() )
		inst->SetInstNoTransfer(false);
...
void Client::Handle_OP_Consider(const EQApplicationPacket *app)
{
	...
	if(tmob->IsNPC())
	{
		con->faction = GetFactionLevel(character_id, tmob->GetNPCTypeID(), race, class_, deity,(tmob->IsNPC()) ? tmob->CastToNPC()->GetPrimaryFaction():0, tmob); // rembrant, Dec. 20, 2001; TODO: Send the players proper deity
			...
			//handle sneaking
		if(sneaking) {
		if ( BehindMob(tmob, GetX(), GetY(), 45.0f ) ) // block 270 degrees of view (they can see 90 degrees, so 45*2)
	...
	// server specific looks for a 4 lvl range
	if(tmob->IsClient() && tmob->CastToClient()->GetPVPValue() > 0 && GetPVPValue() > 0) {
		//con->pvpcon = 2;
		sendPack = false;
		
		char kindly[] = { "kindly considers you" };
		char scowls[] = { "scowls at you" };
		char notworthy[] = { "this would not make a worthy opponent" };
		char worthy[] = { "would make quite a formidable opponent" };
		char wipe[] = { "would stomp you" };
		bool isAttackAllowed = false;
		if ( tmob->CastToClient()->GetPVPValue() != GetPVPValue() || GetPVPValue() == 5 || tmob->CastToClient()->GetPVPValue() == 5 || zone->CanPVPValue() == PVPFFAVALUE 
			|| zone->CanPVPValue() == PVPFFANOLEVELVALUE || zone->CanPVPValue() == PVPALLVALUE )
		{
			isAttackAllowed = true;
		}

			sint16 diff = tmob->GetLevel() - GetLevel();
			switch(zone->CanPVPValue())
			{
			case PVPFFANOLEVELVALUE:
			case PVPALLVALUE:
			case PVPTEAMNOLEVELVALUE:
				{
					diff = -4; // everyone gets a 'white' con because they are all allowed to attack each other
					break;
				}
			}
			if ( diff > 4 )
			{
				if ( isAttackAllowed )
					Message_StringID(13, 12239, tmob->GetName(), scowls, wipe);
				else
					Message_StringID(13, 12239, tmob->GetName(), kindly, wipe);
			}
				//con->level = CON_RED;
			else if ( diff > -5 )
			{
				if ( isAttackAllowed )
					Message_StringID(7, 12239, tmob->GetName(), scowls, worthy);
				else
					Message_StringID(7, 12239, tmob->GetName(), kindly, worthy);
			}
				//con->level = CON_WHITE;
			else
			{
				if ( isAttackAllowed )
					Message_StringID(2, 12239, tmob->GetName(), scowls, notworthy);
				else
					Message_StringID(2, 12239, tmob->GetName(), kindly, notworthy);
			}
				//con->level = CON_GREEN;
	}
	else
...
void Client::Handle_OP_Surname(const EQApplicationPacket *app)
{
	...
	char surName[32];
	int surNameLength = sanitizeCharArray(surname->lastname, (char*)surName, 32);
	...
	if(!database.CheckNameFilter(surName, true))
	{
		Message_StringID(15, SURNAME_REJECTED);
		return;
	}

	ChangeLastName(surName);
	p_timers.Start(pTimerSurnameChange, 604800);

	EQApplicationPacket* outapp = app->Copy();
	outapp = app->Copy();
	surname = (Surname_Struct*) outapp->pBuffer;
	strncpy(surname->lastname,surName,32);
	strncpy(surname->name,GetName(),64);
...
{
	EQApplicationPacket* outapp = new EQApplicationPacket(OP_YellForHelp,0);
...
void Client::Handle_OP_Assist(const EQApplicationPacket *app)
{
	...
	bool skipAssist = true;
	if(entity && entity->IsMob())
	{
		Mob *assistee = entity->CastToMob();
		if( !GetGM() && !entity->CastToMob()->IsCorpse() && ( 
				(!CheckLosFN(assistee) || (DistNoRoot(*assistee) > TARGETING_RANGE*TARGETING_RANGE)) 
				|| (assistee->GetBodyType() == BT_NoTarget2 || assistee->GetBodyType() == BT_Special 
				|| assistee->GetBodyType() == BT_NoTarget) || assistee->IsInvisible(this) )  )
		{
				skipAssist = false;
		}
		
		float targetDist = 0.0f;
		if(assistee->GetTarget() && skipAssist)
		{
			...
			if
				...
				(GetGM() || 
				( (Dist(*assistee) <= TARGETING_RANGE && (targetDist = Dist(*new_target)) <= TARGETING_RANGE) && ( (targetDist < TARGETING_RANGE/3) || CheckLosFN(new_target) ) )
				)
				)
			{
	...
	if ( !skipAssist )
	{
				if ( GetTarget() != NULL )
				{
					eid->entity_id = GetTarget()->GetID();
				}
				else
				{
					safe_delete(outapp);
					outapp = new EQApplicationPacket(OP_TargetReject, sizeof(TargetReject_Struct));
					outapp->pBuffer[0] = 0x2f;
					outapp->pBuffer[1] = 0x01;
					outapp->pBuffer[4] = 0x0d;
					if(GetTarget())
...
void Client::Handle_OP_DuelResponse(const EQApplicationPacket *app)
{
	...
	{
		LogFile->write(EQEMuLog::Debug, "Size mismatch in OP_DuelResponse expected %i got %i", sizeof(DuelResponse_Struct), app->size);
		return;
	}
	...
	if(GetID() == initiator->GetID())
		entity->CastToClient()->Message_StringID(10,DUEL_DECLINE,initiator->CastToClient()->GetName());
	else
		initiator->CastToClient()->Message_StringID(10,DUEL_DECLINE,entity->CastToClient()->GetName());

	// Kings & Bandits - send back to the initiator that the duel was declined.
	ds->unknown = 0;
	ds->entity_id = initiator->GetID();
	ds->target_id = GetID();
	initiator->CastToClient()->QueuePacket(app);

	LogFile->write(EQEMuLog::Debug, "Duel declined between %s and %s (initiator).", entity->GetName(),initiator->GetName());
...
void Client::Handle_OP_DuelResponse2(const EQApplicationPacket *app)
{
	...
	Mob* entity = entity_list.GetMobID(ds->duel_target);
	Mob* initiator = entity_list.GetMobID(ds->duel_initiator);

	if (entity && initiator && entity == this && initiator->IsClient() && entity->IsClient()) {
		if ( ds->duel_target != GetID() || GetDuelTarget() != ds->duel_initiator || initiator->CastToClient()->GetID() != ds->duel_initiator || 
			initiator->CastToClient()->GetDuelTarget() != ds->duel_target )
		{
			Message(13,"Error duel could not be started with %s",initiator->GetName());
			initiator->CastToClient()->Message(13,"%s attempted to start a duel with you but it was rejected.",GetName());

			SetDuelTarget(0);
			SetDueling(false);
			initiator->CastToClient()->SetDuelTarget(0);
			initiator->CastToClient()->SetDueling(false);
			return; // Kings & Bandits - duel issue, person is sending a fake ID
		}

		EQApplicationPacket* outapp = new EQApplicationPacket(OP_RequestDuel, sizeof(OP_DuelResponse2));
...
void Client::Handle_OP_SpawnAppearance(const EQApplicationPacket *app)
{
	...
	if (sa->type == AT_Invis) {
		if(sa->parameter != 0)
		{
			/* KaB / Red69 / Zek - Fix for hackers database flag - this is sent for invis vs undead (parameter of 3)
			...
			}*/ // This is sent by invis vs undead, just ignore it..
			return;
		}
		invisible = false;
		BreakMobHide();
...
{
	LogFile->write(EQEMuLog::Status, "%s sent a logout packet.", GetName());
	//we will save when we get destroyed soon anyhow
	//Save();
	// Kings & Bandits - avoid insta camp!
	if ( dead || instalog || zoning || GetGM() )
	{
		isDespawning = true;
		SendLogout();
		Disconnect();
...
void Client::Handle_OP_FeignDeath(const EQApplicationPacket *app)
{
	...
	if (MakeRandomInt(0, 160) > totalfeign) {
		SetFeigned(false);
...
void Client::Handle_OP_Hide(const EQApplicationPacket *app)
{
	...
	// KaB - Red69 - Zek / new hide class/race/level checks
	bool legalRaceClass = false;
	switch(GetBaseClass())
	{
	case ROGUE:
		{
				if ( GetLevel() > 2 ) // lvl 3 they get it
					legalRaceClass = true;
			break;
		}
	case BARD:
	case RANGER:
		{
				if ( GetLevel() > 24 ) // lvl 25 they get it
					legalRaceClass = true;
			break;
		}
	case SHADOWKNIGHT:
		{
				if ( GetLevel() > 34 ) // lvl 35 they get it
					legalRaceClass = true;
			break;
		}
	}
	...
	if ( !legalRaceClass && Admin() < minStatusToBeGM )
	{
		// they should not be sending hide packets
	char targetName[64];
	memset(targetName,0,64);
	if ( this->GetTarget() != NULL )
		strncpy(targetName,this->GetTarget()->GetName(),64);
	...
	if ( GetTarget() )
	{
		...
		targZ = GetTarget()->GetZ();
			...
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "OPHide", "Improper Race or Class Combination for Hide", 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
		return;
	}
	...
	if ( IsCasting() )
	{
		...
		if ( GetTarget() )
		{
			...
			database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "OPHideCast", "Attempting to Hide while Casting", 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
	...
	if (random < hidechance) {
		...
		entity_list.QueueClients(this, outapp, true);
		safe_delete(outapp);
		if(GetAA(aaShroudofStealth)){ // 2039 is shroud of stealth
...
void Client::Handle_OP_ChannelMessage(const EQApplicationPacket *app)
{
	...
	// Image(2014): language skill fix
	if ( cm->language >= MAX_PP_LANGUAGE )
	{
		cm->language = 0; // default to common tongue
		cm->skill_in_language = m_pp.languages[0];
	}
	else
	{
		if ( m_pp.languages[cm->language] < 1 ) // they have no skill in this language, so they can't speak it
		{
			cm->language = 0; // default to common tongue
			cm->skill_in_language = m_pp.languages[0];
		}
		...
		{
			cm->skill_in_language = m_pp.languages[cm->language];
		}
	}

	ChannelMessageReceived(cm->chan_num, cm->language, cm->skill_in_language, msg, targetName);
	return;
}
...
void Client::Handle_OP_GMZoneRequest(const EQApplicationPacket *app)
{
	...
	// Kings & Bandits - zone command isn't working, last field is ignore, set to 3 so we go to safe coords.
	DoZoneSuccess(NULL,gmzr->zone_id,gmzr->zone_id,tarx,tary,tarz,0.0f,3);
	return;
}
...
void Client::Handle_OP_EndLootRequest(const EQApplicationPacket *app)
{
	...
	uint16 id = GetLootingEntityID();

	SetLooting(0);

	if ( id < 1 ) // we don't care if its not a valid entity id
		return;

	Entity* entity = entity_list.GetID(id);
...
void Client::Handle_OP_GuildDelete(const EQApplicationPacket *app)
{
	...
	if(!IsInAGuild() || !guild_mgr.IsGuildLeader(GuildID(), CharacterID()))
		...
		if ( zone->IsGuildManagementDisabled() || KingsBandits::GuildOwnsCity(GuildID()) )
		{
			Message(13,"Guild may not be deleted at this time as your guild owns a city or you are currently in a contestable city.");
			return;
		}
...
void Client::Handle_OP_GuildInvite(const EQApplicationPacket *app)
{
	...
	if ( zone->IsGuildManagementDisabled () )
	{
		Message(13,"Guild management is disabled.");
		return;
	}
	...
	if (!IsInAGuild())
		...
		if(invitee->IsClient()) {
			...
			{
				if ( GetPVPValue() > 0 && client->GetPVPValue() > 0 &&
					client->GetPVPValue() < 5 && GetPVPValue() < 5 && GetPVPValue() != client->GetPVPValue() )
				{
					Message(13, "Must be on the same team or a discord player to guild invite.");
					return;
				}
			}
...
void Client::Handle_OP_GuildRemove(const EQApplicationPacket *app)
{
	...
	// we can always remove ourself, otherwise, our rank needs remove permissions
		...
		if(char_id > 0 && 
			KingsBandits::PlayerOwnsCity(char_id) )
		{
			Message(0, "This player owns a city and cannot be removed from the guild at this time.");
			return;
		}

		if (!GuildRemoveAllowed(char_id))
			return;

		LogFile->write(EQEMuLog::Status, "%s: Removing from guild charid %i, guild %i.", GetName(), char_id, GuildID());
	...
	else	// ability, or regular memmed spell
	{
		...
		if(castspell->slot == ABILITY_SPELL_SLOT &&
			castspell->spell_id == SPELL_LAY_ON_HANDS && GetClass() == PALADIN) {
				...
				if ( CastSpell(spell_to_cast, castspell->target_id, castspell->slot) )
		...
		else if ( castspell->slot >= MAX_PP_MEMSPELL || (castspell->slot == (MAX_PP_MEMSPELL-1) && !GetAA(aaMnemonicRetention)) ) // Image (Red69/LoZ/KaB): Added to avoid exploiting CastSpell code
		{
				InterruptSpell(castspell->spell_id);
				char hString[320];
				char targetName[64];
				...
				if ( this->GetTarget() != NULL )
					strncpy(targetName,this->GetTarget()->GetName(),64);
				char spellName[64];
				_snprintf(spellName,64,"Unknown");
				if ( castspell->spell_id  > 0 && castspell->spell_id < SPDAT_RECORDS )
					_snprintf(spellName,64,"%s",spells[castspell->spell_id].name);
				...
				sprintf(hString, "Illegal SpellCast (SpellID: %i (%s), SlotID: %i) with location %.2f, %.2f, %.2f zoneid %i", castspell->spell_id, spellName, castspell->slot
					, GetX(), GetY(), GetZ(),GetZoneID());
				database.logevents(this->account_name,this->AccountID(), 0, this->name, targetName, "SpellCast", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true),targX,targY,targZ);
				return;
		}
		...
		if(castspell->slot < MAX_PP_MEMSPELL)
		{
			...
			if(GetLevel() < spells[castspell->spell_id].classes[GetClass()-1])
			{
				char val1[20]={0};
				Message_StringID(13,SPELL_LEVEL_TO_LOW,ConvertArray(spells[castspell->spell_id].classes[GetClass()-1],val1),spells[castspell->spell_id].name);
				InterruptSpell(castspell->spell_id);
				return;
			}
		}
...
void Client::Handle_OP_DeleteItem(const EQApplicationPacket *app)
{
	...
	if (inst && inst->GetItem()->ItemType == ItemTypeAlcohol) {
		...
		int AlcoholTolerance = GetSkill(ALCOHOL_TOLERANCE);
		int IntoxicationIncrease;

		if(CastToClient()->GetClientVersion() < EQClientSoD)
...
void Client::Handle_OP_RezzAnswer(const EQApplicationPacket *app)
{
	...
	// client is accepting rez
	if ( ra->action == 1 )
	{
		QGlobal res;
		// they are in combat, fuck those guys
		if ( GetAttackTarget() != NULL || this->GetAggroCount() > 0 )
		{
			ra->action = 0;
			Message(13,"You are currently in combat and are unable to accept a resurrection at this time.");
		}
...
void Client::Handle_OP_TradeRequest(const EQApplicationPacket *app)
{
	...
	if (tradee && tradee->IsClient()) {
			QGlobal res;
			if ( QGlobalCache::GetQGlobal(res,tradee->CastToClient()->GetQGlobals()->GetBucket(),"notradeallowed",0,tradee->CastToClient()->CharacterID(),zone->GetZoneID()) ) {
				tradee->CastToClient()->Message(13, "You are unable to trade with %s at this time due to a quest flag.",GetName());
				Message(13,"%s is unable to trade at this time due to a quest flag.",tradee->GetName());
			}
			else if ( QGlobalCache::GetQGlobal(res,CastToClient()->GetQGlobals()->GetBucket(),"notradeallowed",0,CastToClient()->CharacterID(),zone->GetZoneID()) ) {
				Message(13,"You are unable to trade at this time due to a quest flag.");
			}
			else
...
void Client::Handle_OP_TradeRequestAck(const EQApplicationPacket *app)
{
	...
	// Kings & Bandits - bad someone is trying to exploit tradereqack
	if ( msg->from_mob_id != this->GetID() )
		return;
...
void Client::Handle_OP_BoardBoat(const EQApplicationPacket *app)
{
	...
	if ( boat && 
		boat->GetBaseRace() == 114 || // ghost ship
		boat->GetBaseRace() == 404 || // discord ship
		boat->GetBaseRace() == 546 || // barrel barge ship
		boat->GetBaseRace() == 550 || // merchant ship
		boat->GetBaseRace() == 551 || // pirate ship
		boat->GetBaseRace() == 552 || // ghost ship
		boat->GetBaseRace() == 72 || // old ship
		boat->GetBaseRace() == 141 || // ship
		boat->GetBaseRace() == 73 ) // skiff
	{
...
void Client::Handle_OP_Buff(const EQApplicationPacket *app)
{
	...
	if(spid == 0xFFFF || (IsValidSpell(spid) && (spells[spid].goodEffect == 0)) )
	{
		...
		if ( RuleB(CheatSystem,ReportBuffStatus) )
		{
			char hString[250];
			if ( !IsBuffToFade(spid) )
				sprintf(hString, "OP_Buff removal attempt of debuff for spellid (BAD: 2+ Tics left!): %i", spid);
			else
				sprintf(hString, "OP_Buff removal attempt of debuff for spellid (OK <2 tics): %i", spid);

				database.logevents(this->account_name,this->AccountID(), 0, this->name, GetTarget() != NULL ? GetTarget()->GetName() : "", "OP_Buff", hString, 15,GetX(),
...
void Client::Handle_OP_GMLastName(const EQApplicationPacket *app)
{
	...
	if (strlen(lastName) >= 64) {
		...
		Client* client = entity_list.GetClientByName(firstName);
		if (client == 0) {
			Message(13, "/LastName: %s not found", firstName);
		}
		else {
			if (this->Admin() < minStatusToUseGMCommands) {
				Message(13, "Your account has been reported for hacking.");
...
void Client::Handle_OP_ShopPlayerBuy(const EQApplicationPacket *app)
{
	...
	if (!item){
		//this does not work so i commented it out
		Message(15,"%s tells you 'Sorry, that item is for display purposes only.'",tmp->GetCleanName());
	...
	if(tmpmer_used && prevcharges > -1 && ((sint32)mp->quantity > prevcharges))
	{
		if ( prevcharges == 0 )
			mp->quantity = 1;
		else
	...
	sint16 freeslotid=0;
	sint16 chargesToGive = (sint16)mp->quantity;
	if ( !tmpmer_used )
	{
		if ( item->MaxCharges == -1 || item->MaxCharges > 1 )
			chargesToGive = item->MaxCharges;
	}

	ItemInst* inst = database.CreateItem(item, chargesToGive);
	uint32 taxPrice = 0;
	int SinglePrice = 0;
	if (RuleB(Merchant, UsePriceMod))
	{
		// Kings & Bandits city tax mod
		taxPrice = item->Price * Client::CalcCityMod(tmp,false);
		SinglePrice = taxPrice + (item->Price * (RuleR(Merchant, SellCostMod)) * item->SellRate * Client::CalcPriceMod(tmp, false));	
	}
	else
	...
	if ( taxPrice > 0 )
	{
		AddCityRevenue(taxPrice * mp->quantity);
	}

	QueuePacket(outapp);
	if(inst && tmpmer_used){
		sint32 new_charges = 0;
		sint32 real_charges = 0;
		if ( prevcharges < 0 )
		{
			new_charges = -1;
			real_charges = 1;
		...
		{
			new_charges = (sint32)chargesToGive*-1;
			real_charges = prevcharges + new_charges;
			if ( real_charges <= 0 )
			{
				real_charges = 0;
			}
		...
		zone->SaveTempItem(merchantid, tmp->GetNPCTypeID(),item_id,new_charges,true);
		if(real_charges<=0){
			EQApplicationPacket* delitempacket = new EQApplicationPacket(OP_ShopDelItem, sizeof(Merchant_DelItem_Struct));
			Merchant_DelItem_Struct* delitem = (Merchant_DelItem_Struct*)delitempacket->pBuffer;
			delitem->itemslot = mp->itemslot;
			...
			inst->SetCharges(real_charges);
			inst->SetPrice(SinglePrice);
			inst->SetMerchantSlot(mp->itemslot);
			inst->SetMerchantCount(real_charges);
...
void Client::Handle_OP_ShopPlayerSell(const EQApplicationPacket *app)
{
	...
	if((freeslot = zone->SaveTempItem(vendor->CastToNPC()->MerchantType, vendor->GetNPCTypeID(),itemid,saveCharges,true)) > 0){
		...
		sint32 MerchantQuantity = zone->GetTempMerchantQuantity(vendor->GetNPCTypeID(), freeslot);
...
void Client::Handle_OP_ClickObject(const EQApplicationPacket *app)
{
	...
	if ( entity && entity->IsObject() )
	{
		float dist = TARGETING_RANGE*TARGETING_RANGE;
		Object* object = entity->CastToObject();
		if (object->IsGroundSpawn () )
			dist = GROUND_CLICK_RANGE*GROUND_CLICK_RANGE;
		if ( (DistNoRootNoZ(object->GetX(),object->GetY()) > dist) )
		{
			char logMsg[256];
			_snprintf(logMsg,256,"Player Attempted Picked Up of item, too far away (%s) (maxdist: %f)",entity->GetName(),dist);
		...
		ClickObject_Struct* clickobj = (ClickObject_Struct*) outapp->pBuffer;
		// Image: we leave the object id to 0 so as to not make the current entity id remove itself
		clickobj->player_id = GetID();
		QueuePacket(outapp);
		safe_delete(outapp);
		}
		else {
...
void Client::Handle_OP_GroupInvite2(const EQApplicationPacket *app)
{
	...
	int inviteeLength = sanitizeCharArray(gis->invitee_name, (char*)invitee_name, 64);
	int inviterLength = sanitizeCharArray(gis->inviter_name, (char*)inviter_name, 64);
	
	Mob *Invitee = entity_list.GetMob(invitee_name);
	...
	if(Invitee) {
		if(Invitee->IsClient()) {
			QGlobal res;
			if ( QGlobalCache::GetQGlobal(res,Invitee->CastToClient()->GetQGlobals()->GetBucket(),"nogroupallowedflag",0,Invitee->CastToClient()->CharacterID(),zone->GetZoneID()) ) {
				Invitee->CastToClient()->Message(13, "You are unable to join a group at this time due to a quest flag.");
				Message(13,"%s is unable to group at this time due to a quest flag.",Invitee->GetName());
				return;
			}
			else if ( QGlobalCache::GetQGlobal(res,GetQGlobals()->GetBucket(),"nogroupallowedflag",0,CharacterID(),zone->GetZoneID()) ) {
				Message(13, "You are unable to join a group at this time due to a quest flag.");
				Invitee->CastToClient()->Message(13,"%s is unable to group at this time due to a quest flag.",GetName());
				return;
			}
			...
			if ( pvpFlag == SERVERTYPE_ZEK )
			{
				if ( GetPVPValue() > 0 && Invitee->CastToClient()->GetPVPValue() > 0 &&
					Invitee->CastToClient()->GetPVPValue() < 5 && GetPVPValue() < 5 && GetPVPValue() != Invitee->CastToClient()->GetPVPValue() )
				{
					Message(13, "Must be on the same team or a discord player to group up.");
					return;
				}
			}
			if(!Invitee->IsGrouped() && !Invitee->IsRaidGrouped()) {
					...
					GroupInvite_Struct* grpinv = (GroupInvite_Struct*) outapp->pBuffer;
					strncpy(grpinv->invitee_name, invitee_name, inviteeLength);
					strncpy(grpinv->inviter_name, inviter_name, inviterLength);
...
void Client::Handle_OP_GroupCancelInvite(const EQApplicationPacket *app)
{
	...
	char name1[64];
	char name2[64];
	int name1Length = sanitizeCharArray(gf->name1, (char*)name1, 64);
	int name2Length = sanitizeCharArray(gf->name2, (char*)name2, 64);
	
	Mob* inviter = entity_list.GetClientByName(name1);
	...
	if(inviter != NULL && inviter->IsClient())
	{
		...
		GroupCancel_Struct* gc = (GroupCancel_Struct*) outapp->pBuffer;
		strncpy(gc->name1, name1, name1Length);
		strncpy(gc->name2, name2, name2Length);
		gc->toggle = gf->toggle;
...
void Client::Handle_OP_GroupFollow2(const EQApplicationPacket *app)
{
	...
	if(inviter != NULL && inviter->IsClient()) {
			QGlobal res;
			if ( QGlobalCache::GetQGlobal(res,inviter->CastToClient()->GetQGlobals()->GetBucket(),"nogroupallowedflag",0,inviter->CastToClient()->CharacterID(),zone->GetZoneID()) ) {
				inviter->CastToClient()->Message(13, "You are unable to join a group at this time due to a quest flag.");
				Message(13,"%s is unable to group at this time due to a quest flag.",inviter->GetName());
				return;
			}
			else if ( QGlobalCache::GetQGlobal(res,GetQGlobals()->GetBucket(),"nogroupallowedflag",0,CharacterID(),zone->GetZoneID()) ) {
				Message(13, "You are unable to join a group at this time due to a quest flag.");
				inviter->CastToClient()->Message(13,"%s is unable to group at this time due to a quest flag.",GetName());
				return;
			}
			...
			if ( pvpFlag == SERVERTYPE_ZEK )
			{
				if ( GetPVPValue() > 0 && inviter->CastToClient()->GetPVPValue() > 0 &&
					inviter->CastToClient()->GetPVPValue() < 5 && GetPVPValue() < 5 && GetPVPValue() != inviter->CastToClient()->GetPVPValue() )
				{
					Message(13, "Must be on the same team or a discord player to group up.");
					return;
				}
		...
		{
			// update members prior to the invite
			group->LearnMembers();
		}
...
#endif
	if((group->IsLeader(this) && (GetTarget() == 0 || GetTarget() == this)) || (group->LearnMembers() && group->GroupCount()<3)) {
		group->DisbandGroup();
	} else {
		Mob* memberToDisband = NULL;
		memberToDisband = GetTarget();
		...
		if(memberToDisband ){
			if(group->IsLeader(this)) // the group leader can kick other members out of the group...
				...
				group->DelMember(this,false, false);
		}
		else
...
void Client::Handle_OP_InspectAnswer(const EQApplicationPacket *app)
{
	...
	char comment[288];
	int commentLength = sanitizeCharArray(insr->text, (char*)comment, 288);
	
	memset(insr->text,0,sizeof(insr->text));
	strncpy(insr->text,comment,commentLength);
...
void Client::Handle_OP_Bug(const EQApplicationPacket *app)
{
	if(app->size!=sizeof(BugStruct))
		...
		worldserver.SendEmoteMessage(0, 0, 80, 15, "%s has made a new bug report.", GetName());
...
void Client::Handle_OP_Petition(const EQApplicationPacket *app)
{
	...
	{
		int32 petitionID = petition_list.GetPetitionIDByAccountName(AccountName());
		if(petitionID > 0)
		{
			// well it exists, lets see if it really does, time for everyone to update!
			...
			if ( newPetID > 0 )
			{
				...
				worldserver.SendEmoteMessage(0, 0, 80, 15, "%s has made a petition update. Petition id: #%i", GetName(), newPetID);
				return;
			}
		}
...
void Client::Handle_OP_PetitionDelete(const EQApplicationPacket *app)
{
	...
	if (petition_list.DeletePetitionByCharName(GetName()) == -1)
...
void Client::Handle_OP_PetCommands(const EQApplicationPacket *app)
{
	...
	if(( mypet->GetPetType() == petFamiliar || mypet->GetNPCTypeID() == RuleI(Zone,EyeOfZommNPCID)) && pet->command != PET_GETLOST)
		return;
	...
	case PET_ATTACK: {
		...
		if ( mypet->IsClient() && ( ( GetTarget()->IsNPC() && GetTarget()->GetOwner() == NULL ) || (GetTarget()->GetOwner() != NULL && GetTarget()->GetOwner()->IsNPC())) )
		{
			Message(13,"Your pet can only attack other players.");
			break;
		}

		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 2) || mypet->GetPetType() != petAnimation) {
			if (GetTarget() != this) {
				if ( RuleB(Pets,OwnerAttackLogic) )
				{
					float distFromOwner = DistNoRootNoZ(*GetTarget());
					float maxRangeDistFromOwner = RuleR(Pets, AttackCommandRange)*RuleR(Pets, AttackCommandRange);
					
					float zoneCustomDistance = zone->GetNoLOSPetDistance();
					if ( zoneCustomDistance > 0.0f )
						maxRangeDistFromOwner = zoneCustomDistance*zoneCustomDistance;
					else if ( zoneCustomDistance < 0.0f )
						maxRangeDistFromOwner = -1.0f;

					bool losCheck = CheckLosFN(GetTarget());
					float maxRangeDistFromOwnerWithLOS = RuleR(Pets, AttackCommandRangeWithLOS)*RuleR(Pets, AttackCommandRangeWithLOS);
					
					if ( maxRangeDistFromOwner == -1.0f && RuleB(Pets,AttackLogicClientOnly) && 
						( (GetTarget()->GetOwner() == NULL && !GetTarget()->IsClient()) || 
						  (GetTarget()->GetOwner() != NULL && !GetTarget()->GetOwner()->IsClient()) )
						  )
					{
						maxRangeDistFromOwner = RuleR(Pets, AttackCommandRange)*RuleR(Pets, AttackCommandRange);
					}
					else if ( maxRangeDistFromOwner == -1.0f && !losCheck )
					{
						Message(13,"This zone requires line of sight for your pet to attack.");
						break;
					}

					if ( ! (( losCheck && distFromOwner <= maxRangeDistFromOwnerWithLOS ) || 
						( maxRangeDistFromOwner > 0.0f && distFromOwner <= maxRangeDistFromOwner )) )
					{
						Message(13,"You are too far away from your target to command your pet to attack.");
						break;
					}
				}
				else
				{
					float distFromPet = mypet->DistNoRootNoZ(*GetTarget());
					float maxRangeDistFromPet = RuleR(Pets, AttackCommandRange)*RuleR(Pets, AttackCommandRange);
					if ( distFromPet > maxRangeDistFromPet )
					{
						Message(13,"You are too far away from your target to command your pet to attack.");
						break;
					}
				...
				// Kings & Bandits - PETATTACK (PET_ATTACK) redesigned to add the target as the highest hate, this will prevent
				// pets from attacking the same target.
				if ( mypet->IsNPC() )
					mypet->AddToHateList(GetTarget(), mypet->CastToNPC()->GetTopHateValue(this)+1000);
				else
				{
					// For charmed clients we will just use wiping a hate list and adding a new target.
					mypet->WipeHateList();
					mypet->AddToHateList(GetTarget(),1000);
				}

				// break invis of pet and owner
				BreakMobInvis();
				BreakMobHide();
	...
	case PET_BACKOFF: {
		if (GetTarget() != NULL && GetTarget()->IsMezzed()) {
			Message_StringID(10, CANNOT_WAKE, mypet->GetCleanName(), GetTarget()->GetCleanName());
			break;
		}
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 3) || mypet->GetPetType() != petAnimation) {
			// KaB - Red69 - Zek changed pet calming to send to owner only
			Message_StringID(10, PET_CALMING);
			mypet->WipeHateList();
		}
		break;
					  }
	...
	case PET_GETLOST: {
		...
		// KaB - Red69 - Zek changed pet get lost to send to owner only
		Message_StringID(10,PET_GETLOST_STRING);
	...
	case PET_GUARDHERE: {
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 1) || mypet->GetPetType() != petAnimation) {
			if(mypet->IsNPC()) {
				mypet->SetHeld(false);
				// KaB - Red69 - Zek changed pet guard to send to owner only
				Message_StringID(10,PET_GUARDINGLIFE);
	...
	case PET_FOLLOWME: {
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 1) || mypet->GetPetType() != petAnimation) {
			mypet->SetHeld(false);
			// KaB - Red69 - Zek changed pet follow to send to owner only
			Message_StringID(10,PET_FOLLOWING);
			mypet->SetPetOrder(SPO_Follow);
	...
	case PET_GUARDME: {
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 1) || mypet->GetPetType() != petAnimation) {
			mypet->SetHeld(false);
			// KaB - Red69 - Zek changed pet guard me to send to owner only
			Message_StringID(10,PET_GUARDME_STRING);
			mypet->SetPetOrder(SPO_Follow);
			mypet->SendAppearancePacket(AT_Anim, ANIM_STAND);
		}
		break;
	...
	case PET_SITDOWN: {		
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 3) || mypet->GetPetType() != petAnimation) {
			// KaB - Red69 - Zek changed pet sit to send to owner only
			Message_StringID(10,PET_SIT_STRING);
	...
	case PET_STANDUP: {
		...
		if((mypet->GetPetType() == petAnimation && GetAA(aaAnimationEmpathy) >= 3) || mypet->GetPetType() != petAnimation) {
			// KaB - Red69 - Zek changed pet stand up to send to owner only
			Message_StringID(10,PET_SIT_STRING);
			mypet->SetPetOrder(SPO_Follow);
			mypet->SendAppearancePacket(AT_Anim, ANIM_STAND);
		}
	...
	case PET_SLUMBER: {
		...
		if(mypet->GetPetType() != petAnimation) {
			// KaB - Red69 - Zek changed pet slumber to send to owner only
			Message_StringID(10,PET_SIT_STRING);
			mypet->SetPetOrder(SPO_Sit);
			mypet->SetRunAnimSpeed(0);
			if(!mypet->UseBardSpellLogic())	// solar: maybe we can have a bard pet
...
void Client::Handle_OP_GMBecomeNPC(const EQApplicationPacket *app)
{
	...
	cli->SendAppearancePacket(AT_NPCName, 2, true);
...
void Client::Handle_OP_Mend(const EQApplicationPacket *app)
{
	...
	int currenthp = GetHP();
	int curMendSkill = (int)GetSkill(MEND);

	// KaB / Red69 - Zek - Modification for Monk Mend to be more useful after 100 skill
	if ( ( curMendSkill > 99 && MakeRandomInt(0,curMendSkill+5) < curMendSkill ) || 
		( MakeRandomInt(0, 105) < curMendSkill ) ) {
...
void Client::Handle_OP_EnvDamage(const EQApplicationPacket *app)
{
	...
	if ( IsAIControlled() )
	{
		Message(13, "Uncontrolled status protects you from %i points of type %i environmental damage.", ed->damage, ed->dmgtype);
		SetHP(GetHP()-1);//needed or else the client wont acknowledge
		return;
	}
...
void Client::Handle_OP_Damage(const EQApplicationPacket *app)
{
	...
	if ( RuleB(Zone, RestrictDamagePackets) )
	{
		if ( damage->source != GetID() || (damage->target != 0 && damage->target != GetID() ) )
		{
			LogFile->write(EQEMuLog::Error, "Unexpected CombatDamage_Struct received.");
			DumpPacket(app);
			return;
		}
	}
...
void Client::Handle_OP_Trader(const EQApplicationPacket *app)
{
	...
	if(app->size==sizeof(Trader_ShowItems_Struct)){ //Show Items
		...
		case BazaarTrader_EndTraderMode: {
			// Kings & Bandits - allows bazaar traders with a flip of the /trader UI.
			// Mar 18 2016 - not needed anymore, stalls have been setup through doors
			/*if ( !this->Trader )
				Trader_StartTrader();
			else*/
				Trader_EndTrader();
		...
		if(ints->Code==BazaarTrader_StartTraderMode){
			...
			for(int i=0;i<80;i++){
				if(gis->Items[i]>0 && database.GetItem(gis->Items[i])!=0)
...
void Client::Handle_OP_PickPocket(const EQApplicationPacket *app)
{
	...
	// per http://everquest.allakhazam.com/db/skills.html?skill=31
	// CancelSneakHide checks if hidden / improved_hidden and then pulls off both hide/sneak.
	CancelSneakHide();
	
	if(DistNoRoot(*victim) > RuleI(Combat, MinRangedAttackDist)*RuleI(Combat, MinRangedAttackDist))
	{
		Message(0,"Target is too far away.");
		SendPickPocketResponse(this,0,0);
		return;
	}
	...
	if (victim == this){
		...
		SendPickPocketResponse(victim,0,0);
	}
	else if (victim->IsNPC()){
		victim->CastToNPC()->PickPocket(this);
	}
	else if ( victim->IsClient() && IsAttackAllowed(victim) && !victim->IsCharmed() )
	{
		if(MakeRandomInt(0, 100) > 95){
			victim->CastToClient()->Message(13,"%s attempts to steal from you!!!", GetName());
			Message(13, "You are noticed trying to steal!");
			SendPickPocketResponse(victim,0,0);
		}
		else if ( !RuleB(PVPCombat,AllowItemSteal) && !RuleB(PVPCombat,AllowMoneySteal) )
		{
			Message(13, "PVP Pick pocket is currently disabled.");
			SendPickPocketResponse(victim,0,0);
		}
		else
		{
			
			int steal_skill = GetSkill(PICK_POCKETS);
			int stealchance = steal_skill*RuleI(PVPCombat,ChanceStealMultiplier)/(5*GetLevel()+5);
			int money[4];
			money[0] = victim->CastToClient()->GetPlatinum();
			money[1] = victim->CastToClient()->GetGold();
			money[2] = victim->CastToClient()->GetSilver();
			money[3] = victim->CastToClient()->GetCopper();
			if (steal_skill < 125)
				money[0] = 0;
			if (steal_skill < 60)
				money[1] = 0;
			bool no_coin = ((money[0] + money[1] + money[2] + money[3]) == 0);
			bool steal_item = (MakeRandomInt(0, 99) < RuleI(PVPCombat,ChanceStealItem));
			
			if ( !RuleB(PVPCombat,AllowItemSteal) )
				steal_item = false;
			
			if ( !RuleB(PVPCombat,AllowMoneySteal) )
				no_coin = true;

			if ( steal_item )
			{
				uint32 itemID = RandomItemSteal(victim->CastToClient(), this);
				if ( itemID > 0 )
				{
					const Item_Struct* item = database.GetItem(itemID);
					SendPickPocketResponse(victim, 0, PickPocketItem, item);
				}
			}
			if ( !no_coin )
			{
				...
				if (MakeRandomInt(0, 100) <= stealchance)
				{
					...
					{
					case 0:{
						if (amt > victim->CastToClient()->GetPlatinum())
							amt = victim->CastToClient()->GetPlatinum();

						victim->CastToClient()->GetPP().platinum -= amt;
						victim->CastToClient()->SendMoneyUpdate();
						AddMoneyToPP(0,0,0,amt,false);
						SendPickPocketResponse(victim, amt, PickPocketPlatinum);
						break;
						   }
					case 1:{
						if (amt > victim->CastToClient()->GetGold())
							amt = victim->CastToClient()->GetGold();

						victim->CastToClient()->GetPP().gold -= amt;
						victim->CastToClient()->SendMoneyUpdate();
						AddMoneyToPP(0,0,amt,0,false);
						SendPickPocketResponse(victim, amt, PickPocketGold);
						break;
						   }
					case 2:{
						if (amt > victim->CastToClient()->GetSilver())
							amt = victim->CastToClient()->GetSilver();

						victim->CastToClient()->GetPP().silver -= amt;
						victim->CastToClient()->SendMoneyUpdate();
						AddMoneyToPP(0,amt,0,0,false);
						SendPickPocketResponse(victim, amt, PickPocketSilver);
						break;
						   }
					case 3:{
						if (amt > victim->CastToClient()->GetCopper())
							amt = victim->CastToClient()->GetCopper();

						victim->CastToClient()->GetPP().copper -= amt;
						victim->CastToClient()->SendMoneyUpdate();
						AddMoneyToPP(amt,0,0,0,false);
						SendPickPocketResponse(victim, amt, PickPocketCopper);
						break;
						   }
					}
				} // end steal chance of coin
				else
				{		// failed no coin
						SendPickPocketResponse(victim, 0, 0);
				}
...
void Client::Handle_OP_Disarm(const EQApplicationPacket *app)
{
	...
	if ( !RuleB(Zone, AllowDisarm) )
	{
		Message(13,"Disarm is disabled in this zone.");
		return;
	}
		
	if(!p_timers.Expired(&database, pTimerDisarm, false)) {
		Message(13,"Ability recovery time not yet met.");
		return;
	}
	p_timers.Start(pTimerDisarm, DisarmReuseTime);
	
	DisarmAbility_Struct* da_atk = (DisarmAbility_Struct*) app->pBuffer;

	if ( da_atk->m_source != GetID() )
		return;

	Mob* target = entity_list.GetMob(da_atk->m_target);

	if ( target == NULL || target->IsCorpse() || !IsAttackAllowed(target) )
		return;
	
	if ( !CombatRange(target) )
	{
		Message(MT_Skills,"%s is not in range!", target->GetCleanName());
		return;
	}

	int16 disarmSkill = GetSkill(DISARM);
	sint8 levelDiff = GetLevel() - target->GetLevel();
	sint32 divideBy = RuleI(Combat,DisarmDivision);
	if ( divideBy == 0 )
		divideBy = 1;

	sint32 chanceToDisarm = 1 + disarmSkill/divideBy + (GetDEX()/divideBy) + levelDiff;
	bool success = false;

	if ( target->IsNPC() )
	{
		int32 primaryItem = target->CastToNPC()->GetEquipment(MATERIAL_PRIMARY);
		int32 secondaryItem = target->CastToNPC()->GetEquipment(MATERIAL_SECONDARY);
		if ( primaryItem == 0 && secondaryItem == 0 )
		{
			Message(MT_Skills,"%s has no item to disarm!", target->GetCleanName());
			return;
		}
		else
		{
			if ( MakeRandomInt(0,100) < chanceToDisarm )
			{
				if ( primaryItem )
				{
					const Item_Struct* item = database.GetItem(primaryItem);
					if ( item && !item->Magic )
					{
						target->CastToNPC()->RemoveItem(primaryItem);
						ItemInst* inst = database.CreateItem(primaryItem);
						Object* object = new Object(inst, target->CastToNPC()->GetX(),target->CastToNPC()->GetY(),target->CastToNPC()->GetZ(),target->CastToNPC()->GetHeading(),300000);						
						entity_list.AddObject(object, true);
						object->Save(); // KaB / Red69 / Zek - Saves to DB the item(s)
						safe_delete(inst);
					}
					target->CastToNPC()->SetEquipment(MATERIAL_PRIMARY, 0);
					target->CastToNPC()->SetPrimSkill(HAND_TO_HAND);
					target->CastToNPC()->SendWearChange(MATERIAL_PRIMARY);
				}
				else if ( secondaryItem )
				{
					const Item_Struct* item = database.GetItem(secondaryItem);
					if ( item && !item->Magic )
					{
						target->CastToNPC()->RemoveItem(secondaryItem);
						ItemInst* inst = database.CreateItem(secondaryItem);
						Object* object = new Object(inst, target->CastToNPC()->GetX(),target->CastToNPC()->GetY(),target->CastToNPC()->GetZ(),target->CastToNPC()->GetHeading(),300000);
						entity_list.AddObject(object, true);
						object->Save(); // KaB / Red69 / Zek - Saves to DB the item(s)
						safe_delete(inst);
					}
					target->CastToNPC()->SetEquipment(MATERIAL_SECONDARY, 0);
					target->CastToNPC()->SetSecSkill(HAND_TO_HAND);
					target->CastToNPC()->SendWearChange(MATERIAL_SECONDARY);
				}
				success = true;
				Message(MT_Skills,"%s has been disarmed!", target->GetCleanName());
			}
			else
				Message(MT_Skills,"Attempt to disarm %s has failed!", target->GetCleanName());

			CheckIncreaseSkill(DISARM, target, 5);
			
			if ( !GetGM() )
				target->CastToNPC()->AddToHateList(this, 1);
		}
	}
	else if ( target->IsClient() )
	{
		if ( !RuleB(PVPCombat, AllowDisarmPVP) )
		{
			Message(13,"PVP Disarm is disabled in this zone.");
			return;
		}

		ItemInst* primary = target->CastToClient()->GetInv().GetItem(SLOT_PRIMARY);
		ItemInst* secondary = target->CastToClient()->GetInv().GetItem(SLOT_SECONDARY);
			if ( primary == 0 && secondary == 0 )
			{
				Message(MT_Skills,"%s has no item to disarm!", target->GetCleanName());
				return;
			}
			else if ( (MakeRandomInt(0,100) < chanceToDisarm) || GetGM() )
			{
				if ( primary )
				{
					int32 itmID = primary->GetItem()->ID;
					sint32 charges = primary->GetCharges();
					uint32 augSlot1 = primary->GetAugmentItemID(0);
					uint32 augSlot2 = primary->GetAugmentItemID(1);
					uint32 augSlot3 = primary->GetAugmentItemID(2);
					uint32 augSlot4 = primary->GetAugmentItemID(3);
					uint32 augSlot5 = primary->GetAugmentItemID(4);

					target->CastToClient()->DeleteItemInInventory(SLOT_PRIMARY,0,true);
					target->CastToClient()->PutItemInOpenSlot(itmID, charges, augSlot1, augSlot2, augSlot3, augSlot4, augSlot5 );
				}
				else if ( secondary )
				{
					int32 itmID = secondary->GetItem()->ID;
					sint32 charges = secondary->GetCharges();
					uint32 augSlot1 = secondary->GetAugmentItemID(0);
					uint32 augSlot2 = secondary->GetAugmentItemID(1);
					uint32 augSlot3 = secondary->GetAugmentItemID(2);
					uint32 augSlot4 = secondary->GetAugmentItemID(3);
					uint32 augSlot5 = secondary->GetAugmentItemID(4);
					target->CastToClient()->DeleteItemInInventory(SLOT_SECONDARY,0,true);
					target->CastToClient()->PutItemInOpenSlot(itmID, charges, augSlot1, augSlot2, augSlot3, augSlot4, augSlot5 );
				}
				success = true;
				Message(MT_Skills,"%s has been disarmed!", target->GetCleanName());
				target->CastToClient()->Message(MT_Skills,"%s has disarmed you!", GetCleanName());
			}
			else
			{
				Message(MT_Skills,"Attempt to disarm %s has failed!", target->GetCleanName());
				target->CastToClient()->Message(MT_Skills,"%s attempted to disarm you!", GetCleanName());
			}
...
void Client::DBAWComplete(int8 workpt_b1, DBAsyncWork* dbaw) {
	...
	case DBA_b1_Entity_Client_MessageQueue:
		{
			
			AsyncLoadMsgQueue(dbaw);
			break;
		}
	case DBA_b1_Entity_Client_LevelUpdate:
		{
			
			AsyncLoadCharacterLevelUpdate(dbaw);
			break;
		}
	case DBA_b1_Entity_Client_AsyncInsert:
		{
			AsyncHandleAsyncInsert(dbaw);
			break;
		}
	default: {
...
bool Client::FinishConnState2(DBAsyncWork* dbaw) {
	...
	for (i=1; i<=3; i++) {
		...
		if (dbaq->QPT() == 1) {
			database.GetAccountInfoForLogin_result(result, 0, account_name, &lsaccountid, &gmspeed, &revoked, &gmhideme, &account_creation, &tellsoff, &ReplayEnabled);
			if(gmhideme)
			{
				trackable = false;
			}
		...
		#ifdef ZONE
		// KaB/Red69/Zek - 5/5/2013 Image - adding transfer of bank coin
		// load bank settings for player and backup their plat in memory temporarily
		KingsBandits::LoadCityBankCoin(&m_pp,&m_inv,CharacterID(),zone->GetZoneID());
		#endif
	...
	int32 cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) Client %s charid %i PlayerProfileLoaded.", cur_time, m_pp.name,CharacterID());
	...
	if ( GetLevel() == 1 && (RuleB(Character,AutoAssignSpells) && GetLevel() <= RuleI(Character,AutoAssignSpellMaxLevel)) )
		LoadCharacterLevelUpdates(1);
	...
	if ( overrideRed && m_pp.pvp != BANDITTEAMID && Admin() < minStatusToBeGM )
	{
		m_pp.pvp = RuleI(PVPCombat,PVPDefaultTeam);
	}
	else
	{
	// ZEK: Race PVP Flag Setting
	if ( !RuleB(PVPCombat,TeamGuildWars) && pvpFlag == SERVERTYPE_ZEK && // only ZEK server
		Admin() < minStatusToBeGM && // doesn't apply to GM's
		m_pp.pvp != BANDITTEAMID ) // don't need to overwrite team 5 (pvp ffa).  They are already PVP!
	{
		switch(GetBaseRace())
		{
		case ERUDITE:
	...
	*/
	...
	//validate adventure points, this cap is arbitrary 0x7FFFFFFF
		...
		for (i = 0; i < buff_count; i++) {
			for(uint32 z = 0; z < buff_count; z++) {
				...
				if(buffs[z].spellid != SPELL_UNKNOWN && buffs[z].spellid == m_pp.buffs[i].spellid) {
					memset(&buffs[z],0,sizeof(Buffs_Struct));
					memset(&m_pp.buffs[i],0,sizeof(SpellBuff_Struct));
					buffs[z].spellid = SPELL_UNKNOWN;
					m_pp.buffs[i].spellid = SPELLBOOK_UNKNOWN;
					memset(&m_epp.buffs[i],0,sizeof(Buffs_Struct));
					m_epp.buffs[i].spellid = SPELLBOOK_UNKNOWN;
				}
			}

			// bard spells will re-apply if the bard is around

			// KaB/Red69/Zek - Sep 14 2012 removing !IsBardSong check.
			if (IsValidSpell(m_pp.buffs[i].spellid) && m_pp.buffs[i].duration > 0) {
				...
				buffs[i].casterid		= 0;
				buffs[i].player_id		= m_pp.buffs[i].player_id;
				buffs[i].player_class		= m_epp.buffs[i].player_class;
				buffs[i].durationformula	= spells[buffs[i].spellid].buffdurationformula;
				buffs[i].poisoncounters		= m_epp.buffs[i].poisoncounters; //CalculatePoisonCounters(m_pp.buffs[i].spellid);
				buffs[i].diseasecounters	= m_epp.buffs[i].diseasecounters; //CalculateDiseaseCounters(m_pp.buffs[i].spellid);
				buffs[i].cursecounters		= m_epp.buffs[i].cursecounters; //CalculateCurseCounters(m_pp.buffs[i].spellid);
				buffs[i].corruptioncounters	= m_epp.buffs[i].corruptioncounters; //CalculateCorruptionCounters(m_pp.buffs[i].spellid);
				buffs[i].numhits		= spells[buffs[i].spellid].numhits;
				buffs[i].persistant_buff	= m_pp.buffs[i].persistant_buff;

				if ( RuleB(PVPCombat,UpdateClientBuffZoning) && buffs[i].player_id > 0 && buffs[i].player_id != MAX_PLAYERID && IsDetrimentalSpell(buffs[i].spellid) )
					buffs[i].UpdateClient		= true;
				else
					buffs[i].UpdateClient		= false;

				buffs[i].dropOnCombat = IsEffectInSpell(m_pp.buffs[i].spellid,SE_NegateIfCombat);

				buffs[i].CountersReady = true;
				...
				memset(&buffs[i],0,sizeof(Buffs_Struct));
				memset(&m_pp.buffs[i],0,sizeof(SpellBuff_Struct));
			...
			if (buffs[j1].spellid <= (int32)SPDAT_RECORDS) {
				for (uint32 x1=0; x1 < EFFECT_COUNT; x1++) {
					...
					case SE_Charm:
						...
						m_pp.buffs[j1].player_id = 0;
						m_pp.buffs[j1].effect = 0;


						memset(&m_epp.buffs[j1],0,sizeof(Buffs_Struct));
						m_epp.buffs[j1].spellid = SPELLBOOK_UNKNOWN;
						x1 = EFFECT_COUNT;
						break;
					case SE_Illusion:
						if(m_pp.buffs[j1].persistant_buff != 1){ //anything other than 1=non persistant
							memset(&buffs[j1],0,sizeof(Buffs_Struct));
							memset(&m_pp.buffs[j1],0,sizeof(SpellBuff_Struct));
							buffs[j1].spellid = SPELL_UNKNOWN;
							m_pp.buffs[j1].spellid = SPELLBOOK_UNKNOWN;
	...
	// Kings & Bandits Achievement Load
	LoadAchievements();
	...
	if(groupid > 0){
		...
		if(!group) {	//nobody from our is here... start a new group
			...
			LogFile->write(EQEMuLog::Status, "%s: Instantiating new group with id %i.", GetName(), groupid);
			group = new Group(groupid);
			if(group->GetID() != 0)
			{
				LogFile->write(EQEMuLog::Status, "%s: group created with id %i.", GetName(), groupid);
				entity_list.AddGroup(group, groupid);
			}
			else	//error loading group members...
			{
				LogFile->write(EQEMuLog::Status, "%s: failed group creation with id %i.", GetName(), groupid);
				delete group;
				group = NULL;
			}
		...
		if(!group)
		{
			LogFile->write(EQEMuLog::Status, "%s: failed to find group with id %i.", GetName(), groupid);

			for(uint32 xy=0;xy < MAX_GROUP_MEMBERS;xy++)
		...
		LogFile->write(EQEMuLog::Status, "%s: no group id exists.", GetName());
	...
	if(group){
		...
		bool update = group->UpdatePlayer(this);
		LogFile->write(EQEMuLog::Status, "%s: update group of player returns %i for group id %i.", GetName(), update, group->GetID());
		LFG = false;
	}
	...
	if(zone->IsPVPZone() && !m_pp.pvp)
		m_pp.pvp=1;

	// temporary hold pvp flag to give client flag on or off
	uint8 pvpflag = m_pp.pvp;

	if ( m_pp.pvp > 0 )
		m_pp.pvp = 1;

	m_pp.timeentitledonaccount = 0;
	...
	uint16 origRace = m_pp.race;
	uint8 origClass = m_pp.class_;
	uint8 origGender = m_pp.gender;

	if ( m_epp.isBecomeNPC )
	{
		m_pp.race = m_epp.npcRace;
		m_pp.class_ = m_epp.npcClass;
		m_pp.gender = m_epp.npcGender;
	}
	...
	// restore pvp flag to normal value
	m_pp.pvp = pvpflag;

	if ( m_epp.isBecomeNPC )
	{
		m_pp.race = origRace;
		m_pp.class_ = origClass;
		m_pp.gender = origGender;
	}
	...
	CheckInfamyLevel(); // check what our infamy state is
	...
	////////////////////////////////////////////////////////////
	// Tribute Packets
	if ( RuleB(TributeSystem,EnableTributeSystem) )
	{
		DoTributeUpdate();
	...
	LoadExtProcs((ExtendedProfile_Procs*)m_epp.PermaProcs, (ExtendedProfile_Procs*)m_epp.SpellProcs);
	// Kings & Bandits - MakePet cross zone.
	// If server is Red69 (1) then we do not want pets cross zone.
	// all other servers it will work
	// moved location to lower in the code
	/*if (m_epp.pet_id > 1 && !GetPet() && m_epp.pet_id <= SPDAT_RECORDS)
	{
		...
		if (GetPet() && GetPet()->IsNPC()) {
			...
			pet->SetPetState(m_epp.pet_buffs, m_epp.pet_items, (ExtendedProfile_Procs*)m_epp.PetPermaProcs, (ExtendedProfile_Procs*)m_epp.PetSpellProcs);
			pet->CalcBonuses();
	...
	cur_time = Timer::GetCurrentTime();
	LogFile->write(EQEMuLog::Status, "(%i) Client %s charid %i moving to ZoneInfoSent.", cur_time, GetName(),CharacterID());
...
void Client::CompleteConnect()
{
	...
	// Kings & Bandits - guild check and force team
	if(pvpFlag == SERVERTYPE_KAB && IsInAGuild() && GetPVPValue()>0)
	{
		bool guildExists = guild_mgr.GuildExists(GuildID());
		if ( guildExists ) 
		{
			uint32 leaderCharID = guild_mgr.GetGuildLeaderID(GuildID());
			uint32 teamID = database.TeamPVPFlag(leaderCharID);
			if ( teamID > 0 )
			{
				if ( m_pp.pvp != teamID )
					Message(13,"You were not on the same team as your guild leader and were forced to change!");
	...
	// Kings & Bandits - zone-in notice for PVP zone status to player
	switch ( zone->CanPVPValue ( ) )
	{
	case NOPVPVALUE:{ 
		Message(4,"**This zone is PVP neutral.");
		break;
					}
	case PVPTEAMVALUE:{ 
		...
		if ( pvpFlag == SERVERTYPE_KAB ) // pvp flag for KaB only
			Message(13,"**This zone is PVP enabled (teams).");
		break;
					  }
	case PVPTEAMNOLEVELVALUE:
		{
			Message(13,"**This zone is PVP enabled (teams) with no level restriction.");
			break;
		}
	case PVPFFANOLEVELVALUE:
		{
			Message(13,"**This zone is PVP enabled (ffa) with no level restriction.");
			break;
		}
	case PVPALLVALUE:
		{
			Message(13,"**This zone is PVP enabled (ffa) with no level restriction including non-pvp players.");
			break;
		}
	case PVPFFAVALUE:{ 
		Message(13,"**This zone is PVP enabled (ffa).");
		break;
					 }
	case DISABLEACTIONVALUE: {
		Message(13,"**This zone is disabled for PVP and all spell casting.");
		break;
							 }
	case NEUTRALPVPVALUE:{ 
		if ( GetLevel() > 39 )
			Message(13,"**This zone is PVP enabled (teams).");
		else
			Message(4,"**This zone is PVP neutral below level 40.");
		break;
						 }
	}

	switch(zone->IsHotzone() )
	{
	case 1: // yes
		{
			Message(13,"This zone is currently a hotzone!  There is a %f percent EXP Bonus!",RuleR(Zone, HotZoneBonus)*100.0f);
			break;
		}
	}

	
	switch(zone->IsPVPHotzone() )
	{
	case 0: // no
		{
			break;
	...
	default: // > 0 yes
		{
			int chanceToDrop = (int) (((float)zone->IsPVPHotzone()/100.0f) * ((float)GetLevel()/((float)RuleI(PVPCombat, PCChanceLevelCap)+1.0f))*100.0f);
			Message(13,"This zone is currently a **PVP** hotzone!  The hot zone offers a %i percent increase in PVP points.",chanceToDrop);
			break;
		}
	}
	...
	if ( !IsOfficialClient() && Admin() < 80 )
	{
			for(int i=0;i<2;i++){
			Message(13,"**WARNING!! You are not running the official client, read the 'How to Play' section on eqpvp.com for more information.");
			Message(13,"**You may only advance only to level %i until the official client is installed.",(int)(RuleI(World,MinLevelCheckBypass) - 1));
			}
	}
	...
	database.UpdateCharacterStats(CharacterID(), GetPP().STR, GetPP().STA, GetPP().DEX, GetPP().AGI, GetPP().INT, GetPP().WIS, GetPP().CHA, 
		GetPP().cur_hp,GetMaxHP(), GetPP().mana, GetMaxMana(), GetPP().endurance, GetPP().endurance, GetPP().timePlayedMin, infamyLevel, bountySet, 1);

	// Initiate replay if necessary
	
	if ( ReplayEnabled )
	{
		ReplayID = database.CreateCharacterReplay(CharacterID(),GetPP().zone_id);
		FrameID = 0;
		int replayValue = RuleI(Replay,SnapshotDelay);
		if ( replayValue < 5 )
			replayValue = 5;
		ReplayTimer.Start(replayValue*1000,true);
	}

	clientinzone_timer.Start(RuleI(Character,ZoneInTimer), true);
}
...
void Client::Handle_OP_LeadershipExpToggle(const EQApplicationPacket *app) {
		...
		return; // whoever coded this UGH just look at Handle_OP_DoGroupLeadershipAbility
	if(app->size != 1) {
...
void Client::Handle_OP_SetTitle(const EQApplicationPacket *app)
{
	...
	// Kings & Bandits, GetPrefix and GetSuffix got an additional Client attribute to cover the check to see if its ok for the client
	// to use this particular title.  So titles.cpp and titles.h have additional changes.
	if(!sts->is_suffix)
	{
		Title = title_manager.GetPrefix(sts->title_id, this);
		SetAATitle(Title.c_str());
	}
	else
	{
		Title = title_manager.GetSuffix(sts->title_id, this);
...
void Client::Handle_OP_Bandolier(const EQApplicationPacket *app) {
	...
	if ( bs->number > MAX_PLAYER_BANDOLIER-1 )
		return;
...
void Client::Handle_OP_DoGroupLeadershipAbility(const EQApplicationPacket *app) {
	...
	return; // Image (2017): NO BAD CODE WHO THE FUCK WROTE THIS SHIT!! NO CHECKS ON POINTS OR HAVING IT?>!?!
...
void Client::Handle_OP_ApplyPoison(const EQApplicationPacket *app) {
	...
	else if ( IsPoisonSpellProcPresent() )
	{
		mlog(SPELLS__CASTING_ERR, "There is already a poison proc applied to the weapon.");
		Message(0, "You may only apply one poison to the weapon.");
	}
	else if(GetClass() == ROGUE)
	{
			...
			(SecondaryWeapon && SecondaryWeapon->GetItem()->ItemType == ItemTypePierce))
		{
			...
			if(ChanceRoll < SuccessChance) {
				...
				// poisons should always proc for rogues on the first attempt
				AddProcToWeapon(PoisonItemInstance->GetItem()->Proc.Effect, false, 100, true);
			}
...
void Client::Handle_OP_PVPLeaderBoardRequest(const EQApplicationPacket *app)
{
	...
	switch(pvplbrs->SortType)
	{
	case 0: // sort by kills
		{
			// Kings & Bandits - pvp leaderboard stuff - copied from memshare
			memcpy(&pvplb->Entries,database.GetPVPLeaderBoardEntriesByKills(),sizeof(PVPLeaderBoardEntry_Struct)*100);
			break;
		}
	case 1: // sort by points
		{
			// Kings & Bandits - pvp leaderboard stuff - copied from memshare
			memcpy(&pvplb->Entries,database.GetPVPLeaderBoardEntriesByPoints(),sizeof(PVPLeaderBoardEntry_Struct)*100);
			break;
		}
	case 2: // infamy
		{
			memcpy(&pvplb->Entries,database.GetPVPLeaderBoardEntriesByInfamy(),sizeof(PVPLeaderBoardEntry_Struct)*100);
			break;
		}
	}
	pvplb->MyDeaths = m_pp.PVPDeaths;
	pvplb->MyKills = m_pp.PVPKills;
	pvplb->MyRank = 999;
	pvplb->MyTotalPoints = m_pp.PVPCareerPoints;
	pvplb->MyInfamy = infamyLevel;
	pvplb->Unknown0000 = 0;
...
void Client::Handle_OP_PVPLeaderBoardDetailsRequest(const EQApplicationPacket *app)
{
	...
	// Kings & Bandits details response
	PVPLeaderBoardDetailsRequest_Struct* inStr = (PVPLeaderBoardDetailsRequest_Struct*)app->pBuffer;
	Client* client = 0;

	bool success = false;

	if ( !strcasecmp(inStr->Name,this->GetName() ) ) // they are trying to get themselves, save the trouble of querying the db lol
	{
		pvplbdrs->Class = this->GetClass();
		pvplbdrs->Race = this->GetRace();
		pvplbdrs->GuildID = this->GuildID();
		pvplbdrs->Level = this->GetLevel();
		strcpy(pvplbdrs->Name,this->GetName());

		pvplbdrs->Points = this->GetPVPCareerPoints();
		pvplbdrs->Kills = this->GetPVPKills();
		pvplbdrs->Deaths = this->GetPVPDeaths();
		pvplbdrs->Infamy = GetInfamyLevel();
		success = true;
	}
	else if ( ( client = entity_list.GetClientByName(inStr->Name) ) != NULL ) // we are in the same zone! how convenient
	{
		pvplbdrs->Class = client->GetClass();
		pvplbdrs->Race = client->GetRace();
		pvplbdrs->GuildID = client->GuildID();
		pvplbdrs->Level = client->GetLevel();
		strcpy(pvplbdrs->Name,client->GetName());

		pvplbdrs->Points = client->GetPVPCareerPoints();
		pvplbdrs->Kills = client->GetPVPKills();
		pvplbdrs->Infamy = client->GetInfamyLevel();

		if ( RuleB(PVPLeaderBoard, TrackDeaths ) )
			pvplbdrs->Deaths = client->GetPVPDeaths();
		else
			pvplbdrs->Deaths = 0;

		success = true;
	}
	else // darn we have to call the db
	{
		PVPLeaderBoardDetailsReply_Struct detailStruct;
		success = database.GetLeaderBoardStructForChar(inStr->Name, pvplbdrs);
...
void Client::Handle_OP_AdventureMerchantSell(const EQApplicationPacket *app)
{
	...
	if(item->LDoNPrice == 0 || item->LDoNSellBackRate == 0)
	{
		Message(13, "The merchant does not want that item.");
		return;
	}

	sint32 price = (sint32)(item->LDoNPrice * item->LDoNSellBackRate);
	...
	LogLDONMerchant(this, vendor, 1, price, inst->GetItem(), false);
...
{
	//Image 2014: ADMIN CHECKS YOU IDIOTS!!!  THIS COMMAND ALLOWS SERVER CRASHES
	if ( this->Admin() < 200 )
		return;
...
void Client::Handle_OP_BuffRemoveRequest(const EQApplicationPacket *app)
{
	...
	{
		char hString[250];
		sprintf(hString, "OP_BuffRemoveRequest removal attempt of debuff for spellid: %i", SpellID);
			database.logevents(this->account_name,this->AccountID(), 0, this->name, GetTarget() != NULL ? GetTarget()->GetName() : "", "OP_BuffRemoveRequest", hString, 15,GetX(),
				GetY(),GetZ(), (char*)database.GetZoneName(GetZoneID(), GetPP().zoneInstance, true) );
	}
}
...
void Client::Handle_OP_CorpseDrag(const EQApplicationPacket *app)
{
	...
	// go for the corpse list not the entire entity list, it is afterall a corpse..
	Entity* ent = entity_list.GetEntityCorpse(cds->CorpseName);
	Mob* corpse = NULL;
...
void Client::Handle_OP_ClientBegin(const EQApplicationPacket *app)
{
