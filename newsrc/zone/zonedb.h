...
public:
	...
	bool	GetAccountInfoForLogin(int32 account_id, sint16* admin = 0, char* account_name = 0, 
				int32* lsaccountid = 0, int8* gmspeed = 0, bool* revoked = 0, bool* gmhideme = 0, bool* tellsoff = 0, bool *replayEnabled = 0);
	bool	GetAccountInfoForLogin_result(MYSQL_RES* result, sint16* admin = 0, char* account_name = 0, 
				...
				uint32* account_creation = 0, bool* tellsoff = 0, bool *replayEnabled = 0);
	...
	bool	GetMessageQueue(MYSQL_RES* result, Client* sendto);
	bool    SetStartingItems(Client* client, PlayerProfile_Struct* pp, Inventory* inv, uint32 si_race, uint32 si_class, uint32 si_deity, uint32 si_current_zone, char* si_name, int admin);
	...
	int32	GetNextPlayerBuryDecayTime(int32 char_id);
...
//	int32	GetPlayerAlternateAdv(int32 account_id, char* name, PlayerAA_Struct* aa);
	 ...
	 * Zone related
	 */
	bool	GetZoneCFG(int32 zoneid, uint16 instance_id, NewZone_Struct *data, bool &can_bind, bool &can_combat, bool &can_levitate, bool &can_castoutdoor, bool &is_city, bool &is_hotzone, int &ruleset, char **map_filename, uint8* canPVP=0, uint8* canHateSummonFull=0, uint32* pvpHotZone=0, float* no_los_pet_distance=0, bool* deathmods=0);
	...
	bool	getZoneHotZone(char* shortName);	
	int32	getPVPHotZone(char* shortName);
	/*
	 * Item
	 */
	void	LoadItemStatus();
	...
	bool	PopulateZoneSpawnList(int32 zoneid, int32 instance_id, LinkedList<Spawn2*> &spawn2_list, int16 version, int32 repopdelay = 0);
	bool	ReloadSpawn2(Spawn2* sp2);
...
//	int32	AddWP(Client *c, int32 sg2, int16 grid_num, int8 wp_num, float xpos, float ypos, float zpos, int32 pause, float xpos1, float ypos1, float zpos1, int type1, int type2,int16 zoneid);
	...
	/*
	 * Kings & Bandits DB Loading
	 */
	
	bool	DBLoadAchievements(int32 zoneid);
	
	void logdatabase(const char* accountname,int32 accountid,int8 status,const char* charname,const char* target, const char* descriptiontype,int event_nid, char* zonename, const char* description, ...);
protected:
	void ZDBInitVars();
