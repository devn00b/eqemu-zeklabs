...
#define _EDLEN	200
	...
	if(caster && caster->IsClient() && GetCastedSpellInvSlot() > 0)
	{
		...
		if(inst)
		{
			if(inst->GetItem()->Click.Level > 0 && !IsDiseaseCounterSpell(spell_id) && !IsPoisonCounterSpell(spell_id) && spell.buffduration < 1)
			{
	...
	// we added a buff to the caster above, we don't want to accidentally change this mobs buffs in the process
	if ( addCasterBuff )
		buffslot = -2;
	...
	if(spells[spell_id].numhits > 0 && buffslot >= 0 && buffslot < buff_count )
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		effect_value = CalcSpellEffectValue(spell_id, buffslot, i, caster_level, caster ? caster : this);
		...
		case SE_CurrentHP:	// nukes, heals; also regen/dot if a buff
			{
				...
				if(dmg < 0)
				{
					...
					// manaconvert dont let people drain themselves to death
					if ( caster != NULL && caster == this && IsClient()
						&& dmg >= GetHP() && (!RuleB(PVPCombat,ManaDrainDeathNoCombat) || (RuleB(PVPCombat,ManaDrainDeathNoCombat) && CastToClient()->GetAttackTarget() != NULL) ) )
						return false;

					Damage(caster, dmg, spell_id, spell.skill, 0, false, buffslot, false, false, false, (buffslot < 0) ? 0 : buffs[buffslot].player_id );
				}
				else if(dmg > 0) {
					//healing spell...
					if(caster)
					{
						...
						{
							if ( !IsAttackLevelPermitted(this,caster,true) )
								caster->CastToClient()->UpdateInfamyLevel(dmg);
						}
					}
		...
		case SE_PercentalHeal:
			{
				...
				if(val > 0)
				{
					...
					{
						if ( !IsAttackLevelPermitted(this,caster,true) )
							caster->CastToClient()->UpdateInfamyLevel(val);
					}
		...
		case SE_CompleteHeal:
			{
				...
				{
					if ( !IsAttackLevelPermitted(this,caster,true) )
						caster->CastToClient()->UpdateInfamyLevel(GetMaxHP()-GetHP());
				}
				Heal();
				break;
			}
		...
		case SE_Translocate:
			{
				...
				{
						QGlobal res;
						if(QGlobalCache::GetQGlobal(res,CastToClient()->GetQGlobals()->GetBucket(),"notranslocateallowed",0,CastToClient()->CharacterID(),zone->GetZoneID()))
						{
		...
		case SE_Teleport2:
			{	
				...
				if(!strcmp(spell.teleport_zone, "same") || ( zone != NULL && !strcasecmp(spell.teleport_zone, zone->GetShortName() ) ) )
				{
					target_zone = 0;
				}
				else
				...
				if (effect == SE_YetAnotherGate && IsClient() && caster && caster->IsClient())
		...
		case SE_Charm:
			{
				...
				if (!caster || buffslot < 0)	// can't be someone's pet unless we know who that someone is
					break;

				// 2018 - mezzed targets should not allow charm to follow through
				if(this->mezzed && IsClient())
				{
					if ( caster && caster->IsClient() )
		...
		case SE_Fear:
			{
				...
				float effectiveness = ResistSpell(spells[spell_id].resisttype, spell_id, caster);

				if ( effectiveness == 0.0f )
				{
					LogFile->write(EQEMuLog::Debug,
						"spell %d has no effectiveness (se_fear) against %s, effectiveness: %f.", spell_id, this->GetName(), effectiveness);
					break;
				}
				...
				// Kings & Bandits - make pvp stun by skipping this statement
				if( (!(caster && caster->IsClient() && IsClient())) && RuleB(Combat, EnableFearPathing)){
					if(IsClient())
					{
						AI_Start();
				...
				else 
				{
					...
					int duration = buffs[buffslot].ticsremaining * 6000 - (6000 - tic_timer.GetRemainingTime());
					
					int rndInt = MakeRandomInt((duration/4),duration);

					if ( rndInt < 1000 )
						rndInt = 1000;

					printf("Fear duration: %i\n", duration);
					Stun(rndInt);
					/*
					if ( caster && caster->GetClass() == BARD )
						Stun(buffs[buffslot].ticsremaining * 100);
					else
						Stun(buffs[buffslot].ticsremaining * 400);*/
				}
				break;
			}
		...
		case SE_CancelMagic:
			{
				...
				uint32 buff_count = GetMaxBuffSlots() + GetMaxSongSlots();
				uint32 buff_dispell_count = 0;
				uint32 try_attempts = 0;
				bool selfCast = false;
				...
				if ( caster == this || caster == NULL )
					selfCast = true;
				else
					attackAllowed = IsAttackAllowed(caster,true);

				bool isLevelAllowed = true;
				
				// handles out of range dispell 10/28/2014 (Image)
				Mob* otherClient = NULL;
				if ( IsClient() )
					otherClient = this;
				else if ( GetOwner() != NULL && GetOwner()->IsClient() )
					otherClient = GetOwner();

				if ( !selfCast && caster != NULL && caster->IsClient() && otherClient != NULL &&
					!IsAttackLevelPermitted(otherClient,caster,false) )
				{
					if ( ( caster->GetGroup() == NULL && otherClient->GetGroup() == NULL ) || 
						( ( caster->GetGroup() != NULL || otherClient->GetGroup() != NULL ) && caster->GetGroup() != otherClient->GetGroup() ) 
						)
						isLevelAllowed = false;
				}

				if ( !isLevelAllowed && !attackAllowed )
					break;
				// end of out of range dispell

				while (outOfOrder 
					&& !selfCast && buff_count > 0 && try_attempts < 20 )
				{
					int slot = MakeRandomInt(0,buff_count);
					if(	IsValidSpell(buffs[slot].spellid) &&
						buffs[slot].durationformula != DF_Permanent &&
						spells[buffs[slot].spellid].dispel_flag < 1 &&
						!IsDiscipline(buffs[slot].spellid))
					{
						if ( GetOwner() != NULL && GetOwner() == caster && !IsDetrimentalSpell(buffs[slot].spellid))
						{
							try_attempts++;
							continue;
						}
						else if ( !attackAllowed && (!IsDetrimentalSpell(buffs[slot].spellid) || IsCharmSpell(buffs[slot].spellid)) )
						{
							try_attempts++;
							continue;
						}
				...
				if ( attackAllowed && caster != NULL && caster != this && HasPet() )
						GetPet()->AddToHateList(caster, 1);

				if ( isLevelAllowed && buff_dispell_count < 1 )
				{
					for(int slot = 0; slot < buff_count; slot++) {
							...
							spells[buffs[slot].spellid].dispel_flag < 1)
						{
							...
							else if ( !selfCast && caster != NULL && caster != GetOwner())
							{
							if ( !attackAllowed && !IsDetrimentalSpell(buffs[slot].spellid) )
							{
								continue;
							}
							}
...
#endif
				if(spell_id==4789 && buffslot >= 0) // Touch of the Divine - Divine Save
		...
		case SE_ShadowStep:
			{
				...
				if ( IsClient() )
					CastToClient()->SetShadowStepUpdate(true);
				break;
			}
		...
		case SE_Levitate:
			{
				...
				if ( IsClient() )
					CastToClient()->SetLevitateFlag(2);
				break;
			}
		...
		case SE_Illusion:
			{
				...
				if(spell.base[i] == -1) {
					...
					if(spell_id == 1732 || spell_id == 1731) {
						...
						// Male
						if(spell_id == 1732 || spell_id == 3090) 
							specific_gender = 0;
						// Female
		...
		case SE_SpinTarget:
			{
				...
				if(max_level == 0)
					max_level = 60; // Default max is 55 level limit
				...
				else
				{
					...
					int rndInt = effect_value * 1000;
					if ( effect_value > 2 )
						rndInt = MakeRandomInt((effect_value/2),effect_value) * 1000;
		...
		case SE_EyeOfZomm:
			{
				...
				if(caster && caster->IsClient()) {
					...
					caster->TemporaryPets(spell_id, NULL, eye_name, duration, true);
				}
				break;
			}
		...
		case SE_Revive:
			{
				...
				if (IsCorpse() && CastToCorpse()->IsPlayerCorpse()) {
					...
					// added check in CastRezz that rezzer exists
...
#endif
				ChangeSize(GetSize(),spells[spell_id].base[i]);
				break;
			}
...
#endif
				LogFile->write(EQEMuLog::Error, "%s attempted to cast spell id %u with the SE_Lull effect.", GetCleanName(), spell_id);
		...
		case SE_PoisonCounter:
			{
				...
				else
				{
					...
					if ( IsNPC() && caster && caster->IsNPC() && effect_value > 0 )
						effect_value *= RuleI(Combat,IncreaseNPCCounterSpells);
					...
					for (int j=0; j < buff_count; j++) {
						...
						if (effect_value >= buffs[j].poisoncounters) {
							...
							if ( RuleB(Spells,SharedCounters) )
...
#endif
				if (IsClient()) { // will tidy this up later so that NPCs can duration ramp from spells too  
		...
		case SE_SkillAttack:
			{
				...
				if(RuleB(Combat, UseIntervalAC))
					caster->DoSpecialAttackDamage(this, spells[spell_id].skill, 0, tDmg, mDmg, (mDmg / 20));
				else
					caster->DoSpecialAttackDamage(this, spells[spell_id].skill, 0, MakeRandomInt(1, tDmg), mDmg, (mDmg / 20));
				break;
			}
		...
		case SE_BardAEDot:
			{
				...
				if(dmg < 0)
				{
					...
					dmg = -dmg;
					Damage(caster, dmg, spell_id, spell.skill,0, false, buffslot, false, false, false, buffs[buffslot].player_id, true );
				} else if(dmg > 0) {
					//healing spell...
					if(caster)
						dmg = caster->GetActSpellHealing(spell_id, dmg);
		...
		case SE_ManaDrainWithDmg:
			{
				...
				if(mana_to_use > -1) {
					...
					Damage(caster, mana_damage, spell_id, spell.skill,0, false, i, true, false, false, buffs[buffslot].player_id );
				}
				else {
		...
		case SE_EndDrainWithDmg:
			{
				...
				if(IsClient()) {
					...
					if(end_to_use > -1) {
						...
						Damage(caster, end_damage, spell_id, spell.skill,0, false, i, true, false, false, buffs[buffslot].player_id );
					}
					else {
		...
		case SE_Leap:
			{
				...
				if ( IsClient() )
					CastToClient()->SetLevitateFlag(0);
	...
	// Image(2017): Testing fixing counters
	if(buffslot >= 0) 
	{
		buffs[buffslot].CountersReady = true;
	}
...
void Mob::BuffProcess()
{
	...
	for (int buffs_i = 0; buffs_i < buff_count; ++buffs_i)
	{
		if (IsValidSpell(buffs[buffs_i].spellid))
		{
			...
			Mob* getMob = entity_list.GetMob(buffs[buffs_i].casterid);
			if ( ( buffs[buffs_i].player_id > 0 && buffs[buffs_i].player_id != MAX_PLAYERID ) && 
				( getMob != NULL && (!getMob->IsClient() || (getMob->IsClient() && getMob->CastToClient()->CharacterID() != buffs[buffs_i].player_id))) ||
				( getMob == NULL && buffs[buffs_i].casterid == 0 ) )
			{
				Client* findClient = entity_list.GetClientByCharID(buffs[buffs_i].player_id);
				if ( findClient )
				{
					getMob = findClient->CastToMob();
					
					if ( RuleB(EventLog,LogSpellDamage) && IsClient() )
						CastToClient()->Message(13,"SpellID: %i, has playerid: %i, casterid: %i, newid established: %s (%i)",buffs[buffs_i].spellid,buffs[buffs_i].player_id,buffs[buffs_i].casterid,getMob ? getMob->GetName() : "Unknown",getMob->GetID());

					loggedMsg = true;

					buffs[buffs_i].casterid = getMob->GetID();
				}
				else
					getMob = NULL;
			}

					if ( !loggedMsg && RuleB(EventLog,LogSpellDamage) && IsClient() )
						CastToClient()->Message(13,"SpellID: %i, has playerid: %i, casterid: %i, established name: %s",buffs[buffs_i].spellid,buffs[buffs_i].player_id,buffs[buffs_i].casterid,getMob ? getMob->GetName() : "Unknown");

			DoBuffTic(buffs[buffs_i].spellid, buffs[buffs_i].ticsremaining, buffs[buffs_i].casterlevel, buffs[buffs_i].player_id, buffs_i, getMob);
			...
			if(IsClient())
			{
				if(buffs[buffs_i].UpdateClient == true && buffs[buffs_i].CountersReady == true)
				{
					uint32 counters = 0;
					if ( IsPoisonCounterSpell(buffs[buffs_i].spellid) )
						counters = buffs[buffs_i].poisoncounters;
					else if ( IsDiseaseCounterSpell(buffs[buffs_i].spellid) )
						counters = buffs[buffs_i].diseasecounters;

					CastToClient()->SendBuffDurationPacket(buffs[buffs_i].spellid, buffs[buffs_i].ticsremaining, buffs[buffs_i].casterlevel, buffs_i, 2, counters);
...
void Mob::DoBuffTic(int16 spell_id, int32 ticsremaining, int8 caster_level, int32 player_id, int buffID, Mob* caster) {
	_ZP(Mob_DoBuffTic);
	...
	for (int i=0; i < EFFECT_COUNT; i++)
	{
		...
		case SE_CurrentHP:
			{
				effect_value = CalcSpellEffectValue(spell_id, buffID, i, caster_level, caster, ticsremaining);
				
				if ( RuleB(EventLog,LogSpellDamage) && IsClient() )
					CastToClient()->Message(13,"SpellID: %i, has playerid: %i, casterid: %i, established caster: %s, value: %i",spell_id,buffs[buffID].player_id,buffs[buffID].casterid,caster ? caster->GetName() : "Unknown", effect_value);
				...
				if(effect_value < 0) {
					...
					// Image: 4/28/2017 iBuffTic set to false as dots update client side
					Damage(caster, effect_value, spell_id, spell.skill,0, false, i, false, false, false, buffs[buffID].player_id, true);
		...
		case SE_BardAEDot:
			{
				...
				effect_value = CalcSpellEffectValue(spell_id, buffID, i, caster_level, caster, ticsremaining);
					...
					// Kings & Bandits - BardAE Instrument fix
			/*	if ( caster && caster->IsClient() )
				{
					int mod = caster->GetInstrumentMod(spell_id);
					effect_value = effect_value * mod / 10;
		...
		case SE_Hate2:{
			...
			break;
					  ...
					  // Kings & Bandits - SE_CurrentMana - support for mana regen like clarity - this was in spelleffect but not in bufftic.
					  // I think this is pooled with general mana, but it feels slow.  If we want to double the tic regen we could uncomment this and apply to all spells.
	/*	case SE_CurrentMana:
			{
		...
		case SE_Charm:			{
			// Kings & Bandits - SE_Charm - support for players being charmed for short periods of time

			if ( IsClient() && caster && caster->IsClient() )
			{
				float effectiveness = ResistSpell(spells[spell_id].resisttype, spell_id, caster);
				int totalEffectiveness = 10 + (effectiveness/7);
				int effectMultiplier = totalEffectiveness * 2;
				int ticMod = effectMultiplier + ticsremaining;
				if ( ticMod < effectMultiplier) // this should correspond to the right value we compare on SpellEffectiveness
				ticMod = effectMultiplier;
			
				int SpellEffectiveness = MakeRandomInt(0,ticMod);
				
				int effectiveResult = (int) (float)totalEffectiveness/(float)1.5;
			LogFile->write(EQEMuLog::Debug,
		"Spell %d attempting chance resist of dot (SE_Root/SE_Blind) against %s, effectiveness: %i (less than %i fades buff).", spell_id, this->GetName(), SpellEffectiveness, effectiveResult);

				if(SpellEffectiveness < effectiveResult) {
					BuffFadeBySpellID(spell_id);
				}
			}
			else if (!caster || !PassCharismaCheck(caster, this, spell_id, ticsremaining)) {
				BuffFadeByEffect(SE_Charm);
				if ( caster && IsNPC() )
					AddToHateList(caster,1,1);
			}
	...
	/*	case SE_CurrentHPOnce: // suffocating spells like enchanters use this, it is a DOT 
			{
			// Kings & Bandits - SE_CurrentHPOnce - detrimental dot based spells handled here
			...
			if ( IsClient() && caster && caster->IsClient() )
			{
				...
				int totalEffectiveness = 20 + (effectiveness/2);
				int effectMultiplier = totalEffectiveness * 2;
				int ticMod = effectMultiplier + ticsremaining; // let dots stick longer, give it a higher tic mod by giving it the original effectiveness.
				if ( ticMod < effectMultiplier) // this should correspond to the right value we compare on SpellEffectiveness
				ticMod = effectMultiplier;
			...
			LogFile->write(EQEMuLog::Debug,
		"Spell %d attempting chance resist of dot (SE_CurrentHPOnce) against %s, effectiveness: %i (less than %i fades buff).", spell_id, this->GetName(), SpellEffectiveness, totalEffectiveness);

			// dont reduce totalEffectiveness here, these last long anyway
				if(SpellEffectiveness < (totalEffectiveness)) {
					BuffFadeBySpellID(spell_id);
				}
			}
		...
		case SE_Fear:
			...
			if ( IsClient() )
			{
				// Kings & Bandits - modified SE_Fear bufftic
				float effectiveness = ResistSpell(spells[spell_id].resisttype, spell_id, caster);
				...
				int totalEffectiveness = 5 + (effectiveness/5);
				int effectMultiplier = totalEffectiveness;
				int ticMod = effectMultiplier + ticsremaining;
				if ( ticMod < effectMultiplier) // this should correspond to the right value we compare on SpellEffectiveness
				ticMod = effectMultiplier;
				...
				int effectiveResult = totalEffectiveness/2;
			LogFile->write(EQEMuLog::Debug,
		"Spell %d attempting chance resist of dot (SE_Fear) against %s, effectiveness: %i (less than %i fades buff).", spell_id, this->GetName(), SpellEffectiveness, effectiveResult);
		...
		// Kings & Bandits - modified Root & Blind bufftic
		float effectiveness = ResistSpell(spells[spell_id].resisttype, spell_id, caster);

			if ( IsClient() && caster && ( caster->IsClient() || caster->IsPet() ) )
			{
				int totalEffectiveness = 10 + (effectiveness/6);
				int effectMultiplier = totalEffectiveness * 2;
				int ticMod = effectMultiplier + ticsremaining/3;
				if ( ticMod < effectMultiplier) // this should correspond to the right value we compare on SpellEffectiveness
				ticMod = effectMultiplier;
				/*
				int totalEffectiveness = 0;
				totalEffectiveness = 15 + (effectiveness/2);
				...
				int effectiveResult = totalEffectiveness;
			LogFile->write(EQEMuLog::Debug,
		"Spell %d attempting chance resist of dot (SE_Root/SE_Blind) against %s, effectiveness: %i (less than %i fades buff).", spell_id, this->GetName(), SpellEffectiveness, effectiveResult);
		...
		case SE_MovementSpeed: {
			// Kings & Bandits - modified Movement speed bufftic (Detrimental)
				if ( !IsDetrimentalSpell(spell_id))
					break;
				else if ( i > 0 )
				{
					LogFile->write(EQEMuLog::Debug,
						"SE_MovementSpeed (%d)  against %s lacks effectiveness test due to being a secondary effect.", spell_id, this->GetName());
				}
			...
			if ( IsClient() && caster && caster->IsClient() )
			{
				...
				int effectiveResult = totalEffectiveness/2;
			LogFile->write(EQEMuLog::Debug,
		"Spell %d attempting chance resist of dot (SE_MovementSpeed) against %s, effectiveness: %i (less than %i fades buff).", spell_id, this->GetName(), SpellEffectiveness, effectiveResult);
...
void Mob::BuffFadeBySlot(int slot, bool iRecalcBonuses)
{
	...
	if ( buffs[slot].spellid == 2039 && IsClient() )
	{
	...
	if(spells[buffs[slot].spellid].viral_targets > 0) {
		...
		for(int i = 0; i < MAX_SPELL_TRIGGER*2; i+=2)
		{
			// KaB - Red69 - Zek / remove viral_spells from existance if we find the spell id we just got to debuff here
			if(viral_spells[i])
			{
				if ( viral_spells[i] != buffs[slot].spellid )
				{
		...
		case SE_Charm:
			{
				...
				if (IsNPC())
				{
					...
					if(tempmob)
					{
						AddToHateList(tempmob, CastToNPC()->GetTopHateValue(this)+1000, 0);
					}
						...
						if(speed > (runs * RuleR(Zone, MQWarpDetectionDistanceFactor)))
						{
							if(!my_c->GetGMSpeed() && (runs >= my_c->GetBaseRunspeed() || (speed > (my_c->GetBaseRunspeed() * RuleR(Zone, MQWarpDetectionDistanceFactor)))))
							{
								...
								if(my_c->IsShadowStepExempted())
								{
									...
									{
										my_c->CheatDetected(MQWarpShadowStep, my_c->GetX(), my_c->GetY(), my_c->GetZ(), my_c->rewind_x, my_c->rewind_y, my_c->rewind_z);
									}
								}
								else if(my_c->IsKnockBackExempted())
								{
									...
									if(speed > 30.0f)
									{
										my_c->CheatDetected(MQWarpKnockBack, my_c->GetX(), my_c->GetY(), my_c->GetZ(), my_c->rewind_x, my_c->rewind_y, my_c->rewind_z);
									}
								}
								else if(!my_c->IsPortExempted())
								{
									if(!my_c->IsMQExemptedArea(zone->GetZoneID(), my_c->GetX(), my_c->GetY(), my_c->GetZ()))
									{
										if(speed > (runs * 2 * RuleR(Zone, MQWarpDetectionDistanceFactor)))
										{
											...
											my_c->CheatDetected(MQWarp, my_c->GetX(), my_c->GetY(), my_c->GetZ(), my_c->rewind_x, my_c->rewind_y, my_c->rewind_z);
											//my_c->Death(my_c, 10000000, SPELL_UNKNOWN, _1H_BLUNT);
										}
										else
										{
											my_c->CheatDetected(MQWarpLight, my_c->GetX(), my_c->GetY(), my_c->GetZ(), my_c->rewind_x, my_c->rewind_y, my_c->rewind_z);
										}
									}
								}
							}
	...
	if (p && p != this && IsValidSpell(buffs[slot].spellid) && !IsBardSong(buffs[slot].spellid))
	{
		Mob *notify = p;
		if(p->IsPet())
			notify = p->GetOwner();
		if(notify && notify->IsClient()) {
			notify->Message_StringID(MT_WornOff, SPELL_WORN_OFF_OF,
				spells[buffs[slot].spellid].name, GetCleanName());
		}
	}
	
	// KaB / Red69 / Zek - Buff changes in spell_effects to clear out old spells 7/22

	memset(&buffs[slot],0,sizeof(Buffs_Struct));
	buffs[slot].spellid = SPELL_UNKNOWN;
	
	for(int i=0;i<EFFECT_COUNT;i++)
		buffs[slot].bardMod[i] = 0;

	if ( IsClient() )
	{
		memset(&CastToClient()->GetPP().buffs[slot],0,sizeof(SpellBuff_Struct));
		CastToClient()->GetPP().buffs[slot].spellid = SPELLBOOK_UNKNOWN;
	}
...
sint16 Client::CalcAAFocusEffect(focusType type, int16 focus_spell, int16 spell_id) 
{
	...
	{	
		// stop a crash trying to grab a non existing pointer
		if ( !this->aa[i] )
		{
			continue;
		}
...
sint16 Mob::CalcFocusEffect(focusType type, int16 focus_id, int16 spell_id) {
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		case SE_ImprovedDamage:
			...
			if ( RuleB( EventLog, LogSpellDamage ) )
				LogFile->write(EQEMuLog::Error, "%s cast the spell %i with a value %i.  Base: %i.  Max: %i.  Type %i.", GetCleanName(), spell_id,value,focus_spell.base[i],focus_spell.max[i], type);
			...
			case 0:
				if (type == focusImprovedDamage && focus_spell.base[i] > value)
				{
					...
					if ( RuleB( EventLog, LogSpellDamage ) )
						LogFile->write(EQEMuLog::Error, "%s focusImprovedDamage, spell id %i with a value %i.  Base: %i.  Max: %i.  Type %i.", GetCleanName(), spell_id,value,focus_spell.base[i],focus_spell.max[i], type);
				}
				break;
			case 1:
...
sint16 Client::GetFocusEffect(focusType type, int16 spell_id) {
	...
	for(int x=0; x<=21; x++)
	{
		...
		if (TempItem && TempItem->Focus.Effect > 0 && TempItem->Focus.Effect != SPELL_UNKNOWN) {
			...
			if ( RuleB( EventLog, LogSpellDamage ) )
				LogFile->write(EQEMuLog::Error, "%s calculated spell id %i, focus id %i, with a value %i.  Type: %i.", GetCleanName(), spell_id, TempItem->Focus.Effect,Total,type);
