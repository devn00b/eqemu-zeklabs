...
public:
	...
	// KaB / Red69 / Zek - Addition for item ground spawns to be set without checking the map z coord 9/19/2012
	Object(const ItemInst* inst, char* name,float max_x,float min_x,float max_y,float min_y,float z,float heading,int32 respawntimer, bool nomapz=false);
	...
	// Event handlers
	void RejectClick(Client* sender);
	...
	int32 GetObjectDataDropID ( ) { return m_data.drop_id; }
	uint32 GetLevelRequirement() { return m_levelrequirement; }
	char* GetQGlobalFlag() { return m_qglobal; }
protected:
	...
	float			m_heading;
	bool			m_ground_spawn;
	bool			m_nomapz; // KaB / Red69 / Zek - Addition for item ground spawns to be set without checking the map z coord 9/19/2012
