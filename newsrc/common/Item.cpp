...
ItemInst::ItemInst(const ItemInst& copy)
{
	...
	m_instnotransfer=copy.m_instnotransfer;
...
void Inventory::SwapItem(sint16 slot_a, sint16 slot_b)
{
	...
	if(inst_a)
	{
		if(!inst_a->IsSlotAllowed(slot_b))
			return;
		if ( RuleB(Character,BlockSecondarySlotItemSwap) && slot_b == SLOT_SECONDARY && m_inv[SLOT_PRIMARY])
		{
			...
			case ItemType2HPierce: // 2 handers cannot wield a secondary
				{
					LogFile->write(EQEMuLog::Error, "Inventory::SwapItem: Blocked Secondary Slot %s (%i)", inst_a->GetItem()->Name, inst_a->GetID());
					return;
					break;
				}
			}
...
void Inventory::purgeInventory()
{
	...
	for (it=m_trade.begin(); it!=m_trade.end(); it++) {
		inst = it->second;
		it->first;
		if(!inst || !inst->GetItem())
			continue;
