...
TCPConnection::State_t TCPConnection::GetState(bool lockState) const {
	State_t ret;
...
bool TCPConnection::ConnectReady(bool lockState) const {
	State_t s = GetState(lockState);
...
bool TCPConnection::TestConnect(int res)
{
	timeval tv;
	fd_set myset; 
	int valopt;
     if (errno == EINPROGRESS) { 
	_log(COMMON__THREADS, "TCPConnectionLoop attempting connection on thread %d", pthread_self());
        do { 
           tv.tv_sec = 15; 
           tv.tv_usec = 0; 
           FD_ZERO(&myset); 
           FD_SET(connection_socket, &myset); 
           res = select(connection_socket+1, NULL, &myset, NULL, &tv); 
           if (res < 0 && errno != EINTR) { 
			   _log(COMMON__THREADS, "TCPConnectionLoop error on thread %d - errno %d - %s", pthread_self(),errno,strerror(errno));
              return false;
           } 
           else if (res > 0) { 
			  socklen_t lon;
              ...
              if (valopt) { 
			   ...
			   _log(COMMON__THREADS, "TCPConnectionLoop error on thread %d - error valopt %d - %s", pthread_self(),valopt,strerror(valopt));
                 return false;
              }
              break; 
           } 
           else { 
			   _log(COMMON__THREADS, "TCPConnectionLoop timeout on thread %d", pthread_self());
			   return false;
           }
        } while (1); 
	 }
	 ...
	 {
		_log(COMMON__THREADS, "TCPConnectionLoop socket state not in progress for thread %d - errno %d", pthread_self(),errno);
		 return false;
	 }
	return true;
}
...
#ifdef WIN32
	if ((res = connect(connection_socket, (PSOCKADDR) &server_sin, sizeof (server_sin))) < 0) {
		if (!TestConnect(res) )
		{
			if (errbuf)
...
#else
	if ((res = connect(connection_socket, (struct sockaddr *) &server_sin, sizeof (server_sin))) < 0) {
		if (!TestConnect(res) )
		{
			if (errbuf)
...
void TCPConnection::ClearBuffers(bool lockMState) {
	MRunLoop.lock();
	MSendQueue.lock();
	if ( lockMState )
		MState.lock();
...
ThreadReturnType TCPConnection::TCPConnectionLoop(void* tmp) {
	...
	tcpc->SetState(TCPS_Disconnected);
