...
bool SharedDatabase::SetGMToggle(int32 account_id, int8 tellsoff)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;

	if (!RunQuery(query, MakeAnyLenString(&query, "UPDATE account SET tellsoff = %i where id = %i", tellsoff, account_id), errbuf)) {
		cerr << "Error in SetGMToggle query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
bool SharedDatabase::SaveInventory(uint32 char_id, const ItemInst* inst, sint16 slot_id, Inventory* inv, int32 zone_id) {
	...
	if (slot_id>=2500 && slot_id<=2600) { // Shared bank inventory
		...
		string tableToUse = "inventory";
		string columnToUse = "charid";
		int32 columnValue = char_id;
		uint32 teamID = KingsBandits::LeaderInCityBank(char_id,zone_id);
		if ( inv != NULL && inv->cityBankLoaded && 
			teamID > 0 && IsBankSlot(slot_id,true))
		{
			tableToUse = "city_bank";
			columnToUse = "teamid";
			columnValue = teamID;
		}
		else if ( inv != NULL && inv->cityBankLoaded && teamID == 0 )
		{
			safe_delete(query);
			return false;
		}
		if (!inst) {
			// Delete item
			ret = RunQuery(query, MakeAnyLenString(&query, "DELETE FROM %s WHERE %s=%i AND slotid=%i",
				tableToUse.c_str(), columnToUse.c_str(), columnValue, slot_id), errbuf);

			// Delete bag slots, if need be
			if (ret && Inventory::SupportsContainers(slot_id)) {
				safe_delete_array(query);
				sint16 base_slot_id = Inventory::CalcSlotId(slot_id, 0);
				ret = RunQuery(query, MakeAnyLenString(&query, "DELETE FROM %s WHERE %s=%i AND slotid>=%i AND slotid<%i",
					tableToUse.c_str(), columnToUse.c_str(), columnValue, base_slot_id, (base_slot_id+10)), errbuf);
			}
			...
			uint32 len_query = MakeAnyLenString(&query, 
				"REPLACE INTO %s "
				"	(%s,slotid,itemid,charges,instnodrop,color,"
				"	augslot1,augslot2,augslot3,augslot4,augslot5,instnotransfer)"
				" VALUES(%lu,%lu,%lu,%lu,%lu,%lu,"
				"	%lu,%lu,%lu,%lu,%lu,%lu)",
				tableToUse.c_str(),columnToUse.c_str(),(unsigned long)columnValue, (unsigned long)slot_id, (unsigned long)inst->GetItem()->ID, (unsigned long)charges, (unsigned long)(inst->IsInstNoDrop() ? 1:0),(unsigned long)inst->GetColor(),
				(unsigned long)augslot[0],(unsigned long)augslot[1],(unsigned long)augslot[2],(unsigned long)augslot[3],(unsigned long)augslot[4], (unsigned long)(inst->IsInstNoTransfer() ? 1:0) );
	...
	if (inst && inst->IsType(ItemClassContainer) && Inventory::SupportsContainers(slot_id)) {
		for (uint8 idx=0; idx<10; idx++) {
			...
			SaveInventory(char_id, baginst, Inventory::CalcSlotId(slot_id, idx), inv, zone_id);
		}
	}
		...
		while ((row = mysql_fetch_row(result))) {
			...
			if (item) {
				...
				if (put_slot_id == SLOT_INVALID) {
					...
					if(is_charid)
						SaveInventory(id,NULL,slot_id,inv, 0);
				}
			}
			else {
				LogFile->write(EQEMuLog::Error,
...
bool SharedDatabase::LoadCityBank(int32 teamid, Inventory* inv, int32 zone_id)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	uint32 len_query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT slotid,itemid,charges,color,augslot1,augslot2,augslot3,augslot4,augslot5,instnodrop,instnotransfer FROM city_bank WHERE teamid=%i ORDER BY slotid", teamid), errbuf, &result)) {

		while ((row = mysql_fetch_row(result))) {	
			sint16 slot_id	= atoi(row[0]);
			LoadItemFromRow(teamid, inv, slot_id, row, zone_id, true);
		}
		mysql_free_result(result);
	}
	inv->cityBankLoaded = true;
	safe_delete_array(query);
	return true;
}

bool SharedDatabase::LoadItemFromRow(int32 char_id, Inventory* inv, sint16 slot_id, MYSQL_ROW row, int32 zoneid, bool teamBank)
{			
	...
	bool instnotransfer = (row[10] && (int16)atoi(row[10])) ? true : false;

	const Item_Struct* item = GetItem(item_id);

	if (item) {

	if ( RuleI(World,NoTransferZoneExemption) > 0 && RuleI(World,NoTransferZoneExemption) == zoneid )
		instnotransfer = false;
		...
		inst->SetInstNoTransfer(instnotransfer);
		...
		if (put_slot_id == SLOT_INVALID) {
			...
			{
			LogFile->write(EQEMuLog::Error,
				"Warning: Invalid slot_id for item in city_bank: teamid=%i, item_id=%i, slot_id=%i",
...
bool SharedDatabase::IsBankSlot(uint32 slot, bool excludeShared)
{
...
bool SharedDatabase::GetInventory(uint32 char_id, Inventory* inv, int32 zone_id) {
...
#ifdef ZONE
	teamID = KingsBandits::LeaderInCityBank(char_id,zone_id);
	if ( teamID > 0 )
	{
		LoadCityBank(teamID, inv, zone_id);
	}
#endif
	// Retrieve character inventory
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT slotid,itemid,charges,color,augslot1,augslot2,augslot3,augslot4,augslot5,instnodrop,instnotransfer FROM inventory WHERE charid=%i ORDER BY slotid", char_id), errbuf, &result)) {

		while ((row = mysql_fetch_row(result))) {	
			sint16 slot_id	= atoi(row[0]);

			if ( teamID > 0 && 
				IsBankSlot(slot_id,true) )
			{
				// we are in the zone where the player will get his inventory from the city bank
				continue;
			}

			LoadItemFromRow(char_id, inv, slot_id, row, zone_id);
...
bool SharedDatabase::GetInventory(uint32 account_id, char* name, Inventory* inv, int32 zone_id) {
	...
	MYSQL_ROW row;
	bool ret = false;
	uint32 char_id = GetCharacterID(name);
...
#undef F
	...
	if (RunQuery(query, sizeof(query), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
			...
			// Kings & Bandits - source field we are using (row[0]) for the item founder
			if ( row[0] != NULL && strlen(row[0]) > 0 )
			{
				// find it in memshare and set it to true, someone found this already
				bool* itemFound = GetIsItemFound(item.ID);
				if ( itemFound )
					*itemFound = true;
			}
...
Item_Struct* SharedDatabase::GetItemNoConst(uint32 id) {
	return EMuShareMemDLL.Items.GetItemNoConst(id);
}
...
bool SharedDatabase::UpdatePlayerProfile(PlayerProfile_Struct* pp, ExtendedProfile_Struct* ext)
{
	if ( !pp || !ext )
		return false;
	...
	if ( ext->profile_version < CURRENTPLAYERPROFILE_VERSION )
	{
		for(int i=ext->profile_version;i<CURRENTPLAYERPROFILE_VERSION;i++ )
		{
			switch(i)
			{
				case 1: // patch to relocate players bind (7/1/2013)
				{
					uint32 zoneID = pp->binds[0].zoneId;

					// kael				temple veeshan		kerafyrm's lair		growth plane    mischief plane	guktop			gukbottom		soldungb		permafrost
					if ( zoneID == 113 || zoneID == 124 || zoneID == 128 || zoneID == 127  || zoneID == 126 || zoneID == 65 || zoneID == 66 || zoneID == 32 || zoneID == 73 )
					{
						pp->binds[0].zoneId = 77;
						pp->binds[0].x = -467.6f;
						pp->binds[0].y = -41.5f;
						pp->binds[0].z = 22.8f;
						pp->binds[0].heading = 63.0f;
					}
						break;
				}
				case 2:
					{
						memset(&ext->buffs[0],0,sizeof(BuffsPriorSep24_Struct)*BUFF_COUNT);
						break;
					}
				case 3: // patch to relocate players bind (10/24/2017)
					{
					uint32 zoneID = pp->binds[0].zoneId;
					// crushbone
					if ( zoneID == 58 )
...
bool SharedDatabase::GetPlayerProfile(int32 account_id, char* name, PlayerProfile_Struct* pp, Inventory* inv, ExtendedProfile_Struct *ext, char* current_zone, uint32 *current_instance) {
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT profile,zonename,x,y,z,extprofile,instanceid,id FROM character_ WHERE account_id=%i AND name='%s'", account_id, name), errbuf, &result)) {
		if (mysql_num_rows(result) == 1) {	
			row = mysql_fetch_row(result);
			...
			if (lengths[0] == sizeof(PlayerProfile_Struct)) {
				...
				if(ext) {
					...
					if ( UpdatePlayerProfile(pp, ext) )
					{
						uint32 instID = 0;
						if ( current_instance )
							instID = *current_instance;

						SetPlayerProfile(account_id,charid,pp,inv,ext,pp->zone_id,instID);
					}
				}

				// Retrieve character inventory
				ret = GetInventory(account_id, name, inv, pp->zone_id);
			}
			else {
...
int32 SharedDatabase::SetPlayerProfile_MQ(char** query, uint32 account_id, uint32 charid, PlayerProfile_Struct* pp, Inventory* inv, ExtendedProfile_Struct *ext, uint32 current_zone, uint32 current_instance) {
	*query = new char[376 + sizeof(PlayerProfile_Struct)*2 + sizeof(ExtendedProfile_Struct)*2 + 4];
	...
	end += sprintf(end, "UPDATE character_ SET timelaston=unix_timestamp(now()),name=\'%s\', zonename=\'%s\', zoneid=%u, instanceid=%u, x = %f, y = %f, z = %f, pvpflag = %i, guildid = %i, race = %i, profile=\'", pp->name, GetZoneName(current_zone), current_zone, current_instance, pp->x, pp->y, pp->z, pp->pvp, pp->guild_id, pp->race);
...
ItemInst* SharedDatabase::CreateItem(uint32 item_id)
{
	const Item_Struct* item = NULL;
	ItemInst* inst = NULL;
	item = GetItem(item_id);
	if (item) {
		inst = CreateBaseItem(item);
		inst->PutAugment(this, 0, 0);
		inst->PutAugment(this, 1, 0);
		inst->PutAugment(this, 2, 0);
		inst->PutAugment(this, 3, 0);
		inst->PutAugment(this, 4, 0);

		if ( item->NoTransfer )
			inst->SetInstNoTransfer(true);
	}
...
// Redesigned for charges on player corpses to properly set
ItemInst* SharedDatabase::CreateBaseItem(const Item_Struct* item) {
	ItemInst* inst = NULL;
	if (item) {
		if(item->CharmFileID != 0 || (item->LoreGroup >= 1000 && item->LoreGroup != -1)) {
			inst = new EvoItemInst(item, item->MaxCharges);
			((EvoItemInst*)inst)->Initialize(this);
		}
		else 
			inst = new ItemInst(item, item->MaxCharges);		
	}
	return inst;
}
...
bool SharedDatabase::FixItem(Item_Struct* item, uint32 itemId)
{
	if (!EMuShareMemDLL.Items.cbAddItem(itemId, item)) {
...
bool SharedDatabase::LoadTeamSettings() {
	...
	if (RunQuery(query, strlen(query), errbuf, &result)) {
		...
		if (row && row[0])
		{
			sint32 count = atoi(row[0])+1;

			if (EMuShareMemDLL.Load())
			{
				// Kings & Bandits - hardcoded max teams to 10 here
				uint32 maxID = MAXKINGDOMS;
...
bool SharedDatabase::DBLoadTeamSettings(sint32 iItemCount, uint32 iMaxItemID) {
	...
	char query[] = "SELECT teamid,bankcopper,banksilver,bankgold,bankplatinum,newbietaxrate,citizentaxrate,kingpcname,kingnpcid,upkeeprate,guildid,charid,ownerlastvisit,bankzoneid,citymode,ownerteamid,x,y,z,radius,zoneid from team_settings";
	if (RunQuery(query, sizeof(query), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
#if EQDEBUG >= 6
			LogFile->write(EQEMuLog::Status, "Loading %s:%i", row[ItemField::name], row[ItemField::id]);
			...
			team.bankcopper = atoul(row[1]);
			team.banksilver = atoul(row[2]);
			team.bankgold = atoul(row[3]);
			team.bankplatinum = atoul(row[4]);
			team.newbieTaxRate = atoul(row[5]);
			team.citizenTaxRate = atoul(row[6]);
			strcpy(team.kingPCName,row[7]);
			team.kingNPCTypeID = atoul(row[8]);
			team.upkeepRate = atoul(row[9]);
			team.guildid = atoul(row[10]);
			team.charid = atoul(row[11]);
			team.ownerlastvisit = atoul(row[12]);
			team.bankzoneid = atoul(row[13]);
			team.citymode = atoul(row[14]);
			team.ownerteamid = atoul(row[15]);
			team.x = atof(row[16]);
			team.y = atof(row[17]);
			team.z = atof(row[18]);
			team.radius = atof(row[19]);
			team.zoneid = atoul(row[20]);
			// each city gives a 5% xp bonus by default
			team.teamXPBonus = 5;
	...
	// load zone guard settings
	DBLoadGuardTeamValues();

	// Load PVP Leader Board into memshare (points) - points first since its weird like that
	DBLoadPVPLeaderBoard("SELECT t1.charid,t1.pvpkills,t1.pvpdeaths,t1.pvpcareerpts,t2.name,t3.infamy from characterdb_pvpstats as t1, character_ as t2, characterdb_stats as t3 where t2.id=t1.charid and t2.id=t3.charid and NOW()<DATE_ADD(lastupdated,INTERVAL 7 DAY) order by t1.pvpcareerpts desc limit 100");
	
	// Load PVP Leader Board into memshare (kills)
	DBLoadPVPLeaderBoard("SELECT t1.charid,t1.pvpkills,t1.pvpdeaths,t1.pvpcareerpts,t2.name,t3.infamy from characterdb_pvpstats as t1, character_ as t2, characterdb_stats as t3 where t2.id=t1.charid and t2.id=t3.charid and NOW()<DATE_ADD(lastupdated,INTERVAL 7 DAY) order by t1.pvpkills desc limit 100");

	// Load PVP Leader Board into memshare (infamy)
	DBLoadPVPLeaderBoard("SELECT t1.charid,t1.pvpkills,t1.pvpdeaths,t1.pvpcareerpts,t2.name,t3.infamy from characterdb_pvpstats as t1, character_ as t2, characterdb_stats as t3 where t2.id=t1.charid and t2.id=t3.charid and NOW()<DATE_ADD(lastupdated,INTERVAL 7 DAY) order by t3.infamy desc limit 100");

	return ret;
}

uint32 SharedDatabase::AddTeam(uint32 guildid, uint32 charid, uint8 citymode, uint32 ownerteamid, float x, float y, float z, float radius, uint32 zoneid) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	query = new char[256];
	strcpy(query, "SELECT max(teamid) FROM team_settings");
	...
	if (RunQuery(query, strlen(query), errbuf, &result)) {
		...
		if (row && row[0])
		{
			...
			team.zoneid = zoneid;
			if (!EMuShareMemDLL.TeamSettings.cbAddTeamSettings(team.teamID, &team)) {
				LogFile->write(EQEMuLog::Error, "Database::AddTeam: Failure reported from EMuShareMemDLL.TeamSettings.cbAddTeamSettings(%i)", team.teamID);
				return 0;
			}
		...
		cerr << "Error in AddTeam query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return 0;
	}
	return 0;
...
bool SharedDatabase::DBLoadPVPLeaderBoard(char* query)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	MYSQL_RES *result;
	MYSQL_ROW row;
	...
	if (RunQuery(query, strlen(query), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
			...
			memset(&lbe, 0, sizeof(PVPLeaderBoardEntry_Struct));
			...
			lbe.Infamy = atoul(row[5]); // kings & bandits - infamy leaderboard fix

			if (!EMuShareMemDLL.TeamSettings.cbUpdatePVPLeaderBoard(&lbe)) {
				LogFile->write(EQEMuLog::Error, "Database::DBLoadPVPLeaderBoard: Failure reported from EMuShareMemDLL.TeamSettings.cbUpdatePVPLeaderBoard()");
				break;
			}
		}
...
bool SharedDatabase::DBLoadGuardTeamValues()
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	MYSQL_RES *result;
	MYSQL_ROW row;
	bool ret = false;
	char query[] = "select teamid,guardnpcid,guardclass,hp,mana,hpregen,manaregen,ac,guardlevel,spawngroupid from team_guards";
	if (RunQuery(query, sizeof(query), errbuf, &result)) {
		while((row = mysql_fetch_row(result))) {
#if EQDEBUG >= 6
			LogFile->write(EQEMuLog::Status, "Loading %s:%i", row[ItemField::name], row[ItemField::id]);
#endif				
			GuardSettings_Struct gss;
			memset(&gss, 0, sizeof(GuardSettings_Struct));
			gss.teamID = atoul(row[0]);
			gss.guardNPCTypeID = atoul(row[1]);
			gss.classID = atoul(row[2]);
			gss.hpBonus = atoul(row[3]);
			gss.manaBonus = atoul(row[4]);
			gss.hpRegenBonus = atoul(row[5]);
			gss.manaRegenBonus = atoul(row[6]);
			gss.AC = atoul(row[7]);
			gss.level = atoul(row[8]);
			gss.spawngroupid = atoul(row[9]);
			gss.didMobSpawn = false;

			if (!EMuShareMemDLL.TeamSettings.cbUpdateGuardSettings(&gss)) {
				LogFile->write(EQEMuLog::Error, "Database::DBLoadGuardTeamValues: Failure reported from EMuShareMemDLL.TeamSettings.cbUpdateGuardSettings()");
				break;
			}
		}
...
PVPLeaderBoardEntry_Struct* SharedDatabase::GetPVPLeaderBoardEntriesByKills() {
	return EMuShareMemDLL.TeamSettings.GetPVPLeadershipBoardByKills();
}

PVPLeaderBoardEntry_Struct* SharedDatabase::GetPVPLeaderBoardEntriesByPoints() {
	return EMuShareMemDLL.TeamSettings.GetPVPLeadershipBoardByPoints();
}

PVPLeaderBoardEntry_Struct* SharedDatabase::GetPVPLeaderBoardEntriesByInfamy() {
	return EMuShareMemDLL.TeamSettings.GetPVPLeadershipBoardByInfamy();
}

GuardSettings_Struct* SharedDatabase::GetGuardSettings(uint32 kingdomid, uint32 npctypeid, uint32 spawngroupid) {
	return EMuShareMemDLL.TeamSettings.GetGuardSettings(kingdomid, npctypeid, spawngroupid);
}

GuildHate_Struct* SharedDatabase::GetGuildHateList(uint32 kingdomid, uint32 guildid) {
	return EMuShareMemDLL.TeamSettings.GetGuildHateList(kingdomid, guildid);
}

bool* SharedDatabase::GetIsItemFound(uint32 id) {
	return EMuShareMemDLL.TeamSettings.GetIsItemFound(id);
}
...
bool SharedDatabase::GetRandomName(uint32 templateID, uint32 weight,string* data) {
	...
	//cout<<"NPC Has Random Name Group: "<< templateID << ", Weight: " << weight <<endl;
	...
	if( weight > 0 )
	{
		return GetRandomName(templateID, weight-1,data);
	}
...
bool SharedDatabase::GenerateRandomLootGroupResult(RandomLootDropResult_Struct* rldr, uint32 groupid, char* npcName, char* pcName,int32 providedCRC)
{
	...
	if ( rld != NULL )
	{
		...
		default:
			{
				if ( rld->maxItemDrop < 1 )
					itemCountResult = rld->minItemDrop;
				else
		...
		for(int i=0;i<itemCountResult;i++)
		{
			...
			if ( itmTemplate->itemRandCount < 1 )
			{
				...
				for ( int i=0;i<attempts;i++)
				{
					...
					{
						actualDropType = COMMON;
						resultCRC = providedCRC;
						gotExistingItem = GetExistingRandomDropID(actualDropType,resultCRC,itmTemplate->itemid,&outID,false);
					}
					else
					{
						resultCRC = (MakeRandomFloat(0.00000001,0.99999999) * 10000000) + attempts * MakeRandomInt(0,10000);
						gotExistingItem = GetExistingRandomDropID(actualDropType,resultCRC,itmTemplate->itemid,&outID,true);
					}

					// If a CRC is provided then we are looking for an item, we either found it or we didn't
					if ( (providedCRC>0) ||  (!providedCRC && !gotExistingItem ) )
						break;
				}
			}
			else
...
bool SharedDatabase::GenerateRandItemStruct(float baseValue, float dbValue, Item_Struct* itmStruct, const RandomItemTemplate_Struct* rndItem, RNDITEM_DROP_TYPE dropType, char* npcNameStr, char* pcNameStr)
{
	...
	if ( rndItem->loregroup != 0 )
		strncpy(itmStruct->Lore,itmStruct->Name,64);
	else
	...
	if ( rndItem->maxmaxcharges < 1 )
		itmStruct->MaxCharges = rndItem->minmaxcharges;
	else
...
bool SharedDatabase::GetExistingRandomDropID(uint8 typeID, uint32 checksum, uint32 itemTemplateID, uint32* itemID, bool randomize) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	MYSQL_RES *result;
	MYSQL_ROW row;
	uint32 itemDBID = 0;
	...
	char* query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT id FROM items where templatehash='Random%i %i' and templateid=%i %slimit 1"
		, typeID, checksum,itemTemplateID, randomize ? "order by rand() " : ""), errbuf, &result)) {
			safe_delete_array(query);
			if (mysql_num_rows(result) == 1)
			{
				row = mysql_fetch_row(result);
...
const char* SharedDatabase::GetZoneName(int32 zoneID, int32 instanceID, bool ErrorUnknown, bool ReturnStaticZone) {
	...
	while ((zns = IterateZoneNames(&it))) {
		...
		if ( instanceID > 0 && zns->instanceID != instanceID && instanceID < 10000 )
			continue;
...
bool SharedDatabase::MoveCharacterToZone(int32 iCharID, const char* iZonename) {
	...
	char *query = 0;
	int32	affected_rows = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "UPDATE character_ SET zonename = '%s', zoneid=%i, instanceid=0, x=-1, y=-1, z=-1 WHERE id=%i", iZonename, GetZoneID(iZonename), iCharID), errbuf, 0,&affected_rows)) {
...
int32 SharedDatabase::GetCharacterInfo(const char* iName, int32* oAccID, int32* oZoneID, int32* oInstanceID, float* oX, float* oY, float* oZ, uint8* pvpFlag,uint8* level) {
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	...
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT id, account_id, zonename, instanceid, x, y, z, pvpflag, level FROM character_ WHERE name='%s'", char_name), errbuf, &result)) {
		safe_delete_array(query);
...
bool SharedDatabase::logevents(const char* accountname,int32 accountid,int8 status,const char* charname, const char* target,const char* descriptiontype, const char* description,int event_nid,
	float charXPos, float charYPos, float charZPos, char* charZoneName, float targXPos, float targYPos, float targZPos){
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		...
		if (!RunQuery(query, MakeAnyLenString(&query, 
			...
			accountname,accountid,status,charname,targetarr,descriptiontype,descriptiontext,event_nid, charXPos, charYPos, charZPos, charZoneName, targXPos, targYPos, targZPos), errbuf))	{
...
bool SharedDatabase::logchatevents(const char* accountname,int32 accountid,int8 status,const char* charname,const char* target, const char* chattext,int chatgroup_id,
	float charXPos, float charYPos, float charZPos, char* charZoneName, float targXPos, float targYPos, float targZPos){
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		uint32 len = strlen(chattext);
		uint32 len2 = strlen(target);
		char* descriptiontext = new char[2*len+1];
		char* targetarr = new char[2*len2+1];
		memset(descriptiontext, 0, 2*len+1);
		memset(targetarr, 0, 2*len2+1);
		DoEscapeString(descriptiontext, chattext, len);
		DoEscapeString(targetarr, target, len2);
		if (!RunQuery(query, MakeAnyLenString(&query, 
			"Insert into eventlog_chat (accountname,accountid,status,charname,target,chattext,chatgroup_id,charxpos,charypos,charzpos,charzonename,targxpos,targypos,targzpos) " 
			"values('%s',%i,%i,'%s','%s','%s','%i','%f','%f','%f','%s','%f','%f','%f')", 
			accountname,accountid,status,charname,targetarr,descriptiontext,chatgroup_id, charXPos, charYPos, charZPos, charZoneName, targXPos, targYPos, targZPos), errbuf))	{
				cerr << "Error in logchatevents" << query << "' " << errbuf << endl;
				safe_delete_array(query);
				safe_delete_array(descriptiontext);
				safe_delete_array(targetarr);
				return false;
...
bool SharedDatabase::logcombatevents(const char* accountname,int32 accountid,int8 status,const char* charname,const char* target, const char* actiontext,int actiongroup_id,
	float charXPos, float charYPos, float charZPos, char* charZoneName, float targXPos, float targYPos, float targZPos){
		char errbuf[MYSQL_ERRMSG_SIZE];
		char *query = 0;
		uint32 len = strlen(actiontext);
		uint32 len2 = strlen(target);
		char* descriptiontext = new char[2*len+1];
		char* targetarr = new char[2*len2+1];
		memset(descriptiontext, 0, 2*len+1);
		memset(targetarr, 0, 2*len2+1);
		DoEscapeString(descriptiontext, actiontext, len);
		DoEscapeString(targetarr, target, len2);
		if (!RunQuery(query, MakeAnyLenString(&query, 
			"Insert into eventlog_combat (accountname,accountid,status,charname,target,actiontext,actiongroup_id,charxpos,charypos,charzpos,charzonename,targxpos,targypos,targzpos) " 
			"values('%s',%i,%i,'%s','%s','%s','%i','%f','%f','%f','%s','%f','%f','%f')", 
			accountname,accountid,status,charname,targetarr,descriptiontext,actiongroup_id, charXPos, charYPos, charZPos, charZoneName, targXPos, targYPos, targZPos), errbuf))	{
				cerr << "Error in logcombatevents" << query << "' " << errbuf << endl;
				safe_delete_array(query);
				safe_delete_array(descriptiontext);
				safe_delete_array(targetarr);
				return false;
...
{	
	...
	if (!RunQuery(query, MakeAnyLenString(&query, "insert into characterdb_pvpstats set charid=%i,lastupdated=NOW()", charid), errbuf))	{
...
{	
	...
	if (!RunQuery(query, MakeAnyLenString(&query, "update characterdb_pvpstats set pvpkills=%i,pvpdeaths=%i,pvpcurpts=%i,pvpcareerpts=%i,pvpbestkillstreak=%i,"
		"pvpworstdeathstreak=%i,pvpcurrentkillstreak=%i,killsinlastday=%i,lastkillname='%s',lastdeathname='%s',lastupdated=NOW() where charid=%i", 
...
void SharedDatabase::AddCharacterPVPKill(uint32 charidattacker, uint32 charidkilled, uint32 ptsearned, char* attackerName, char* killedName, char* zoneShortName, uint8 attackerLevel, uint8 killedLevel,
	int32 attackerguildid, int32 killedguildid, int32 attackerteamid, int32 killedteamid, bool instanced)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "insert into characterdb_pvpkills set charidattacker=%i,charidkilled=%i,ptsearned=%i,attackername='%s',killedname='%s',zoneshortname='%s',attackerlevel=%i,killedlevel=%i,attackerguildid=%i,killedguildid=%i,attackerteamid=%i,killedteamid=%i,instanced=%i", 
		charidattacker, charidkilled, ptsearned, attackerName, killedName, zoneShortName, attackerLevel, killedLevel, attackerguildid, killedguildid, attackerteamid, killedteamid, instanced ), errbuf))	{
...
void SharedDatabase::AddGuardSettings(uint32 teamid, uint32 guardnpcid, uint8 classid, uint32 hpBonus, uint32 manaBonus, uint32 hpRegen, uint32 manaRegen, uint32 AC, uint8 level, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "insert into team_guards set teamid=%i,guardnpcid=%i,guardclass=%i,hp=%i,mana=%i,hpregen=%i,manaregen=%i,ac=%i,guardlevel=%i,spawngroupid=%i", 
		teamid, guardnpcid, classid, hpBonus, manaBonus, hpRegen, manaRegen, AC, level, spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;

			GuardSettings_Struct data;
			memset(&data, 0, sizeof(GuardSettings_Struct));
			data.teamID = teamid;
			data.guardNPCTypeID = guardnpcid;
			data.classID = classid;
			data.hpBonus = hpBonus;
			data.manaBonus = manaBonus;
			data.hpRegenBonus = hpRegen;
			data.manaRegenBonus = manaRegen;
			data.AC = AC;
			data.level = level;
			data.lastkilledtimestamp = 0;
			data.lastkilledcharid = 0;
			data.lastkilledguildid = 0;
			data.lastkilledteamid = 0;
			data.lastkilledlevel = 0;
			data.didMobSpawn = true;
			data.spawngroupid = spawngroupid;

			if (!EMuShareMemDLL.TeamSettings.cbUpdateGuardSettings(&data)) {
				LogFile->write(EQEMuLog::Error, "Database::UpdateGuardSettings: Failure reported from EMuShareMemDLL.TeamSettings.cbUpdateGuardSettings(%i)", data.guardNPCTypeID);
			}
	}
	safe_delete_array(query);
	//return true;
...
void SharedDatabase::AddGuardSpellOmitList(uint32 teamID, uint32 guardnpcid, uint32 effect_type, uint32 spell_id, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "insert into team_guard_omit_spells set teamid=%i,npcid=%i,effect_type=%i,spell_id=%i,spawngroupid=%i", 
		teamID, guardnpcid, effect_type, spell_id, spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::DeleteGuardSpellOmitList(uint32 teamID, uint32 guardnpcid, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "delete from team_guard_omit_spells where teamid=%i and npcid=%i and nodelete=0 and (spawngroupid=0 or spawngroupid=%i)", 
		teamID, guardnpcid,spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::AddGuardSpellList(uint32 teamID, uint32 guardnpcid, uint32 npc_spell_list, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "insert into team_guard_spells set teamid=%i,npcid=%i,npc_spell_list=%i,spawngroupid=%i", 
		teamID, guardnpcid, npc_spell_list, spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::DeleteGuardSpellList(uint32 teamID, uint32 guardnpcid, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "delete from team_guard_spells where teamid=%i and npcid=%i and nodelete=0 and (spawngroupid=0 or spawngroupid=%i)", 
		teamID, guardnpcid,spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
...
void SharedDatabase::AddGuardItem(uint32 teamid, uint32 guardnpcid, uint32 zoneid, uint32 spawngroupid, uint32 itemid, sint16 charges)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "insert into team_guard_items set teamid=%i,npcid=%i,zoneid=%i,spawngroupid=%i,itemid=%i,charges=%i", 
		teamid,guardnpcid,zoneid,spawngroupid,itemid,charges), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::DeleteGuardItems(uint32 teamid, uint32 guardnpcid, uint32 zoneid, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "delete from team_guard_items where teamid=%i and npcid=%i and zoneid=%i and (spawngroupid=0 or spawngroupid=%i)", 
		teamid, guardnpcid, zoneid, spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::DeleteGuardSettings(uint32 teamid, uint32 guardnpcid, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "delete from team_guards where teamid=%i and guardnpcid=%i and (spawngroupid=%i or spawngroupid=0)", 
		teamid, guardnpcid, spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
...
void SharedDatabase::UpdateGuardSettings(uint32 teamID, uint32 guardnpcid, uint8 classid, uint32 hpBonus, uint32 manaBonus, uint32 hpRegen, uint32 manaRegen, uint32 AC, uint8 level, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "update team_guards set guardclass=%i,hp=%i,mana=%i,hpregen=%i,manaregen=%i,ac=%i,guardlevel=%i,spawngroupid=%i where teamid=%i and guardnpcid=%i and (spawngroupid=%i or spawngroupid=0)", 

		classid, hpBonus, manaBonus, hpRegen, manaRegen, AC, level, spawngroupid, teamID, guardnpcid,spawngroupid), errbuf))	{
			//cerr << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//return false;
	}
...
void SharedDatabase::UpdateGuildHateList(uint32 teamid, uint32 guildid, uint32 timestamp, bool purgeAll)
{
	if ( guildid == 0 || guildid == GUILD_NONE )
		return;
	
	GuildHate_Struct ghs;
	memset(&ghs, 0, sizeof(GuildHate_Struct));
	ghs.taggedguild = guildid;
	ghs.taggedtimestamp = timestamp;

	if (!EMuShareMemDLL.TeamSettings.cbUpdateGuildHateList(teamid,&ghs,purgeAll)) {
		LogFile->write(EQEMuLog::Error, "Database::UpdateGuildHateList: Failure reported from EMuShareMemDLL.TeamSettings.cbUpdateGuildHateList");
	}
}
...
void SharedDatabase::UpdateCharacterStats(uint32 charid, uint32 astr, uint32 asta, uint32 adex, uint32 aagi, uint32 aint, uint32 awis, uint32 acha, uint32 hp, uint32 maxhp, 
	uint32 mana, uint32 maxmana, uint32 endurance, uint32 maxendurance, uint32 timeplayed, uint32 infamyLevel, bool bountySet, uint8 ischaronline)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "update characterdb_stats set astr=%i,asta=%i,adex=%i,aagi=%i,aint=%i,awis=%i,acha=%i,hp=%i,maxhp=%i,mana=%i,maxmana=%i,endurance=%i,maxendurance=%i,timeplayedminutes=%i,infamy=%i,bountyset=%i,ischaronline=%i,lastzonetime=CURRENT_TIMESTAMP where charid=%i", 
		astr, asta, adex, aagi, aint, awis, acha, hp, maxhp, mana, maxmana, endurance, maxendurance, timeplayed, infamyLevel, bountySet, ischaronline, charid), errbuf))	{
			//cout << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//	return false;
	}
...
bool SharedDatabase::GetLeaderBoardStructForChar(char* name, PVPLeaderBoardDetailsReply_Struct* pvpBoardStruct)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	...
	int len = strlen(name);
	char* char_name = new char[2*len+1];
	DoEscapeString(char_name, name, len);

	if (RunQuery(query, MakeAnyLenString(&query, "SELECT t1.id,t1.name,t1.class,t1.race,t1.level,t1.guildid,t2.charid,t2.pvpkills,t2.pvpdeaths,t2.pvpcareerpts,t3.infamy" 
		" from character_ as t1, characterdb_pvpstats as t2, characterdb_stats as t3 where t1.name='%s' and t1.id = t2.charid and t1.id= t3.charid;", char_name), 
		errbuf, &result))
	{
		safe_delete_array(query);
		safe_delete_array(char_name);
		...
		{
			// id (charid) : 0
			strcpy(pvpBoardStruct->Name,row[1]);
			pvpBoardStruct->Class = atoul(row[2]);
			pvpBoardStruct->Race = atoul(row[3]);
			pvpBoardStruct->Level = atoul(row[4]);
			pvpBoardStruct->GuildID = atoul(row[5]);
			// charid : 6
			pvpBoardStruct->Kills = atoul(row[7]);
			pvpBoardStruct->Deaths = atoul(row[8]);
			pvpBoardStruct->Points = atoul(row[9]);
			pvpBoardStruct->Infamy = atoul(row[10]);
			set = true;
		}
		mysql_free_result(result);
		return set;
...
bool SharedDatabase::UpdateItemDiscovery(uint32 itemID, char* name, bool isGM)
{
	if ( isGM ) // if their GM flag is enabled we return false, no disco for GM's
		return false;

	// Kings & Bandits item discovery
	bool* itemFound = GetIsItemFound(itemID);
	if ( !itemFound ) // this item doesnt exist, thats not good but we better return
		return false;
	else if ( *itemFound ) // found it but its already found!
		return false;
	// it hasn't been found, so lets make it so it is!

	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "update items set updated=NOW(),source='%s' where id=%i", 
		name,itemID), errbuf))	{
			//cout << "Error in SharedDatabase::UpdateCharacterPVPStats" << query << "' " << errbuf << endl;
			safe_delete_array(query);
			//	return false;
	...
	// we made it to the DB save it in memshare
	*itemFound = true;
	return true;
}


bool SharedDatabase::IsZoneUp(char* shortname)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT isup from zone where short_name='%s'", shortname), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
			int8 value = atoi(row[0]);
			mysql_free_result(result);
			return (bool)value;
		}
		else
	...
	else
	{
		...
		cerr << "Error in IsZoneUp query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
int8 SharedDatabase::TeamPVPFlag(uint32 charID)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT pvpflag from character_ where id='%i'", charID), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
	...
	else
	{
		...
		cerr << "Error in TeamPVPFlag query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
int8 SharedDatabase::GuardTeamFlag(uint32 npctypeid, uint32 spawngroupid)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT teamid from team_guards where guardnpcid='%i' and spawngroupid='%i'", npctypeid,spawngroupid), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
...
uint32 SharedDatabase::GetCharacterGuildID(uint32 charID)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT guildid from character_ where id='%i'", charID), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
			uint32 value = atoi(row[0]);
			mysql_free_result(result);
			return value;
		}
		else
	...
	else
	{
		...
		cerr << "Error in GetCharacterGuildID query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
bool SharedDatabase::GetPlayerZoneKills(uint32 charID,char* zoneShortName,uint32* kills, uint32 secondsElapsed)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	...
	if ( secondsElapsed > 0 )
		_snprintf(sqlMsg,512,"SELECT count(id) from characterdb_pvpkills where charidattacker=%i and zoneshortname='%s' and now()<DATE_ADD(eventtime,INTERVAL %i SECOND)", charID, zoneShortName, secondsElapsed);
	else
		_snprintf(sqlMsg,512,"SELECT count(id) from characterdb_pvpkills where charidattacker=%i and zoneshortname='%s'", charID, zoneShortName);

	if (RunQuery(sqlMsg, (int)strlen(sqlMsg), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
			int32 value = atoul(row[0]);
			mysql_free_result(result);
			if ( kills )
				*kills = value;
	...
	{
		cerr << "Error in GetPlayerZoneKills query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
bool SharedDatabase::GetPlayerZoneDeaths(uint32 charID,char* zoneShortName,uint32* deaths, uint32 secondsElapsed, bool skipInstanced)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	...
	if ( skipInstanced )
		_snprintf(append,32," and instanced=0");
	else
		_snprintf(append,32,"");
	
	if ( secondsElapsed > 0 )
		_snprintf(sqlMsg,512,"SELECT count(id) from characterdb_pvpkills where charidkilled=%i and zoneshortname='%s' and now()<DATE_ADD(eventtime,INTERVAL %i SECOND)%s", charID, zoneShortName, secondsElapsed, append);
	else
		_snprintf(sqlMsg,512,"SELECT count(id) from characterdb_pvpkills where charidkilled=%i and zoneshortname='%s'%s", charID, zoneShortName, append);
	...
	{
		cerr << "Error in GetPlayerZoneDeaths query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
int32 SharedDatabase::GetSyncAccountByName(char* accountName, char* key)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	int32 accountID = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT id from account where name='%s' and sync_key='%s'", accountName, key), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
	...
	{
		cerr << "Error in GetSyncAccountByName query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
int8 SharedDatabase::GetSyncCharacterCount(uint32 accountID, uint32 syncAcctID)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	int8 count = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT count(id) from character_ where account_id=%i or account_id=%i", accountID, syncAcctID), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
			int8 value = atoul(row[0]);
			mysql_free_result(result);
			count = value;
	...
	{
		cerr << "Error in GetSyncCharacterCount query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
void SharedDatabase::UpdateAccountSetSyncID(uint32 acctID, uint32 syncAcctID)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "update account SET sync_accid=%i,sync_key='' WHERE id=%i",syncAcctID,acctID), errbuf))	{
		cout << "Error in SharedDatabase::UpdateAccountSetSyncID" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		//	return false;
	}
	safe_delete_array(query);
...
int32 SharedDatabase::GetSyncAccountID(uint32 acctID, bool* iseqemuacct)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	int32 accountID = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT sync_accid,eqemutable FROM account where id=%i", acctID), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);
	...
	{
		cerr << "Error in GetSyncAccountID query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return false;
	}
...
// Kings & Bandits
// AddNewItem - Support for creation of new items on the fly and transitioning it across the server.
uint32 SharedDatabase::CreateCharacterReplay(uint32 charID, uint32 zoneID)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MYSQL_RES *result;
	MakeAnyLenString(&query,
		"INSERT into replaylist set charid=%i,zoneid=%i,starttime=NOW(),endtime=NOW()",charID,zoneID);
	...
	if (!RunQuery(query, strlen(query), errbuf, &result, &affected_rows, &insert_id)) {
		...
		if ( insert_id < 1 )
		{
			LogFile->write(EQEMuLog::Error, "Database::CreateCharacterReplay: Failure, no replay id reported back");
			return 0;
		}
	}
	else
	{
		safe_delete_array(query);
		LogFile->write(EQEMuLog::Error, "Database::CreateCharacterReplay: no replay id reported back (insert failed).");
		return false;
	}
...
void SharedDatabase::AddReplayFrame(uint32 replayID, uint32 frameID, float x, float y, float z, float heading, float speed, float recordedspeed, uint32 animation, uint8 levitate, 
		float targetx, float targety, float targetz, float targetheading, float targetspeed, char* targetName)
{
	...
	MYSQL_RES *result;
	MakeAnyLenString(&query,
		"INSERT into replayframes set replayid=%i,frameid=%i,x=%f,y=%f,z=%f,heading=%f,speed=%f,recordedspeed=%f,animation=%i,levitate=%i,targetx=%f,targety=%f,targetz=%f,targetheading=%f,targetspeed=%f,targetname='%s',inserted=NOW()",
		replayID, frameID, x, y, z, heading, speed, recordedspeed, animation, levitate, targetx, targety, targetz, targetheading, targetspeed, targetName);
...
void SharedDatabase::UpdateEndReplay(uint32 replayID)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "UPDATE replaylist set endtime=NOW() where id=%i",replayID), errbuf))	{
		cout << "Error in SharedDatabase::UpdateEndReplay" << query << "' " << errbuf << endl;
		safe_delete_array(query);
	}
	safe_delete_array(query);
}

bool SharedDatabase::GetReplayList(int32 charid, int32 zoneid, LinkedList<ReplayList_Struct*>* list)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	uint32 len_query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT id,starttime,endtime from replaylist where charid=%i and zoneid=%i", charid, zoneid), errbuf, &result)) {

		while ((row = mysql_fetch_row(result))) {
			ReplayList_Struct* replayStruct = new ReplayList_Struct;
			replayStruct->replayid = atoi(row[0]);
			_snprintf(replayStruct->startTime,64,"%s",row[1]);
			_snprintf(replayStruct->endTime,64,"%s",row[2]);
			if ( list )
				list->Insert(replayStruct);
		}
		mysql_free_result(result);
	}
	safe_delete_array(query);
...
bool SharedDatabase::GetReplayFrame(uint32 replayID, uint32 frameID, float* x, float* y, float* z, float* heading, float* speed, float* recordedspeed, uint32* animation, uint8* levitate, 
		float* targetx, float* targety, float* targetz, float* targetheading, float* targetspeed, char* targetName)
{
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	...
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT x,y,z,heading,speed,recordedspeed,animation,levitate,targetx,targety,targetz,targetheading,targetspeed,targetname from replayframes where replayid=%i and frameid=%i", replayID, frameID), errbuf, &result)) {
		...
		if ((row = mysql_fetch_row(result))) {
			...
			*heading = atof(row[3]);
			*speed = atof(row[4]);
			*recordedspeed = atof(row[5]);
			*animation = atoi(row[6]);
			*levitate = atoi(row[7]);
			*targetx = atof(row[8]);
			*targety = atof(row[9]);
			*targetz = atof(row[10]);
			*targetheading = atof(row[11]);
			*targetspeed = atof(row[12]);
			_snprintf(targetName,64,"%s",row[13]); 
			mysql_free_result(result);
			return true;
		}
		return false;
...
bool SharedDatabase::InsertMessageQueue(char* toName, char* fromName, int16 channelID, int8 language, int16 langskill, char* message, int32 petitionID, bool bypassMessageLimit, uint32 command, uint32 commandValue, uint32 guildid, char* lastAttacker, int32 fromgroupid, char* accountName, int32 optionalfield1, int32 optionalfield2)
{
	if ( !bypassMessageLimit && IsMaxedMessages(toName) ) // if we have no more queue space, return false
		return false;

	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	MakeAnyLenString(&query,
		"INSERT into tell_queue set toname='%s',fromname='%s',chanid=%i,language=%i,langskill=%i,message='%s',petitionid=%i,inserted=NOW(),command=%i,commandvalue=%i,guildid=%i,lastattacker='%s', fromgroupid=%i,accountname='%s',optional1=%i,optional2=%i",
		toName, fromName, channelID, language, langskill, message, petitionID,command,commandValue, guildid, lastAttacker, fromgroupid, accountName, optionalfield1, optionalfield2);
...
bool SharedDatabase::IsMaxedMessages(char* toName)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
	uint32 len_query = 0;
	MYSQL_RES *result;
	MYSQL_ROW row;
	if (RunQuery(query, MakeAnyLenString(&query, "select count(toname) from tell_queue where toname='%s' and petitionid=0", toName), errbuf, &result)) {

		if ((row = mysql_fetch_row(result))) {
			int count = atoi(row[0]);
			mysql_free_result(result);
			if ( count > RuleI(World,MaxMessageQueue) )
				return true;
			else
				return false;
		}
...
uint32 SharedDatabase::GetCharacterStatsUnsignedInt(uint32 charID, char* fieldName)
{
	if ( fieldName == NULL )
		return false;
	...
	MYSQL_ROW row;
	int32 accountID = 0;
	if (RunQuery(query, MakeAnyLenString(&query, "SELECT %s FROM characterdb_stats where charid=%i", fieldName, charID), errbuf, &result)) {
		safe_delete_array(query);
		if (mysql_num_rows(result) == 1)
		{
			row = mysql_fetch_row(result);

			uint32 val = (uint32)atoul(row[0]);

			// Image: mem leak fix 5/1/2017
			mysql_free_result(result);
	...
	{
		cerr << "Error in GetCharacterStatsUnsignedInt query '" << query << "' " << errbuf << endl;
		safe_delete_array(query);
		return 0;
	}
...
void SharedDatabase::SetCharacterStatsUnsignedInt(uint32 charID, char* fieldName, uint32 value)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "UPDATE characterdb_stats set %s=%i where charid=%i",fieldName,value,charID), errbuf))	{
		cout << "Error in SharedDatabase::SetCharacterStatsUnsignedInt" << query << "' " << errbuf << endl;
		safe_delete_array(query);
	}
	safe_delete_array(query);
}

void SharedDatabase::SetCharacterStatsStatic(uint32 charID, char* fieldName, char* value)
{	
	char errbuf[MYSQL_ERRMSG_SIZE];
	char *query = 0;
	if (!RunQuery(query, MakeAnyLenString(&query, "UPDATE characterdb_stats set %s=%s where charid=%i",fieldName,value,charID), errbuf))	{
		cout << "Error in SharedDatabase::SetCharacterStatsStatic" << query << "' " << errbuf << endl;
		safe_delete_array(query);
	}
	safe_delete_array(query);
}