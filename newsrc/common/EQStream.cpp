...
#ifdef LOGINSERVER
	retransmittimeout = 1500; //use 500ms as base before we have connection stats
#else
...
void EQStream::ProcessPacket(EQProtocolPacket *p)
{
	...
	case OP_SessionResponse: {
		...
		_log(NET__ERROR, _L "Received OP_SessionResponse: session %lu, maxlen %d, key %lu, compressed? %s, encoded? %s, port %i, src port %i" __L, (unsigned long)Session, MaxLen, (unsigned long)Key, compressed?"yes":"no", encoded?"yes":"no", remote_port, p->src_port);
				...
				if (index < sqsize) {
					...
					for (sitr = SequencedQueue.begin(); sitr != SequencedQueue.end() && count < index; ++sitr, ++count) {
						if (!(*sitr)->acked && (*sitr)->sent_time > 0 && (((*sitr)->sent_time + timeout) < Timer::GetCurrentTime())) {
							(*sitr)->sent_time = 0;
							_log(NET__NET_TRACE, _L "OP_OutOfOrderAck Flagging packet %d for retransmission" __L, SequencedBase + count);
						}
}
				}
#ifndef LOGINSERVER
	...
	case OP_SessionStatRequest: {
		...
		if (RuleR(EQStream, RetransmitTimeoutMult) && ntohl(Stats->average_delta)) {
...
#ifdef LOGINSERVER
				retransmittimeout = (ntohl(Stats->last_local_delta) + ntohl(Stats->last_remote_delta)) * 3;
#else
				retransmittimeout = (ntohl(Stats->last_local_delta) + ntohl(Stats->last_remote_delta)) * RuleR(EQStream, RetransmitTimeoutMult);
#endif
			} else {
#ifdef LOGINSERVER
				retransmittimeout = ntohl(Stats->average_delta) * 6;
#else
			...
			if(retransmittimeout > 5000)
				retransmittimeout = 5000;
#else
...
void EQStream::SendPacket(uint16 opcode, EQApplicationPacket *p)
{
	...
	if (p->size>(MaxLen-8)) { // proto-op(2), seq(2), app-op(2) ... data ... crc(2)
		...
		*(uint32 *)(out->pBuffer+2)=htonl(length);
		...
		_log(NET__FRAGMENT, _L "First fragment: used %d/%d. Put size %d in the packet" __L, used, p->size, length);
		SequencedPush(out);
		...
		if (sitr!=SequencedQueue.end()) {
			...
			if(SequencedQueue.empty()) {
				...
				if (p) {
					...
					// Combine worked
					_log(NET__NET_COMBINE, _L "Combined seq packet %d of len %d, yielding %d combined." __L, seq_send, (*sitr)->size, p->size);
					(*sitr)->sent_time = Timer::GetCurrentTime();
					++sitr;
					++count;
			}
...
void EQStream::Decay()
{
	...
	if (BytesWritten>0) {
		...
		deque<EQProtocolPacket *>::iterator sitr;
		for (sitr = SequencedQueue.begin(); sitr != SequencedQueue.end(); ++sitr, count++) {
			if (!(*sitr)->acked && (*sitr)->sent_time > 0 && ((*sitr)->sent_time + retransmittimeout) < Timer::GetCurrentTime()) {
				(*sitr)->sent_time = 0;
			_log(NET__DEBUG, _L "Timeout exceeded for seq %d.  Flagging packet for retransmission" __L, SequencedBase + count);
			}
		}
MOutboundQueue.unlock();
#endif
...
{
        if (average_delta && (average_delta <= 2500)) {
                MRate.lock();
                ...
                _log(NET__RATES, _L "Not adjusting data rate because avg delta over max (%d > %d)" __L, average_delta, 2500);
        }
}
#else
void EQStream::AdjustRates(uint32 average_delta)
{
        if (average_delta && (average_delta <= RuleI(EQStream, AverageDeltaMax))) {
				...
				AverageDelta = RuleI(EQStream, AverageDeltaMax);
        }
}
#endif
...
EQStream::MatchState EQStream::CheckSignature(const Signature *sig) {
	...
	if (!InboundQueue.empty()) {
		...
		if(p == NULL) {
		...
		} else if(p->opcode == sig->first_eq_opcode || p->opcode == sig->first_eq_opcode_backup) {
			...
			_log(NET__IDENT_TRACE, "%s:%d: First opcode 0x%x did not match expected 0x%x or 0x%x", long2ip(GetRemoteIP()).c_str(), ntohs(GetRemotePort()), p->opcode, sig->first_eq_opcode, sig->first_eq_opcode_backup);
			res = MatchFailed;
		}
	}
	MInboundQueue.unlock();
