...
void Register(EQStreamIdentifier &into) {
	...
	signature.first_eq_opcode_backup = opcodes->EmuToEQ(OP_SendLoginInfoOld);
...
#define IsFieldSame(fieldname, value) !strcasecmp(TOSTRING(fieldname), value)
...
char *SerializeItem(const ItemInst *inst, sint16 slot_id, uint32 *length, uint8 depth) {
	...
	int8 inheritTransferStatus = inst->IsInstNoDrop() ? 1 : 0;
	...
	for(i=0;i<10;i++) {
		...
		if (sub) {
			sub->SetInstNoTransfer(inheritTransferStatus);
...
#include "Titanium_itemfields.h"
		...
		,depth,protection
		,item->ItemClass
#define I(field) , IsFieldSame(field,"notransfer") ? inst->IsInstNoTransfer() : item->field
