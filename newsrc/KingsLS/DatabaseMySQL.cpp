...
/*
Changed a shitload of stuff. just what tables go where commented here rather than each one cuz im lazy.
-devn00b
*/
...
bool DatabaseMySQL::GetLoginDataFromAccountName(string name, string &password, unsigned int &id, unsigned int &groupid, unsigned int& is_activated)
{
	if(!db)
	{
		return false;
	...
	query << "SELECT id_member, eqemupwd, id_group, is_activated FROM " << server.options.GetAccountTable() << " WHERE member_name = '";
	query << name;
	query << "'";
...
void DatabaseMySQL::UpdateLSAccountData(unsigned int id, string ip_address)
{
	...
	stringstream query(stringstream::in | stringstream::out);
	query << "UPDATE " << server.options.GetAccountTable() << " SET member_ip2 = '";
	query << ip_address;
	query << "', last_login = now() where id_member = ";
	query << id;
...
void DatabaseMySQL::UpdateLSAccountInfo(unsigned int id, string name, string password, string email)
{
	...
	stringstream query(stringstream::in | stringstream::out);
	query << "REPLACE " << server.options.GetAccountTable() << " SET id_member = ";
	query << id << ", member_name = '" << name << "', eqemupwd = sha('";
	query << password << "'), date_registered = now(), email_address = '" << email;
	query << "', member_ip2 = '0.0.0.0', last_login = now()";
