...
MMF ItemsMMF;
MMFItems_Struct* MMFItemsData = 0;
MMFItems_Struct* MMFItemsData_Writable = 0;

DLLFUNC bool AddItem(uint32 id, const Item_Struct* item) {
	if ( !MMFItemsData_Writable && ItemsMMF.IsLoaded())
	{
	MMFItemsData = (MMFItems_Struct*) ItemsMMF.GetHandle();
	uint32 nextid = MMFItemsData->NextFreeIndex++;
	MMFItemsData->ItemIndex[id] = nextid;
	memcpy(&MMFItemsData->Items[nextid], item, sizeof(Item_Struct));
	return true;
	}
	if (!MMFItemsData_Writable) {
		return false;
	}
	if (id > MMF_EQMAX_ITEMS) {
		return false;
	}
...
DLLFUNC bool DLLLoadItems(CALLBACK_DBLoadItems cbDBLoadItems, int32 iItemStructSize, sint32* iItemCount, int32* iMaxItemID) {
	...
	*iItemCount = MMF_EQMAX_ITEMS;
	//MMFItemsData_Writable = 0;
	//Allocate the shared memory for the item structures
	int32 tmpMemSize = sizeof(MMFItems_Struct) + 256 + (sizeof(Item_Struct) * (MMF_EQMAX_ITEMS));
...
DLLFUNC Item_Struct* GetItemNoConst(uint32 id) {
	if (MMFItemsData == 0 || (!ItemsMMF.IsLoaded()) || id > MMF_EQMAX_ITEMS || MMFItemsData->ItemIndex[id] == 0xFFFF)
		return 0;
	return &MMFItemsData->Items[MMFItemsData->ItemIndex[id]];
}
