...
bool MMF::Open(const char* iName, int32 iSize) {
	...
	if (strlen(iName) < 2) {
		...
		cout << "Memory Allocation(" << iName << "): " << iSize/1000 << "KB" << endl;
	...
	switch (MMFname[16]) {
		...
		case 'A': load_share = 10;  break;
		case 'B': load_share = 11;  break;
		case 'R': load_share = 12;  break;
		case 'T': load_share = 13;  break;
#ifdef CATCH_CRASH
		default:
	...
	switch (load_share) {
		...
		// random loot drops
		case 10: share_key = ftok(".", 'A'); break;
		// random drop rates
		case 11: share_key = ftok(".", 'B'); break;
		// random name groups
		case 12: share_key = ftok(".", 'R'); break;
		// team settings
		case 13: share_key = ftok(".", 'T'); break;
		// ERROR Fatal
	...
	//if (!tmpSize) {
		...
		if ( share_id <= 0) {
			...
			if (share_id <= 0) {
			    ...
			    if ((lpvMem = shmat(share_id, NULL,SHM_R|SHM_W)) == (void *)-1) {
