##Ornate
###Rog
##9806 - Bidilis' Helm of the Elusive
##11141 - Bidilis' Bracer of the Elusive
##11432 - Bidilis' Sleeves of the Elusive
##12598 - Bidilis' Gauntlet of the Elusive
##16717 - Bidilis' Greaves of the Elusive
##19443 - Bidilis' Boots of the Elusive
##47648 - Bidilis' Hauberk of the Elusive
### Rog with aug?
###84121 - Bidilis' Boots of the Elusive
###84122 - Bidilis' Bracer of the Elusive
###84123 - Bidilis' Gauntlet of the Elusive
###84124 - Bidilis' Greaves of the Elusive
###84125 - Bidilis' Hauberk of the Elusive
###84126 - Bidilis' Helm of the Elusive
###84127 - Bidilis' Sleeves of the Elusive
###Druid
##9833 - Kerasha's Sylvan Cap
##11281 - Kerasha's Sylvan Bracer
##11575 - Kerasha's Sylvan Sleeves
##13603 - Kerasha's Sylvan Gloves
##16811 - Kerasha's Sylvan Leggings
##20422 - Kerasha's Sylvan Boots
##47651 - Kerasha's Sylvan Tunic
### Druid with aug?
###84142 - Kerasha's Sylvan Boots
###84143 - Kerasha's Sylvan Bracer
###84144 - Kerasha's Sylvan Cap
###84145 - Kerasha's Sylvan Gloves
###84146 - Kerasha's Sylvan Leggings
###84147 - Kerasha's Sylvan Sleeves
###84148 - Kerasha's Sylvan Tunic
###War
##9629 - Raex's Helm of Destruction
##11138 - Raex's Bracer of Destruction
##11429 - Raex's Armplates of Destruction
##12595 - Raex's Gauntlets of Destruction
##16693 - Raex's Greaves of Destruction
##19440 - Raex's Boots of Destruction
##47642 - Raex's Chestplate of Destruction
###War with aug?
###84114 - Raex's Armplates of Destruction
###84115 - Raex's Boots of Destruction
###84116 - Raex's Bracer of Destruction
###84117 - Raex's Chestplate of Destruction
###84118 - Raex's Gauntlets of Destruction
###84119 - Raex's Greaves of Destruction
###84120 - Raex's Helm of Destruction
###CLR
##9827 - Ultor's Helm of Faith
##11199 - Ultor's Bracer of Faith
##13563 - Ultor's Gauntlets of Faith
##16797 - Ultor's Greaves of Faith
##20074 - Ultor's Boots of Faith
##32012 - Ultor's Vambraces of Faith
##47646 - Ultor's Chestguard of Faith
###clr with aug
###84107 - Ultor's Boots of Faith
###84108 - Ultor's Bracer of Faith
###84109 - Ultor's Chestguard of Faith
###84110 - Ultor's Gauntlets of Faith
###84111 - Ultor's Greaves of Faith
###84112 - Ultor's Helm of Faith
###84113 - Ultor's Vambraces of Faith
###WIZ
##9950 - Maelin's Skullcap of Lore
##11309 - Maelin's Bracelet of Lore
##11876 - Maelin's Sleeves of Lore
##13627 - Maelin's Gloves of Lore
##19027 - Maelin's Leggings of Lore
##20434 - Maelin's Shoes of Lore
##47654 - Maelin's Robe of Lore
###wiz with aug
###84128 - Maelin's Bracelet of Lore
###84129 - Maelin's Gloves of Lore
###84130 - Maelin's Leggings of Lore
###84131 - Maelin's Robe of Lore
###84132 - Maelin's Shoes of Lore
###84133 - Maelin's Skullcap of Lore
###84134 - Maelin's Sleeves of Lore
###ENC
##9836 - Romar's Circlet of Visions
##11298 - Romar's Bracer of Visions
##11578 - Romar's Sleeves of Visions
##13614 - Romar's Gloves of Visions
##16931 - Romar's Pantaloons of Visions
##20425 - Romar's Slippers of Visions
##47656 - Romar's Robe of Visions
###enc with aug
###84135 - Romar's Bracer of Visions
###84136 - Romar's Circlet of Visions
###84137 - Romar's Gloves of Visions
###84138 - Romar's Pantaloons of Visions
###84139 - Romar's Robe of Visions
###84140 - Romar's Sleeves of Visions
###84141 - Romar's Slippers of Visions
####################### Done!!
###Ranger:
##9821 - Askr's Thunderous Helm
##11193 - Askr's Thunderous Bracer
##11447 - Askr's Thunderous Armplates
##12816 - Askr's Thunderous Gauntlets
##16777 - Askr's Thunderous Greaves
##19838 - Askr's Thunderous Boots
##47647 - Askr's Thunderous Chainmail
###
#146611 - Askr's Thunderous Helm
#146612 - Askr's Thunderous Bracer
#146613 - Askr's Thunderous Armplates
#146614 - Askr's Thunderous Gauntlets
#146615 - Askr's Thunderous Greaves
#146616 - Askr's Thunderous Boots
#146617 - Askr's Thunderous Chainmail
###MAG
##9944 - Magi`Kot's Tiara of Convergence
##11303 - Magi`Kot's Bangle of Convergence
##11598 - Magi`Kot's Sleeves of Convergence
##13619 - Magi`Kot's Gloves of Convergence
##19021 - Magi`Kot's Pants of Convergence
##20428 - Magi`Kot's Shoes of Convergence
##47655 - Magi`Kot's Robe of Convergence
###
#146618
#146619
#146620
#146621
#146622
#146623
#146624
###NEC
##9947 - Miragul's Crown of Risen Souls
##11306 - Miragul's Bracelet of Risen Souls
##11618 - Miragul's Sleeves of Risen Souls
##13623 - Miragul's Gloves of Risen Souls
##19024 - Miragul's Greaves of Risen Souls
##20431 - Miragul's Shoes of Risen Souls
##47653 - Miragul's Shroud of Risen Souls
###
#146625
#146626
#146627
#146628
#146629
#146630
#146631
###Pal
##9812 - Trydan's Helm of Nobility
##11163 - Trydan's Bracer of Nobility
##11438 - Trydan's Armplates of Nobility
##12627 - Trydan's Gauntlets of Nobility
##16757 - Trydan's Greaves of Nobility
##19449 - Trydan's Boots of Nobility
##47644 - Trydan's Chestplate of Nobility
###
#146632
#146633
#146634
#146635
#146636
#146637
#146638
###Monk
##9809 - Ton Po's Helm of Composure
##11146 - Ton Po's Wristwraps of Composure
##11435 - Ton Po's Sleeves of Composure
##12624 - Ton Po's Gloves of Composure
##16727 - Ton Po's Leggings of Composure
##19446 - Ton Po's Sandals of Composure
##47650 - Ton Po's Chestwraps of Composure
###
#146639
#146640
#146641
#146642
#146643
#146644
#146645
###SK
##9815 - Grimror's Helm of the Plagues
##11173 - Grimror's Bracer of the Plagues
##11441 - Grimror's Sleeves of the Plagues
##12637 - Grimror's Gauntlets of the Plagues
##16763 - Grimror's Greaves of the Plagues
##19546 - Grimror's Boots of the Plagues
##47645 - Grimror's Guard of the Plagues
###
#146646
#146647
#146648
#146649
#146650
#146651
#146652
###Shaman
##55508 - Galladan's Stormwrath Sleeves
##55509 - Galladan's Stormwrath Boots
##55510 - Galladan's Stormwrath Bracer
##55511 - Galladan's Stormwrath Tunic
##55512 - Galladan's Stormwrath Leggings
##55513 - Galladan's Stormwrath Coif
##55514 - Galladan's Stormwrath Gloves
###
#146653
#146654
#146655
#146656
#146657
#146658
#146659

sub EVENT_SAY {
####DISABLE
quest::say("Sorry $name, This season has no need of me, so I\'m just here to take up space I guess.");
return;
####

  if($text =~ /Hail/i) {
  $interestedLink = quest::saylink("interested"); quest::say("Hello, $name. I am the master of the forge of devn00b. He has permitted me to appear here and offer you a chance to upgrade your ornate armor! Let me know if you're 
actually $interestedLink"); quest::doanim(15);
  }
if($text=~/interested/i) {
    quest::say("Of course you are interested! Items from the gods are rare! Who wouldn't want them?");
    quest::say(" Here is how it works, you bring me: ");
    quest::say(" (1) Golden Token of Service which is sold here in bazaar");
    quest::say(" (1) Titanium Token of Bravery which comes from some high level monsters.");
    quest::say("");
    quest::say("Once you have both of those I will forge them into 1 random Ornate armor with an augment slot for you. I will only give you something you dont already have, so dont worry about that.");
  }
}

################### ITEM CODE ##################
sub EVENT_ITEM {
####DISABLE
quest::say("Sorry $name, This season has no need of me, so I\'m just here to eat your items..Nom. Nom. Nom.");
return;
####

#
# Golden Token of Service - 146609 - 3000 Points
#
# Titanium Token of Bravery - 14610 - Drop
#

if (plugin::check_handin(\%itemcount, 146609 => 1) && plugin::check_handin(\%itemcount, 146610 => 1)){ #### Token + Drop Token
## If they are too low a level stop before they can do anything
if($ulevel <45)
{
quest::say("Sorry $name, but $ulevel is too low to give this quality of gear to you. Come back when your 45+.");
quest::say("Oh and before I forget here are your tokens back. Please save them for when you get stronger, the gods will not return lost tokens!");
quest::summonitem(146609);
quest::summonitem(146610);

return;
}
##


if($class eq 'Enchanter')
{
##enc with aug
#84135 - Romar's Bracer of Visions
#84136 - Romar's Circlet of Visions
#84137 - Romar's Gloves of Visions
#84138 - Romar's Pantaloons of Visions
#84139 - Romar's Robe of Visions
#84140 - Romar's Sleeves of Visions
#84141 - Romar's Slippers of Visions

#random to see what item they get
my $mystuff = quest::ChooseRandom(84135,84136,84137,84138,84139,84140,84141);

if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84135) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}
quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
 }

if($class eq 'Wizard')
{
##wiz with aug
#84128 - Maelin's Bracelet of Lore
#84129 - Maelin's Gloves of Lore
#84130 - Maelin's Leggings of Lore
#84131 - Maelin's Robe of Lore
#84132 - Maelin's Shoes of Lore
#84133 - Maelin's Skullcap of Lore
#84134 - Maelin's Sleeves of Lore

#random to see what item they get
my $mystuff = quest::ChooseRandom(84128,84129,84130,84131,84132,84133,84134);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84128) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Cleric')
{
##ckr with aug
#84107 - Ultor's Boots of Faith
#84108 - Ultor's Bracer of Faith
#84109 - Ultor's Chestguard of Faith
#84110 - Ultor's Gauntlets of Faith
#84111 - Ultor's Greaves of Faith
#84112 - Ultor's Helm of Faith
#84113 - Ultor's Vambraces of Faith

#random to see what item they get
my $mystuff = quest::ChooseRandom(84107,84108,84109,84110,84111,84112,84113);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84108) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}
quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Warrior')
{
##War with aug?
#84114 - Raex's Armplates of Destruction
#84115 - Raex's Boots of Destruction
#84116 - Raex's Bracer of Destruction
#84117 - Raex's Chestplate of Destruction
#84118 - Raex's Gauntlets of Destruction
#84119 - Raex's Greaves of Destruction
#84120 - Raex's Helm of Destruction

#random to see what item they get
my $mystuff = quest::ChooseRandom(84114,84115,84116,84117,84118,84119,84120);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84116) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}


quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Druid')
{
## Druid with aug?
#84142 - Kerasha's Sylvan Boots
#84143 - Kerasha's Sylvan Bracer
#84144 - Kerasha's Sylvan Cap
#84145 - Kerasha's Sylvan Gloves
#84146 - Kerasha's Sylvan Leggings
#84147 - Kerasha's Sylvan Sleeves
#84148 - Kerasha's Sylvan Tunic

#random to see what item they get
my $mystuff = quest::ChooseRandom(84142,84143,84144,84145,84146,84147,84148);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84143) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Rogue')
{
## Rog with aug?
#84121 - Bidilis' Boots of the Elusive
#84122 - Bidilis' Bracer of the Elusive
#84123 - Bidilis' Gauntlet of the Elusive
#84124 - Bidilis' Greaves of the Elusive
#84125 - Bidilis' Hauberk of the Elusive
#84126 - Bidilis' Helm of the Elusive
#84127 - Bidilis' Sleeves of the Elusive

#random to see what item they get
my $mystuff = quest::ChooseRandom(84121,84122,84123,84124,84125,84126,84127);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 84122) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}
quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}
###########################################################

if($class eq 'Ranger')
{
#146611 - Askr's Thunderous Helm
#146612 - Askr's Thunderous Bracer
#146613 - Askr's Thunderous Armplates
#146614 - Askr's Thunderous Gauntlets
#146615 - Askr's Thunderous Greaves
#146616 - Askr's Thunderous Boots
#146617 - Askr's Thunderous Chainmail

#random to see what item they get
my $mystuff = quest::ChooseRandom(146611,146612,146613,146614,146615,146616,146617);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146612) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}


if($class eq 'Magician')
{
#146618
#146619
#146620
#146621
#146622
#146623
#146624

#random to see what item they get
my $mystuff = quest::ChooseRandom(146618,146619,146620,146621,146622,146623,146624);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146619) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}


quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Necromancer')
{
#146625
#146626
#146627
#146628
#146629
#146630
#146631

#random to see what item they get
my $mystuff = quest::ChooseRandom(146625,146626,146627,146628,146629,146630,146631);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146626) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}


quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Paladin')
{
#146632
#146633
#146634
#146635
#146636
#146637
#146638
#random to see what item they get
my $mystuff = quest::ChooseRandom(146632,146633,146634,146635,146636,146637,146638);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146633) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}


if($class eq 'Monk')
{
#146639
#146640
#146641
#146642
#146643
#146644
#146645

#random to see what item they get
my $mystuff = quest::ChooseRandom(146639,146640,146641,146642,146643,146644,146645,);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146640) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Shadowknight')
{
#146646
#146647
#146648
#146649
#146650
#146651
#146652

#random to see what item they get
my $mystuff = quest::ChooseRandom(146646,146647,146648,146649,146650,146651,146652);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146647) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}

if($class eq 'Shaman')
{
##146660	Rosrak's Helm of the Primal
##146661	Rosrak's Bracer of the Primal
##146662	Rosrak's Sleeves of the Primal
##146663	Rosrak's Gauntlets of the Primal
##146664	Rosrak's Greaves of the Primal
##146665	Rosrak's Boots of the Primal
##146666	Rosrak's Hauberk of the Primal

#random to see what item they get
my $mystuff = quest::ChooseRandom(146660,146661,146662,146663,146664,146665,146666);
if(plugin::check_hasitem($client, $mystuff)){
$winning = quest::varlink($mystuff);
quest::summonitem(146609);
quest::summonitem(146610);
quest::say("Sorry $name, you already had that item ($winning), Lets Try Again!!");
return;
}else{
if($mystuff eq 146661) {
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning! Since I'm sooo nice I gave you 2 of them! Please bring me more tokens to get the rest of the set (7 in total).");
quest::summonitem($mystuff);
quest::summonitem($mystuff);

return;
}

quest::summonitem($mystuff);
 }
$winning = quest::varlink($mystuff);
quest::say("Fantastic! You have been awarded $winning. Bring me more tokens to get the rest of the set (7 in total)");
return;
}



return;
}
  }

