sub EVENT_SAY {
  if ($text=~/hail/i) {
    my $link = quest::saylink("teleport near there");
    my $link2 = quest::saylink("travel to north ro");
    quest::say("Hello there. There seems to be some strange problems with the boats in this area. The Academy of Arcane Sciences has sent a small team of us to investigate them. If you need to [$link2] in the meantime, I can transport you to my companion there. We also just recently discovered that Joshel has been stranded over in the middle of the ocean since the problems with the boats started. If you'd be willing to go see if he's ok, I may be able to [$link]. Keep in mind though that it will be a one way trip.");
  }
