	...
	$var = plugin::guardevent_say($npc,13,$client,$text);
	if ( $var == 0 )
	{
	...
	{
	  if($npc->GetClass() != 41 && ( $npc->GetName()=~/Clockwork/i || $npc->GetName()=~/Watchman/i) ) {
		plugin::setupguard_enter(13,$npc);
	  }
	}
	...
	sub EVENT_ENTER {
	  if($npc->GetClass() != 41 && ( $npc->GetName()=~/Clockwork/i || $npc->GetName()=~/Watchman/i) ) {
	plugin::enterguard_area(13,$client,$npc);
	  }
	...
	{
	  if($npc->GetClass() != 41 && ( $npc->GetName()=~/Clockwork/i || $npc->GetName()=~/Watchman/i) ) {
	quest::resetguardfields(13,$npc->GetNPCTypeID(),$userid);
	  }
