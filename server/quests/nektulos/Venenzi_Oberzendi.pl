...
sub EVENT_SPAWN {
   	$epicsEnabled = $npc->CheckRule("epicsenabled");
   	if ( $epicsEnabled == 0 )
	{
...
sub EVENT_ITEM {
  ...
  elsif(plugin::check_handin($client, 20643 => 1, 1320 => 1)) {
#&& $itemcount {1320} == 1) {
#if(($itemcount{20643} == 1) && ($itemcount{1320} == 1)) { # Rolling Stone // Flowing Black Robe
#if(plugin::check_handin(\%itemcount, 20643 => 1)) {
