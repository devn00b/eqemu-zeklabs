...
if($class eq 'Beastlord' && !defined $qglobals{blracechange} ){
$fuckbl = quest::saylink("wood");
quest::say("Would you like to become a [$fuckbl]elf? **Note $name you can only do this once for free**.");
}
...
elsif($text=~/wood/i && !defined($blsuckx) ){
quest::setglobal("blracechange",0,5,"F");
