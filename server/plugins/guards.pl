...
#begin big if statement
	my $fieldToPull = "ownerteamid"; #formerly guildid
	my $fieldToCompare = $client->GetPVPFlag(); #formerly GuildID()
	my $teamHalas = quest::getguardfield(2,0, $fieldToPull); #halas
	my $teamFreeport = quest::getguardfield(3,0, $fieldToPull); #freeport
	my $teamFelwithe = quest::getguardfield(7,0, $fieldToPull); #felwithe
	my $teamErudin = quest::getguardfield(8,0, $fieldToPull); #erudin

	my $teamQeynos = quest::getguardfield(1,0, $fieldToPull); #qeynos
	my $teamRivervale = quest::getguardfield(4,0, $fieldToPull); #rivervale

	my $greatdivideCamp = quest::getguardfield(9,0, $fieldToPull);

	my $toxCamp = quest::getguardfield(10,0, $fieldToPull);
	my $gfayCamp = quest::getguardfield(11,0, $fieldToPull);
	my $lavaCamp = quest::getguardfield(12,0, $fieldToPull);

	my $teamNeriak = quest::getguardfield(14,0, $fieldToPull);
	my $teamKaladim = quest::getguardfield(15,0, $fieldToPull);
	my $teamOggok = quest::getguardfield(19,0, $fieldToPull);
	my $teamGrobb = quest::getguardfield(20,0, $fieldToPull);
...
elsif ($text =~/Halas/i && ( $fieldToCompare == $teamHalas || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(29,153,142.7,3.1,255);
}
elsif ($text =~/Freeport/i && ( $fieldToCompare == $teamFreeport || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(10,-657,168.9,-52.8,450);
}
elsif ($text =~/Felwithe/i && ( $fieldToCompare == $teamFelwithe || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(61,-231.5,-20.9,3.1,170);
}
elsif ($text =~/Erudin/i && ( $fieldToCompare == $teamErudin || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(24,-372.6,-931,66.1,60);
}
elsif ($text =~/Qeynos/i && ( $fieldToCompare == $teamQeynos || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(2,6.4,-10.5,3.1,0);
}
elsif ($text =~/Rivervale/i && ( $fieldToCompare == $teamRivervale || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(19,-153.7,68.5,4.1,255);
}
elsif ($text =~/Neriak/i && ( $fieldToCompare == $teamNeriak || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(40,-182.6,-2.1,3.0,125);
}
elsif ($text =~/Kaladim/i && ( $fieldToCompare == $teamNeriak || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(60,-0.6,143.3,3.1,0);
}
elsif ($text =~/Oggok/i && ( $fieldToCompare == $teamOggok || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(49,379.7,-194.8,13.1,250);
}
elsif ($text =~/Grobb/i && ( $fieldToCompare == $teamGrobb || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(52,-52.7,239.4,3.1,295);
}
elsif (($text =~/Tox Camp/i || $text =~/Toxxulia Camp/i || $text =~/Toxxulia Forest Camp/i) && ( $fieldToCompare == $toxCamp || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(38,1078.7,-980.1,-45.7,190.6);
}
elsif ($text =~/Greater Faydark Camp/i && ( $fieldToCompare == $gfayCamp || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(54,-1676.2,1667.4,69.2,230.9);
}
elsif ($text =~/Lavastorm Camp/i && ( $fieldToCompare == $lavaCamp || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
$client->MovePC(27,-500.3,176.6,99.1,112.9);
}
elsif ($text =~/Great Divide Camp/i && ( $fieldToCompare == $greatdivideCamp || $client->GetGM() ) ){
quest::setglobal($flagName,$flagValue,$flagType,$flagTime);
...
{
#quest::say("Team pvp: $fieldToCompare");
...
$link16 = quest::saylink("Neriak");
$link17 = quest::saylink("Kaladim");
$link18 = quest::saylink("Oggok");
$link19 = quest::saylink("Grobb");
...
$zones = "You may return to ";
	...
	if ( $fieldToCompare == $teamFreeport )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link;
	}
	if ( $fieldToCompare == $teamHalas )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link15;
	}
	if ( $fieldToCompare == $teamNeriak )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link16;
	}
	if ( $fieldToCompare == $teamKaladim )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link17;
	}
	if ( $fieldToCompare == $teamOggok )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link18;
	}
	if ( $fieldToCompare == $teamGrobb )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link19;
	}
	if ( $fieldToCompare == $teamFelwithe )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link2;
	}
	if ( $fieldToCompare == $teamErudin )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link3;
	}
	if ( $fieldToCompare == $teamQeynos )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link4;
	}
	if ( $fieldToCompare == $teamRivervale )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link5;
	}
	if ( $fieldToCompare == $toxCamp )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link6;
	}
	if ( $fieldToCompare == $gfayCamp )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link7;
	}
	if ( $fieldToCompare == $lavaCamp )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link8;
	}
	if ( $fieldToCompare == $greatdivideCamp )
	{
		$length = length $zones;
		if ( $length > 19 ) { $zones = $zones . " or "; }
		$zones = $zones . $link9;
